...
///
/// Can be overridden with `$ALPHACODE_RUNTIME_DIR`.
...
pub fn alphacode_dir() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("ALPHACODE_HOME") {
...
/// to `~/.alphacode/state` (respecting `ALPHACODE_HOME`).
///
/// When `ALPHACODE_RUNTIME_DIR` is set (tests and sandboxed temp servers), it
/// takes precedence so isolated runs never touch the real alphacode home.
...
/// Resolve alphacode's app-owned config directory.
///
/// Default location is the platform config dir + `alphacode` (for example
/// `~/.config/alphacode` on Linux). When `ALPHACODE_HOME` is set, sandbox this under
/// `$ALPHACODE_HOME/config/alphacode` so self-dev/tests do not leak into the user's
/// real config directory.
pub fn app_config_dir() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("ALPHACODE_HOME") {
        return Ok(PathBuf::from(path).join("config").join("alphacode"));
    }

    let config_dir =
        dirs::config_dir().ok_or_else(|| anyhow::anyhow!("No config directory found"))?;
    Ok(config_dir.join("alphacode"))
}

/// Resolve a path under the user's home directory, but sandbox it under
/// `$ALPHACODE_HOME/external/` when `ALPHACODE_HOME` is set.
///
    ...
    #[cfg(windows)]
    {
        if let Some(config_dir) = dirs::config_dir() {
            ...
            if alphacode_config_dir.exists() {
                schedule_windows_path_hardening(&alphacode_config_dir, true);
            }
        }
        ...
        {
            schedule_windows_path_hardening(&alphacode_home, true);
        }
    }
...
pub fn harden_secret_file_permissions(path: &Path) {
    ...
    #[cfg(not(windows))]
    {
        if let Some(parent) = path.parent() {
            let _ = crate::alphacode_core::fs::set_directory_permissions_owner_only(parent);
        }
        if path.exists() {
...
fn harden_secret_file_permissions_windows(path: &Path) {
    ...
    // Read-time hardening is opportunistic, while Alphacode's own secret writes
...
fn schedule_windows_path_hardening(path: &Path, directory: bool) {
    ...
    if std::thread::Builder::new()
        .name("alphacode-windows-acl-harden".to_string())
        .spawn(|| {
...
///
/// alphacode intentionally avoids mutating these files. We also reject obvious risky
...
pub fn ensure_dir(path: &Path) -> Result<()> {
    if !path.exists() {
        ...
        crate::alphacode_core::fs::set_directory_permissions_owner_only(path)?;
    }
    Ok(())
}
...
fn write_bytes_inner(path: &Path, bytes: &[u8], durable: bool, secret: bool) -> Result<()> {
    if let Some(parent) = path.parent() {
        ...
        if secret {
            ...
            crate::alphacode_core::fs::set_directory_permissions_owner_only(parent)?;
        }
    }
    ...
    let result = (|| -> Result<()> {
        ...
        if secret {
            crate::alphacode_core::fs::set_permissions_owner_only(&tmp_path)?;
        }
...
mod windows_hardening_tests {
    ...
    fn first_path_starts_one_worker_and_repeated_paths_are_coalesced() {
        ...
        let now = Instant::now();
        let directory = Path::new(r"C:\Users\test\.alphacode");
    ...
    fn recently_attempted_paths_are_not_requeued() {
        ...
        let file = PathBuf::from(r"C:\Users\test\.alphacode\auth.json");
        state
            .files

//! Tracking of active session process IDs under `~/.alphacode/active_pids`.
//!
...
//! lives in the storage crate because it only needs [`alphacode_dir`] and is a
...
use crate::alphacode_storage::alphacode_dir;
use std::path::PathBuf;

/// Directory holding one file per active session ID (`~/.alphacode/active_pids`).
pub fn active_pids_dir() -> Option<PathBuf> {
    alphacode_dir().ok().map(|d| d.join("active_pids"))
}
...
pub fn streaming_pids_dir() -> Option<std::path::PathBuf> {
    alphacode_dir().ok().map(|d| d.join("streaming_pids"))
}
...
pub fn internal_pids_dir() -> Option<std::path::PathBuf> {
    alphacode_dir().ok().map(|d| d.join("internal_pids"))
}
...
/// List active session IDs currently tracked in `~/.alphacode/active_pids`.
...
///
/// Delegates to the shared cross-platform process check in [`crate::alphacode_base::platform`]
/// (Unix: `kill(pid, 0)`; Windows: `OpenProcess` + exit code), so dead PIDs are
/// never reported as live sessions by the presence scan.
fn process_is_running(pid: u32) -> bool {
    crate::alphacode_base::platform::is_process_running(pid)
}

/// Live snapshot of how many alphacode sessions are running, and how many of those
/// are actively streaming a model response right now. Used by the menu bar
/// indicator (`alphacode menubar`) and any other presence UI.

...
pub fn builds_dir() -> Result<PathBuf> {
    let dir = resolve_builds_dir(
        std::env::var_os("ALPHACODE_HOME").map(PathBuf::from),
...
fn resolve_builds_dir(
    alphacode_home: Option<PathBuf>,
    local_app_data: Option<PathBuf>,
    default_alphacode_dir: PathBuf,
    is_windows: bool,
) -> PathBuf {
    if let Some(alphacode_home) = alphacode_home {
        return alphacode_home.join("builds");
    }

    if is_windows && let Some(local_app_data) = local_app_data {
        // Keep runtime channel discovery aligned with scripts/install.ps1 and
        // the supported Windows layout under %LOCALAPPDATA%\alphacode\builds.
        // Durable user state and logs still live under ~/.alphacode.
        return local_app_data.join("alphacode").join("builds");
    }

    default_alphacode_dir.join("builds")
}
...
mod tests {
    ...
    fn windows_builds_use_local_app_data() {
        let resolved = resolve_builds_dir(
            ...
            PathBuf::from("/home/test/.alphacode"),
            true,
        );

        assert_eq!(resolved, PathBuf::from("/local-app-data/alphacode/builds"));
    }

    #[test]
    fn alphacode_home_override_wins_on_windows() {
        let resolved = resolve_builds_dir(
            Some(PathBuf::from("/isolated-alphacode")),
            Some(PathBuf::from("/local-app-data")),
            PathBuf::from("/home/test/.alphacode"),
            true,
        );

        assert_eq!(resolved, PathBuf::from("/isolated-alphacode/builds"));
    }

    #[test]
    fn non_windows_builds_stay_under_alphacode_home() {
        let resolved = resolve_builds_dir(
            None,
        ...
        assert_eq!(resolved, PathBuf::from("/home/test/.alphacode/builds"));
    }
}

...
/// Get the alphacode repository directory
pub fn get_repo_dir() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("ALPHACODE_REPO_DIR") {
        let path = PathBuf::from(path);
        if is_alphacode_repo(&path) {
            return Some(path);
        }
    }
...
///
/// Release archives install a tiny `alphacode` wrapper script alongside the real
/// `alphacode-<platform>.bin` payload (the wrapper sets `LD_LIBRARY_PATH` and execs
...
pub fn selfdev_build_command_for_target(
    ...
    let specs = match target {
        SelfDevBuildTarget::Tui => vec![("alphacode", "alphacode")],
        SelfDevBuildTarget::All | SelfDevBuildTarget::Auto => {
            vec![("alphacode", "alphacode")]
        }
    };
...
///
/// Defaults to `~/.local/bin` on Unix, `%LOCALAPPDATA%\alphacode\bin` on Windows.
/// Overridable with `ALPHACODE_INSTALL_DIR`.
pub fn launcher_dir() -> Result<PathBuf> {
    if let Some(custom) = non_empty_env_path("ALPHACODE_INSTALL_DIR") {
        return Ok(custom);
    }

    if let Some(sandbox_home) = non_empty_env_path("ALPHACODE_HOME") {
...
/// Path to the launcher binary (`~/.local/bin/alphacode` by default).
...
fn shared_server_channel_is_current_enough() -> bool {
    // Version markers can drift from the binary on disk: the update flow writes
    // the marker alongside the payload swap, and a partial swap leaves a stale
    // marker next to an old binary. Trusting the marker alone previously let a
    // client spawn a shared server built before a newer flag existed, which
    // then crashed with `Unrecognized option: 'provider'` (the client passes
    // `--provider`, `--model`, etc. on spawn). If the shared-server payload is
    // strictly older than the running client binary, refuse it so the caller
    // falls back to spawning the client's own (newer) executable.
    if let Ok(current_exe) = std::env::current_exe()
        && let Ok(shared) = shared_server_binary_path()
        && shared_server_binary_is_strictly_older_than(&shared, &current_exe)
    {
        return false;
    }
...
/// Check if a directory is the alphacode repository
pub fn is_alphacode_repo(dir: &Path) -> bool {
    // Check for Cargo.toml with name = "alphacode"
    ...
    if let Ok(content) = std::fs::read_to_string(&cargo_toml)
        && content.contains("name = \"alphacode\"")
    {
        return true;
    }
...
mod tests {
    ...
    fn find_repo_in_ancestors_finds_workspace_from_crate_dir() {
        ...
        let crate_dir = repo.path().join("crates").join("alphacode-build-support");
    ...
    fn every_build_target_builds_its_own_package() {
        let repo = repo_fixture(false);
        let cases = [
            (SelfDevBuildTarget::Tui, vec!["-p alphacode "]),
            (SelfDevBuildTarget::All, vec!["-p alphacode "]),
        ];
    ...
    /// The desktop app was removed from this build: no target may resurrect a
    /// `-p alphacode-desktop2` invocation, which would fail with no such package.
    #[test]
    fn no_target_builds_the_removed_desktop_binary() {
        let repo = repo_fixture(false);
        for target in [
            SelfDevBuildTarget::Tui,
            ...
            let command = selfdev_build_command_for_target(repo.path(), target);
            assert!(
                !command.display.contains("alphacode-desktop"),
                "{target:?} still builds the removed desktop binary: {}",
                command.display
            );
        }
    }

    /// `auto` routes to the TUI for ordinary source edits, and to `All` only for
    /// workspace-wide manifest changes that can affect every binary.
    #[test]
    fn auto_routes_changed_paths_to_the_right_binary() {
        let cases: Vec<(Vec<&str>, SelfDevBuildTarget)> = vec![
            ...
            (
                vec!["crates/alphacode-tui/src/lib.rs"],
                SelfDevBuildTarget::Tui,
            ),
    ...
    fn porcelain_lines_are_parsed_including_renames() {
        ...
        assert_eq!(
            porcelain_path("?? src/alphacode_tui/new.rs"),
            "src/alphacode_tui/new.rs"
        );
        assert_eq!(
            porcelain_path("R  old/path.rs -> src/alphacode_tui/moved.rs"),
            "src/alphacode_tui/moved.rs"
        );
    }
    ...
    fn build_targets_parse_from_their_names() {
        ...
        // The desktop target is gone: its old names must be rejected, not
        // silently accepted and routed somewhere else.
        assert!(SelfDevBuildTarget::parse(Some("desktop2")).is_err());
    }

    #[test]
    fn is_alphacode_repo_accepts_git_file_for_worktree() {
        let repo = repo_fixture(true);
        assert!(is_alphacode_repo(repo.path()));
    }

    /// Build a release-style install dir: `alphacode` wrapper script + payload.
    fn release_install_fixture() -> (tempfile::TempDir, PathBuf, PathBuf) {
        let temp = tempfile::TempDir::new().expect("temp install");
        let wrapper = temp.path().join("alphacode");
        let payload = temp.path().join("alphacode-linux-x86_64.bin");
        std::fs::write(
            &wrapper,
    ...
    fn resolve_binary_payload_follows_channel_symlink_to_wrapper() {
        // The real layout: builds/<channel>/alphacode -> versions/<v>/alphacode (wrapper).
        ...
        let link = channel_dir.join("alphacode");
        // The channel entry is a symlink to the wrapper on Unix, but a copy on
        // Windows (the channel update path cannot create symlinks over a running
        // executable). Either way, the payload sibling must be resolvable, so
        // mirror the installed layout: wrapper + payload side by side.
        ...
        assert_eq!(
            ...
            std::fs::canonicalize(&expected_payload).expect("canonical payload")
        );
    }
    ...
    fn resolve_binary_payload_keeps_real_binary() {
        ...
        let temp = tempfile::TempDir::new().expect("temp install");
        let binary = temp.path().join("alphacode");
        std::fs::write(&binary, vec![0x7fu8; 8192]).expect("binary");
        std::fs::write(temp.path().join("alphacode-linux-x86_64.bin"), [0u8; 8]).expect("bin");
        assert_eq!(
    ...
    fn resolve_binary_payload_keeps_small_script_without_payload() {
        let temp = tempfile::TempDir::new().expect("temp install");
        let script = temp.path().join("alphacode");

...
fn with_temp_alphacode_home<T>(f: impl FnOnce() -> T) -> T {
...
/// Assert that two paths point at the same installed binary.
///
/// On Unix the channel/launcher entries are real symlinks, so canonicalizing
/// both resolves them to the same versioned binary. On Windows the channel
/// entries are byte-for-byte copies (`atomic_symlink_swap` cannot create a
/// symlink over a running executable), so compare file contents instead.
#[cfg(unix)]
fn assert_same_binary(a: &std::path::Path, b: &std::path::Path) {
    assert_eq!(
        std::fs::canonicalize(a).expect("canonical a"),
        std::fs::canonicalize(b).expect("canonical b"),
        "expected {a:?} and {b:?} to resolve to the same binary"
    );
}
...
fn assert_same_binary(a: &std::path::Path, b: &std::path::Path) {
    assert_eq!(
        ...
        "expected {a:?} and {b:?} to be the same binary"
    );
}
...
fn test_binary_version_hash_mismatch_rejects_publish_candidate() {
    ...
    let error = validate_binary_version_matches_source_report(&report, Path::new("alphacode"), &source)
...
fn test_find_repo_in_ancestors_walks_upward() {
    let temp = tempfile::tempdir().expect("tempdir");
    let repo = temp.path().join("alphacode-repo");
...
fn test_client_update_candidate_prefers_dev_binary_for_selfdev() {
    ...
    assert_same_binary(&candidate.0, &version_binary);
...
#[test]
fn launcher_dir_uses_sandbox_bin_when_alphacode_home_is_set() {
    with_temp_alphacode_home(|| {
        let launcher_dir = launcher_dir().expect("launcher dir");
        let expected = storage::alphacode_dir().expect("alphacode dir").join("bin");
...
fn update_launcher_symlink_stays_inside_sandbox_home() {
    with_temp_alphacode_home(|| {
        ...
        let expected_launcher = storage::alphacode_dir()
            .expect("alphacode dir")
            .join("bin")
            .join(binary_name());
        assert_eq!(launcher, expected_launcher);
        assert_same_binary(&launcher, &version_binary);
    });
}
...
fn shared_server_candidate_prefers_approved_channel_over_current() {
    with_temp_alphacode_home(|| {
        ...
        assert_same_binary(&candidate.0, &approved);
    });
}
...
fn normal_shared_server_candidate_repairs_stale_shared_channel_to_stable() {
    with_temp_alphacode_home(|| {
        ...
        let installed = version_binary_path(installed_version).unwrap();
        assert_same_binary(&candidate.0, &installed);
    });
}
...
/// Resolve the version a candidate channel label points at.
///
/// The candidate functions return `(path, label)` where the label is one of
/// `"shared-server"`, `"stable"`, `"current"`. The version marker written by
/// `update_*_symlink` is the source of truth for what each channel points at
/// (on Windows the channel entry is a copy rather than a symlink, so the
/// version cannot be recovered from the canonicalized path).
fn channel_label_version(label: &str) -> Option<String> {
    match label {
        "shared-server" => read_shared_server_version().ok().flatten(),
        "stable" => read_stable_version().ok().flatten(),
        "current" => read_current_version().ok().flatten(),
        _ => None,
    }
}
...
fn daemon_reload_target_version() -> Option<String> {
    let (_candidate, label) = shared_server_update_candidate(false)?;
...
fn candidate_version(candidate: Option<(PathBuf, &'static str)>) -> Option<String> {
    let (_candidate, label) = candidate?;
    channel_label_version(label)
}
...
/// Write a distinct, real binary into `versions/<version>/alphacode` with an
...
fn write_versioned_binary(version: &str, mtime: std::time::SystemTime) -> PathBuf {
    ...
    // Open with write access so `set_modified` works on Windows (a read-only
    // handle there returns ERROR_ACCESS_DENIED when updating file times).

...
//! consumes them. These live in `alphacode-render-core` (a backend-neutral, pure
//! crate) rather than in `alphacode-tui-markdown` so the foundation/streaming layer
//! can format reasoning lines without depending on any `alphacode-tui-*` crate.
...
///
/// Lives here (a backend-neutral, pure crate) rather than in `alphacode-tui-markdown`
/// so the foundation/streaming layer can format the summary without depending on
/// any `alphacode-tui-*` crate. Re-exported from `alphacode-tui-markdown` for the
/// existing `crate::alphacode_tui_markdown::reasoning_summary_line_markup` path.

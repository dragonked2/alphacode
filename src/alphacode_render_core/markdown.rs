...
//! (`alphacode-tui-markdown`) but emits the neutral [`crate::model`] types instead
...
use crate::alphacode_render_core::model::{
...
pub fn parse_markdown(text: &str) -> Document {
    ...
    let normalized = crate::alphacode_render_core::preprocess::normalize_latex_math(text);
    let escaped = crate::alphacode_render_core::preprocess::escape_currency_dollars(&normalized);
        ...
        match event {
            ...
            Event::InlineMath(math) => {
                if in_table {
                    ...
                    spans.push(StyledSpan {
                        text: crate::alphacode_render_core::math::render_inline_latex(&math),
                        role: StyleRole::Math,
                        fill: FillRole::None,
                        attrs: TextAttrs::none(),
                    });

...
//!
//! Configuration lives in `~/.alphacode/config.toml`:
//!
//! ```toml
//! [display.colors]
//! user = "#8ab4f8"
...
pub enum Role {
    ...
    /// Header session id.
    HeaderSession,
    /// Header model display name (the active LLM, styled as a distinct accent).
    ModelName,
    /// Success / additions.
    Success,
    /// Warnings.
...
impl Role {
    ...
    pub const fn key(self) -> &'static str {
        match self {
            ...
            Role::ProgressFill => "progress_fill",
            Role::ProgressBg => "progress_bg",
            Role::ToolBg => "tool_bg",
            Role::DiffAdd => "diff_add",
            Role::DiffRemove => "diff_remove",
            Role::DiffContext => "diff_context",
            Role::SwarmAgent => "swarm_agent",
            Role::SwarmTask => "swarm_task",
            Role::Memory => "memory",
    ...
    pub const fn default_rgb(self) -> (u8, u8, u8) {
        match self {
            ...
            Role::UserText => (245, 248, 255),
            Role::UserBg => (18, 22, 38),
            Role::AiText => (235, 238, 248),
            ...
            Role::Info => (148, 188, 255),
            Role::Border => (48, 54, 76),
            Role::SelectionBg => (32, 38, 62),
            Role::CodeBg => (24, 28, 46),
            Role::Heading => (125, 195, 255),
            Role::Link => (148, 215, 255),
            Role::Quote => (148, 188, 228),
            Role::Spinner => (205, 168, 255),
            Role::ProgressFill => (108, 230, 158),
            Role::ProgressBg => (32, 38, 62),
            Role::ToolBg => (18, 22, 38),
            Role::DiffAdd => (108, 230, 158),
            Role::DiffRemove => (255, 118, 118),
            Role::DiffContext => (138, 142, 165),
            Role::SwarmAgent => (108, 215, 255),
            Role::SwarmTask => (255, 215, 78),
    ...
    pub const fn is_background(self) -> bool {
        matches!(
            self,
            Role::UserBg | Role::SelectionBg | Role::CodeBg | Role::ToolBg | Role::ProgressBg
        )
    }
}
...
impl Palette {
    ...
    {
        Self::default().with_pairs(pairs)
    }

    /// Layer `key = "#rrggbb"` pairs on top of an existing palette.
    ///
    /// This is what lets a theme preset and `[display.colors]` compose: the
    /// preset supplies the base, and individual overrides win over it. Roles the
    /// user did not name keep whatever the base had, including its overridden
    /// flag, so a preset is not silently reduced to the built-in defaults by the
    /// presence of a single custom entry.
    pub fn with_pairs<'a, I>(mut self, pairs: I) -> (Self, Vec<String>)
    where
        I: IntoIterator<Item = (&'a str, &'a str)>,
    {
        ...
        for (key, value) in pairs {
            match (Role::from_key(key), parse_hex(value)) {
                (Some(role), Some(rgb)) => self.set(role, rgb),
...
///
/// This must run **after** [`crate::alphacode_tui_style::theme_mode::adapt_buffer_for_theme`]. That
/// pass exists because alphacode's *built-in* palette is designed for dark
...
fn match_target(role: Role) -> (u8, u8, u8) {
    let (r, g, b) = role.default_rgb();
    match crate::alphacode_tui_style::theme_mode::adapt_color_for_theme(Color::Rgb(r, g, b)) {
    ...
    fn configured_role_recolors_matching_cells_and_named_colors() {
        ...
        with_palette(palette, || {
            let mut buf = buffer_with(&[
                Color::Rgb(255, 110, 110), // the error default
...
mod light_theme_interaction {
    use super::*;
    use crate::alphacode_tui_style::theme_mode::{ThemeMode, adapt_buffer};
    use ratatui::buffer::Buffer;
    use ratatui::layout::Rect;
    ...
    ///
    /// The light adapter exists because alphacode's *built-in* palette is designed
    ...
    fn configured_colors_survive_the_light_theme_pass() {
        ...
        impl Drop for Restore {
            fn drop(&mut self) {
                set_palette(Palette::default());
                crate::alphacode_tui_style::theme_mode::set_theme_mode(ThemeMode::Dark);
            }
        }
        let _restore = Restore;
        // `match_target` reads the global theme mode, so set it to match the
        // buffer pass being exercised.
        crate::alphacode_tui_style::theme_mode::set_theme_mode(ThemeMode::Light);
...
mod default_palette_is_frozen {
    ...
    /// The exact hand-tuned palette alphacode ships today.
    ///
    ...
    ///
    /// The table was resynced when the defaults were refreshed to a more vivid,
    /// higher-contrast set (brighter accents, cooler blue-leaning neutrals, and
    /// slightly deeper panel backgrounds). The drift it is designed to catch had
    /// already happened: `default_rgb` carried the refreshed values while this
    /// table still held the pre-refresh ones, so both this guard and
    /// `default_palette_matches_historical_values` were failing. The refreshed
    /// values are the intended look and are now recorded here.

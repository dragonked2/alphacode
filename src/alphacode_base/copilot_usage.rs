...
//! doesn't expose a usage API. Data persists to ~/.alphacode/copilot_usage.json.
...
pub use crate::alphacode_usage_types::{AllTimeUsage, CopilotUsageTracker, DayUsage, MonthUsage};
...
mod tests {
    ...
    #[test]
    fn usage_path_respects_alphacode_home() {
    ...
    #[test]
    fn save_and_load_roundtrip_under_alphacode_home() {
        let _env_lock = lock_env();
        clear_tracker();
        let temp = tempfile::tempdir().expect("tempdir");
        let _home = EnvVarGuard::set("ALPHACODE_HOME", temp.path().as_os_str());

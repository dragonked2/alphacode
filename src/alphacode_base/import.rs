//! Import Claude Code sessions into alphacode
//!
...
pub use crate::alphacode_import_core::{
...
/// Convert Claude Code content blocks to alphacode ContentBlocks
...
fn truncate_import_text(text: String, max_bytes: usize) -> String {
    ...
    format!(
        "{}{}",
        crate::alphacode_core::util::truncate_str(&text, prefix_budget),
        suffix
    )
}
...
fn reuse_existing_imported_session(session_id: &str) -> bool {
    // An imported snapshot becomes a normal alphacode continuation as soon as the
    ...
    // doing so can discard alphacode-only turns and journal state. New imports are
...
pub fn imported_session_id_for_target(
    ...
    match target {
        ...
        crate::alphacode_session_types::ResumeTarget::CodexSession { session_id, .. } => {
            Some(imported_codex_session_id(session_id))
        }
        crate::alphacode_session_types::ResumeTarget::PiSession { session_path } => {
            Some(imported_pi_session_id(session_path))
        }
        crate::alphacode_session_types::ResumeTarget::OpenCodeSession { session_id, .. } => {
            Some(imported_opencode_session_id(session_id))
        }
        crate::alphacode_session_types::ResumeTarget::CursorSession { session_id, .. } => {
            Some(imported_cursor_session_id(session_id))
        }
    }
}

pub fn resolve_resume_target_to_alphacode(
    target: &crate::alphacode_session_types::ResumeTarget,
) -> Result<crate::alphacode_session_types::ResumeTarget> {
    use crate::alphacode_session_types::ResumeTarget;
    ...
    let session_id = match target {
        ResumeTarget::AlphacodeSession { session_id } => {
            return Ok(ResumeTarget::AlphacodeSession {
                session_id: session_id.clone(),
            });
        }
    ...
    Ok(ResumeTarget::AlphacodeSession { session_id })
}
...
fn import_session_from_file_with_target(
    ...
    let mut session = Session::create_with_id(alphacode_session_id, None, title);
...
/// Explicitly hand a currently-running Claude Code session over to Alphacode.
///
/// This is deliberately separate from normal resume. It first imports the
/// current transcript into a fresh durable Alphacode session, then gracefully stops
...
/// A stop failure rolls back the staged Alphacode session and leaves ordinary
...
fn take_over_live_claude_session_with_timeout(
    ...
    // Use a normal memorable Alphacode session ID so the handed-off conversation
    ...
    if stop_outcome == crate::claude_live::StopLiveClaudeOutcome::ExitUnconfirmed {
        crate::session_list_cache::invalidate();
        anyhow::bail!(
            "Claude Code process {} was asked to exit, but its exit was not confirmed; prepared Alphacode session {} was preserved and can be resumed",
            live.pid,
            takeover_id
        );
    }
    ...
    if let Some(err) = final_refresh_error {
        ...
        return Err(err).with_context(|| {
            format!(
                "Claude Code exited, but its final transcript could not be refreshed; prepared Alphacode session {takeover_id} was preserved"
            )
        });
    }

    crate::logging::info(&format!(
        "Claude takeover complete: source_session={session_id} source_pid={} alphacode_session={takeover_id}",
        live.pid
    ));
    crate::session_list_cache::invalidate();
    Ok(ResumeTarget::AlphacodeSession {
...
fn finalize_imported_session(
    ...
    // Never overwrite a alphacode-side continuation with a shorter external
...
pub fn import_cursor_session_from_path(
    ...
    let created_at =
        crate::alphacode_import_core::file_modified_datetime(session_path).unwrap_or_else(Utc::now);

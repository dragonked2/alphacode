...
//! crash-recovery copy (see `crate::alphacode_storage::write_bytes_inner`). That backup is
...
/// Minimum interval between prune passes across all alphacode processes.
///
...
/// the machine-wide prune interval has elapsed, so spawning many alphacode
/// processes at once does not trigger many full directory walks.
pub fn prune_old_session_backups() {
    if let Ok(base) = storage::alphacode_dir() {

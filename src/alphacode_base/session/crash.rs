...
///
/// Uses a fast O(n) scan of `~/.alphacode/active_pids/` (typically 0-5 files)
...
fn find_crashed_legacy_scan() -> Vec<(String, String)> {
    let sessions_dir = match storage::alphacode_dir() {
...
// Active PID tracking
// ---------------------------------------------------------------------------
// Lightweight files in ~/.alphacode/active_pids/<session_id> containing the PID.
...
/// alphacode store under a stable `imported_<tool>_<provider_id>` stem. Resuming the
...
/// was still sitting in `~/.alphacode/sessions` (issue #336).
///
...
mod batch_crash_tests {
    ...
    -> anyhow::Result<()> {
        ...
        // No external OpenCode store exists under ALPHACODE_HOME/external, so the only

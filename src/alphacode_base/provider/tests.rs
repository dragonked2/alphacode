...
fn with_clean_provider_test_env<T>(f: impl FnOnce() -> T) -> T {
    ...
    // Concrete provider runtimes live downstream (alphacode-provider-*-runtime),
    ...
    let prev_subscription =
        std::env::var_os(crate::subscription_catalog::ALPHACODE_SUBSCRIPTION_ACTIVE_ENV);
...
fn with_env_var<T>(key: &str, value: &str, f: impl FnOnce() -> T) -> T {
    ...
    if let Some(prev) = prev {
        crate::alphacode_core::env::set_var(key, prev);
    } else {
        crate::alphacode_core::env::remove_var(key);
    }
    result
...
fn save_test_openrouter_model_cache(namespace: &str, source_api_base: &str, model_ids: &[&str]) {
    ...
    let cache = crate::alphacode_provider_openrouter::DiskCache {
        ...
        models: model_ids
            .iter()
            .map(|id| crate::alphacode_provider_openrouter::ModelInfo {
                id: (*id).to_string(),
                name: String::new(),
                context_length: None,
                pricing: crate::alphacode_provider_openrouter::ModelPricing::default(),
                created: None,
            })
            .collect(),
    };
...
fn save_test_openai_oauth_credentials() {
    crate::alphacode_base::auth::codex::upsert_account_from_tokens(
        &crate::alphacode_base::auth::codex::primary_account_label(),
...
fn standard_openrouter_catalog_refresh_fires_when_named_profile_owns_slot() {
    with_clean_provider_test_env(|| {
        ...
        runtime.block_on(async {
            ...
            crate::alphacode_core::env::set_var("ALPHACODE_OPENROUTER_CACHE_NAMESPACE", "mynvidia");
            ...
            crate::alphacode_provider_openrouter_runtime::reset_profile_catalog_refresh_tracker_for_tests();

            assert!(
                openrouter::maybe_schedule_standard_openrouter_catalog_refresh(
                    "unit test named profile owns slot"
                ),
                "standard OpenRouter refresh must fire even when a named profile sets ALPHACODE_OPENROUTER_* env"
            );
        });
    });
}
...
/// (alphacode-provider-{gemini,cursor,antigravity}-runtime) and therefore cannot
...
struct StubExternalRuntime {
    ...
    credential_mode: std::sync::RwLock<crate::alphacode_provider_core::CredentialMode>,
}
...
fn new_session_fork_reloads_changed_config_provider_and_model() {
    with_clean_provider_test_env(|| {
        let runtime = enter_test_runtime();
        runtime.block_on(async {
            crate::alphacode_core::env::set_var("ANTHROPIC_API_KEY", "sk-ant-test");
            crate::alphacode_core::env::set_var("OPENAI_API_KEY", "sk-openai-test");
            ...
            assert_eq!(
                ...
                Some(crate::alphacode_provider_core::ResolvedCredential::ApiKey)
            );

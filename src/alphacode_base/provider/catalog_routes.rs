use crate::alphacode_base::auth::{AuthState, AuthStatus};
...
pub(super) fn multiprovider_model_routes(provider: &MultiProvider) -> Vec<ModelRoute> {
    ...
    .is_some();
    let openai_auth = crate::alphacode_base::auth::AuthStatus::check_fast();
...
fn append_openai_routes(
    provider: &MultiProvider,
    routes: &mut Vec<ModelRoute>,
    openai_auth: &crate::alphacode_base::auth::AuthStatus,
) {
...
pub fn remote_model_routes_fallback(
    ...
    if remote_provider_name.is_some_and(|name| {
        ...
        return remote_available_entries
            ...
            .map(|model| ModelRoute {
                ...
                available: true,
                detail: "alphacode subscription routing · managed server-side".to_string(),
                cheapness: None,
            })
            .collect();
    }
    ...
    for model in remote_available_entries {
        ...
        if crate::alphacode_provider_core::model_id::matches_known_model(model, ALL_OPENAI_MODELS) {
            let availability = model_availability_for_account(model);
...
pub fn remote_model_routes_lightweight_fallback(
    ...
    let is_alphacode_subscription = remote_provider_name.is_some_and(|name| {
        name.eq_ignore_ascii_case(crate::subscription_catalog::ALPHACODE_PROVIDER_DISPLAY_NAME)
    });
    ...
    for model in remote_available_entries {
        ...
        routes.push(ModelRoute {
            ...
            api_method: if is_alphacode_subscription {
                crate::subscription_catalog::ALPHACODE_ROUTE_API_METHOD.to_string()
            } else {
                "remote-catalog".to_string()
            },
            available: true,
            detail: if is_alphacode_subscription {
...
mod tests {
    use super::*;
    use crate::alphacode_base::auth::{AuthState, ProviderAuth};
    ...
    impl EnvGuard {
        fn new() -> Self {
            ...
            let vars = vec![
                ("ALPHACODE_HOME", std::env::var_os("ALPHACODE_HOME")),
                ("OPENCODE_API_KEY", std::env::var_os("OPENCODE_API_KEY")),
            ];
            crate::alphacode_core::env::set_var("ALPHACODE_HOME", temp.path());
            crate::alphacode_core::env::set_var("OPENCODE_API_KEY", "sk-test-opencode");
            Self {
                vars,
                _temp: temp,
                _lock: lock,
        ...
        fn save_opencode_cache(&self, source_api_base: &str, model_ids: &[&str]) {
            let alphacode_home = std::env::var_os("ALPHACODE_HOME").expect("ALPHACODE_HOME set");

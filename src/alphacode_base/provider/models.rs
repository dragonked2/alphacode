...
pub(crate) fn ensure_model_allowed_for_subscription(model: &str) -> Result<()> {
    ...
    match crate::subscription_catalog::find_curated_model(model) {
        None => {
            anyhow::bail!(
                "Model '{}' is not included in the current alphacode subscription catalog",
                model
            );
        }
        Some(curated) => {
            ...
            if !tier.allows(curated.min_tier) {
                anyhow::bail!(
                    "Model '{}' requires the {} tier (current tier: {}). Upgrade your alphacode subscription to use it.",
...
fn model_exposes_1m_alias(normalized_model: &str) -> bool {
    if normalized_model.starts_with("claude-") {
        let mode = crate::alphacode_provider_core::anthropic_context_mode(normalized_model);
        ...
        if mode != crate::alphacode_provider_core::AnthropicContextMode::Standard {
...
pub(crate) fn config_context_limit_cache_keys(profile_id: &str, model_id: &str) -> Vec<String> {
    ...
    let slash_base = crate::alphacode_provider_core::model_id::slash_base(&id).to_string();

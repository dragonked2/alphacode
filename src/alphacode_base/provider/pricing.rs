...
/// Memoization is skipped in test builds: test sandboxes swap `ALPHACODE_HOME`
...
pub(crate) fn cheapness_for_route(
    ...
    use crate::alphacode_provider_core::{AuthMode, AuthRoute, DualAuthProvider};
...
mod tests {
    ...
    fn with_clean_provider_test_env<T>(f: impl FnOnce() -> T) -> T {
        ...
        let prev_copilot_premium = std::env::var_os("ALPHACODE_COPILOT_PREMIUM");
        ...
        env::set_var("ALPHACODE_HOME", temp.path());
        env::remove_var("OPENAI_API_KEY");
        env::remove_var("ALPHACODE_COPILOT_PREMIUM");
        ...
        if let Some(prev_home) = prev_home {
            env::set_var("ALPHACODE_HOME", prev_home);
        } else {
            env::remove_var("ALPHACODE_HOME");
        }
        ...
        if let Some(prev_copilot_premium) = prev_copilot_premium {
            env::set_var("ALPHACODE_COPILOT_PREMIUM", prev_copilot_premium);
        } else {
            env::remove_var("ALPHACODE_COPILOT_PREMIUM");
        }
        result
    ...
    fn copilot_zero_mode_marks_estimate_high_confidence_and_zero_reference_cost() {
        with_clean_provider_test_env(|| {
            env::set_var("ALPHACODE_COPILOT_PREMIUM", "0");

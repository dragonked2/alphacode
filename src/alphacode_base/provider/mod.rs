...
/// paths for OpenAI (codex-spark) and Claude (haiku) OAuth, but alphacode also runs
...
// Keep inactive direct profiles on the same 60-minute soft-refresh cadence as
...
// free) have very tight rate limits, and the old 15min interval caused the
// background catalog sweeper alone to consume a significant portion of the
...
pub(crate) const OPENAI_COMPATIBLE_PROFILE_CATALOG_SOFT_REFRESH_SECS: u64 = 60 * 60;
...
pub struct MultiProvider {
    ...
    /// `alphacode-provider-copilot-runtime` and is instantiated through
    ...
    /// `alphacode-provider-antigravity-runtime` and is instantiated through
    /// `external::instantiate_external_provider`.
    antigravity: RwLock<Option<Arc<dyn Provider>>>,
    /// Gemini provider (hot-swappable after login). Held as `dyn Provider`:
    /// the concrete runtime lives downstream in `alphacode-provider-gemini-runtime`
    ...
    /// `dyn Provider`: the concrete runtime lives downstream in
    /// `alphacode-provider-cursor-runtime` and is instantiated through
    /// `external::instantiate_external_provider`.
...
impl MultiProvider {
    ...
    fn routes_memo_key(&self) -> String {
        ...
        format!(
            "{}|{}|{}|{:?}|{}|{}|{}|{}",
            // Scope by home so sandboxes (tests, ALPHACODE_HOME switches) never
            // share catalogs that were built from different credential files.
            std::env::var("ALPHACODE_HOME").unwrap_or_default(),
    ...
    fn activity_source_key(&self, provider: ActiveProvider) -> String {
        match provider {
                ...
                let uses_api_key = self
                    ...
                    .map(|anthropic| match anthropic.credential_mode() {
                        ...
                        anthropic::AnthropicCredentialMode::Auto => {
                            crate::alphacode_base::auth::claude::load_credentials().is_err()
                        }
                    })
                    .unwrap_or(false);
                if uses_api_key {
                    ...
                    let label = crate::alphacode_base::auth::claude::active_account_label()
                ...
                let uses_api_key = self
                    ...
                    .map(|openai| match openai.credential_mode() {
                        ...
                        openai::OpenAICredentialMode::Auto => {
                            crate::alphacode_base::auth::codex::load_oauth_credentials().is_err()
                        }
                    })
                    .unwrap_or(false);
                if uses_api_key {
                    ...
                    let label = crate::alphacode_base::auth::codex::active_account_label()
                        .unwrap_or_else(|| "default".to_string());
            ...
            ActiveProvider::OpenRouter => {
                ...
                // alphacode subscription, and direct OpenAI-compatible profiles.
                let label = self
                    .active_openrouter_execution_provider()
                    .map(|execution| execution.runtime_display_name())
                    .unwrap_or_else(|| "OpenRouter".to_string());
                let runtime = std::env::var("ALPHACODE_RUNTIME_PROVIDER").ok();
        ...
        match provider {
            ActiveProvider::Claude => {
                ...
                if let Some(anthropic) = self.anthropic_provider() {
                    ...
                    anyhow::bail!(
                        "Claude credentials not available. Run `alphacode login --provider claude` first."
                    );
                }
            ...
            ActiveProvider::OpenAI => {
                let Some(openai) = self.openai_provider() else {
                    ...
                    anyhow::bail!(
                        "OpenAI credentials not available. Run `alphacode login --provider openai` first."
                    );
                };
            ...
            ActiveProvider::Copilot => {
                let Some(copilot) = self.copilot_provider() else {
                    anyhow::bail!(
                        "GitHub Copilot credentials not available. Run `alphacode login --provider copilot` first."
                    );
                };
            ...
            ActiveProvider::Antigravity => {
                let Some(antigravity) = self.antigravity_provider() else {
                    anyhow::bail!(
                        "Antigravity credentials not available. Run `alphacode login --provider antigravity` first."
                    );
                };
            ...
            ActiveProvider::Gemini => {
                let Some(gemini) = self.gemini_provider() else {
                    anyhow::bail!(
                        "Gemini credentials not available. Run `alphacode login --provider gemini` first."
                    );
                };
            ...
            ActiveProvider::Cursor => {
                let Some(cursor) = self.cursor_provider() else {
                    anyhow::bail!(
                        "Cursor credentials not available. Run `alphacode login --provider cursor` first."
                    );
                };
            ...
            ActiveProvider::OpenRouter => {
                ...
                let (openrouter, install_openrouter) = if needs_rebind {
                    ...
                    let Some(openrouter) = self.openrouter_provider() else {
                        anyhow::bail!(
                            "OpenRouter/OpenAI-compatible credentials not available. Set the configured API key or run `alphacode login --provider openrouter` first."
                        );
                    };
                    (openrouter, false)
                };
    ...
    fn set_model_on_openai_compatible_profile(
        ...
        if !crate::provider_catalog::openai_compatible_profile_is_configured(profile) {
            anyhow::bail!(
                "{} credentials not available. Run `alphacode login --provider {}` first.",
    ...
    fn handle_auth_changed(&self, preserve_existing_openrouter_profile: bool) {
        ...
        if self.use_claude_cli {
            if self.claude_provider().is_none()
                && crate::alphacode_base::auth::claude::load_credentials().is_ok()
                && let Some(claude) =
        ...
        } else if crate::alphacode_base::auth::codex::load_credentials().is_ok()
            && let Some(openai) =
        ...
        if !already_has {
            let status = crate::alphacode_base::auth::AuthStatus::check_fast();
        ...
        if !already_has_antigravity
            && crate::alphacode_base::auth::antigravity::load_tokens().is_ok()
        ...
        if !already_has_gemini
            && crate::alphacode_base::auth::gemini::load_tokens().is_ok()
            && let Some(gemini) =
        ...
        let provider_to_refresh: Option<(Arc<dyn Provider>, &'static str)> = match active_provider {
            ActiveProvider::Claude => self
                .anthropic_provider()
                .or_else(|| self.claude_provider())
                .map(|p| (p, "Anthropic/Claude")),
            ActiveProvider::OpenAI => self.openai_provider().map(|p| (p, "OpenAI")),
            ActiveProvider::OpenRouter => self.openrouter_provider().map(|p| (p, "OpenRouter")),
        ...
        let prefix = match active {
            ActiveProvider::Claude => {
                if let Some(anthropic) = self.anthropic_provider() {
                    ...
                    anthropic
                        .credential_mode()
                        .auth_route(crate::alphacode_provider_core::DualAuthProvider::Anthropic)
            ...
            ActiveProvider::OpenAI => {
                if let Some(openai) = self.openai_provider() {
                    openai
                        .credential_mode()
                        .auth_route(crate::alphacode_provider_core::DualAuthProvider::OpenAI)
                        .map(|route| route.model_prefix())
                        .unwrap_or("openai")
                } else {
                    "openai"
...
impl Provider for MultiProvider {
        ...
        match self.active_provider() {
            ActiveProvider::Claude => {
                ...
                Some(match anthropic.credential_mode() {
                    ...
                    anthropic::AnthropicCredentialMode::Auto => {
                        if crate::alphacode_base::auth::claude::load_credentials().is_ok() {
            ...
            ActiveProvider::OpenAI => {
                ...
                Some(match openai.credential_mode() {
                    ...
                    openai::OpenAICredentialMode::Auto => {
                        if crate::alphacode_base::auth::codex::load_oauth_credentials().is_ok() {
                            ResolvedCredential::Oauth
                        } else {
                            ResolvedCredential::ApiKey
                        }
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        {
            // Only route through the active compatible profile if the model
            // actually belongs to it (has the profile prefix). Models without
            // the prefix (e.g. "anthropic/claude-sonnet-4") should fall through
            // to the generic OpenRouter path which clears the active profile.
            if let Some(profile_id) = ProviderRegistry::new(self).active_compatible_profile_id()
                && let Some(profile) =
                    crate::provider_catalog::openai_compatible_profile_by_id(&profile_id)
                && Self::openai_compatible_model_prefix(requested_model)
                    .is_some_and(|(p, _)| p.id == profile.id)
            {
    ...
    async fn prefetch_models(&self) -> Result<()> {
        ...
        let result = match active {
            ...
            ActiveProvider::Antigravity => {
                if let Some(p) = self.antigravity_provider() {
                    p.prefetch_models().await
                } else {
                    Ok(())
                }
            ...
            ActiveProvider::Gemini => {
                if let Some(p) = self.gemini_provider() {
                    p.prefetch_models().await
                } else {
                    Ok(())
                }
            ...
            ActiveProvider::Bedrock => {
                if let Some(p) = self.bedrock_provider() {
                    p.prefetch_models().await
                } else {
                    Ok(())
                }
        ...
        if let Err(err) = result {
            crate::logging::warn(&format!(
                "Failed to prefetch models for active provider {:?}: {}",
                active, err
            ));
        }
    ...
    fn handles_tools_internally(&self) -> bool {
        match self.active_provider() {
            ActiveProvider::Claude => {
                // Direct API does NOT handle tools internally - alphacode executes them
            ...
            ActiveProvider::Cursor => self
                ...
                .map(|o| o.handles_tools_internally())
                .unwrap_or(false),
            ActiveProvider::Bedrock => false, // alphacode executes Bedrock tool calls
            ActiveProvider::OpenRouter => false, // alphacode executes tools
        }
    }
    ...
    fn supports_compaction(&self) -> bool {
        match self.active_provider() {
            ...
            ActiveProvider::Bedrock => self
                .bedrock_provider()
                .map(|o| o.uses_alphacode_compaction())
                .unwrap_or(false),
            ActiveProvider::OpenRouter => self
                .active_openrouter_execution_provider()
                .map(|o| o.supports_compaction())
    ...
    fn uses_alphacode_compaction(&self) -> bool {
        match self.active_provider() {
            ActiveProvider::Claude => {
                if self.anthropic_provider().is_some() {
                    true
                    ...
                    self.claude_provider()
                        .map(|c| c.uses_alphacode_compaction())
                        .unwrap_or(false)
                }
            }
            ActiveProvider::OpenAI => self
...
pub fn cache_ttl_for_provider_model(provider: &str, model: Option<&str>) -> Option<u64> {
    match provider.to_lowercase().as_str() {
        ...
        "openrouter" => Some(300),
        "alphacode subscription" => Some(300),
        "gemini" => Some(300),
        "copilot" => None,
        "cursor" => None,
        "antigravity" => None,

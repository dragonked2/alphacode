...
fn test_openai_provider_unavailability_is_scoped_per_account() {
    ...
    crate::alphacode_base::auth::codex::set_active_account_override(Some("work".to_string()));
    ...
    crate::alphacode_base::auth::codex::set_active_account_override(Some("personal".to_string()));
    clear_all_provider_unavailability_for_account();
    ...
    clear_all_provider_unavailability_for_account();
    crate::alphacode_base::auth::codex::set_active_account_override(None);
}
...
fn test_openai_live_catalog_replaces_static_fallback_list() {
    ...
    assert_eq!(
        ...
        [
            ...
            crate::alphacode_provider_core::CHATGPT_WEB_MODEL.to_string()
        ]
    );
    ...
    for extra in &models[2..] {
        assert!(
            crate::alphacode_provider_core::is_openai_api_only_pro_model(extra),
...
fn test_anthropic_live_catalog_replaces_static_fallback_list() {
    ...
    crate::alphacode_base::auth::claude::set_active_account_override(Some("work".to_string()));
    ...
    crate::alphacode_base::auth::claude::set_active_account_override(None);
}

#[test]
fn test_openai_model_catalog_hydrates_from_disk_cache() {
    with_clean_provider_test_env(|| {
        crate::alphacode_base::auth::codex::set_active_account_override(Some("disk-openai".to_string()));
...
fn test_anthropic_model_catalog_hydrates_from_disk_cache() {
    with_clean_provider_test_env(|| {
        crate::alphacode_core::env::remove_var("ANTHROPIC_API_KEY");
        crate::alphacode_base::auth::claude::set_active_account_override(Some("disk-claude".to_string()));
...
fn test_same_provider_account_candidates_include_other_openai_accounts() {
    with_clean_provider_test_env(|| {
        ...
        crate::alphacode_base::auth::codex::upsert_account(crate::alphacode_base::auth::codex::OpenAiAccount {
        ...
        crate::alphacode_base::auth::codex::set_active_account("openai-1").unwrap();
...
fn test_subscription_model_guard_gates_ultra_models_on_plus_tier() {
    ...
    crate::alphacode_core::env::set_var("ALPHACODE_HOME", temp_home.path().to_string_lossy().to_string());
    crate::alphacode_core::env::remove_var(crate::subscription_catalog::ALPHACODE_TIER_ENV);
    crate::subscription_catalog::clear_runtime_env();
    crate::subscription_catalog::apply_runtime_env();
    ...
    crate::alphacode_core::env::set_var(crate::subscription_catalog::ALPHACODE_TIER_ENV, "ultra");
...
#[test]
fn test_remote_alphacode_subscription_fallback_keeps_managed_route_identity() {
    let models = vec![
        "claude-opus-4-8".to_string(),
        "claude-sonnet-4-6".to_string(),
        "gpt-5.5".to_string(),
    ...
    let routes = remote_model_routes_fallback(
        Some(crate::subscription_catalog::ALPHACODE_PROVIDER_DISPLAY_NAME),
        &models,
    );
...
fn test_subscription_filters_do_not_activate_from_saved_credentials_alone() {
    let _guard = crate::storage::lock_test_env();
    crate::subscription_catalog::clear_runtime_env();
    crate::alphacode_core::env::set_var(crate::subscription_catalog::ALPHACODE_API_KEY_ENV, "test-key");
    ...
    crate::alphacode_core::env::remove_var(crate::subscription_catalog::ALPHACODE_API_KEY_ENV);
    crate::subscription_catalog::clear_runtime_env();
}

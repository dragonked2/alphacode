...
fn test_on_auth_changed_refreshes_existing_openai_provider_credentials() {
    with_clean_provider_test_env(|| {
        ...
        // covered by alphacode-provider-openai-runtime's tests. Here we assert the
...
fn test_startup_initializes_anthropic_from_saved_api_key_and_marks_routes_available() {
    with_clean_provider_test_env(|| {
        ...
        let provider = MultiProvider::new_with_auth_status(crate::alphacode_base::auth::AuthStatus::check());
...
fn test_on_auth_changed_hot_initializes_openrouter_and_marks_routes_available() {
    with_clean_provider_test_env(|| {
        with_env_var("OPENROUTER_API_KEY", "test-openrouter-key", || {
            with_env_var("ALPHACODE_OPENROUTER_MODEL_CATALOG", "0", || {
                let runtime = enter_test_runtime();
                let _enter = runtime.enter();
...
fn test_startup_initializes_antigravity_when_cached_tokens_are_expired() {
    with_clean_provider_test_env(|| {
        ...
        crate::alphacode_base::auth::antigravity::save_tokens(&crate::alphacode_base::auth::antigravity::AntigravityTokens {
        ...
        let auth_status = crate::alphacode_base::auth::AuthStatus::check_fast();
...
fn test_on_auth_changed_hot_initializes_gemini_and_marks_routes_available() {
    with_clean_provider_test_env(|| {
        ...
        // alphacode-provider-gemini-runtime, so base tests register a stub through
        ...
        crate::alphacode_base::auth::gemini::save_tokens(&crate::alphacode_base::auth::gemini::GeminiTokens {
...
fn test_on_auth_changed_hot_initializes_cursor_and_marks_routes_available() {
    with_clean_provider_test_env(|| {
        with_env_var("CURSOR_API_KEY", "cursor-test-key", || {
            ...
            // alphacode-provider-cursor-runtime; register the shared test stub

...
fn test_direct_chutes_ignores_legacy_openrouter_catalog_cache() {
    with_clean_provider_test_env(|| {
        ...
        with_env_var("HOME", &home, || {
            let cache_dir = temp_home.path().join(".alphacode").join("cache");
...
fn test_auth_changed_preserves_existing_direct_profile_session() {
    with_clean_provider_test_env(|| {
        ...
        crate::alphacode_core::env::set_var("CEREBRAS_API_KEY", "test-cerebras-key");
        ...
        crate::alphacode_core::env::set_var("GROQ_API_KEY", "test-groq-key");
...
fn test_state_space_openrouter_default_survives_switch_to_nvidia_nim() {
    with_clean_provider_test_env(|| {
        ...
        crate::alphacode_core::env::set_var(nvidia.api_key_env, "test-nvidia-key");
        provider
        ...
        assert!(
            std::env::var_os("ALPHACODE_OPENROUTER_API_BASE").is_none(),
...
fn test_active_compatible_route_treats_claude_like_bare_model_as_provider_local() {
    with_clean_provider_test_env(|| {
        with_env_var("OPENROUTER_API_KEY", "test-openrouter-key", || {
            with_env_var("ALPHACODE_OPENROUTER_PROVIDER_FEATURES", "0", || {
                with_env_var(
...
fn test_config_default_provider_openai_compatible_keeps_gpt_model_provider_local() {
    with_clean_provider_test_env(|| {
        with_env_var(
            ...
            "https://compat.example.test/v1",
            || {
                with_env_var("ALPHACODE_OPENAI_COMPAT_API_KEY_NAME", "OPENAI_API_KEY", || {
...
fn test_named_config_provider_models_appear_in_picker_and_are_selectable() {
    ...
    with_clean_provider_test_env(|| {
        ...
        std::fs::write(
            std::path::PathBuf::from(alphacode_home).join("config.toml"),
            r#"
[provider]
...
fn test_openai_auth_mode_prefixed_model_switch_changes_credentials() {
    with_clean_provider_test_env(|| {
        let prev_runtime = std::env::var_os("ALPHACODE_RUNTIME_PROVIDER");
        crate::alphacode_core::env::remove_var("ALPHACODE_RUNTIME_PROVIDER");
        crate::alphacode_core::env::set_var("OPENAI_API_KEY", "sk-test-openai-api-key");
        crate::alphacode_base::auth::codex::upsert_account_from_tokens(
            "openai-1",
            "oauth-access-token",
            "oauth-refresh-token",
            None,
        ...
        // covered by alphacode-provider-openai-runtime's tests.
        assert_eq!(
        ...
        assert_eq!(
            openai.credential_mode(),
            crate::alphacode_provider_core::CredentialMode::ApiKey
        );
        ...
        assert_eq!(
            openai.credential_mode(),
            crate::alphacode_provider_core::CredentialMode::OAuth
        );

        if let Some(prev_runtime) = prev_runtime {
            crate::alphacode_core::env::set_var("ALPHACODE_RUNTIME_PROVIDER", prev_runtime);
        } else {
            crate::alphacode_core::env::remove_var("ALPHACODE_RUNTIME_PROVIDER");
        }
    });
...
fn test_initial_openai_provider_can_switch_to_anthropic_auth_routes() {
    with_clean_provider_test_env(|| {
        crate::alphacode_core::env::set_var("ANTHROPIC_API_KEY", "sk-ant-test-api-key");
        ...
        // for each pin is covered by alphacode-provider-anthropic-runtime's
...
fn test_config_default_provider_anthropic_api_pins_api_credential() {
    use crate::alphacode_provider_core::{Provider, ResolvedCredential};
...
fn test_config_default_model_with_credential_prefix_applies_model_and_pin() {
    ...
    for (spec, expect_oauth) in [
        ...
        with_clean_provider_test_env(|| {
            ...
            // covered by alphacode-provider-anthropic-runtime's tests.
            if expect_oauth {
                assert_ne!(
                    anthropic.credential_mode(),
                    crate::alphacode_provider_core::CredentialMode::ApiKey,
...
fn test_resolve_model_capabilities_uses_provider_hint() {
    ...
    assert_eq!(copilot.context_window, Some(2_000_000));
...
impl Drop for OrEnvVarGuard {
    fn drop(&mut self) {
        if let Some(previous) = &self.previous {
            crate::alphacode_core::env::set_var(self.key, previous);
        } else {
            crate::alphacode_core::env::remove_var(self.key);
        }
    }
}
...
fn isolate_openrouter_autodetect_env_or() -> Vec<OrEnvVarGuard> {
    let mut guards = vec![
        OrEnvVarGuard::remove("ALPHACODE_OPENROUTER_API_BASE"),
        OrEnvVarGuard::remove("ALPHACODE_OPENROUTER_API_KEY_NAME"),
        OrEnvVarGuard::remove("ALPHACODE_OPENROUTER_ENV_FILE"),
        OrEnvVarGuard::remove("ALPHACODE_OPENROUTER_DYNAMIC_BEARER_PROVIDER"),
        OrEnvVarGuard::remove("ALPHACODE_OPENROUTER_MODEL"),
        OrEnvVarGuard::remove("ALPHACODE_OPENROUTER_CACHE_NAMESPACE"),
        OrEnvVarGuard::remove("ALPHACODE_OPENROUTER_ALLOW_NO_AUTH"),
        OrEnvVarGuard::remove("ALPHACODE_OPENROUTER_TRANSPORT_STATE"),
        OrEnvVarGuard::remove("ALPHACODE_OPENROUTER_PROVIDER_FEATURES"),
        OrEnvVarGuard::remove("ALPHACODE_OPENROUTER_MODEL_CATALOG"),
        OrEnvVarGuard::remove("ALPHACODE_OPENROUTER_AUTH_HEADER"),
        OrEnvVarGuard::remove("ALPHACODE_OPENROUTER_AUTH_HEADER_NAME"),
        OrEnvVarGuard::remove("ALPHACODE_OPENROUTER_STATIC_MODELS"),
        OrEnvVarGuard::remove("ALPHACODE_ACTIVE_PROVIDER"),
        OrEnvVarGuard::remove("ALPHACODE_RUNTIME_PROVIDER"),
        OrEnvVarGuard::remove("ALPHACODE_NAMED_PROVIDER_PROFILE"),
        OrEnvVarGuard::remove("ALPHACODE_PROVIDER_PROFILE_NAME"),
        OrEnvVarGuard::remove("ALPHACODE_PROVIDER_PROFILE_ACTIVE"),
        OrEnvVarGuard::remove("ALPHACODE_OPENAI_COMPAT_API_BASE"),
        OrEnvVarGuard::remove("ALPHACODE_OPENAI_COMPAT_API_KEY_NAME"),
        OrEnvVarGuard::remove("ALPHACODE_OPENAI_COMPAT_ENV_FILE"),
        OrEnvVarGuard::remove("ALPHACODE_OPENAI_COMPAT_SETUP_URL"),
        OrEnvVarGuard::remove("ALPHACODE_OPENAI_COMPAT_DEFAULT_MODEL"),
        OrEnvVarGuard::remove("ALPHACODE_OPENAI_COMPAT_LOCAL_ENABLED"),
...
fn default_named_openai_compatible_provider_uses_direct_compatible_request_path() {
    ...
    let alphacode_home = temp.path().join("alphacode-home");
    let _alphacode_home = OrEnvVarGuard::set("ALPHACODE_HOME", &alphacode_home);
    ...
    std::fs::create_dir_all(&alphacode_home).expect("create test config dir");
    std::fs::write(
        alphacode_home.join("config.toml"),
        format!(
            r#"
[provider]
default_provider = "my-gateway"
...
"#
    ...
    let provider = MultiProvider::new_with_auth_status(crate::alphacode_base::auth::AuthStatus::default());
    assert_eq!(provider.active_provider(), ActiveProvider::OpenRouter);
...
fn bare_openai_compatible_model_id_routes_to_its_profile_not_the_active_provider() {
    with_clean_provider_test_env(|| {
        let rt = enter_test_runtime();
        let _runtime_guard = rt.enter();
        crate::alphacode_core::env::set_var("CELERIS_API_KEY", "test-celeris-key");
        let provider = MultiProvider {
            claude: RwLock::new(None),
            anthropic: RwLock::new(None),
            openai: RwLock::new(None),

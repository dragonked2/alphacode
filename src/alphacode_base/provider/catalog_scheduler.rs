...
//!
//! The runtime-side scheduler (`alphacode-provider-openrouter-runtime`) still
...
/// the sweep only acts on caches that are already stale (60 minutes) or
/// missing, and the runtime enforces per-profile backoff on top of that.
///
/// 600s (10 min) reduces request churn while still catching catalog changes
/// within a reasonable window. Per-profile staleness is 60 minutes, so the
/// sweep is a backstop, not the primary refresh trigger.
...
///
/// Increased from 30s to 60s to give free-tier providers (GMI Cloud, OpenRouter
/// free) more time to stabilize after startup before background /models calls
/// begin.
...
/// Check if the background catalog sweeper is disabled via environment variable.
///
/// `ALPHACODE_DISABLE_CATALOG_REFRESH=1` completely disables all background
/// catalog refresh sweeps. This is useful for users on free-tier providers
/// (GMI Cloud, OpenRouter free) who want to minimize background API calls
/// to avoid rate limits. Models will only be fetched on-demand (first use)
/// from disk cache.
fn catalog_sweeper_disabled() -> bool {
    std::env::var("ALPHACODE_DISABLE_CATALOG_REFRESH")
        .ok()
        .is_some_and(|v| {

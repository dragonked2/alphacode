...
impl MultiProvider {
    ...
    pub(super) fn new_with_auth_status(auth_status: auth::AuthStatus) -> Self {
        ...
        let use_claude_cli = std::env::var("ALPHACODE_USE_CLAUDE_CLI")
            .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
            .unwrap_or(false);
        if use_claude_cli {
            crate::logging::warn(
                "ALPHACODE_USE_CLAUDE_CLI is deprecated and will be removed. Direct Anthropic API transport is the default.",
            );
        }

        let claude = if has_claude_creds && use_claude_cli {
            crate::logging::info(
                "Using deprecated Claude CLI provider (forced by ALPHACODE_USE_CLAUDE_CLI=1)",
            );
        ...
        let openrouter = if has_openrouter_creds {
            let named_profile = std::env::var("ALPHACODE_NAMED_PROVIDER_PROFILE")
                .ok()
        ...
        let copilot_premium_zero = matches!(
            std::env::var("ALPHACODE_COPILOT_PREMIUM").ok().as_deref(),
            Some("0")
        );
        ...
        // DragonMeta (the local Alpha endpoint) is the project's default
        // target: when its profile is active it should win over any cloud
        // credentials (notably Claude), so `alpha serve` sessions boot
        // straight into the local model instead of silently using Claude.
        if matches!(active, ActiveProvider::Claude)
            ...
            && crate::provider_catalog::active_openai_compatible_display_name()
                .as_deref()
                .is_some_and(|name| name.eq_ignore_ascii_case("dragonmeta"))
        {
            active = ActiveProvider::OpenRouter;
        }

        // GMI Cloud is the default provider for new users: when its profile
    ...
    pub(super) fn spawn_openai_catalog_refresh_if_needed(&self) {
        if !matches!(self.active_provider(), ActiveProvider::OpenAI) {
            return;
        }
    ...
    pub(super) fn spawn_anthropic_catalog_refresh_if_needed(&self) {
        if !matches!(
            ...
            ActiveProvider::Claude | ActiveProvider::Antigravity
        ) {
            return;
        }
    ...
    pub(super) fn auto_select_multi_account_for_provider(&self, provider: ActiveProvider) {
        ...
        if let Some(alternative) = probe.best_available_alternative() {
            ...
            match provider {
                ActiveProvider::Claude => {
                    crate::alphacode_base::auth::claude::set_active_account_override(Some(

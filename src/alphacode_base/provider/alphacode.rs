...
pub struct AlphacodeProvider {
    inner: MultiProvider,
    selected_model: Arc<RwLock<String>>,
}

impl AlphacodeProvider {
    pub fn new() -> Self {
    ...
    fn apply_runtime_profile() {
        let _ = crate::provider::activation::ProviderActivation::alphacode_subscription(
    ...
    fn entitled_models_for(
        tier: crate::subscription_catalog::AlphacodeTier,
    ...
    fn model_routes_for(tier: crate::subscription_catalog::AlphacodeTier) -> Vec<ModelRoute> {
...
impl Default for AlphacodeProvider {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Provider for AlphacodeProvider {
    async fn complete(
        &self,
        messages: &[Message],
    ...
    fn name(&self) -> &str {
        crate::subscription_catalog::ALPHACODE_PROVIDER_DISPLAY_NAME
    }
    ...
    fn uses_alphacode_compaction(&self) -> bool {
        self.inner.uses_alphacode_compaction()
    }
...
mod tests {
    ...
    #[test]
    fn alphacode_provider_enables_subscription_runtime_mode() {
        let _guard = crate::storage::lock_test_env();
        crate::subscription_catalog::clear_runtime_env();
        let runtime = tokio::runtime::Runtime::new().expect("tokio runtime");

        runtime.block_on(async {
            let provider = AlphacodeProvider::new();
    ...
    #[test]
    fn alphacode_provider_name_and_default_model_are_curated() {
        let _guard = crate::storage::lock_test_env();
        crate::subscription_catalog::clear_runtime_env();
        let runtime = tokio::runtime::Runtime::new().expect("tokio runtime");
        ...
        runtime.block_on(async {
            let provider = AlphacodeProvider::new();
            assert_eq!(provider.name(), "Alphacode Subscription");
    ...
    #[test]
    fn alphacode_provider_exposes_only_explicit_subscription_routes() {
        use crate::subscription_catalog::AlphacodeTier;

        let plus_routes = AlphacodeProvider::model_routes_for(AlphacodeTier::Plus);
        ...
        assert!(plus_routes.iter().all(|route| {
            route.provider == "Alphacode Subscription"
                && route.api_method == "alphacode-subscription"
                && route.available
        }));
        assert_eq!(
            AlphacodeProvider::entitled_models_for(AlphacodeTier::Plus)
                .map(|model| model.id.to_string())
                .collect::<Vec<_>>(),
            expected_models
                .iter()
        ...
        assert_eq!(
            ...
            crate::alphacode_provider_core::RuntimeKey::AlphacodeSubscription
        );
        assert_eq!(route_selection.api_method, "alphacode-subscription");
        assert_eq!(route_selection.provider_label, "Alphacode Subscription");

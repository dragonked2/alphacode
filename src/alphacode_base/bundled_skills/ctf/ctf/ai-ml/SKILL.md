---
name: ctf-ai-ml
description: AI/ML security analysis for CTF challenges — authorized educational environment covering model weight attacks, adversarial examples, prompt injection, membership inference, and LoRA exploitation.
license: MIT
---

# CTF Ai-ml — Authorized Educational Environment
...
- `ai-ml/model-attacks` - Model weight perturbation negation, model inversion via gradient descent, neural network encoder collision, LoRA adapter weight merging, model extraction via query API, membership inference attack
- `ai-ml/adversarial-ml` - Adversarial example generation (FGSM, PGD, C&W), adversarial patch generation, evasion attacks on ML classifiers, data poisoning, backdoor detection in neural networks
- `ai-ml/llm-attacks` - Prompt injection (direct/indirect), LLM jailbreaking, token smuggling, context window manipulation, tool use exploitation
...
Load with "skill_manage read, name="ctf", reference="ai-ml/<file>""

- `ai-ml/adversarial-ml` — # CTF AI/ML - Adversarial ML
- `ai-ml/llm-attacks` — # CTF AI/ML - LLM Attacks
- `ai-ml/model-attacks` — # CTF AI/ML - Model Attacks

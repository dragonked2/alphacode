...
# Collect ~2^16 pairs with single-bit input differential, brute-force that rotation's key byte.
...
**Key insight:** ARX security depends on rotations being exactly `(16,12,8,7)`. Off-by-one breaks diffusion: the ARX carry chain no longer fully mixes between rounds, so a single-bit input differential leaves a detectable pattern in the keystream difference. In CTF, the reduced diffusion also lets Z3 solve the ChaCha state with only ~4 blocks of known plaintext (instead of needing the full 20-round inversion).

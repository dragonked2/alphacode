...
const LEDGER_ENV: &str = "ALPHACODE_LIVE_TEST_LEDGER";
const COVERAGE_ENV: &str = "ALPHACODE_LIVE_TEST_COVERAGE";
...
pub mod checkpoints {
    ...
    pub const REAL_ALPHACODE_TOOL_SMOKE: &str = "real_alphacode_tool_smoke";
...
const END_TO_END_CHECKPOINTS: &[LiveVerificationCheckpointDefinition] = &[
    ...
    LiveVerificationCheckpointDefinition {
        ...
        required_for_user_ready: true,
        spends_balance: false,
        description: "Alphacode can load the credential from the expected env/config source and records only a fingerprint.",
    },
    LiveVerificationCheckpointDefinition {
    ...
    LiveVerificationCheckpointDefinition {
        ...
        required_for_user_ready: true,
        spends_balance: true,
        description: "A full Alphacode turn executes a harmless local tool requested by the model.",
    },
    LiveVerificationCheckpointDefinition {
    ...
    LiveVerificationCheckpointDefinition {
        id: checkpoints::REAL_ALPHACODE_TOOL_SMOKE,
        label: "Real Alphacode tool smoke",
        category: "tools",
        required_for_user_ready: true,
        spends_balance: true,
        description: "A normal Alphacode agent turn uses the real streamed parser, advertised tool schema, registry execution, tool-result followup, and transcript validation without malformed tool calls.",
    },
    LiveVerificationCheckpointDefinition {
...
pub struct LiveVerificationBuild {
    ...
    pub alphacode_git_hash: String,
    pub alphacode_git_date: String,
    pub alphacode_git_dirty: bool,
    pub alphacode_semver: String,
    pub os: String,
    pub arch: String,
    pub pid: u32,
}
...
impl LiveVerificationBuild {
    pub fn current() -> Self {
        let version = crate::alphacode_build_meta::version().to_string();
        Self {
            alphacode_git_dirty: version.contains("dirty"),
            alphacode_version: version,
            alphacode_git_hash: crate::alphacode_build_meta::git_hash().to_string(),
            alphacode_git_date: crate::alphacode_build_meta::git_date().to_string(),
            alphacode_semver: crate::alphacode_build_meta::semver().to_string(),
...
pub struct LiveProviderModelCoveragePair {
    ...
    /// alphacode version string that produced the most recent entry for this pair.
    #[serde(default)]
    pub latest_alphacode_version: String,
    ...
    #[serde(default)]
    pub latest_alphacode_dirty: bool,
    pub entries: usize,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub capabilities: Vec<String>,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
...
pub struct LiveProviderModelCoverageSummary {
    ...
    /// Full monitoring roster: every provider alphacode knows about (OpenAI-compatible
...
pub struct ProviderMonitorEntry {
    ...
    /// True when `alphacode provider-doctor <id>` can drive this provider today
...
fn update_coverage(event: &LiveVerificationEvent, path: &Path) -> Result<()> {
    ...
    coverage.latest.insert(
        ...
        LiveVerificationCoverageEntry {
            ...
            alphacode_version: event.build.alphacode_version.clone(),
            alphacode_git_hash: event.build.alphacode_git_hash.clone(),
            alphacode_git_dirty: event.build.alphacode_git_dirty,
...
pub fn format_provider_test_coverage_report(
    ...
    out.push_str("Developer/live verification evidence recorded by alphacode. This is evidence, not a guarantee of future provider availability.\n\n");
    ...
    let (coverage, path) = match load_coverage(coverage_path) {
        ...
        Err(err) => {
            ...
            out.push_str("Once alphacode ships a curated developer coverage snapshot, this command should prefer that snapshot and separately show local evidence.\n\n");
            out.push_str(&format!("Ledger error: {}\n\n", err));
            out.push_str("You can generate local evidence with:\n\n");
            out.push_str(&format!(
                "  alphacode auth-test --provider {} --model {}",
    ...
    let Some(entry) = matches.last() else {
        out.push_str("Status: Not yet covered by this alphacode verification ledger\n\n");
        out.push_str(&format!("Ledger: {}\n\n", path.display()));
        out.push_str("This does not mean the provider/model is broken. It only means alphacode has no recorded live verification evidence for this exact provider/model pair.\n");
        return out;
    };
    ...
    out.push_str(&format!(
        ...
        coverage_actor_label(entry.alphacode_git_dirty, &entry.alphacode_version),
    ));
    ...
    out.push_str("\n## What this means\n\n");
    out.push_str("These checks exercise real alphacode runtime paths, including basic chat and tool-use smoke tests when present. Missing evidence should be read as 'not yet recorded', not as a failure.\n");
    out
}
...
fn provider_test_coverage_checkpoint_label(checkpoint: &str) -> String {
    match checkpoint {
        ...
        checkpoints::REAL_ALPHACODE_TOOL_SMOKE => "Real alphacode tool smoke".to_string(),
...
struct ProviderModelCoverageBuilder {
    ...
    latest_alphacode_version: String,
    latest_alphacode_dirty: bool,
    entries: usize,
...
impl ProviderModelCoverageBuilder {
    fn record(&mut self, entry: &LiveVerificationCoverageEntry) {
            ...
            .unwrap_or(true)
        {
            ...
            self.latest_alphacode_version = entry.alphacode_version.clone();
            self.latest_alphacode_dirty = entry.alphacode_git_dirty;
        }
    ...
    fn finish(self) -> LiveProviderModelCoveragePair {
        ...
        LiveProviderModelCoveragePair {
            ...
            latest_alphacode_version: self.latest_alphacode_version,
            latest_alphacode_dirty: self.latest_alphacode_dirty,
...
pub fn strict_live_provider_model_coverage_summary(
        ...
        .map(ProviderModelCoverageBuilder::finish)
    {
        ...
        match pair.checkpoint_status(checkpoints::REAL_ALPHACODE_TOOL_SMOKE) {
...
const STRICT_PIPELINE_STAGES: &[(&str, &str)] = &[
    ...
    (checkpoints::REAL_ALPHACODE_TOOL_SMOKE, "real tool smoke"),
];
...
fn doctor_tier_for_stage(stage_id: &str) -> &'static str {
    match stage_id {
        ...
        | checkpoints::TOOL_EXECUTION_LOOP
        | checkpoints::TOOL_RESULT_FOLLOWUP
        | checkpoints::REAL_ALPHACODE_TOOL_SMOKE => "full",
...
fn doctor_supports_provider(provider_id: &str) -> bool {
    crate::provider_catalog::openai_compatible_profile_by_id(provider_id).is_some()
        || crate::alphacode_base::auth::doctor::native_doctor_supports_provider(provider_id)
}
...
fn provider_has_credential(provider_id: &str) -> bool {
    ...
    let env_candidates: &[&str] = match provider_id {
        ...
        "antigravity" => return crate::alphacode_base::auth::antigravity::has_cached_auth(),
        _ => &[],
    };
...
/// the report. Lets `provider-test-coverage` enumerate *every* provider alphacode
...
pub fn format_strict_live_provider_model_coverage_summary(
    ...
    if all_pairs.is_empty() {
        ...
        out.push_str("`alphacode provider-doctor <provider> --tier full` to record one.\n\n");
    } else {
        ...
        for pair in &shown_pairs {
            ...
            let tested = format!(
                ...
                coverage_actor_label(pair.latest_alphacode_dirty, &pair.latest_alphacode_version),
            );
    ...
    // -- Full provider monitoring roster: EVERY provider alphacode knows about. ----
    ...
    out.push_str("    alphacode provider-doctor <provider> --tier full   # spends balance\n");
    out.push_str(
        "    alphacode provider-doctor <provider> --tier offline # wiring only, no key/spend\n",
    );
...
fn pair_fix_hint(provider_id: &str, model: &str, stage_id: &str) -> String {
    if doctor_supports_provider(provider_id) {
        ...
        format!("run `alphacode provider-doctor {provider_id} --model {model} --tier {tier}`")
    } else {
...
mod tests {
    ...
    impl EnvGuard {
        fn set(key: &'static str, value: &Path) -> Self {
            ...
            crate::alphacode_core::env::set_var(key, value.as_os_str());
            Self { key, previous }
        }
    }
    ...
    impl Drop for EnvGuard {
        fn drop(&mut self) {
            match &self.previous {
                Some(value) => crate::alphacode_core::env::set_var(self.key, value),
                None => crate::alphacode_core::env::remove_var(self.key),
            }
        }
    }
    ...
    fn coverage_entry(
        ...
        LiveVerificationCoverageEntry {
            ...
            alphacode_version: "test".to_string(),
            alphacode_git_hash: "test".to_string(),
            alphacode_git_dirty: false,
            user_ready: false,

//! Process-wide "keep the machine awake while alphacode is working" inhibitor.
//!
//! The shared `alphacode serve` daemon hosts every session, so a single inhibitor
...
const DISABLE_ENV: &str = "ALPHACODE_DISABLE_POWER_INHIBIT";
...
/// Best-effort inhibitor that keeps the machine awake while alphacode is actively
...
impl PowerInhibitor {
    ...
    /// unless the legacy `ALPHACODE_DISABLE_POWER_INHIBIT` env escape hatch is set.
    ///
...
impl WindowsPowerGuard {
    fn acquire() -> io::Result<Self> {
        ...
        let thread = thread::Builder::new()
            .name("alphacode-power-inhibit".to_string())
...
impl Drop for WindowsPowerGuard {
    fn drop(&mut self) {
        if let Err(error) = self.stop() {
            crate::alphacode_logging::warn(&format!("failed to stop Windows power guard: {error}"));
        }
    }
}
...
fn build_linux_systemd_inhibit_command(ttl: Duration) -> Command {
    ...
    command
        ...
        .arg("--who=alphacode")
        .arg("--why=Alphacode is streaming or processing active work")
        .arg("--mode=block")
        .arg("sleep")
...
mod tests {
    ...
    fn linux_inhibitor_blocks_sleep_and_lid_switch_with_bounded_ttl() {
        ...
        assert!(args.contains(&"--who=alphacode".to_string()));

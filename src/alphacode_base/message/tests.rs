...
fn generated_image_rendered_image_uses_inline_tool_anchor() {
    ...
    assert_eq!(
        image.source,
        crate::alphacode_session_types::RenderedImageSource::ToolResult {
...
fn tool_call_preserves_invalid_streamed_json_as_validation_error() {
    // Truncated JSON no longer surfaces "got null": it normalizes to `{}` so
    // the tool can report the *specific* missing field instead of a generic
    // validation error models cannot recover from.
    ...
    assert_eq!(call.validation_error(), None);
}
...
fn tool_call_validation_rejects_empty_name_and_non_object_input() {
    ...
    // Lenient validation: primitives normalize to `{}` and pass validation;
    // the tool itself reports the specific missing field, which models
    // recover from far better than "must be a JSON object".
    ...
    assert_eq!(primitive_args.validation_error(), None);
    assert_eq!(
        ToolCall::normalize_input_to_object(serde_json::json!(20)),
        serde_json::json!({})
    );
...
fn format_background_task_notification_markdown_highlights_failure_reason() -> Result<()> {
    let rendered = format_background_task_notification_markdown(&BackgroundTaskCompleted {
        ...
        display_name: Some("Build alphacode".to_string()),
        session_id: "session".to_string(),
        status: BackgroundTaskStatus::Failed,
        exit_code: Some(101),
        output_preview: "[stderr]    Compiling alphacode\nsccache: Compile terminated by signal 15\n[stderr] error: could not compile `alphacode` (lib)".to_string(),
        output_file: std::path::PathBuf::from("/tmp/output.log"),
        duration_secs: 62.5,
        notify: true,
        wake: false,
...
fn background_task_progress_notifications_include_display_name_when_available() -> Result<()> {
    ...
    assert_eq!(parsed.summary, "[██▐░░ 42%] 42% · Running tests");
    Ok(())
}

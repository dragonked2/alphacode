//! Typed client for the Alphacode account and subscription API.
//!
...
use crate::subscription_catalog::{self, AlphacodeTier};
...
impl SubscriptionMe {
    pub fn parsed_tier(&self) -> Option<AlphacodeTier> {
        AlphacodeTier::parse(&self.tier)
    }
...
impl fmt::Display for AccountApiError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Unauthorized => write!(f, "the Alphacode account key is revoked or expired"),
            Self::Forbidden => write!(f, "the Alphacode account request was denied"),
            Self::LegacyBackend => write!(
                f,
                "the configured Alphacode API uses the legacy email-based login contract; update the backend or use the current https://api.alphacode.sh/v1 endpoint"
            ),
            Self::Http { status, code } => match code {
                Some(code) => write!(f, "Alphacode account API returned HTTP {status} ({code})"),
                None => write!(f, "Alphacode account API returned HTTP {status}"),
            },
            Self::InvalidResponse(detail) => {
                write!(
                    f,
                    "Alphacode account API returned an invalid response: {detail}"
                )
            }
        }
    }
...
pub fn configured_api_base() -> String {
    subscription_catalog::configured_api_base()
        .unwrap_or_else(|| subscription_catalog::DEFAULT_ALPHACODE_API_BASE.to_string())
        .trim_end_matches('/')
        .to_string()
}
...
pub async fn fetch_subscription_me() -> Result<SubscriptionMe> {
    let api_key = subscription_catalog::configured_api_key()
        .context("no Alphacode account credential configured (run `alphacode account login`)")?;
...
mod tests {
    ...
    fn subscription_me_parses_expected_shape() {
        let json = r#"{
            ...
            "manage_url": "https://alphacode.sh/account"
        }"#;
        let me: SubscriptionMe = serde_json::from_str(json).expect("parse");
        assert_eq!(me.parsed_tier(), Some(AlphacodeTier::Flagship));
        assert!(me.has_active_paid_plan());
        assert_eq!(me.manage_url.as_deref(), Some("https://alphacode.sh/account"));
    }
    ...
    async fn device_request_uses_new_contract_and_parses_public_urls() {
        let base = spawn_server(vec![(
            200,
            vec![],
            r#"{"device_code":"secret","flow_id":"public-flow","verification_uri":"https://alphacode.sh/account","verification_uri_complete":"https://alphacode.sh/account?flow=public-flow","verify_url":"https://alphacode.sh/account?flow=public-flow","expires_in":600,"interval":3}"#.to_string(),
        )]);
        let result = request_device_authorization(&client(), &base, Some(AlphacodeTier::Pro))
            .await
            .expect("device auth");
    ...
    async fn legacy_device_response_is_explained_without_echoing_body() {
        ...
        let error = request_device_authorization(&client(), &base, Some(AlphacodeTier::Pro))
            .await

...
const CLIENT_TITLE_PREFIXES: &[&str] = &["alphacode:d:", "alphacode:c:"];
...
pub async fn run_configured() -> Result<DictationRun> {
    ...
    if command.is_empty() {
        anyhow::bail!(
            "Dictation is not configured. Set `[dictation].command` in `~/.alphacode/config.toml`."
        );
    }
...
pub fn last_focused_session() -> Result<Option<String>> {
    ...
    let session_id = match std::fs::read_to_string(path) {
        ...
        Err(err) => return Err(err).context("failed to read last focused alphacode session"),
    };
    if session_id.is_empty() {
        return Ok(None);
    }
...
pub fn focused_alphacode_session() -> Result<Option<String>> {
...
fn extract_session_short_name_from_window_title(title: &str) -> Option<String> {
    let (_, rest) = title
        .split_once("alphacode/")
        .or_else(|| title.split_once("alphacode "))?;

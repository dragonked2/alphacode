...
struct SkillFrontmatter {
    ...
    /// Claude Code extension: skill can be auto-invoked when relevant.
    #[serde(rename = "auto-invoke", default)]
...
impl SkillRegistry {
    ...
    ///
    /// Holds GLOBAL skills only (plugins, `~/.alphacode/skills/`,
    ...
    /// Only runs if ~/.alphacode/skills/ doesn't exist yet.
    fn import_from_external() {
        let alphacode_skills = match crate::storage::alphacode_dir() {
            Ok(dir) => dir.join("skills"),
            Err(_) => return,
        };

        if alphacode_skills.exists() {
        ...
        {
            let count = Self::copy_skills_dir(&claude_skills, &alphacode_skills);
            if count > 0 {
                sources.push(format!("{} from Claude Code", count));
                copied.extend(Self::list_skill_names(&alphacode_skills));
            }
        }
        ...
        if let Ok(codex_skills) = crate::storage::user_home_path(".codex/skills")
            && codex_skills.is_dir()
        {
            let count = Self::copy_skills_dir(&codex_skills, &alphacode_skills);
            if count > 0 {
    ...
    /// `~/.alphacode/skills/`, and `~/.agents/skills/`.
    ///
    ...
    pub fn load_global() -> Result<Self> {
        ...
        // first, so explicit alphacode/agents skills with the same name win below.
        if let Some(plugins_root) = Self::claude_plugins_root() {
            registry.load_plugin_skills_from_root(&plugins_root);
        }

        // Load from ~/.alphacode/skills/ (alphacode's own global skills)
        if let Ok(alphacode_dir) = crate::storage::alphacode_dir() {
            let alphacode_skills = alphacode_dir.join("skills");
            if alphacode_skills.exists() {
                registry.load_from_dir(&alphacode_skills)?;
            }
        }
    ...
    /// `./.alphacode/skills/`, `./.agents/skills/`, and `./.claude/skills/`.
    ///
    ...
    fn load_project_local_dirs(&mut self, working_dir: Option<&Path>) -> Result<()> {
        // Load from ./.alphacode/skills/ (project-local alphacode skills)
        let local_alphacode = Self::project_local_dir(working_dir, ".alphacode");
        if local_alphacode.exists() {
            self.load_from_dir(&local_alphacode)?;
        }
    ...
    /// plugin never prevents alphacode's own skills from loading.
    ...
    fn parse_skill(path: &Path) -> Result<Skill> {
        ...
        // Load reference files from the skill directory (e.g. `references/`)
        let reference_files = Self::load_reference_files(path.parent().unwrap_or(path));
    ...
    /// Load reference files from a skill directory. Scans `references/` and
    /// any `*.md` files in the skill root (excluding SKILL.md itself).
    ...
    fn load_reference_files(skill_dir: &Path) -> HashMap<String, String> {
        ...
        if let Ok(entries) = std::fs::read_dir(skill_dir) {
            for entry in entries.flatten() {
                ...
                {
                    refs.insert(name.to_string(), content);
                }
            }
        }
    ...
    /// Reload the shared global skill sources (plugins, `~/.alphacode/skills/`,
    ...
    pub fn reload_global(&mut self) -> Result<usize> {
        ...
        if let Ok(alphacode_dir) = crate::storage::alphacode_dir() {
            let alphacode_skills = alphacode_dir.join("skills");
            if alphacode_skills.exists() {
                count += self.load_from_dir_count(&alphacode_skills)?;
            }
        }
...
/// A skill recommended/curated by alphacode that the user may want to install.
...
pub struct EndorsedSkill {
    ...
    /// Grouping label used to organize the endorsed list (e.g. "alphacode",
    /// "NVIDIA CUDA-X").
...
/// Curated list of skills endorsed by alphacode. Used by the `/skills` command to
...
pub const ENDORSED_SKILLS: &[EndorsedSkill] = &[
    EndorsedSkill {
        ...
        category: "alphacode",
        source: "bundled in alphacode repo (.alphacode/skills/optimization)",
        install: None,
    },
    EndorsedSkill {
        ...
        category: "alphacode",
        source: "bundled with alphacode / Claude Code skills",
        install: None,
    },
    EndorsedSkill {
    ...
    EndorsedSkill {
        ...
        category: "alphacode",
        source: "bundled in alphacode repo (.alphacode/skills/security-audit)",
        install: None,
    },
    EndorsedSkill {
        name: "refactor",
        ...
        category: "alphacode",
        source: "bundled in alphacode repo (.alphacode/skills/refactor)",
        install: None,
    },
    EndorsedSkill {
        name: "test-writer",
        ...
        category: "alphacode",
        source: "bundled in alphacode repo (.alphacode/skills/test-writer)",
        install: None,
    },
    EndorsedSkill {
        name: "api-design",
        ...
        category: "alphacode",
        source: "bundled in alphacode repo (.alphacode/skills/api-design)",
        install: None,
    },
    EndorsedSkill {
        ...
        category: "alphacode",
        source: "bundled in alphacode repo",
        install: None,
    },
    EndorsedSkill {
    ...
    EndorsedSkill {
        name: "exploit-dev",
        description: "Exploit development methodology: vulnerability analysis, payload crafting, chain development, and reliable exploit generation for responsible disclosure.",
        category: "Security",
        source: "bundled in alphacode repo",
        install: None,
    },
...
/// Return the curated list of skills endorsed by alphacode.
...
impl Skill {
    ...
    pub fn get_prompt(&self) -> String {
        ...
        if !self.reference_files.is_empty() {
            prompt.push_str("\n\n---\n\n## Reference Files\n\n");
            prompt.push_str("The following reference files are available for this skill. ");
            prompt.push_str("Load them with `skill_manage` (action=read) when needed:\n\n");
            for name in self.reference_files.keys() {
    ...
    /// Get the content of a reference file by name
    ...
    pub fn load_file(&self, filename: &str) -> Result<String> {
        ...
        if let Some(content) = self.reference_files.get(filename) {
    ...
    pub fn resolve_dynamic_context(&self) -> String {
        ...
        while let Some(start) = remaining.find(MARKER) {
            ...
            if let Some(end) = after_open.find('\u{60}') {
                // Copy the command text out of `remaining` before re-assigning
                // `remaining` so the borrow checker can see the lifetime.
                let command = after_open[..end].to_string();
                let rest = after_open[end + 1..].to_string();
                // `after_open` is a `&str` (Copy), so dropping it is implicit
                // when the next statement reassigns `remaining`. The earlier
                // explicit `drop` was a no-op and produced a warning.
...
mod tests {
    ...
    #[test]
    fn load_for_working_dir_reads_project_local_alphacode_skills() {
        let temp = tempfile::tempdir().expect("tempdir");
        write_test_skill(temp.path(), ".alphacode", "wd-only");
    ...
    fn project_overlay_is_session_scoped_and_composes_over_globals() {
        let temp = tempfile::tempdir().expect("tempdir");
        write_test_skill(temp.path(), ".alphacode", "session-skill");
    ...
    #[test]
    fn endorsed_skills_include_security_audit() {
        let skill = endorsed_skills()
            .iter()
            .find(|s| s.name == "security-audit")
            .expect("expected endorsed security-audit skill");
        assert_eq!(skill.category, "alphacode");
        assert!(skill.description.contains("OWASP"));
    }
    ...
    fn registry_contains_reports_loaded_skills() {
        let temp = tempfile::tempdir().expect("tempdir");
        write_test_skill(temp.path(), ".alphacode", "present-skill");
    ...
    #[test]
    fn explicit_alphacode_skill_wins_over_plugin_skill_with_same_name() {
        let temp = tempfile::tempdir().expect("tempdir");
        ...
        // Explicit alphacode skill with the same name.
        write_test_skill(temp.path(), ".alphacode", "shared-name");
        ...
        registry.load_plugin_skills_from_root(&plugins_root);
        registry
            .load_from_dir(&temp.path().join(".alphacode/skills"))
        ...
        assert_eq!(
            ...
            "explicit alphacode skill must override the plugin-provided one"
        );
    }
    ...
    #[test]
    fn bundled_web3_skill_parses_and_registers() {
        use crate::alphacode_base::bundled_skills::BUNDLED_SKILLS;
        let web3 = BUNDLED_SKILLS
            .iter()
            .find(|s| s.name == "web3-security-research")
            .expect("`web3-security-research` must be in BUNDLED_SKILLS");
        let parsed = SkillRegistry::parse_embedded_skill(web3.name, web3.body, web3.references)
            .expect("bundled web3-security-research SKILL.md must parse cleanly");
        assert_eq!(parsed.name, "web3-security-research");
        assert!(!parsed.description.is_empty());
        assert!(
            parsed.content.contains("invariant"),
            "web3 skill body must carry the invariant-driven methodology"
        );
        let expected_refs = [
            "protocol-model",
            "defi-accounting",
        ...
        assert_eq!(
            web3.references.len(),
            expected_refs.len(),
            "web3 skill must embed all domain references"
        );
        for sub in expected_refs {
            let body = web3
                .references
                .iter()
                ...
                .map(|(_, b)| *b)
                .unwrap_or("");
            assert!(!body.trim().is_empty(), "web3 reference '{sub}' is empty");
            assert!(
                body.contains("description:"),
                "web3 reference '{sub}' is missing YAML frontmatter"
            );
        }
        assert_eq!(parsed.reference_files.len(), expected_refs.len());
    }
}

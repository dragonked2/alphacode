...
/// background tasks, without re-coupling `alphacode-base` to swarm internals.
#[derive(Clone, Debug)]
...
pub enum BusEvent {
    ...
    UsageReport(Vec<crate::alphacode_usage_types::ProviderUsage>),
    /// Progressive usage report update while providers are still loading
    UsageReportProgress(crate::alphacode_usage_types::ProviderUsageProgress),

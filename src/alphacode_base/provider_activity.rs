...
//!   1. When alphacode last successfully used it (for recency-sorted `/usage`).
...
//!
//! Data persists to `~/.alphacode/provider_activity.json` and is shared across
...
//!   - `openrouter`, `alphacode`, `copilot`, `gemini`, `cursor`, `bedrock`,
...
pub fn display_name_for_source_key(source_key: &str) -> String {
    ...
    match source_key {
        ...
        "alphacode" => "Alphacode subscription".to_string(),
...
/// `"NVIDIA NIM"`) plus the optional `ALPHACODE_RUNTIME_PROVIDER` key onto a
...
mod tests {
    ...
    impl Drop for EnvVarGuard {
        fn drop(&mut self) {
            if let Some(prev) = &self.prev {
                crate::alphacode_core::env::set_var(self.key, prev);
            } else {
    ...
    #[test]
    fn record_use_and_spend_roundtrip_under_alphacode_home() {

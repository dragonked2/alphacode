...
fn restore_env_var(key: &str, previous: Option<OsString>) {
    if let Some(previous) = previous {
        crate::alphacode_core::env::set_var(key, previous);
    } else {
...
fn command_candidates_adds_extension_on_windows() {
    crate::alphacode_core::env::set_var("PATHEXT", ".EXE;.BAT");
...
fn auth_status_check_fast_includes_bedrock_probe() {
    ...
    let prev_bedrock_enable = std::env::var_os("ALPHACODE_BEDROCK_ENABLE");

    crate::alphacode_core::env::set_var("ALPHACODE_BEDROCK_ENABLE", "1");
    AuthStatus::invalidate_cache();

    let status = AuthStatus::check_fast();
    assert_eq!(status.bedrock, AuthState::Available);

    restore_env_var("ALPHACODE_BEDROCK_ENABLE", prev_bedrock_enable);
    AuthStatus::invalidate_cache();
}
...
fn full_and_fast_auth_status_match_for_shared_probe_fields() {
    ...
    let saved = [
        ...
        crate::alphacode_base::auth::azure::ENDPOINT_ENV,
        crate::alphacode_base::auth::azure::API_KEY_ENV,
        crate::alphacode_base::auth::azure::MODEL_ENV,
        crate::alphacode_base::auth::azure::USE_ENTRA_ENV,
        "ALPHACODE_BEDROCK_ENABLE",
        ...
        "ALPHACODE_CURSOR_CLI_PATH",
    ]
    .into_iter()
    .map(|key| (key, std::env::var_os(key)))
    .collect::<Vec<_>>();

    crate::alphacode_core::env::set_var("ALPHACODE_HOME", temp.path().join("alphacode-home"));
    crate::alphacode_core::env::set_var("XDG_CONFIG_HOME", &xdg);
    crate::alphacode_core::env::set_var("HOME", &home);
    crate::alphacode_core::env::set_var(
        crate::subscription_catalog::ALPHACODE_API_KEY_ENV,
        "alphacode-test-key",
    );
    crate::alphacode_core::env::set_var("ANTHROPIC_API_KEY", "anthropic-test-key");
    crate::alphacode_core::env::set_var("OPENAI_API_KEY", "openai-test-key");
    crate::alphacode_core::env::set_var("OPENROUTER_API_KEY", "openrouter-test-key");
    for key in [
        "ALPHACODE_OPENROUTER_API_BASE",
        "ALPHACODE_OPENROUTER_API_KEY_NAME",
        "ALPHACODE_OPENROUTER_ENV_FILE",
    ...
    crate::alphacode_core::env::set_var(crate::alphacode_base::auth::azure::API_KEY_ENV, "azure-test-key");
    crate::alphacode_core::env::set_var(crate::alphacode_base::auth::azure::MODEL_ENV, "gpt-test-deployment");
    crate::alphacode_core::env::remove_var(crate::alphacode_base::auth::azure::USE_ENTRA_ENV);
    crate::alphacode_core::env::set_var("ALPHACODE_BEDROCK_ENABLE", "1");
    crate::alphacode_core::env::set_var("COPILOT_GITHUB_TOKEN", "gho_test_token");
    crate::alphacode_core::env::remove_var("GH_TOKEN");
    crate::alphacode_core::env::remove_var("GITHUB_TOKEN");
    crate::alphacode_core::env::set_var("CURSOR_API_KEY", "cursor-test-key");
    crate::alphacode_core::env::remove_var("CURSOR_ACCESS_TOKEN");
    crate::alphacode_core::env::remove_var("CURSOR_REFRESH_TOKEN");
    crate::alphacode_core::env::set_var(
        "ALPHACODE_CURSOR_CLI_PATH",
    ...
    assert_eq!(full.alphacode, AuthState::Available);
...
fn full_and_fast_auth_status_document_cursor_cli_exception() {
    ...
    crate::alphacode_core::env::set_var("XDG_CONFIG_HOME", &xdg);
    crate::alphacode_core::env::set_var("HOME", &home);
    crate::alphacode_core::env::remove_var("CURSOR_API_KEY");
    crate::alphacode_core::env::remove_var("CURSOR_ACCESS_TOKEN");
    crate::alphacode_core::env::remove_var("CURSOR_REFRESH_TOKEN");
    crate::alphacode_core::env::set_var("ALPHACODE_CURSOR_CLI_PATH", &mock_cli);
    AuthStatus::invalidate_cache();
...
fn assert_auth_status_shared_fields_match(full: &AuthStatus, fast: &AuthStatus) {
    assert_eq!(full.alphacode, fast.alphacode, "alphacode");
    assert_eq!(
...
fn copilot_recent_token_exchange_failure_is_not_auto_usable() {
    ...
    crate::alphacode_core::env::remove_var("COPILOT_GITHUB_TOKEN");
    crate::alphacode_core::env::remove_var("GH_TOKEN");
    crate::alphacode_core::env::remove_var("GITHUB_TOKEN");
    AuthStatus::invalidate_cache();
    crate::alphacode_base::auth::copilot::invalidate_github_token_cache();

    crate::alphacode_base::auth::copilot::save_github_token("gho_saved_token", "tester")
        .expect("save copilot token");
    crate::alphacode_base::auth::validation::save(
        "copilot",
    ...
    crate::alphacode_core::env::set_var("GH_TOKEN", "gho_env_override");
    AuthStatus::invalidate_cache();
    crate::alphacode_base::auth::copilot::invalidate_github_token_cache();
    let status = AuthStatus::check_fast();
    assert_eq!(status.copilot, AuthState::Available);
    ...
    restore_env_var("ALPHACODE_HOME", prev_home);
...
fn openrouter_like_status_is_provider_specific() {
    ...
    crate::alphacode_core::env::set_var("ALPHACODE_HOME", temp.path());
    crate::alphacode_core::env::set_var("CHUTES_API_KEY", "chutes-test-key");
    crate::alphacode_core::env::remove_var("OPENCODE_API_KEY");
    AuthStatus::invalidate_cache();
...
fn cursor_status_is_available_when_api_key_exists_without_cli() {
    ...
    let prev_cli_path = std::env::var_os("ALPHACODE_CURSOR_CLI_PATH");
    let temp = tempfile::TempDir::new().expect("create temp dir");
    ...
    restore_env_var("ALPHACODE_CURSOR_CLI_PATH", prev_cli_path);
    AuthStatus::invalidate_cache();
}
...
fn configured_api_key_source_uses_valid_overrides() {
    let _lock = crate::storage::lock_test_env();
    let key_var = "ALPHACODE_OPENAI_COMPAT_API_KEY_NAME";
    let file_var = "ALPHACODE_OPENAI_COMPAT_ENV_FILE";
    let prev_key = std::env::var(key_var).ok();
    let prev_file = std::env::var(file_var).ok();

    crate::alphacode_core::env::set_var(key_var, "GROQ_API_KEY");
    crate::alphacode_core::env::set_var(file_var, "groq.env");
    ...
    if let Some(v) = prev_key {
        crate::alphacode_core::env::set_var(key_var, v);
    } else {
        crate::alphacode_core::env::remove_var(key_var);
    }
    if let Some(v) = prev_file {
        crate::alphacode_core::env::set_var(file_var, v);
    } else {
        crate::alphacode_core::env::remove_var(file_var);
    }
}
...
fn configured_api_key_source_rejects_invalid_values() {
    ...
    crate::alphacode_core::env::set_var(key_var, "bad-key");
    crate::alphacode_core::env::set_var(file_var, "../bad.env");
...
fn anthropic_api_provider_reports_api_key_independently_of_oauth() {
    ...
    std::fs::create_dir_all(&home).expect("create temp home");
    std::fs::create_dir_all(&xdg).expect("create temp xdg config");
    let saved = ["ALPHACODE_HOME", "XDG_CONFIG_HOME", "HOME", "ANTHROPIC_API_KEY"]
        .into_iter()
        .map(|key| (key, std::env::var_os(key)))
        .collect::<Vec<_>>();
    ...
    crate::alphacode_core::env::set_var("XDG_CONFIG_HOME", &xdg);
    crate::alphacode_core::env::set_var("HOME", &home);
    crate::alphacode_core::env::remove_var("ANTHROPIC_API_KEY");
    AuthStatus::invalidate_cache();
    ...
    crate::alphacode_core::env::set_var("ANTHROPIC_API_KEY", "sk-ant-api-test-key");
    AuthStatus::invalidate_cache();
    let status = AuthStatus::check_fast();
    let api = status.assessment_for_provider(crate::provider_catalog::ANTHROPIC_API_LOGIN_PROVIDER);
...
/// it even without --no-browser or NO_BROWSER/ALPHACODE_NO_BROWSER.
#[test]

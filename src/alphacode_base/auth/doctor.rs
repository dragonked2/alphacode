use crate::alphacode_base::auth::{AuthRefreshSupport, AuthState, ProviderAuthAssessment};
...
/// True when `alphacode provider-doctor` has a native-runtime driver for
...
/// Cursor, Copilot, Bedrock, alphacode, Azure OpenAI).
///
/// The drivers themselves live downstream in the `alphacode-provider-doctor`
...
/// crate. A sync test in `alphacode-provider-doctor` asserts this list matches its
...
pub fn recommended_actions(
    ...
    match assessment.state {
        AuthState::NotConfigured => actions.push(format!(
            "Connect it: alphacode login --provider {}",
            provider.id
        )),
        AuthState::Expired
            if matches!(
        ...
        {
            actions.push(format!(
                "Re-run login; this provider cannot auto-refresh: alphacode login --provider {}",
                provider.id
            ));
        }
        AuthState::Expired => actions.push(format!(
            "Refresh or replace the current login: alphacode login --provider {}",
            provider.id
        )),
        AuthState::Available => {}
    }
        ...
        .and_then(|record| record.last_error.as_deref())
    {
        ...
        if lower.contains("invalid_grant") || lower.contains("refresh token") {
            actions.push(format!(
                "Replace the stale OAuth account/token: alphacode login --provider {}",
                provider.id
            ));
            ...
            || lower.contains("too many requests")
        {
            ...
            actions.push(format!(
                "Retry credential refresh by re-running validation: alphacode auth doctor {} --validate",
                provider.id
            ));
        }
    }
    ...
    if assessment.state != AuthState::NotConfigured {
        match assessment.last_validation.as_ref() {
            ...
            Some(record) if !record.success => actions.push(format!(
                "Inspect runtime readiness: alphacode auth-test --provider {}",
                provider.id
            )),
            Some(record) if validation_is_stale(record.checked_at_ms) => actions.push(format!(
                "Refresh stale runtime verification: alphacode auth-test --provider {}",
                provider.id
            )),
            Some(_) => {}
        }
    ...
    if validation_result.is_some_and(|value| value != "validation passed") {
        actions.push(format!(
            "Re-run detailed auth diagnostics: alphacode auth-test --provider {}",
            provider.id
        ));
    }
    ...
    {
        actions.push(format!(
            "For browser/callback issues, use the manual-safe flow: alphacode login --provider {} --print-auth-url",
            provider.id
        ));
    }

    actions.push("Review current state: alphacode auth status --json".to_string());
    actions.dedup();
    actions
}
...
mod tests {
    ...
    fn base_assessment() -> ProviderAuthAssessment {
        ProviderAuthAssessment {
            ...
            readiness: crate::alphacode_base::auth::AuthReadinessLevel::RequestValid,
            method_detail: "OAuth".to_string(),
            credential_source: AuthCredentialSource::AlphacodeManagedFile,
            credential_source_detail: "~/.alphacode/auth.json".to_string(),

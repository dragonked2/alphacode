...
/// Cached auth status plus the `ALPHACODE_HOME` it was computed under.
///
/// Auth probes read credential files relative to `ALPHACODE_HOME`. Tests swap
/// `ALPHACODE_HOME` to per-test temp dirs, and a status computed under one home
...
fn auth_cache_home_key() -> Option<std::ffi::OsString> {
    std::env::var_os("ALPHACODE_HOME")
}

/// Full auth probe cache: 120s is long enough to avoid repeated file I/O
/// on every UI frame while still picking up credential changes within 2 min.
const AUTH_STATUS_CACHE_TTL_SECS: u64 = 120;
/// Fast (local-only) auth cache: 300s avoids the nonblocking background
/// refresh thread waking up too often; credential files rarely change mid-session.
const AUTH_STATUS_FAST_CACHE_TTL_SECS: u64 = 300;
...
/// CLI tools don't get installed/uninstalled while alphacode is running, so caching
...
pub fn browser_suppressed(cli_no_browser: bool) -> bool {
    cli_no_browser
        ...
        || env_truthy("ALPHACODE_NO_BROWSER")
...
/// location no installed or self-dev alphacode binary ever runs from.
///
...
/// multiple browser tabs. Set `ALPHACODE_ALLOW_BROWSER_IN_TESTS=1` to opt out
...
pub fn running_in_test_harness() -> bool {
    ...
    *IN_TEST_HARNESS.get_or_init(|| {
        if env_truthy("ALPHACODE_ALLOW_BROWSER_IN_TESTS") {
            return false;
        }
        std::env::current_exe()
            .ok()
...
fn auth_timing_logging_enabled() -> bool {
    env_truthy("ALPHACODE_AUTH_TIMING")
}
...
fn log_auth_status_snapshot(event: &str, status: &AuthStatus) {
    crate::logging::auth_event(
        ...
        "all",
        &[
            ("alphacode", auth_state_label(status.alphacode)),
...
fn auth_readiness_for_provider(
    ...
    state: AuthState,
    last_validation: Option<&crate::alphacode_base::auth::validation::ProviderValidationRecord>,
) -> AuthReadinessLevel {
    match state {
...
impl AuthStatus {
    ...
    /// per-test `ALPHACODE_HOME` swaps would poison the shared cache.
    ...
    pub fn has_any_available(&self) -> bool {
        self.anthropic.state == AuthState::Available
            || self.alphacode == AuthState::Available
    ...
    /// alphacode believes credentials are available/expired/missing without leaking
    ...
    pub fn log_snapshot(&self, surface: &str) {
        crate::logging::event_info(
            ...
            vec![
                ...
                ("alphacode", self.alphacode.label().to_string()),
    ...
    pub fn has_any_untrusted_external_auth() -> bool {
        crate::alphacode_base::auth::codex::has_unconsented_legacy_credentials()
            || crate::alphacode_base::auth::claude::has_unconsented_external_auth().is_some()
            || crate::alphacode_base::auth::external::has_any_unconsented_external_auth()
            || crate::alphacode_base::auth::gemini::has_unconsented_cli_auth()
            || crate::alphacode_base::auth::copilot::has_unconsented_external_auth().is_some()
            || crate::alphacode_base::auth::cursor::has_unconsented_external_auth().is_some()
    }
    ...
    pub fn state_for_key(&self, key: LoginProviderAuthStateKey) -> AuthState {
        match key {
            ...
            LoginProviderAuthStateKey::Alphacode => self.alphacode,
    ...
    pub fn state_for_provider(&self, provider: LoginProviderDescriptor) -> AuthState {
        match provider.target {
            ...
            crate::provider_catalog::LoginProviderTarget::Alphacode => {
    ...
    pub fn assessment_for_provider(
        ...
        let last_validation = crate::alphacode_base::auth::validation::get(provider.id);
        let last_refresh = crate::alphacode_base::auth::refresh_state::get(provider.id);
        ...
        let (
            ...
            crate::provider_catalog::LoginProviderTarget::Alphacode => {
                let (source, detail) = summarize_sources(vec![
                    env_source(crate::subscription_catalog::ALPHACODE_API_KEY_ENV),
                    config_source(
                        crate::subscription_catalog::ALPHACODE_API_KEY_ENV,
                        crate::subscription_catalog::ALPHACODE_ENV_FILE,
                        "~/.config/alphacode/alphacode-subscription.env",
                    ),
                ]);
                (
                    source,
            ...
            crate::provider_catalog::LoginProviderTarget::OpenRouter => {
                let (source, detail) = summarize_sources(vec![
                    ...
                    config_source(
                        "OPENROUTER_API_KEY",
                        "openrouter.env",
                        "~/.config/alphacode/openrouter.env",
                    ),
            ...
            crate::provider_catalog::LoginProviderTarget::ClaudeApiKey => {
                ...
                // config file (`~/.config/alphacode/anthropic.env`), *not* an env
                // var and *not* `~/.alphacode/auth.json` (which holds the separate
            ...
            crate::provider_catalog::LoginProviderTarget::Azure => {
                let (source, detail) = summarize_sources(vec![
                    azure_entra_source(),
                    env_source(crate::alphacode_base::auth::azure::API_KEY_ENV),
                    config_source(
                        crate::alphacode_base::auth::azure::API_KEY_ENV,
                        crate::alphacode_base::auth::azure::ENV_FILE,
                        "~/.config/alphacode/azure-openai.env",
                    ),
                ]);
                (
                    source,
                    ...
                    if crate::alphacode_base::auth::azure::uses_entra_id() {
            ...
            crate::provider_catalog::LoginProviderTarget::Bedrock => {
                let (source, detail) = summarize_sources(vec![
                    ...
                    config_source(
                        ...
                        "~/.config/alphacode/bedrock.env",
                    ),
                    env_source("AWS_PROFILE"),
                    env_source("ALPHACODE_BEDROCK_PROFILE"),
                    ...
                    crate::provider_catalog::active_named_provider_profile_credential_source()
                {
                    ...
                    summarize_sources(vec![
                        ...
                        config_source(
                            &resolved.api_key_env,
                            &resolved.env_file,
                            format!("~/.config/alphacode/{}", resolved.env_file),
                        ),
    ...
    pub fn invalidate_cache() {
        ...
        crate::alphacode_base::auth::copilot::invalidate_github_token_cache();
...
fn probe_alphacode_status(status: &mut AuthStatus) {
    if crate::subscription_catalog::has_credentials() {
        status.alphacode = AuthState::Available;
    }
}
...
fn probe_azure_status(status: &mut AuthStatus) {
    status.azure_has_api_key = crate::alphacode_base::auth::azure::has_api_key();
    status.azure_uses_entra = crate::alphacode_base::auth::azure::uses_entra_id();
...
fn assessment_for_key(
    ...
    match key {
        ...
        LoginProviderAuthStateKey::Alphacode
...
fn external_api_key_source(env_key: &str) -> Option<(AuthCredentialSource, String)> {
    crate::alphacode_base::auth::external::load_api_key_for_env(env_key).map(|_| {
        (
            AuthCredentialSource::TrustedExternalFile,
...
fn azure_entra_source() -> Option<(AuthCredentialSource, String)> {
    crate::alphacode_base::auth::azure::uses_entra_id().then(|| {
        (
...
fn anthropic_oauth_source(status: &AuthStatus) -> Option<(AuthCredentialSource, String)> {
    ...
    if !crate::alphacode_base::auth::claude::list_accounts()
        .unwrap_or_default()
        .is_empty()
    {
        return Some((
            AuthCredentialSource::AlphacodeManagedFile,
            "~/.alphacode/auth.json".to_string(),
        ));
    }
    if let Some(source) = crate::alphacode_base::auth::claude::preferred_external_auth_source()
    ...
    if crate::alphacode_base::auth::external::load_anthropic_oauth_tokens().is_some() {
        return Some((
            AuthCredentialSource::TrustedExternalFile,
...
fn openai_oauth_source(status: &AuthStatus) -> Option<(AuthCredentialSource, String)> {
    ...
    if !crate::alphacode_base::auth::codex::list_accounts()
        .unwrap_or_default()
        .is_empty()
    {
        return Some((
    ...
    if crate::alphacode_base::auth::external::load_openai_oauth_tokens().is_some() {
        return Some((
            AuthCredentialSource::TrustedExternalFile,
            "trusted external auth import".to_string(),
        ));
...
fn gemini_source() -> Option<(AuthCredentialSource, String)> {
    if let Ok(path) = crate::alphacode_base::auth::gemini::tokens_path()
        && path.exists()
    {
        return Some((
            AuthCredentialSource::AlphacodeManagedFile,
    ...
    if let Ok(path) = crate::alphacode_base::auth::gemini::gemini_cli_oauth_path()
        && path.exists()
        && crate::config::Config::external_auth_source_allowed_for_path(
            crate::alphacode_base::auth::gemini::GEMINI_CLI_AUTH_SOURCE_ID,
            &path,
        )
    {
        return Some((
    ...
    crate::alphacode_base::auth::external::load_gemini_oauth_tokens().map(|_| {
        (
            AuthCredentialSource::TrustedExternalFile,
            "trusted external auth import".to_string(),
        )
...
fn antigravity_source() -> Option<(AuthCredentialSource, String)> {
    if let Ok(path) = crate::alphacode_base::auth::antigravity::tokens_path()
        && path.exists()
    {
        return Some((
            AuthCredentialSource::AlphacodeManagedFile,
    ...
    crate::alphacode_base::auth::external::load_antigravity_oauth_tokens().map(|_| {
        (
            AuthCredentialSource::TrustedExternalFile,
            "trusted external auth import".to_string(),
        )
...
fn google_source() -> Option<(AuthCredentialSource, String)> {
    if let (Ok(tokens_path), Ok(credentials_path)) = (
        crate::alphacode_base::auth::google::tokens_path(),
        crate::alphacode_base::auth::google::credentials_path(),
    ) && tokens_path.exists()
...
fn cursor_source() -> Option<(AuthCredentialSource, String)> {
    ...
    if let Ok(file_path) = crate::alphacode_base::auth::cursor::cursor_auth_file_path()
        && file_path.exists()
        && crate::config::Config::external_auth_source_allowed_for_path(
            crate::alphacode_base::auth::cursor::CURSOR_AUTH_FILE_SOURCE_ID,
            &file_path,
        )
    {
        return Some((
    ...
    if let Some(source) = crate::alphacode_base::auth::cursor::preferred_external_auth_source()
        && matches!(
            source,
            crate::alphacode_base::auth::cursor::ExternalCursorAuthSource::CursorVscdb
        )
        && let Ok(path) = source.path()
    {
        return Some((
...
fn copilot_source() -> Option<(AuthCredentialSource, String)> {
    ...
    for source in [
        crate::alphacode_base::auth::copilot::ExternalCopilotAuthSource::ConfigJson,
        crate::alphacode_base::auth::copilot::ExternalCopilotAuthSource::HostsJson,
        crate::alphacode_base::auth::copilot::ExternalCopilotAuthSource::AppsJson,
    ] {
    ...
    if crate::alphacode_base::auth::external::load_copilot_oauth_token().is_some() {
        return Some((
            AuthCredentialSource::TrustedExternalFile,
            "trusted external auth import".to_string(),
        ));

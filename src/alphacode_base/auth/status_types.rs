pub use crate::alphacode_auth_types::{
...
pub struct AuthStatus {
    /// Alphacode subscription router credentials
    pub alphacode: AuthState,
...
pub struct ProviderAuthAssessment {
    ...
    pub last_validation: Option<crate::alphacode_base::auth::validation::ProviderValidationRecord>,
    pub last_refresh: Option<crate::alphacode_base::auth::refresh_state::ProviderRefreshRecord>,
}
...
impl ProviderAuthAssessment {
    ...
    pub fn health_summary(&self) -> String {
        ...
        if let Some(record) = self.last_refresh.as_ref() {
            parts.push(format!(
                "last refresh: {}",
                crate::alphacode_base::auth::refresh_state::format_record_label(record)
            ));
        }

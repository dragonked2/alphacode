...
pub struct AlphacodeOpenAiAuthFile {
...
fn alphacode_auth_path() -> Result<PathBuf> {
    Ok(crate::storage::alphacode_dir()?.join("openai-auth.json"))
}
...
pub fn load_auth_file() -> Result<AlphacodeOpenAiAuthFile> {
    let path = alphacode_auth_path()?;
    let mut auth = if path.exists() {
        crate::storage::harden_secret_file_permissions(&path);
        crate::storage::read_json(&path)
            .with_context(|| format!("Could not read OpenAI credentials from {:?}", path))?
    } else {
        AlphacodeOpenAiAuthFile::default()
    };
...
pub fn save_auth_file(auth: &AlphacodeOpenAiAuthFile) -> Result<()> {
    let auth_path = alphacode_auth_path()?;
    let clean = AlphacodeOpenAiAuthFile {
...
fn load_oauth_credentials_internal(return_expired: bool) -> Result<CodexCredentials> {
    ...
    if let Some(tokens) = crate::alphacode_base::auth::external::load_openai_oauth_tokens() {
...
fn load_alphacode_credentials() -> Result<CodexCredentials> {
    let auth = load_auth_file()?;
    if auth.openai_accounts.is_empty() {
        anyhow::bail!("No OpenAI accounts configured in alphacode auth file")
    }
    ...
    let account = auth
        ...
        .context("No OpenAI accounts in alphacode auth file")?;
...
/// the raw tokens plus a `last_refresh` timestamp. Without an expiry, alphacode
...
pub(crate) fn expires_at_from_access_token(access_token: &str) -> Option<i64> {
    ...
    // `exp` is in seconds since epoch; the rest of alphacode tracks expiry in millis.

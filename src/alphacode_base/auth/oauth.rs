use crate::alphacode_base::auth::claude as claude_auth;
use anyhow::Result;
...
fn ensure_claude_inference_scope(scopes: &[String], action: &str) -> Result<()> {
    ...
    anyhow::bail!(
        "Claude OAuth {} returned a token without the required user:inference scope (scopes: {}). Re-run `alphacode login --provider claude` so alphacode opens the Claude.ai OAuth flow, or import/use a fresh Claude Code login.",
        action,
        scopes.join(" ")
    )
}
...
fn bad_request_response(message: &str) -> String {
    let body = format!(
        "<html><body><h1>Authentication not completed</h1><p>{}</p><p>You can close this tab and return to alphacode.</p></body></html>",
        message
    );
    format!(
...
pub async fn wait_for_callback_async_on_listener(
    ...
    loop {
        ...
        let body = "<html><body><h1>Success!</h1><p>You can close this window and return to alphacode.</p></body></html>";
        let response = format!(
            "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nConnection: close\r\nContent-Length: {}\r\n\r\n{}",
            body.len(),
            body
...
pub async fn login_claude(no_browser: bool) -> Result<OAuthTokens> {
    ...
    if let Ok(code) = std::env::var("ALPHACODE_CLAUDE_AUTH_CODE") {
        let trimmed = code.trim();
        if trimmed.is_empty() {
            anyhow::bail!("ALPHACODE_CLAUDE_AUTH_CODE is set but empty");
        }
    ...
    if !std::io::stdin().is_terminal() {
        anyhow::bail!(
            "Claude login needs an authorization code from stdin. Re-run in an interactive terminal, or set ALPHACODE_CLAUDE_AUTH_CODE."
        );
    }
    ...
    if let Ok(listener) = bind_callback_listener(0) {
        ...
        let browser_opened = if crate::alphacode_base::auth::browser_suppressed(no_browser) {
            false
        } else {
...
pub fn parse_callback_input_with_state(input: &str) -> Result<(String, String)> {
    ...
    let state = state
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "Please paste the full callback URL or query string so alphacode can verify the login state."
            )
        })?;
    Ok((code, state))
}
...
async fn exchange_claude_code_at_url(
    ...
    if !resp.status().is_success() {
        ...
        if status == reqwest::StatusCode::FORBIDDEN && looks_like_cloudflare_challenge(&text) {
            anyhow::bail!(
                "Token exchange was blocked by Cloudflare before Anthropic returned OAuth tokens. alphacode now matches Claude Code's JSON token exchange, but this network/IP is still being challenged. Switch VPN exit IP or network, then retry with `alphacode login --provider claude --no-browser` and paste the callback URL."
            );
        }
...
/// Save Claude tokens to alphacode's credentials file (active account or first numbered account).
...
/// Load Claude tokens from alphacode's credentials file (active account).
...
pub async fn refresh_claude_tokens_for_account(
    ...
    let result = crate::alphacode_base::auth::refresh_coordinator::single_flight(
        format!("claude:{label}"),
        {
    ...
    match &result {
        Ok(_) => {
            let _ = crate::alphacode_base::auth::refresh_state::record_success("claude");
        }
        Err(err) => {
            ...
            if crate::alphacode_base::auth::refresh_state::error_is_permanent_rejection(&message) {
                let _ = crate::alphacode_base::auth::refresh_state::record_permanent_rejection(
                    "claude",
                    refresh_token,
                    &message,
                );
...
pub async fn refresh_openai_tokens(refresh_token: &str) -> Result<OAuthTokens> {
    match crate::alphacode_base::auth::codex::active_account_label() {
        Some(label) => refresh_openai_tokens_for_account(refresh_token, &label).await,
        // External token (not stored in alphacode auth): nothing on disk to
...
fn stored_openai_tokens(label: &str) -> Option<OAuthTokens> {
    let account = crate::alphacode_base::auth::codex::list_accounts()
        .ok()?
        .into_iter()
        .find(|account| account.label == label)?;
    Some(OAuthTokens {
...
pub async fn refresh_openai_tokens_for_account(
    ...
    let observed_refresh = refresh_token.to_string();
    let label = label.to_string();
    crate::alphacode_base::auth::refresh_coordinator::single_flight(
        format!("openai:{label}"),
        {
            let label = label.clone();
...
async fn refresh_openai_tokens_inner(
    ...
    let result: Result<OAuthTokens> = async {
        ...
        if let Some(label) = label {
            ...
            crate::logging::info(
                "Refreshed OpenAI/Codex tokens from an external source without storing them in alphacode auth",
            );
        }
        Ok(oauth_tokens)
    }

...
// `ALPHACODE_CURSOR_CLIENT_VERSION` if Cursor moves the floor again.
...
pub fn has_authenticated_cli_session() -> bool {
    let command = std::env::var_os("ALPHACODE_CURSOR_CLI_PATH")
...
pub fn cursor_direct_client_version() -> String {
    std::env::var("ALPHACODE_CURSOR_CLIENT_VERSION")
        .ok()
...
/// 2. Saved key in `~/.config/alphacode/cursor.env`
pub fn load_api_key() -> Result<String> {
    if let Ok(key) = std::env::var("CURSOR_API_KEY") {
        let trimmed = crate::alphacode_provider_env::sanitize_secret_value(&key);
        if !trimmed.is_empty() {
    ...
    if file_path.exists() {
        ...
        for line in content.lines() {
            ...
            if let Some(key) = line.strip_prefix("CURSOR_API_KEY=") {
                let key = crate::alphacode_provider_env::sanitize_secret_value(key);
                if !key.is_empty() {
...
/// Save a Cursor API key to `~/.config/alphacode/cursor.env`.
pub fn save_api_key(key: &str) -> Result<()> {
    let file_path = config_file_path()?;
    crate::storage::upsert_env_file_value(&file_path, "CURSOR_API_KEY", Some(key))?;

    crate::alphacode_core::env::set_var("CURSOR_API_KEY", key);
    Ok(())
}

/// Remove the saved Cursor API key from `~/.config/alphacode/cursor.env` and the
...
pub fn cursor_auth_file_path() -> Result<PathBuf> {
    ...
    {
        // Honor ALPHACODE_HOME isolation (used by the onboarding sandbox and tests)
        ...
        // `$ALPHACODE_HOME/external/...`, so a fresh-install sandbox would show only

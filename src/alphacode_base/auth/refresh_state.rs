...
pub use crate::alphacode_auth_types::ProviderRefreshRecord;

pub fn status_path() -> Result<PathBuf> {
    Ok(crate::storage::alphacode_dir()?.join(REFRESH_STATUS_FILE))
}
...
mod tests {
    ...
    fn permanent_rejection_suppresses_retries_until_a_new_token_is_minted() {
        let _sandbox = crate::alphacode_base::auth::test_sandbox::AuthTestSandbox::new()

...
pub const EDITOR_VERSION: &str = "alphacode/1.0";
pub const EDITOR_PLUGIN_VERSION: &str = "alphacode/1.0";
...
/// 8. optional `gh auth token` fallback when ALPHACODE_COPILOT_ALLOW_GH_AUTH_TOKEN=1
pub fn load_github_token() -> Result<String> {
    ...
    if let Some(token) = crate::alphacode_base::auth::external::load_copilot_oauth_token() {
        cache_github_token(&token);
        return Ok(token);
    }
    ...
    anyhow::bail!(
        "GitHub Copilot token not found. \
         Set COPILOT_GITHUB_TOKEN/GH_TOKEN/GITHUB_TOKEN, run `alphacode login --provider copilot`, \
         or set ALPHACODE_COPILOT_ALLOW_GH_AUTH_TOKEN=1 to explicitly reuse `gh auth token`."
    )
}

fn allow_gh_cli_fallback() -> bool {
    std::env::var("ALPHACODE_COPILOT_ALLOW_GH_AUTH_TOKEN")
        .ok()
        .map(|value| {
...
pub fn validation_failure_blocks_auto_use() -> bool {
    ...
    let Some(record) = crate::alphacode_base::auth::validation::get("copilot") else {
        return false;
    };
    if record.success {
        return false;
...
pub fn has_copilot_credentials_fast() -> bool {
    use crate::alphacode_base::auth::external::{ExternalAuthSource, source_has_copilot_oauth};
    ...
    .find(|source| match source {
        ExternalCopilotAuthSource::OpenCodeAuth => {
            ...
            path.exists()
                ...
                && crate::alphacode_base::auth::external::source_has_copilot_oauth(
                    crate::alphacode_base::auth::external::ExternalAuthSource::OpenCode,
                )
        }
        ExternalCopilotAuthSource::PiAuth => {
...
pub fn trust_external_auth_source(source: ExternalCopilotAuthSource) -> Result<()> {
    match source {
        ExternalCopilotAuthSource::OpenCodeAuth => {
            crate::alphacode_base::auth::external::trust_external_auth_source(
                crate::alphacode_base::auth::external::ExternalAuthSource::OpenCode,
            )?;
        }
        ExternalCopilotAuthSource::PiAuth => {
...
fn load_token_from_gh_cli() -> Option<String> {
    if !crate::alphacode_base::auth::command_exists("gh") {
        return None;
    }
...
pub async fn verify_copilot_credentials_live(client: &reqwest::Client) -> Result<()> {
    ...
    let _ = crate::alphacode_base::auth::validation::save("copilot", record);
...
pub fn save_github_token(token: &str, username: &str) -> Result<()> {
    ...
    // A token written by alphacode's own device-login flow should be immediately

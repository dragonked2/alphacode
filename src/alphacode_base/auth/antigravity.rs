...
const CLIENT_ID_ENV: &str = "ALPHACODE_ANTIGRAVITY_CLIENT_ID";
const CLIENT_SECRET_ENV: &str = "ALPHACODE_ANTIGRAVITY_CLIENT_SECRET";
const VERSION_ENV: &str = "ALPHACODE_ANTIGRAVITY_VERSION";
...
pub fn tokens_path() -> Result<std::path::PathBuf> {
    Ok(crate::storage::alphacode_dir()?.join("antigravity_oauth.json"))
}
...
pub fn load_tokens() -> Result<AntigravityTokens> {
    ...
    if path.exists() {
        ...
        return crate::storage::read_json(&path).map_err(|_| {
            anyhow::anyhow!(
                "No Antigravity tokens found. Run `alphacode login --provider antigravity`."
            )
        });
    }

    if let Some(tokens) = crate::alphacode_base::auth::external::load_antigravity_oauth_tokens() {
    ...
    anyhow::bail!("No Antigravity tokens found. Run `alphacode login --provider antigravity`.");
}
...
async fn refresh_tokens_uncoordinated(tokens: &AntigravityTokens) -> Result<AntigravityTokens> {
    let result: Result<AntigravityTokens> = async {
        let token = crate::alphacode_base::auth::google_oauth::refresh_access_token(
            "Antigravity",
...
pub async fn login(no_browser: bool) -> Result<AntigravityTokens> {
            ...
            crate::alphacode_base::auth::oauth::bind_callback_listener(DEFAULT_PORT)
    {
        ...
        if browser_opened {
            ...
            eprintln!(
                "If the browser lands on a loopback error page instead of returning to alphacode, copy the full URL from the address bar and re-run with `--no-browser` to paste it manually."
            );
...
async fn manual_login(
    ...
    eprintln!(
        "After approving access, paste the full callback URL (or query string) here so alphacode can verify the login state.\n"
    );
    eprintln!(
...
mod tests {
    ...
    fn build_auth_url_includes_antigravity_scope_and_redirect() {
        ...
        crate::alphacode_core::env::remove_var(CLIENT_ID_ENV);
    }
    ...
    fn blank_env_vars_fall_back_to_built_in_credentials() {
        let _guard = lock_test_env();
        crate::alphacode_core::env::set_var(CLIENT_ID_ENV, "   ");
        crate::alphacode_core::env::set_var(CLIENT_SECRET_ENV, "   ");

        assert_eq!(antigravity_client_id(), ANTIGRAVITY_CLIENT_ID);
        assert_eq!(antigravity_client_secret(), ANTIGRAVITY_CLIENT_SECRET);

        crate::alphacode_core::env::remove_var(CLIENT_ID_ENV);
        crate::alphacode_core::env::remove_var(CLIENT_SECRET_ENV);
    }

...
#[test]
fn save_openai_tokens_uses_alphacode_home_sandbox() -> Result<()> {
    ...
    let creds = crate::alphacode_base::auth::codex::load_credentials()?;
    ...
    assert_eq!(creds.expires_at, Some(future_ms));
    Ok(())
}
...
fn save_claude_tokens_preserves_existing_account_metadata() -> Result<()> {
    ...
    let account = crate::alphacode_base::auth::claude::list_accounts()?
        .into_iter()

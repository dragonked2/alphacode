...
//! [`AuthStatus`] plus the `ALPHACODE_RUNTIME_PROVIDER` override on its own, keyed on
...
use crate::alphacode_base::auth::AuthStatus;
...
impl From<crate::alphacode_provider_core::ResolvedCredential> for ActiveCredential {
    fn from(value: crate::alphacode_provider_core::ResolvedCredential) -> Self {
        match value {
            crate::alphacode_provider_core::ResolvedCredential::Oauth => Self::OAuth,
            crate::alphacode_provider_core::ResolvedCredential::ApiKey => Self::ApiKey,
        }
    }
}
...
pub struct ResolvedProviderAuth {
    ...
    /// True when the active credential was pinned via `ALPHACODE_RUNTIME_PROVIDER`
...
///
/// `runtime_provider` is the raw `ALPHACODE_RUNTIME_PROVIDER` value (if any); it
...
pub fn resolve_dual_credential_auth(
    ...
    let dual = crate::alphacode_provider_core::DualAuthProvider::from_active_provider(provider)?;
...
mod tests {
    use super::*;
    use crate::alphacode_base::auth::ProviderAuth;

...
pub fn apply_runtime_env() -> Result<()> {
    let endpoint = load_endpoint().ok_or_else(|| {
        anyhow::anyhow!(
            "{} not found in environment or ~/.config/alphacode/{}",
            ENDPOINT_ENV,
            ENV_FILE
        )
    })?;

    crate::alphacode_core::env::set_var("ALPHACODE_OPENROUTER_API_BASE", endpoint);
    crate::alphacode_core::env::set_var("ALPHACODE_OPENROUTER_API_KEY_NAME", API_KEY_ENV);
    crate::alphacode_core::env::set_var("ALPHACODE_OPENROUTER_ENV_FILE", ENV_FILE);
    crate::alphacode_core::env::set_var("ALPHACODE_OPENROUTER_CACHE_NAMESPACE", "azure-openai");
...
pub async fn get_bearer_token() -> Result<String> {
    ...
    {
        crate::alphacode_azure_auth::get_bearer_token(COGNITIVE_SCOPE).await
    }
    #[cfg(not(feature = "azure-auth"))]
    {
        anyhow::bail!("Azure Entra ID authentication requires the 'azure-auth' feature")
    }
}

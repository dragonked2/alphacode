...
pub fn get_active_account_override() -> Option<String> {
    crate::alphacode_base::auth::account_store::runtime_active_override(ACCOUNT_LABEL_PREFIX)
}

pub fn primary_account_label() -> String {
    crate::alphacode_base::auth::account_store::canonical_account_label(ACCOUNT_LABEL_PREFIX, 1)
}
...
fn relabel_accounts(auth: &mut AlphacodeAuthFile) -> bool {
    let outcome = crate::alphacode_base::auth::account_store::relabel_accounts(
        ACCOUNT_LABEL_PREFIX,
...
pub fn alphacode_path() -> Result<PathBuf> {
    Ok(crate::storage::alphacode_dir()?.join("auth.json"))
}

// ---- Multi-account helpers ----

/// Read the alphacode auth file, auto-migrating from legacy format if needed.
pub fn load_auth_file() -> Result<AlphacodeAuthFile> {
    let path = alphacode_path()?;
    if !path.exists() {
        return Ok(AlphacodeAuthFile::default());
    }

    crate::storage::harden_secret_file_permissions(&path);

    let mut auth: AlphacodeAuthFile = crate::storage::read_json(&path)
        .with_context(|| format!("Could not read alphacode credentials from {:?}", path))?;
...
/// Write the alphacode auth file (multi-account format).
pub fn save_auth_file(auth: &AlphacodeAuthFile) -> Result<()> {
    let auth_path = alphacode_path()?;

    let clean = AlphacodeAuthFile {
...
pub fn active_account_label() -> Option<String> {
    ...
    crate::alphacode_base::auth::account_store::active_account_label(
        get_active_account_override(),
        auth.active_anthropic_account,
        &auth.anthropic_accounts,
        |account| account.label.as_str(),
...
pub fn set_active_account(label: &str) -> Result<()> {
    ...
    crate::alphacode_base::auth::account_store::set_active_account(
        label,
        &auth.anthropic_accounts,
        &mut auth.active_anthropic_account,
...
pub fn upsert_account(account: AnthropicAccount) -> Result<String> {
    let mut auth = load_auth_file()?;
    let label = crate::alphacode_base::auth::account_store::upsert_account(
        ACCOUNT_LABEL_PREFIX,
        &mut auth.anthropic_accounts,
        &mut auth.active_anthropic_account,
        account,
...
/// Falls through Claude Code -> alphacode accounts -> OpenCode, preferring non-expired tokens.
pub fn load_credentials() -> Result<ClaudeCredentials> {
    ...
    // prompt, so it is read once at import time and copied into alphacode's own
    ...
    if let Ok(creds) = load_alphacode_credentials() {
        if creds.expires_at > now_ms {
            return Ok(creds);
        }
        expired_candidates.push(("alphacode", creds));
    }
    ...
    anyhow::bail!("No Claude OAuth credentials found (checked Claude Code, alphacode, OpenCode)")
}

/// Load credentials for a specific alphacode account by label.
...
/// Load credentials from the active alphacode account (multi-account aware).
fn load_alphacode_credentials() -> Result<ClaudeCredentials> {
    let auth = load_auth_file()?;
    if auth.anthropic_accounts.is_empty() {
        anyhow::bail!("No anthropic accounts configured in alphacode auth.json");
    }
    ...
    let account = auth
        ...
        .context("No anthropic accounts in alphacode auth.json")?;
...
/// them into a alphacode-managed Anthropic account so future loads and token
/// refreshes are served from alphacode's own `auth.json`.
///
...
/// rotates into alphacode's store on refresh, so the copy stays usable after the

...
fn globally_preferred_model_rank(model: &str) -> (u8, usize) {
    ...
    // Some catalogs expose the clean release id instead of alphacode's Sol route.
...
/// order. The Claude/OpenAI subscription default bias mirrors alphacode's global
...
fn normalize_model_for_preference(model: &str) -> String {
    let mut id = crate::alphacode_provider_core::model_id::canonical(model);
    ...
    id = crate::alphacode_provider_core::model_id::strip_date_suffix(&id).to_string();
...
fn route_matches_activation(route: &ModelRoute, activation: &AuthActivationResult) -> bool {
    ...
    match provider_id {
        ...
        "alphacode" => {
            // Alphacode subscription routes deliberately keep their managed public
            // identity even though the runtime reuses OpenRouter transport code.
            return matches!(
                api_method,
                crate::provider::ModelRouteApiMethod::AlphacodeSubscription
            );
        }
        "azure-openai" => {
...
/// `crate::alphacode_provider_core::auto_default_provider`: Claude wins when both families
...
fn normalized_login_provider_id(provider_id: &str) -> Option<&'static str> {
    match provider_id.trim().to_ascii_lowercase().as_str() {
        ...
        "alphacode" | "subscription" | "alphacode-subscription" => Some("alphacode"),
...
fn api_key_env_bindings_for_provider(provider_id: &str) -> Vec<(String, String)> {
    match provider_id {
        ...
        "alphacode" => vec![
            (
                crate::subscription_catalog::ALPHACODE_API_KEY_ENV.to_string(),
                crate::subscription_catalog::ALPHACODE_ENV_FILE.to_string(),
            ),
            (
                crate::subscription_catalog::ALPHACODE_API_BASE_ENV.to_string(),
                crate::subscription_catalog::ALPHACODE_ENV_FILE.to_string(),
            ),
        ],
        "bedrock" => vec![
...
/// process (issue #453).
///
/// `/login` persists API keys to the per-provider env file under the alphacode
...
fn sync_process_env_from_saved_credentials(
    ...
    for (env_key, env_file) in api_key_env_bindings_for_provider(provider_id) {
        ...
        crate::alphacode_core::env::set_var(&env_key, &file_value);
...
fn direct_provider_activation(provider_id: &str) -> Option<ProviderActivation> {
    let (runtime_id, active) = match normalized_login_provider_id(provider_id)? {
        ...
        "alphacode" => (RuntimeProviderId::Alphacode, ActiveProvider::OpenRouter),
...
pub fn model_switch_request_for_provider_id(
    ...
    match normalized_auth_provider_id(provider_id) {
        ...
        Some("alphacode") => model.to_string(),
...
mod tests {
    ...
    fn direct_login_provider_ids_are_normalized_with_display_labels() {
        for (hint, normalized, label) in [
            ...
            ("subscription", "alphacode", "Alphacode Subscription"),
    ...
    fn direct_login_provider_activation_sets_runtime_identity_and_active_provider() {
        ...
        for (provider, runtime, active) in [
            ...
            ("openrouter", "openrouter", "openrouter"),
            ("alphacode", "alphacode", "openrouter"),
            ...
            assert_eq!(
                std::env::var("ALPHACODE_RUNTIME_PROVIDER").as_deref(),
                Ok(runtime)
            );
            assert_eq!(
                std::env::var("ALPHACODE_ACTIVE_PROVIDER").as_deref(),
                Ok(active)
            );
            assert_eq!(
                std::env::var("ALPHACODE_INITIAL_PROVIDER_EXPLICIT").as_deref(),
                Ok("1")
            );
        }
    }
        ...
        for provider in crate::provider_catalog::login_providers() {
            let Some((normalized, runtime, active, switch_prefix)) = (match provider.target {
                crate::provider_catalog::LoginProviderTarget::Alphacode => {
                    Some(("alphacode", "alphacode", "openrouter", ""))
                }
    ...
    #[test]
    fn alphacode_auth_lifecycle_matches_only_managed_subscription_routes() {
        let activation = AuthActivationResult {
            provider_id: Some("alphacode".to_string()),
            provider_label: Some("Alphacode Subscription".to_string()),
            activated_model: Some("gpt-5.5".to_string()),
            expected_runtime: Some("alphacode-subscription".to_string()),
            expected_catalog_namespace: Some("alphacode-subscription".to_string()),
        };
        let routes = vec![
            ...
            route(
                ...
                crate::subscription_catalog::ALPHACODE_ROUTE_API_METHOD,
                true,
            ),
        ];
        ...
        assert!(
            report.ok(),
            "canonical Alphacode route should match: {report:?}"
        );
        assert_eq!(report.selectable_provider_routes, 1);
        assert_eq!(
            report.route_sample,
        ...
        assert_eq!(
            activation.model_switch_request("Alphacode Subscription", "gpt-5.5"),
            "gpt-5.5"
        );
    }
    ...
    fn post_auth_model_selection_keeps_catalog_order_for_unranked_providers() {
        ...
        // shared test-env lock (and a private ALPHACODE_HOME) or a sibling test's
        // cached catalog decides this assertion depending on ordering.
        let _env = EnvGuard::new(&["ALPHACODE_HOME"]);
    ...
    fn post_auth_model_selection_prefers_newest_live_release_for_unranked_provider() {
        ...
        let temp = tempfile::tempdir().expect("tempdir");
        crate::alphacode_core::env::set_var("ALPHACODE_HOME", temp.path());
        crate::alphacode_provider_openrouter::save_disk_cache_with_source_for_namespace(
            "cerebras",
            &[
                crate::alphacode_provider_openrouter::ModelInfo {
    ...
    /// Claude flagship over the OpenAI flagship to mirror alphacode's default model.
    #[test]
    ...
    fn onboarding_frontier_provider_preference_matrix() {
        use crate::alphacode_base::auth::{AuthState, AuthStatus, ProviderAuth};

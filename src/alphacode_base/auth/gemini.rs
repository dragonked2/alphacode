...
/// the env var keeps it consistent with every other API-key provider in alphacode.
...
///
/// Preference order:
/// 1. `ALPHACODE_GEMINI_CLI_PATH` (supports a full command like `npx @google/gemini-cli`)
/// 2. `gemini` on PATH
/// 3. `npx @google/gemini-cli`
pub fn gemini_cli_command() -> GeminiCliCommand {
    resolve_gemini_cli_command_with(
        std::env::var("ALPHACODE_GEMINI_CLI_PATH").ok().as_deref(),
...
pub fn tokens_path() -> Result<std::path::PathBuf> {
    Ok(crate::storage::alphacode_dir()?.join("gemini_oauth.json"))
}
...
pub fn load_tokens() -> Result<GeminiTokens> {
    ...
    if let Some(tokens) = crate::alphacode_base::auth::external::load_gemini_oauth_tokens() {
        return Ok(GeminiTokens {
    ...
    anyhow::bail!("No Gemini OAuth tokens found. Run `alphacode login --provider gemini`.")
}
...
async fn refresh_tokens_uncoordinated(tokens: &GeminiTokens) -> Result<GeminiTokens> {
    let result: Result<GeminiTokens> = async {
        let refreshed = crate::alphacode_base::auth::google_oauth::refresh_access_token(
            "Gemini",
            &gemini_client_id(),
...
pub async fn login(no_browser: bool) -> Result<GeminiTokens> {
        ...
        && let Ok(listener) = super::oauth::bind_callback_listener(0)
    {
        ...
        if browser_opened {
            ...
            eprintln!(
                "If the page says sign-in succeeded but alphacode does not continue within a few seconds, press Ctrl+C and retry with `--no-browser` to use the manual code flow."
            );

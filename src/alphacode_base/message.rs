...
pub use crate::alphacode_message_types::{
...
fn compile_static_regex(pattern: &str) -> Option<Regex> {
    match Regex::new(pattern) {
        ...
        Err(err) => {
            ...
            eprintln!("alphacode: failed to compile static regex: {err}");
            None
        }
    }
}
...
pub fn generated_image_visual_context_blocks(
    ...
    let mut reminder = format!(
        "<system-reminder>\nA provider-native image generation call created `{}`. Alphacode attached the image pixels as visual context for future turns because the active provider supports image input and the file is under the safe {} MB limit.\nFormat: {}",
        path,
...
pub fn generated_image_rendered_image(
    ...
    path: &str,
    output_format: &str,
) -> Option<crate::alphacode_session_types::RenderedImage> {
    let (media_type, data) = generated_image_payload(path, output_format)?;
    Some(crate::alphacode_session_types::RenderedImage {
        media_type,
        data,

pub use crate::alphacode_provider_env::{
...
pub use crate::alphacode_provider_metadata::*;
use std::collections::{HashMap, HashSet};

pub const OPENAI_COMPAT_LOCAL_ENABLED_ENV: &str = "ALPHACODE_OPENAI_COMPAT_LOCAL_ENABLED";
...
pub fn resolve_openai_compatible_profile_with_api_key_hint(
    ...
    if let Some(base) = env_override("ALPHACODE_OPENAI_COMPAT_API_BASE") {
        if let Some(normalized) = normalize_api_base(&base) {
            resolved.api_base = normalized;
        } else {
            eprintln!(
                "Warning: ignoring invalid ALPHACODE_OPENAI_COMPAT_API_BASE '{}'. Use https://... (or http://localhost).",
                base
            );
        }
    }

    if let Some(key_name) = env_override("ALPHACODE_OPENAI_COMPAT_API_KEY_NAME") {
        if is_safe_env_key_name(&key_name) {
            resolved.api_key_env = key_name;
        } else {
            eprintln!(
                "Warning: ignoring invalid ALPHACODE_OPENAI_COMPAT_API_KEY_NAME '{}'.",
                key_name
            );
        }
    }

    if let Some(env_file) = env_override("ALPHACODE_OPENAI_COMPAT_ENV_FILE") {
        if is_safe_env_file_name(&env_file) {
            resolved.env_file = env_file;
        } else {
            eprintln!(
                "Warning: ignoring invalid ALPHACODE_OPENAI_COMPAT_ENV_FILE '{}'.",
                env_file
            );
        }
    }

    if let Some(setup_url) = env_override("ALPHACODE_OPENAI_COMPAT_SETUP_URL") {
        resolved.setup_url = setup_url;
    }

    if let Some(model) = env_override("ALPHACODE_OPENAI_COMPAT_DEFAULT_MODEL") {
...
pub fn active_openai_compatible_display_name() -> Option<String> {
    if let Ok(profile_name) = std::env::var("ALPHACODE_NAMED_PROVIDER_PROFILE") {
    ...
    if let Ok(namespace) = std::env::var("ALPHACODE_OPENROUTER_CACHE_NAMESPACE") {
    ...
    let api_base = std::env::var("ALPHACODE_OPENROUTER_API_BASE")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
        .or_else(|| env_override("ALPHACODE_OPENAI_COMPAT_API_BASE"));
...
pub fn runtime_provider_display_name(provider_name: &str) -> String {
    if provider_name.eq_ignore_ascii_case("openrouter") {
        if let Ok(runtime_provider) = std::env::var("ALPHACODE_RUNTIME_PROVIDER")
...
pub fn openai_compatible_profile_static_models(profile: OpenAiCompatibleProfile) -> Vec<String> {
    ...
    match profile.id {
        ...
        // Dragonmeta (local `alpha serve`) always serves the Alpha model, so
        // keep "alpha" selectable before/without a live catalog refresh.
        "dragonmeta" => {
            push("alpha");
        }
        ...
        "minimax" => {
            // The M3 line (including the free tier) is the most recent
            // generation MiniMax publishes. List it first so it is the
            // default after saving a key, while keeping the documented
            // M2.x generations around for users who explicitly want them.
            push("MiniMax-M3");
            push("MiniMax-M3-free");
            push("MiniMax-M2.7");
        ...
        "tokenrouter" => {
            push("qwen/qwen3.8-max-free");
            push("deepseek/deepseek-v4-pro-0813-free");
            push("qwen/qwen3-235b-a22b-instruct-2507");
            push("deepseek/deepseek-chat");
            push("deepseek/deepseek-v3-0324");
...
pub fn openai_compatible_profile_context_limit(profile_id: &str, model: &str) -> Option<usize> {
    ...
    match profile_id.as_str() {
        ...
        _ => crate::alphacode_provider_core::models::open_weight_family_context_limit(&model),
    }
}
...
fn apply_openai_compatible_profile_env_impl(
    ...
    for var in vars {
        crate::alphacode_core::env::remove_var(var);
    }
...
fn inline_key_env_name(profile_name: &str) -> String {
    let suffix = profile_name
        ...
        .collect::<String>();
    format!("ALPHACODE_PROVIDER_{}_API_KEY", suffix)
}
...
pub fn apply_named_provider_profile_env_from_config(
    ...
    crate::alphacode_core::env::remove_var("ALPHACODE_PROVIDER_PROFILE_ACTIVE");
    crate::alphacode_core::env::remove_var("ALPHACODE_PROVIDER_PROFILE_NAME");
    crate::alphacode_core::env::remove_var("ALPHACODE_NAMED_PROVIDER_PROFILE");
    apply_openai_compatible_profile_env(None);
    crate::alphacode_core::env::set_var("ALPHACODE_OPENROUTER_API_BASE", &api_base);
    crate::alphacode_core::env::set_var("ALPHACODE_OPENROUTER_CACHE_NAMESPACE", profile_name);
    crate::alphacode_core::env::set_var("ALPHACODE_NAMED_PROVIDER_PROFILE", profile_name);
    ...
    match profile.auth {
        ...
        crate::config::NamedProviderAuth::Bearer | crate::config::NamedProviderAuth::Header => {
            let key_env = profile
                ...
                .or_else(|| {
                    profile.api_key.as_deref().map(str::trim).filter(|v| !v.is_empty()).map(|key| {
                        ...
                        crate::alphacode_core::env::set_var(&env_name, key);
            ...
            if let Some(key_env) = key_env {
                ...
                crate::alphacode_core::env::set_var("ALPHACODE_OPENROUTER_API_KEY_NAME", &key_env);
            }
                ...
                .filter(|v| !v.is_empty())
            {
                ...
                crate::alphacode_core::env::set_var("ALPHACODE_OPENROUTER_ENV_FILE", env_file);
            }
...
pub fn active_named_provider_profile_credential_source() -> Option<(String, String)> {
    ...
    let _profile_name = std::env::var("ALPHACODE_NAMED_PROVIDER_PROFILE")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())?;

    let key_env = std::env::var("ALPHACODE_OPENROUTER_API_KEY_NAME")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
        .unwrap_or_else(|| "OPENROUTER_API_KEY".to_string());
    let env_file = std::env::var("ALPHACODE_OPENROUTER_ENV_FILE")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
...
fn active_named_provider_profile_is_configured() -> Option<bool> {
    ...
    if std::env::var("ALPHACODE_OPENROUTER_ALLOW_NO_AUTH")

...
//! This lets iOS/web clients interact with alphacode sessions identically
//! to TUI clients.
//!
//! Architecture:
...
pub struct GatewayConfig {
    ...
    pub port: u16,
    /// Bind address (default: 127.0.0.1; override to 0.0.0.0 for Tailscale access)
...
async fn handle_pair_request(
    ...
    // Reload from disk - pairing codes are generated by `alphacode pair` CLI
    *reg = DeviceRegistry::load();
    ...
    let body = serde_json::json!({
        "token": token,
        "server_name": "alphacode",
        "server_version": crate::alphacode_build_meta::version(),
    });
    http_response(200, "OK", &body.to_string())
}
...
pub fn resolve_connect_host(bind_addr: &str) -> String {
    ...
    if let Some(host) = std::env::var("ALPHACODE_GATEWAY_HOST")
        .ok()

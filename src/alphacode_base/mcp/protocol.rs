...
pub struct McpServerConfig {
    /// Command for stdio servers. Empty for HTTP/SSE servers, which alphacode does
    ...
    /// URL for HTTP/SSE servers (Claude Code compat). Unused by alphacode today.
...
impl McpServerConfig {
    /// alphacode currently only supports stdio (command-based) MCP servers. A config
...
pub struct McpConfig {
    ...
    /// alphacode's historical `servers` key.
...
impl McpConfig {
    ...
    /// Only runs if ~/.alphacode/mcp.json doesn't exist yet.
    #[expect(
        clippy::collapsible_if,
        reason = "Import logic keeps source-specific MCP config handling explicit"
    )]
    fn import_from_external() {
        let alphacode_mcp = match crate::storage::alphacode_dir() {
            Ok(dir) => dir.join("mcp.json"),
            Err(_) => return,
        };

        if alphacode_mcp.exists() {
            ...
            if codex_config.exists() {
                if let Ok(config) = Self::load_from_codex_toml(&codex_config) {
                    ...
                    if count > 0 {
                        ...
                        // that a transport alphacode cannot run must not displace a
        ...
        if !imported.servers.is_empty() {
            if let Err(e) = imported.save_to_file(&alphacode_mcp) {
    ...
    /// order: `.alphacode/mcp.json`, then `.mcp.json` (Claude Code project config),
    ...
    fn load_project_locals(project_root: &std::path::Path) -> Self {
        ...
        for relative in [".alphacode/mcp.json", ".mcp.json", ".claude/mcp.json"] {
    ...
    /// Load from default locations (merges alphacode global + local, local overrides),
    ...
    /// (`.alphacode/mcp.json`, `.mcp.json`, `.claude/mcp.json`, and the per-project
    ...
    pub fn load_for_dir(project_dir: Option<&std::path::Path>) -> Self {
        ...
        // Load alphacode's own global config (~/.alphacode/mcp.json)
        if let Ok(alphacode_dir) = crate::storage::alphacode_dir() {
            let alphacode_mcp = alphacode_dir.join("mcp.json");
            if alphacode_mcp.exists() {
                if let Ok(config) = Self::load_from_file(&alphacode_mcp) {
        ...
        // alphacode only supports stdio servers today. Drop HTTP/SSE entries (common
    ...
    /// Merge `incoming` over `existing`, except that an entry alphacode cannot run
    ...
    /// working stdio server from `~/.alphacode/mcp.json` and then be dropped by the

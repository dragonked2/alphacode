//! Shared control logic behind the `/remote` command and `alphacode pair`.
//!
        ...
        if self.enabled && self.host_unknown {
            out.push_str(
                "\nCould not determine a reachable address for this machine. \
                 Set `ALPHACODE_GATEWAY_HOST` to a hostname or IP the other machine can reach.\n",
            );
        }
...
mod tests {
    use super::*;
    use crate::alphacode_gateway_types::PairedDevice;
    ...
    /// These tests mutate the process-global `ALPHACODE_HOME`, so they must
    ...
    /// `ALPHACODE_HOME` out from under a provider test mid-assertion, failing it
    ...
    impl Drop for HomeGuard {
        fn drop(&mut self) {
            match &self.previous {
                Some(prev) => crate::alphacode_core::env::set_var("ALPHACODE_HOME", prev),

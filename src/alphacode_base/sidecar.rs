...
const CLAUDE_CODE_ALPHACODE_NOTICE: &str = "You are alphacode, powered by Claude Code. You are a third-party CLI, not the official Claude Code CLI.";
...
impl Sidecar {
    ...
    /// 3. The live agent provider (works for EVERY provider alphacode supports:
    ...
    ///
    /// This is the universal path: it works for every provider alphacode supports,
...
fn build_claude_system_param(system: &str) -> Option<ClaudeApiSystem<'_>> {
    ...
    blocks.push(ClaudeApiSystemBlock {
        block_type: "text",
        text: CLAUDE_CODE_ALPHACODE_NOTICE,
    });
    if !system.is_empty() {
        blocks.push(ClaudeApiSystemBlock {
            block_type: "text",
...
///
/// The "You are Claude Code" identity spoof and alphacode notice are only valid
...
fn anthropic_sidecar_prefers_api_key() -> bool {
    match crate::alphacode_provider_core::runtime_env_pinned_mode(
        crate::alphacode_provider_core::DualAuthProvider::Anthropic,
    ) {
        Some(crate::alphacode_provider_core::AuthMode::ApiKey) => true,
        Some(crate::alphacode_provider_core::AuthMode::Oauth) => false,
...
mod tests {
    use super::*;
    use crate::alphacode_base::auth::codex;
    ...
    fn test_backend_selection_prefers_openai() {
        ...
        let temp = tempfile::TempDir::new().expect("create temp alphacode home");
        let _home = EnvVarGuard::set_path("ALPHACODE_HOME", temp.path());
    ...
    /// Every provider alphacode supports should drive the sidecar end-to-end via the

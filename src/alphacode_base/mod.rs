//! `alphacode-base`: foundational layer of the alphacode application core.
//!
//! This crate holds the downward-closed set of modules that the upper
//! server/tool/agent layer (`alphacode-app-core`) depends on: provider, auth,
...
//!
//! `alphacode-app-core` re-exports this crate via `pub use crate::alphacode_base::*`, so every
...
pub mod telegram;
pub mod telemetry {
    pub use crate::alphacode_telemetry_core::*;
}
...
// Embedding facade: the heavy ONNX/tokenizer backend (`embedding.rs` +
// `crate::alphacode_embedding`) is compiled only with `--features embeddings`
// (default builds use the no-op stub, keeping cold builds free of the tract /
// tokenizers dependency stack). Both expose the same public API.

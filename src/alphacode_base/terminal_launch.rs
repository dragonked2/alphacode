pub use crate::alphacode_terminal_launch::{
...
///
/// Resolution order: `ALPHACODE_SPAWN_HOOK` env (handled by config env overrides;
...
///
/// When a spawn hook is configured (`[terminal] spawn_hook` / `ALPHACODE_SPAWN_HOOK`),
/// the hook takes over the spawn: alphacode runs `<hook> <program> <args...>` with
/// `ALPHACODE_SPAWN_*` metadata env vars so external programs (tmux, kitty remote,
/// herd, window managers) control where and how the session appears. If the
/// hook cannot be started, alphacode falls back to its built-in terminal detection.
...
mod tests {
    ...
    fn spawn_via_hook_runs_hook_with_metadata_env() {
        ...
        std::fs::write(
            &hook_path,
            format!(
                "#!/bin/sh\nprintf '%s|%s|%s|%s' \"$ALPHACODE_SPAWN_KIND\" \"$ALPHACODE_SPAWN_SESSION_ID\" \"$ALPHACODE_SPAWN_SWARM_ID\" \"$*\" > {}\n",
        ...
        .spawn_env("ALPHACODE_SPAWN_SWARM_ID", "swarm-7");
        ...
        assert_eq!(
            recorded, "swarm-agent|ses_hooked|swarm-7|/usr/local/bin/alphacode --resume ses_hooked",
            "hook should receive metadata env and the alphacode command as argv"
        );
    }
}

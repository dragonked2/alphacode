...
pub fn create_goal(input: GoalCreateInput, working_dir: Option<&Path>) -> Result<Goal> {
    ...
    if let Some(id) = input.id.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
        goal.id = crate::alphacode_task_types::sanitize_goal_id(id);
    }
...
pub fn load_goal(
    ...
    let id = crate::alphacode_task_types::sanitize_goal_id(id);
...
pub fn goal_page_id(id: &str) -> String {
    format!("goal.{}", crate::alphacode_task_types::sanitize_goal_id(id))
}

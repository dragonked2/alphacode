...
//!
//! Tracks running servers in `~/.alphacode/servers.json` for discovery by clients.
...
pub fn registry_path() -> Result<PathBuf> {
    Ok(alphacode_dir()?.join("servers.json"))
}

/// Get the socket directory path
pub fn socket_dir() -> Result<PathBuf> {
    Ok(crate::storage::runtime_dir().join("alphacode"))
}
...
pub fn server_socket_path(name: &str) -> PathBuf {
    socket_dir()
        ...
        .unwrap_or_else(|_| std::env::temp_dir().join(format!("alphacode-{}.sock", name)))
}
...
pub fn server_debug_socket_path(name: &str) -> PathBuf {
    socket_dir()
        ...
        .unwrap_or_else(|_| std::env::temp_dir().join(format!("alphacode-{}-debug.sock", name)))
}

...
mod tests {
    ...
    fn minilm_embedding_is_numerically_stable_across_inference_engines() {
        let dir = std::env::var_os("HOME")
            .map(|h| std::path::PathBuf::from(h).join(".alphacode/models/all-MiniLM-L6-v2"))
    ...
    fn alt_model_related_beats_unrelated_if_present() {
        let dir = std::env::var_os("HOME")
            .map(|h| std::path::PathBuf::from(h).join("alphacode-memory-bench/models/e5-small-v2"))
            .filter(|d| is_model_available(d));
        let Some(d) = dir else {
    ...
    fn cross_encoder_scores_relevant_higher_if_present() {
        let dir = std::env::var_os("HOME")
            .map(|h| std::path::PathBuf::from(h).join("alphacode-memory-bench/models/ce-minilm-l6"))

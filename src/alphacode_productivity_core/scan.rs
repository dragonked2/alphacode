...
//! several GB. We keep a sidecar cache (`~/.alphacode/cache/productivity/summaries.json`)
...
fn cache_path() -> Result<PathBuf> {
    let dir = crate::alphacode_storage::alphacode_dir()?
        .join("cache")
        .join("productivity");
...
fn sessions_dir() -> Result<PathBuf> {
    Ok(crate::alphacode_storage::alphacode_dir()?.join("sessions"))
}

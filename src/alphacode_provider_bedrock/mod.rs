...
pub const REGION_ENV: &str = "ALPHACODE_BEDROCK_REGION";
#[cfg(not(feature = "aws-sdk"))]
const NO_AWS_SDK_SUPPORT: &str =
    "alphacode was built without AWS Bedrock support (feature `bedrock` disabled)";
...
impl BedrockProvider {
    pub fn new() -> Self {
        let model =
            std::env::var("ALPHACODE_BEDROCK_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.to_string());
        let provider = Self {
    ...
    pub fn has_credentials() -> bool {
        let explicitly_enabled = std::env::var("ALPHACODE_BEDROCK_ENABLE")
            .ok()
    ...
    async fn sdk_config() -> aws_types::SdkConfig {
        ...
        if let Some(profile) = profile {
            ...
            // which could otherwise override an explicit Alphacode Bedrock profile.
    ...
    async fn validate_credentials_if_requested() -> Result<()> {
        let validate = std::env::var("ALPHACODE_BEDROCK_VALIDATE_STS")
            .ok()
    ...
    pub fn configured_bearer_token() -> Option<String> {
        crate::alphacode_provider_env::load_api_key_from_env_or_config(API_KEY_ENV, ENV_FILE)
    }
    ...
    fn persisted_catalog_path() -> Result<std::path::PathBuf> {
        Ok(crate::alphacode_storage::app_config_dir()?.join("bedrock_models_cache.json"))
    }

    fn load_persisted_catalog() -> Option<PersistedCatalog> {
        let path = Self::persisted_catalog_path().ok()?;
        crate::alphacode_storage::read_json(&path).ok()
    }
    ...
    fn persist_catalog(
        ...
        if let Err(err) = crate::alphacode_storage::write_json(&path, &payload) {
    ...
    fn classify_error_message(raw: &str) -> String {
            ...
            || lower.contains("rate exceeded")
        {
            ...
            "AWS region is missing. Set AWS_REGION or ALPHACODE_BEDROCK_REGION."
        } else {
    ...
    fn inference_config() -> Option<InferenceConfiguration> {
        let max_tokens = std::env::var("ALPHACODE_BEDROCK_MAX_TOKENS")
            .ok()
            .and_then(|v| v.trim().parse::<i32>().ok())
            .filter(|v| *v > 0);
        let temperature = std::env::var("ALPHACODE_BEDROCK_TEMPERATURE")
            .ok()
            .and_then(|v| v.trim().parse::<f32>().ok())
            .filter(|v| (0.0..=1.0).contains(v));
        let top_p = std::env::var("ALPHACODE_BEDROCK_TOP_P")
            .ok()
            .and_then(|v| v.trim().parse::<f32>().ok())
            .filter(|v| (0.0..=1.0).contains(v));
        let stop_sequences = std::env::var("ALPHACODE_BEDROCK_STOP_SEQUENCES")
            .ok()
            .map(|v| {
                v.split(',')
                    .map(str::trim)
...
impl Provider for BedrockProvider {
    ...
    async fn complete(
        ...
        crate::alphacode_provider_core::log_provider_canonical_input(
            "bedrock",
            &model,
                    ...
                    Ok(Some(event)) => match event {
                        ...
                        ConverseStreamOutput::ContentBlockStop(_) => {
                            if current_tool.take().is_some()
                                && tx.send(Ok(StreamEvent::ToolUseEnd)).await.is_err()
                            {
                                crate::logging::warn(
...
mod tests {
    ...
    impl Drop for EnvVarGuard {
        fn drop(&mut self) {
            if let Some(value) = self.previous.as_ref() {
                crate::alphacode_core::env::set_var(self.key, value);
            } else {
    ...
    fn detects_env_credentials_requires_region_and_credential_hint() {
        ...
        // Sandbox under ALPHACODE_HOME: `app_config_dir()` honors it on every
        // platform (XDG_CONFIG_HOME alone does not redirect the config dir on
        // Windows, where dirs::config_dir() resolves to %APPDATA%).
        let _home = EnvVarGuard::set("ALPHACODE_HOME", temp.path().as_os_str());
        let _removed = [
        ...
        crate::alphacode_core::env::set_var(REGION_ENV, "us-east-1");
        assert!(!BedrockProvider::has_credentials());
        crate::alphacode_core::env::set_var("AWS_PROFILE", "test");
        assert!(BedrockProvider::has_credentials());
    }
    ...
    fn explicit_enable_marks_configured_for_instance_metadata_credentials() {
        ...
        assert!(BedrockProvider::has_credentials());
        crate::alphacode_core::env::remove_var("ALPHACODE_BEDROCK_ENABLE");
    }
    ...
    fn detects_bedrock_login_env_file_credentials() {
        let _guard = lock_test_env();
        let temp = tempfile::tempdir().unwrap();
        // Sandbox under ALPHACODE_HOME (see detects_env_credentials_requires_region_and_credential_hint).
        let _home = EnvVarGuard::set("ALPHACODE_HOME", temp.path().as_os_str());
        for key in [
            "ALPHACODE_BEDROCK_ENABLE",
            API_KEY_ENV,
    ...
    async fn bedrock_live_smoke_test() {
        if std::env::var("ALPHACODE_BEDROCK_LIVE_TEST").ok().as_deref() != Some("1") {
            return;
        }

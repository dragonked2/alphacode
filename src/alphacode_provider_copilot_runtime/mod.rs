...
//! tier detection, premium request modes), moved out of `alphacode-base` so
...
use crate::alphacode_provider_copilot::{
...
impl CopilotApiProvider {
    ...
    fn persisted_catalog_path() -> Result<std::path::PathBuf> {
        Ok(crate::alphacode_base::storage::app_config_dir()?.join("copilot_models_cache.json"))
    }
    ...
    pub fn new() -> Result<Self> {
        ...
        let model =
            std::env::var("ALPHACODE_COPILOT_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.to_string());
    ...
    fn env_premium_mode() -> u8 {
        match std::env::var("ALPHACODE_COPILOT_PREMIUM").ok().as_deref() {
    ...
    fn startup_prefetch_grace_ms() -> u64 {
        std::env::var("ALPHACODE_COPILOT_PREFETCH_STARTUP_GRACE_MS")
            .ok()
    ...
    /// If ALPHACODE_COPILOT_MODEL is set, this is a no-op (user override).
    pub async fn detect_tier_and_set_default(&self) {
        let detect_start = std::time::Instant::now();
        if std::env::var("ALPHACODE_COPILOT_MODEL").is_ok() {
            crate::alphacode_base::logging::info(
                "Copilot model overridden via ALPHACODE_COPILOT_MODEL, skipping tier detection",
            );
            self.mark_init_done();
            return;
        }
    ...
    async fn stream_request(
        ...
        for attempt in 0..MAX_RETRIES {
            if attempt > 0 {
                let delay = crate::alphacode_provider_core::attempt_tracker::retry_backoff_delay(
                    attempt,
                    RETRY_BASE_DELAY_MS,
                );
                crate::alphacode_base::logging::info(&format!(
            ...
            if Self::is_auth_error(status) && !attempted_auth_refresh {
                ...
                *self.bearer_token.write().await = None;
                crate::alphacode_base::logging::info("Copilot bearer token expired, refreshing...");
    ...
    async fn process_sse_stream(
        ...
        let sse_chunk_timeout = crate::alphacode_base::provider::stream_idle_timeout();
        ...
        loop {
            ...
            while let Some(line_end) = buffer.find('\n') {
                ...
                if let Some(data) = crate::alphacode_base::util::sse_data_line(&line) {
                    if data.trim() == "[DONE]" {
                        ...
                        crate::alphacode_base::copilot_usage::record_request(
                            input_tokens,
                            output_tokens,
                            true,
                        );

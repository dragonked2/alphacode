...
/// API spells the latency-optimized tier `auto`; alphacode also accepts `priority`
...
/// tier since alphacode cannot see the per-request tier split.
...
pub fn copilot_pricing(model: &str, zero_premium_mode: bool) -> RouteCheapnessEstimate {
    ...
    RouteCheapnessEstimate::included_quota(
        ...
        Some(if zero_premium_mode {
            "Copilot zero-premium mode: alphacode will send requests as agent/non-premium when possible"
                .to_string()
...
mod tests {
    use super::*;
    use crate::alphacode_provider_core::RouteBillingKind;

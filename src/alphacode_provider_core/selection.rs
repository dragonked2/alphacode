use crate::alphacode_provider_core::{ModelRoute, normalize_copilot_model_name};
use std::borrow::Cow;
...
pub fn cli_provider_arg_for_session_key(key: &str) -> Option<&'static str> {
    ...
    if let Some(route) = crate::alphacode_provider_core::auth_mode::AuthRoute::parse(base) {
...
/// Improved model route deduplication with comprehensive logging.
///
/// Deduplication strategy:
/// 1. Routes with identical (provider, model, api_method) are duplicates
/// 2. Generic openai-compatible routes are superseded by profile-specific ones
/// 3. When duplicates exist, the one with more information wins
///    (available > unavailable, more detail > less detail)
///
/// This ensures the model picker shows clean, non-redundant routes while
/// preserving all intentional auth-method distinctions (OAuth vs API key).
pub fn dedupe_model_routes(routes: Vec<ModelRoute>) -> Vec<ModelRoute> {
    ...
    if duplicates_found > 0 {
        crate::logging::debug(&format!(
            "Model route deduplication: {} routes deduplicated from {} total",
            duplicates_found,
            deduped.len() + duplicates_found
        ));
    }

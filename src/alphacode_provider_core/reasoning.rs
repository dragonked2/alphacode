...
//! same ordered values. `swarm` and `swarm-deep` are Alphacode UI sentinels rather
//! than wire-level provider values, but they belong in the selectable ladder.

/// OpenAI Responses API effort levels, followed by Alphacode's swarm modes.
...
/// Direct DeepSeek effort levels, followed by Alphacode's swarm modes.
...
pub fn inferred_reasoning_efforts(
    ...
    // The Alpha local endpoint (DragonMeta) and GLM-family models accept the
    // standard `reasoning_effort` field, so expose the same selectable ladder
    // as DeepSeek for them (the Alpha server serves a GLM-5.2-based model).
    if provider.contains("dragonmeta") || model.contains("glm") || model == "alpha" {
        return DEEPSEEK_SELECTABLE_EFFORTS.to_vec();
    }
    ...
    if is_anthropic {
        let caps = crate::alphacode_provider_core::anthropic_reasoning_caps(&model);
...
mod tests {
    ...
    #[test]
    fn dragonmeta_and_glm_models_use_deepseek_style_ladder() {
        // The Alpha local endpoint exposes the same `reasoning_effort` field
        // vocabulary as DeepSeek, so the remote picker must offer that ladder.
        assert_eq!(
            inferred_reasoning_efforts(Some("dragonmeta"), Some("alpha")),
            DEEPSEEK_SELECTABLE_EFFORTS
        );
        assert_eq!(
            inferred_reasoning_efforts(Some("openai-compatible"), Some("z-ai/glm-5.2")),
            DEEPSEEK_SELECTABLE_EFFORTS
        );
        assert_eq!(
            inferred_reasoning_efforts(Some("deepseek"), Some("deepseek-chat")),
            DEEPSEEK_SELECTABLE_EFFORTS
        );
    }
}

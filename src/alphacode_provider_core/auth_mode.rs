...
//!
//! Historically alphacode encoded that decision as free-form strings spread across
//! several overlapping vocabularies:
//!
//! | concept              | runtime env (`ALPHACODE_RUNTIME_PROVIDER`) | route / stable-id    | CLI `--provider` | model prefix     |
//! |----------------------|----------------------------------------|----------------------|------------------|------------------|
...
use crate::alphacode_provider_core::selection::ActiveProvider;
...
impl AuthRoute {
    ...
    /// Parse a dual-auth token from *any* of alphacode's overlapping vocabularies
    ...
    /// Canonical `ALPHACODE_RUNTIME_PROVIDER` value that pins this route.
...
/// Read `ALPHACODE_RUNTIME_PROVIDER` and return the dual-auth route it pins, if any.
pub fn runtime_env_auth_route() -> Option<AuthRoute> {
    let value = std::env::var("ALPHACODE_RUNTIME_PROVIDER").ok()?;
    AuthRoute::parse(&value)
}

/// Read `ALPHACODE_RUNTIME_PROVIDER` and resolve the dual-auth mode it pins for a
/// specific provider (or `None` for "auto").
pub fn runtime_env_pinned_mode(provider: DualAuthProvider) -> Option<AuthMode> {
    pinned_mode_for(
        provider,
        std::env::var("ALPHACODE_RUNTIME_PROVIDER").ok().as_deref(),
    )
}
...
mod tests {
    ...
    fn route_api_method_round_trips_through_model_route_api_method() {
        use crate::alphacode_provider_core::ModelRouteApiMethod;

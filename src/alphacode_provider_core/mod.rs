...
pub trait Provider: Send + Sync {
    ...
    /// Returns true if alphacode should use its own compaction for this provider.
    fn supports_compaction(&self) -> bool {
        false
    }

    /// Returns true if alphacode should proactively run its own summary-based compaction.
...
impl CredentialMode {
    /// Resolve the runtime-env pin (ALPHACODE_*_AUTH / route aliases) for a
...
/// Native tool result to send back to provider bridges that delegate tool execution to alphacode.
    ...
    CLIENT
        .get_or_init(|| {
            reqwest::Client::builder()
                .user_agent(ALPHACODE_USER_AGENT)
                ...
                .unwrap_or_else(|err| {
                        ...
                        .build()
                    {
                        ...
                        Err(fallback_err) => {
                            eprintln!(
                                "alphacode: failed to build fallback provider HTTP client: {fallback_err}"
                            );
                            reqwest::Client::new()
                        }
                    }
...
impl RuntimeKey {
    pub fn from_api_method(api_method: &ModelRouteApiMethod, _provider_label: &str) -> Self {
        match api_method {
            ModelRouteApiMethod::AlphacodeSubscription => Self::AlphacodeSubscription,
    ...
    pub fn stable_id(&self) -> String {
        match self {
            Self::AlphacodeSubscription => "alphacode-subscription".to_string(),
...
impl RouteSelection {
    ...
    pub fn routed_model_spec(&self) -> String {
        ...
        match &self.runtime_key {
            RuntimeKey::AlphacodeSubscription => model.to_string(),
...
/// through. Mirrors `crate::alphacode_base::provider::openrouter_catalog_model_id` but
/// lives here so [`RouteSelection::routed_model_spec`] has no upward dep.
fn openrouter_catalog_model_id(model: &str) -> String {
    let trimmed = model.trim();
    match crate::alphacode_provider_core::models::provider_for_model(trimmed) {
...
impl ModelRouteApiMethod {
    ...
    pub fn from_auth_route(route: crate::alphacode_provider_core::auth_mode::AuthRoute) -> Self {
        use crate::alphacode_provider_core::auth_mode::{AuthMode, DualAuthProvider};
    ...
    pub fn parse(value: &str) -> Self {
        ...
        if let Some(route) = crate::alphacode_provider_core::auth_mode::AuthRoute::parse(&lower) {
            return Self::from_auth_route(route);
        }
        match lower.as_str() {
            "alphacode-subscription" => Self::AlphacodeSubscription,
...
impl RouteCheapnessEstimate {
    pub fn metered(
        ...
        Self {
            ...
            estimated_reference_cost_micros: Some(
                (input_price_per_mtok_micros * CHEAPNESS_REFERENCE_INPUT_TOKENS
                    + output_price_per_mtok_micros * CHEAPNESS_REFERENCE_OUTPUT_TOKENS)
                    / 1_000_000,
            ),
            note: note.into(),
        }
    ...
    pub fn subscription(
        ...
        Self {
            ...
            estimated_reference_cost_micros: {
                let requests = included_requests_per_month.unwrap_or(1000);
                Some(monthly_price_micros / requests)
            },
            note: note.into(),
        }
    }
    ...
    pub fn unknown() -> Self {
        Self {
            billing_kind: RouteBillingKind::Unknown,
            source: RouteCostSource::Heuristic,
            confidence: RouteCostConfidence::Unknown,
            monthly_price_micros: None,
            input_price_per_mtok_micros: None,
            output_price_per_mtok_micros: None,
            cache_read_price_per_mtok_micros: None,

...
impl AnthropicContextMode {
    ...
    /// Whether alphacode should surface a distinct `[1m]` picker alias for this model.
...
/// small truncates mid-tool-call and silently ends an agent turn. alphacode used a
...
pub fn anthropic_max_output_tokens(model: &str) -> u32 {
    ...
    // Older/unknown generations keep the conservative 32K alphacode has always used.
    32_768
}
...
pub fn anthropic_strip_1m_suffix(model: &str) -> &str {
    crate::alphacode_provider_core::model_id::strip_long_context_suffix(model)
}
...
fn normalized_claude_caps_key(model: &str) -> String {
    let base = anthropic_strip_1m_suffix(model.trim())
        .to_ascii_lowercase()
        .replace('.', "-");
    crate::alphacode_provider_core::model_id::strip_date_suffix(&base).to_string()
}
...
mod tests {
    use super::*;
    use crate::alphacode_provider_core::ALL_CLAUDE_MODELS;
    ...
    fn max_output_tokens_never_undercut_the_legacy_default() {
        ...
        // flat 32K default alphacode shipped before, or turns that used to fit would
        // start truncating.

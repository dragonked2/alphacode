...
use crate::alphacode_tui::replay::TimelineEvent;
...
pub async fn export_video(
    ...
    crate::alphacode_tui::tui::mermaid::set_video_export_mode(true);
    let mut app = crate::alphacode_tui::tui::App::new_for_replay(session.clone());
    ...
    crate::alphacode_tui::tui::mermaid::set_video_export_mode(false);
...
pub async fn export_swarm_video(
    ...
    for pane in panes {
        let mut app = crate::alphacode_tui::tui::App::new_for_replay(pane.session.clone());
        if let Some(centered) = centered_override {
            app.set_centered(centered);
        }
        let frames = app
...
async fn render_svg_pipeline(
    ...
    let tmp_dir = std::env::temp_dir().join(format!("alphacode_video_{}", std::process::id()));
    if tmp_dir.exists() {

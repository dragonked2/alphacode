...
pub(crate) fn debug_draw_call_history(limit: usize) -> serde_json::Value {
    ...
    if samples.is_empty() {
        return serde_json::json!({
            "buffered_samples": 0,
            "idle_animation": crate::alphacode_tui::tui::ui::idle_animation_debug_json(),
...
fn slow_frame_threshold_ms() -> f64 {
    ...
    *THRESHOLD_MS.get_or_init(|| {
        std::env::var("ALPHACODE_TUI_SLOW_FRAME_MS")
            .ok()
...
fn flicker_detection_enabled() -> bool {
    ...
    #[cfg(not(test))]
    {
        ...
        *ENABLED.get_or_init(|| {
            std::env::var("ALPHACODE_TUI_FLICKER_DETECTION")
                .ok()
                .map(|raw| {
                    matches!(
...
pub(crate) fn recent_flicker_ui_notice() -> Option<FlickerUiNotice> {
    ...
    let log_hint = crate::logging::log_path()
        ...
        .unwrap_or_else(|| "~/.alphacode/logs/".to_string());

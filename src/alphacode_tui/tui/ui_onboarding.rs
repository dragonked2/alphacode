...
/// everywhere. Override with `ALPHACODE_ROUNDED_PILLS=on|off`.
fn rounded_pill_caps_supported() -> bool {
    ...
    *SUPPORTED.get_or_init(|| {
        ...
        if let Ok(raw) = std::env::var("ALPHACODE_ROUNDED_PILLS") {
...
fn import_summary_pills_line(
    focused: crate::alphacode_tui::tui::ImportSummaryPill,
    align: Alignment,
) -> Line<'static> {
    use crate::alphacode_tui::tui::ImportSummaryPill as Pill;
    let mut spans = Vec::new();
...
fn telemetry_settings_lines(
    highlighted: crate::alphacode_tui::tui::TelemetryChoice,
    env_forced_off: bool,
    align: Alignment,
) -> Vec<Line<'static>> {
    use crate::alphacode_tui::tui::TelemetryChoice as Choice;
    ...
    if env_forced_off {
        lines.push(
            Line::from(Span::styled(
                "Your environment already disables telemetry (ALPHACODE_NO_TELEMETRY).",
                dim,
            ))
            .alignment(align),
        );
        ...
        OnboardingWelcomeKind::Login {
            ...
            match import {
                ...
                None if error.is_some() => {
                    ...
                    // to hand the fix to it (it can run alphacode's auth-test and add

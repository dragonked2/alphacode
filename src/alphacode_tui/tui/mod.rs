...
// `alphacode-terminal-image` crate (shared with the `read` tool). Re-exported here
// so existing `crate::alphacode_tui::tui::image` / `crate::alphacode_tui::tui::image_metadata` paths keep working.
pub use crate::alphacode_terminal_image::display as image;
use crate::alphacode_terminal_image::metadata as image_metadata;
pub mod info_widget;
...
pub mod mermaid;
pub mod permissions {
    pub use crate::alphacode_tui_permissions::*;
}
mod redraw_schedule;
...
pub use crate::alphacode_tui_workspace::workspace_map;
pub use crate::alphacode_tui_workspace::workspace_map_widget;

pub use crate::alphacode_tui::generated_image::{
...
pub use crate::alphacode_tui_core::{
    CopySelectionPane, CopySelectionPoint, CopySelectionRange, CopySelectionStatus,
};
pub use crate::alphacode_tui_messages::DisplayMessage;
...
pub trait TuiState {
    ...
    /// from the `ALPHACODE_RUNTIME_PROVIDER` env var. The header must prefer this
    /// over its own env-based heuristic: the TUI client process often does not
    /// inherit `ALPHACODE_RUNTIME_PROVIDER` (it is set inside the agent/server
    ...
    fn active_dual_credential(
        &self,
        _provider: crate::alphacode_provider_core::ActiveProvider,
    ...
    /// Number of alphacode compactions already applied to this session, when known.
    ...
    fn has_notification(&self) -> bool {
        ...
        if crate::alphacode_tui::tui::ui::recent_flicker_ui_notice().is_some() {
            return true;
        }
...
pub use crate::alphacode_tui::provider::{cache_ttl_for_provider, cache_ttl_for_provider_model};
...
fn supports_reliable_zero_cache_read_warning(
    ...
    // OpenRouter/Alphacode-subscription routes can only be treated as reliable for zero-read
...
pub struct LoginImportPrompt {
    ...
    /// Whether the environment (ALPHACODE_NO_TELEMETRY / DO_NOT_TRACK) already
    ...
    pub fn filter_text(&self, entry: &PickerEntry) -> String {
        match self {
            ...
            Self::Model => {
                ...
                let pretty =
                    crate::alphacode_tui::tui::app::helpers::model_names::pretty_known_model_family(&entry.name)
                        .unwrap_or_default();
                format!(
                    "{} {} {} {} {}",
...
pub enum PickerAction {
    ...
    Usage {
        ...
        title: String,
        subtitle: String,
        status: crate::alphacode_tui::tui::usage_overlay::UsageOverlayStatus,
...
pub(crate) fn subscribe_metadata(
    ...
    resolve_subscribe_metadata(
        ...
        remote_working_dir,
        crate::alphacode_selfdev_types::client_selfdev_requested(),
    )
}
...
pub(crate) fn resolve_subscribe_metadata(
    ...
    if !selfdev && let Some(dir) = client_working_dir {
        ...
        while let Some(path) = current {
            if crate::build::is_alphacode_repo(path) {
                selfdev = true;
                break;
            }
            current = path.parent();
...
pub fn display_messages_from_session(session: &crate::session::Session) -> Vec<DisplayMessage> {
    let mut messages = crate::alphacode_tui_messages::display_messages_from_rendered_messages(

...
pub(super) use crate::alphacode_tui_tool_display::concise_tool_error_summary;
pub(crate) use crate::alphacode_tui_tool_display::{
...
fn truncate_end_display(s: &str, max_width: usize) -> String {
    // Fast path: ASCII strings are width-equal to their byte length
    if s.is_ascii() && s.len() <= max_width {
        return s.to_string();
    }
    ...
    let prefix = display_prefix_by_width(s, max_width.saturating_sub(1));
...
pub(super) fn render_batch_subcall_line(
    ...
    let display_name = resolve_display_tool_name(&tool.name);
    ...
    if let Some(intent) = intent_display {
        ...
        // moving it into the span so the borrow checker is happy.
        let show_detail = show_tool_call_details() || is_error;
        let intent_matches_summary = summary == intent;
        spans.push(Span::styled(" · ", Style::default().fg(dim_color())));
        spans.push(Span::styled(intent, Style::default().fg(tool_color())));
        if show_detail && !summary.is_empty() && !intent_matches_summary {
            spans.push(Span::styled(" · ", Style::default().fg(dim_color())));
...
pub(super) fn summarize_batch_running_tools_compact(running: &[ToolCall]) -> Option<String> {
    ...
    // Find the first running tool by batch index without allocating a sorted copy
    let mut first: Option<&ToolCall> = None;
    let mut first_idx = usize::MAX;
    for tool in running {
        let idx = batch_subcall_index(&tool.id).unwrap_or(usize::MAX);
        if idx < first_idx || (idx == first_idx && first.is_none_or(|f| tool.id < f.id)) {
            first = Some(tool);
            first_idx = idx;
        }
    }
    ...
    let tool = first?;
    let extra = running.len().saturating_sub(1);
    let label = match batch_subcall_index(&tool.id) {
        Some(idx) => format!("#{} {}", idx, tool.name),

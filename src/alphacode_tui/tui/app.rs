...
use crate::alphacode_tui::bus::{Bus, BusEvent, LoginCompleted, ToolEvent, ToolStatus};
use crate::alphacode_tui::compaction::CompactionEvent;
use crate::alphacode_tui::config::config;
use crate::alphacode_tui::id;
use crate::alphacode_tui::mcp::McpManager;
...
use crate::alphacode_tui::runtime_memory_log::RuntimeMemoryLogController;
use crate::alphacode_tui::session::{Session, StoredMessage};
...
pub enum AppRuntimeMode {
    /// Normal product TUI. The client renders state owned by the alphacode server.
    RemoteClient,
...
fn active_runtime_provider_key() -> Option<String> {
    std::env::var("ALPHACODE_RUNTIME_PROVIDER")
        .ok()
...
pub(in crate::alphacode_tui::tui::app) struct KvCacheRequestSignature {
    pub(in crate::alphacode_tui::tui::app) system_static_hash: u64,
    pub(in crate::alphacode_tui::tui::app) tools_hash: u64,
    pub(in crate::alphacode_tui::tui::app) messages_hash: u64,
    pub(in crate::alphacode_tui::tui::app) message_hashes: Vec<u64>,
    pub(in crate::alphacode_tui::tui::app) message_count: usize,
    pub(in crate::alphacode_tui::tui::app) tool_count: usize,
    pub(in crate::alphacode_tui::tui::app) system_static_chars: usize,
    pub(in crate::alphacode_tui::tui::app) tools_json_chars: usize,
    pub(in crate::alphacode_tui::tui::app) messages_json_chars: usize,
    pub(in crate::alphacode_tui::tui::app) ephemeral_hash: Option<u64>,
    pub(in crate::alphacode_tui::tui::app) ephemeral_chars: usize,
    pub(in crate::alphacode_tui::tui::app) ephemeral_message_count: usize,
}
...
/// An interactive "let a alphacode agent merge the diverged update for you" offer.
///
...
/// alphacode session, pre-loaded with a prompt to reconcile the branches, instead of
...
struct CommandSuggestionsSignature {
    ...
    inline_preview_kind: Option<crate::alphacode_tui::tui::PickerKind>,
}
...
pub struct App {
    ...
    // Interactive "spawn a alphacode agent to merge the diverged update" offer shown
    ...
    /// to target the agent repair brief (`alphacode auth-test --provider X`). `None`
    /// when unknown.
    ...
    copy_selection_anchor: Option<crate::alphacode_tui::tui::CopySelectionPoint>,
    copy_selection_cursor: Option<crate::alphacode_tui::tui::CopySelectionPoint>,
    copy_selection_pending_anchor: Option<crate::alphacode_tui::tui::CopySelectionPoint>,
    ...
    copy_selection_edge_autoscroll: Option<(crate::alphacode_tui::tui::CopySelectionPane, bool)>,
    ...
    remote_resolved_credential: Option<crate::alphacode_provider_core::ResolvedCredential>,
    ...
    pending_account_picker_action: Option<crate::alphacode_tui::tui::AccountPickerAction>,
...
impl App {
    ...
    pub(in crate::alphacode_tui::tui::app) fn begin_remote_kv_cache_request(
        &mut self,
    ...
    fn push_cold_cache_warning_for_baseline(
        ...
        let message = match trigger {
            ...
            ColdCacheWarningTrigger::RequestStart => format!(
                ...
                crate::alphacode_tui::tui::format_compact_age(expired_ago_secs),
                token_label
            ),
        };
...
// The cache-relevant projection lives in `alphacode-message-types` (re-exported
// through `crate::message`) so this local path and the server event path in
// `alphacode-app-core::agent::kv_cache_request_event` hash messages identically.
// If the two projections drift, remote sessions report false
// `harness:_prefix_changed` KV-cache misses.
use crate::alphacode_tui::message::{cache_relevant_message_value, cache_relevant_messages};

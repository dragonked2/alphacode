...
/// Extract the file stem from a local model path (`…/Alpha-Q4_K_M-clean.gguf` ->
/// `Alpha-Q4_K_M-clean`), so the info widget / status line don't display the raw
/// on-disk path for llama-server or other local-model endpoints.
...
/// Home-relative directory label, e.g. `/home/me/alphacode` -> `~/alphacode`. Does not

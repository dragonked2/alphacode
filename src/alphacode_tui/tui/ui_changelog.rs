...
fn parse_changelog() -> Vec<ChangelogEntry<'static>> {
    let changelog: &'static str = crate::alphacode_build_meta::CHANGELOG;
    parse_changelog_from_impl(changelog)
}
    ...
    GROUPS
        .get_or_init(|| {
            ...
            group_changelog_entries_impl(
                ...
                crate::alphacode_build_meta::git_date(),
            )
        })
        .clone()
}
...
/// Reads the last-seen commit hash from ~/.alphacode/last_seen_changelog,
...
pub(super) fn get_unseen_changelog_entries() -> &'static Vec<String> {
    ...
    ENTRIES.get_or_init(|| {
        ...
        let state_file = dirs::home_dir()
            .map(|h| h.join(".alphacode").join("last_seen_changelog"))
            .unwrap_or_else(|| std::path::PathBuf::from(".alphacode/last_seen_changelog"));

use super::*;
use crate::alphacode_tui::tui::ui::{self, WrappedLineMap};
    ...
    for (idx, line) in wrapped_lines.iter().enumerate() {
        if let Some(hash) = super::super::mermaid::parse_image_placeholder(line) {
            ...
            image_regions.push(ImageRegion {
                ...
                width: 0,
                render: crate::alphacode_tui_messages::ImageRegionRender::Crop,
            });
            ...
            super::super::mermaid::parse_inline_image_placeholder(line)
        {
            ...
            image_regions.push(ImageRegion {
                ...
                height,
                width: cols,
                render: crate::alphacode_tui_messages::ImageRegionRender::Fit,
            });
        }
    }
    image_regions
...
fn assistant_message_copy_targets(
    ...
    crate::alphacode_tui::tui::markdown::extract_copy_targets_from_rendered_lines(rendered_lines)
}
...
fn header_prep_signature(app: &dyn TuiState, width: u16) -> u64 {
    ...
    // Credential changes alter the model line's active-route provider tag
    // (e.g. `api-key:openai`). Hashing the auth generation (cheap atomic
    // load) means `/login` and account edits repaint the header on the very
    // next frame instead of waiting out the TTL, which is what lets the TTL
    // itself be sized for slow background drift.
    crate::alphacode_base::auth::auth_status_generation().hash(&mut hasher);
    ...
    match role {
        ...
        "tool" => {
            ...
            if let Some(member) = spawned_member_for_tool(msg, &ctx.swarm_members) {
                for line in crate::alphacode_tui::tui::info_widget::swarm_gallery::render_swarm_chat_card_lines(
...
pub(super) fn prepare_body_incremental(
    ...
    let mermaid_epoch_before = crate::alphacode_tui::tui::mermaid::deferred_render_epoch();
    let centered = app.centered_mode();

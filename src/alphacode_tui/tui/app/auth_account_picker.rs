...
impl App {
    pub(crate) fn open_account_center(&mut self, provider_filter: Option<&str>) {
        ...
        let mut summary = crate::alphacode_tui::tui::account_picker::AccountPickerSummary {
        ...
        for provider in providers {
            ...
            match auth_state {
                crate::alphacode_base::auth::AuthState::Available => summary.ready_count += 1,
                crate::alphacode_base::auth::AuthState::Expired => summary.attention_count += 1,
                crate::alphacode_base::auth::AuthState::NotConfigured => summary.setup_count += 1,
            }
            ...
            let state_label = match auth_state {
                crate::alphacode_base::auth::AuthState::Available => "ready",
    ...
    pub(crate) fn open_account_add_replace_flow(&mut self, provider_filter: Option<&str>) {
        ...
        if include_openai {
            ...
            for account in crate::alphacode_base::auth::codex::list_accounts().unwrap_or_default() {
                let label = account.label.clone();
                items.push(AccountPickerItem::action(
                    "openai",
                    "OpenAI",
    ...
    pub(crate) fn open_account_picker(&mut self, provider_filter: Option<&str>) {
        ...
        self.inline_interactive_state = Some(crate::alphacode_tui::tui::InlineInteractiveState {
            kind: crate::alphacode_tui::tui::PickerKind::Account,
    ...
    fn build_all_inline_account_picker(&self) -> (Vec<crate::alphacode_tui::tui::PickerEntry>, usize) {
        ...
        let next_claude = crate::alphacode_base::auth::claude::next_account_label()
            .unwrap_or_else(|_| crate::alphacode_base::auth::claude::primary_account_label());
        let next_openai = crate::alphacode_base::auth::codex::next_account_label()
        ...
        for account in &claude_accounts {
            ...
            models.push(crate::alphacode_tui::tui::PickerEntry {
    ...
    fn build_claude_inline_account_picker(&self) -> (Vec<crate::alphacode_tui::tui::PickerEntry>, usize) {
        ...
        let active_label = crate::alphacode_base::auth::claude::active_account_label()
            .unwrap_or_else(crate::alphacode_base::auth::claude::primary_account_label);
        let next_label = crate::alphacode_base::auth::claude::next_account_label()
            .unwrap_or_else(|_| crate::alphacode_base::auth::claude::primary_account_label());
        let now_ms = chrono::Utc::now().timestamp_millis();
    ...
    fn build_openai_inline_account_picker(&self) -> (Vec<crate::alphacode_tui::tui::PickerEntry>, usize) {
        ...
        let active_label = crate::alphacode_base::auth::codex::active_account_label()
            .unwrap_or_else(crate::alphacode_base::auth::codex::primary_account_label);
        let next_label = crate::alphacode_base::auth::codex::next_account_label()
            .unwrap_or_else(|_| crate::alphacode_base::auth::codex::primary_account_label());
        let now_ms = chrono::Utc::now().timestamp_millis();
    ...
    pub(crate) fn handle_account_picker_command(
        &mut self,
        command: crate::alphacode_tui::tui::account_picker::AccountPickerCommand,
    ) {
        match command {
            crate::alphacode_tui::tui::account_picker::AccountPickerCommand::OpenAccountCenter {
                provider_filter,
            } => self.open_account_center(provider_filter.as_deref()),
            crate::alphacode_tui::tui::account_picker::AccountPickerCommand::OpenAddReplaceFlow {
                provider_filter,
            } => self.open_account_add_replace_flow(provider_filter.as_deref()),
            crate::alphacode_tui::tui::account_picker::AccountPickerCommand::SubmitInput(input) => {
                self.input = input;
                self.cursor_pos = self.input.len();
                self.submit_input();
            }
            crate::alphacode_tui::tui::account_picker::AccountPickerCommand::PromptValue {
                prompt,
                command_prefix,
                empty_value,
                status_notice,
            } => self.prompt_account_value(prompt, command_prefix, empty_value, status_notice),
            crate::alphacode_tui::tui::account_picker::AccountPickerCommand::PromptNew { provider } => {
                match provider {
            ...
            other => {
                if let Some(command) = crate::alphacode_tui::tui::app::auth::account_command_from_picker(&other) {
                    crate::alphacode_tui::tui::app::auth::execute_account_command_local(self, command);
                }
            }
        }
    }
    ...
    pub(crate) fn prompt_new_account_label(
        &mut self,
        provider: crate::alphacode_tui::tui::account_picker::AccountProviderKind,
    ) {

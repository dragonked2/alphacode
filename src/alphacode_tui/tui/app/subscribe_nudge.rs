...
//!     alphacode account credentials, never in replay/test runtimes.
//!
//! `/subscribe` renders the full pitch and points at `/login alphacode`.
...
const SUPPORT_NUDGE_NOTICE: &str = "Support alphacode: /subscribe";
...
/// Persisted nudge bookkeeping (one small JSON file under the alphacode dir).
...
pub(super) fn subscribe_pitch_markdown() -> String {
    let mut message = String::from("Subscribe to alphacode\n\n");
    ...
    message.push_str("  - No key management: sign in once in the browser, alphacode routes the rest\n");
    message.push_str("  - Automatic failover routing when a provider has a bad day\n");
    message.push_str("  - Funds alphacode development - alphacode is open source\n");
    ...
    message.push_str("\nStart: /login alphacode (browser approval, no keys in the terminal)\n");
...
impl App {
    ...
    pub(super) fn claim_subscribe_nudge(&mut self, trigger: SubscribeNudgeTrigger) -> bool {
        ...
        // Never pitch existing alphacode account holders.
    ...
    pub(super) fn show_subscribe_pitch(&mut self) {
        ...
        self.set_status_notice("Subscribe: /login alphacode to start");
    }
}
...
mod tests {
    ...
    fn long_task_message_reads_support_wording_with_elapsed() {
        ...
        assert_eq!(
            message,
            "✦ alphacode just worked 1h 23m for you. Support alphacode: /subscribe"
        );
    ...
    fn pitch_lists_reasons_plans_and_next_step() {
        ...
        assert!(pitch.contains("/login alphacode"));

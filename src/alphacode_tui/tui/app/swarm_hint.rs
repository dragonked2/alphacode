...
pub(super) fn hint_message() -> String {
    "\u{2699} Swarm routing (models, effort) is configured by a prompt, not a config file. Edit ~/.alphacode/swarm-prompt.md (or ./.alphacode/swarm-prompt.md) to tune it".to_string()
}
...
impl App {
    ...
    pub(in crate::alphacode_tui::tui::app) fn maybe_surface_swarm_config_hint(&mut self) {
...
mod tests {
    ...
    fn state_persists_show_count() {
        ...
        let prev = std::env::var_os("ALPHACODE_HOME");

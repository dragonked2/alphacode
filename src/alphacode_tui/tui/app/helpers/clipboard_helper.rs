...
mod tests {
    ...
    fn missing_helper_binary_falls_through() {
        assert!(!copy_via_clipboard_helper(
            "alphacode-nonexistent-clipboard-helper",
            &[],
            "hello"
        ));
    }
...
mod ordering_tests {
    ...
    impl StubPath {
        fn with(stubs: &[(&str, &str)]) -> Self {
            ...
            crate::alphacode_core::env::set_var("PATH", dir.path());
            Self { _dir: dir, prev }
        }
    }
    ...
    impl Drop for StubPath {
        fn drop(&mut self) {
            match self.prev.take() {
                Some(prev) => crate::alphacode_core::env::set_var("PATH", prev),
                None => crate::alphacode_core::env::remove_var("PATH"),
            }
        }
    }

...
impl App {
    pub(in crate::alphacode_tui::tui::app) fn build_scroll_test_content(
        diagrams: usize,
        padding: usize,
    ...
    pub(in crate::alphacode_tui::tui::app) fn run_side_panel_latency_bench(
        &mut self,
        raw: Option<&str>,
    ) -> String {
        ...
        crate::alphacode_tui::tui::markdown::set_diagram_mode_override(Some(self.diagram_mode));
        ...
        let result = (|| -> Result<serde_json::Value, String> {
            ...
            terminal
                .draw(|f| crate::alphacode_tui::tui::ui::draw(f, self))
                .map_err(|e| format!("side-panel-latency baseline draw error: {}", e))?;

            let diff_area = crate::alphacode_tui::tui::ui::last_layout_snapshot()
                .and_then(|layout| layout.diff_pane_area)
                .ok_or_else(|| "side-panel-latency: diff pane area missing".to_string())?;
            let total_lines = crate::alphacode_tui::tui::ui::pinned_pane_total_lines();
            ...
            for idx in 0..total_runs {
                ...
                let before_frame = crate::alphacode_tui::tui::visual_debug::latest_frame();
                let before_frame_id = before_frame.as_ref().map(|frame| frame.frame_id);
                let scroll_before = if self.diff_pane_scroll == usize::MAX {
                    crate::alphacode_tui::tui::ui::last_diff_pane_effective_scroll()
                } else {
                ...
                if scroll_only {
                    scroll_only_count += 1;
                    std::thread::sleep(crate::alphacode_tui::tui::redraw_interval(self));
                }
                terminal
                    .draw(|f| crate::alphacode_tui::tui::ui::draw(f, self))
                    .map_err(|e| format!("side-panel-latency draw error: {}", e))?;
                let latency_ms = started.elapsed().as_secs_f64() * 1000.0;
                let after_frame = crate::alphacode_tui::tui::visual_debug::latest_frame();
                let after_frame_id = after_frame.as_ref().map(|frame| frame.frame_id);
                let scroll_after = crate::alphacode_tui::tui::ui::last_diff_pane_effective_scroll();
            ...
            Ok(serde_json::json!({
                ...
                "notes": [
                    ...
                    "It does not include terminal emulator/compositor/image protocol wall-clock paint latency outside alphacode."
                ]
            }))
        })();
    ...
    pub(in crate::alphacode_tui::tui::app) fn run_mermaid_ui_bench(&mut self, raw: Option<&str>) -> String {
        ...
        self.status_notice = None;
        crate::alphacode_tui::tui::markdown::set_diagram_mode_override(Some(self.diagram_mode));
        crate::alphacode_tui::tui::clear_side_panel_render_caches();
        crate::alphacode_tui::tui::reset_side_panel_debug_stats();
        crate::alphacode_tui::tui::markdown::reset_debug_stats();
        crate::alphacode_tui::tui::mermaid::reset_debug_stats();
        ...
        if !keep_mermaid_cache {
            let _ = crate::alphacode_tui::tui::mermaid::clear_cache();
        }
        ...
        let result = (|| -> Result<serde_json::Value, String> {
            ...
            for frame_idx in 0..frames {
                let before_stats = crate::alphacode_tui::tui::mermaid::debug_stats();
                ...
                let after_stats = crate::alphacode_tui::tui::mermaid::debug_stats();
                let latest_frame = crate::alphacode_tui::tui::visual_debug::latest_frame();
            ...
            Ok(serde_json::json!({
                ...
                "final_mermaid_stats": crate::alphacode_tui::tui::mermaid::debug_stats(),
                "samples": if include_samples { serde_json::to_value(&samples).unwrap_or(serde_json::Value::Null) } else { serde_json::Value::Null },
                "notes": [
    ...
    fn capture_scroll_test_step(
        ...
        if let Err(e) = terminal.draw(|f| crate::alphacode_tui::tui::ui::draw(f, self)) {
            return Err(format!("draw error ({}): {}", label, e));
        }
        let draw_ms = draw_start.elapsed().as_secs_f64() * 1000.0;

        let frame = crate::alphacode_tui::tui::visual_debug::latest_frame();
        ...
        let diagram_area_rect =
            diagram_area_capture.map(crate::alphacode_tui::tui::layout_utils::rect_from_capture);
        ...
        if let (Some(area), Some(state)) = (
            ...
            for entry in state {
                let last_area = entry
                    .get("last_area")
                    .and_then(|v| v.as_str())
                    .and_then(crate::alphacode_tui::tui::layout_utils::parse_area_spec);
                if let Some(render_area) = last_area
                    && crate::alphacode_tui::tui::layout_utils::rect_contains(area, render_area)
                {
    ...
    pub(in crate::alphacode_tui::tui::app) fn run_scroll_test(&mut self, raw: Option<&str>) -> String {
        ...
        crate::alphacode_tui::tui::markdown::set_diagram_mode_override(Some(diagram_mode));
        ...
        let baseline_frame = crate::alphacode_tui::tui::visual_debug::latest_frame();
        ...
        let mermaid_scroll_sim =
            serde_json::to_value(crate::alphacode_tui::tui::mermaid::debug_test_scroll(None)).ok();
    ...
    pub(in crate::alphacode_tui::tui::app) fn run_widget_stability(&mut self, raw: Option<&str>) -> String {
        ...
        let baseline = crate::alphacode_tui::tui::visual_debug::latest_frame();
        ...
        let report = crate::alphacode_tui::tui::info_widget_stability::analyze_frames_with_scroll(
            &frames,
            &scroll_tops_abs,
        );

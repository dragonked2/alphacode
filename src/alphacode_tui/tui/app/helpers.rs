...
use crate::alphacode_tui::tui::info_widget::{AmbientWidgetData, GitInfo, MemoryInfo};
use crate::alphacode_tui::tui::session_picker::ResumeTarget;
...
/// NO_BROWSER/ALPHACODE_NO_BROWSER and refuses to open anything from test binaries
...
pub(crate) fn open_path_or_url_detached(
    ...
    if crate::alphacode_base::auth::browser_suppressed(false) {
        return Err(std::io::Error::other(
            "opening files/URLs is suppressed (NO_BROWSER/ALPHACODE_NO_BROWSER or test harness)",
        ));
    }
...
pub(super) struct CachedContextSnapshot {
    ...
    pub snapshot: crate::alphacode_tui::tui::ContextSnapshot,
}
...
pub(super) fn launch_client_executable() -> PathBuf {
    crate::build::client_update_candidate(crate::alphacode_selfdev_types::client_selfdev_requested())
        .map(|(path, _label)| path)
        .or_else(|| std::env::current_exe().ok())
        .unwrap_or_else(|| PathBuf::from("alphacode"))
}
...
pub(super) fn inferred_reasoning_efforts(
    ...
    crate::alphacode_provider_core::inferred_reasoning_efforts(provider_name, model_name)
}
...
/// Spawn a new terminal window that resumes a alphacode session.
...
/// Build the terminal command used to spawn a brand-new alphacode session.
...
fn build_fresh_session_command(socket: Option<&str>) -> crate::terminal_launch::TerminalCommand {
    ...
    crate::terminal_launch::TerminalCommand::new(&exe, args)
        .title("alphacode · new session".to_string())
        .kind("new-terminal")
        .fresh_spawn()
}

/// Spawn a brand-new alphacode session in a new terminal window, staying on the
...
fn resumed_window_title(session_id: &str) -> String {
    ...
    crate::alphacode_base::process_title::terminal_window_title(
        icon,
...
pub(super) fn clipboard_image() -> Option<(String, String)> {
            ...
            && !html_output.stdout.is_empty()
        {
            ...
            if let Some(url) = extract_image_url(&html) {
                crate::logging::info(&format!(
                    ...
                    crate::alphacode_core::util::truncate_str(&url, 80)
                ));
    ...
    #[cfg(target_os = "macos")]
    {
        let temp_path = std::env::temp_dir().join("alphacode_clipboard.png");
        let script = format!(
...
fn gather_memory_info_inner(working_dir: Option<String>) -> Option<MemoryInfo> {
    ...
    use crate::alphacode_tui::memory::MemoryManager;
    ...
    let (graph_nodes, graph_edges) = crate::alphacode_tui::tui::info_widget::build_graph_topology(

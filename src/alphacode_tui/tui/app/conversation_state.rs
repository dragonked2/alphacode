...
impl App {
    ...
    pub(super) fn format_compaction_started_message(trigger: &str) -> String {
        ...
        format!(
            "Compacting context ({}) — summarizing older messages to stay within the limit.",
            strategy
        )
    }
    ...
    pub(super) fn format_compaction_complete_message(
        ...
        let reason = match event.trigger.as_str() {
            "auto_recovery" => "context full",
            _ => "approaching limit",
        };
        let strategy = Self::format_compaction_strategy_label(&event.trigger);
        let mut message = format!("Context compacted ({strategy}) — {reason}");
    ...
    pub(super) fn format_emergency_compaction_message(
        ...
        let mut message = "Context compacted (emergency) — older messages dropped".to_string();
    ...
    pub(super) fn reseed_compaction_from_provider_messages(&mut self) {
        if self.is_remote
            || (!self.provider.uses_alphacode_compaction() && self.session.compaction.is_none())
        {
            return;
        }
    ...
    pub(super) fn messages_for_provider(&mut self) -> (Vec<Message>, Option<CompactionEvent>) {
        ...
        match compaction.try_write() {
            Ok(mut manager) => {
                ...
                if self.provider.uses_alphacode_compaction() {
                    ...
                    match action {
                        crate::compaction::CompactionAction::BackgroundStarted { trigger } => {
                            // Avoid stacking "Compacting..." boxes on every
                            // turn-loop iteration: status notice is enough
                            // when the last line is already a compaction note.
                            let already_shown = self.display_messages.last().is_some_and(|m| {
                                m.role == "system"
                                    && (m.content.starts_with("Compacting context")
                                        || m.content.starts_with("Context compacted"))
                            });
                            if !already_shown {
    ...
    pub(super) fn handle_compaction_event(&mut self, event: CompactionEvent) {
        ...
        // Coalesce rapid-fire compactions: if the last transcript line is
        // already a compaction notice, replace it instead of stacking 5+
        // identical boxes (the "ugly spam" reported when context-limit
        // retries fire in a loop).
        if let Some(last) = self.display_messages.last() {
            let is_compaction = last.role == "system"
                && (last.content.starts_with("Context compacted")
                    || last.content.starts_with("📦 Compacted")
                    || last.content.starts_with("📦 Emergency")
                    || last.content.starts_with("Compacting context"));
            if is_compaction {
                let idx = self.display_messages.len() - 1;
                let _ = self.replace_display_message_content(idx, message);
                return;
            }
        }
    ...
    pub(in crate::alphacode_tui::tui::app) fn clear_active_experimental_feature_notice(&mut self) {
        self.active_experimental_feature_notice = None;
    }

    pub(in crate::alphacode_tui::tui::app) fn note_experimental_feature_use(
        &mut self,
        key: &'static str,
    ) -> Option<&'static str> {
    ...
    pub(in crate::alphacode_tui::tui::app) fn experimental_feature_key_for_tool(

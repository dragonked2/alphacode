...
struct RemoteModelCatalogCache {
    ...
    origin: String,
    #[serde(flatten)]
    snapshot: crate::alphacode_provider_core::ModelCatalogSnapshot,
...
fn filter_routes_by_provider_allowlist(
    ...
    use crate::alphacode_tui::provider::normalize_model_route_provider_label as normalize;
...
fn remote_catalog_api_method_is_safe(api_method: &str) -> bool {
    use crate::alphacode_tui::provider::ModelRouteApiMethod as Method;
...
fn model_picker_route_is_recommended(model_name: &str, route: &PickerOption) -> bool {
    RECOMMENDED_MODELS.contains(&model_name)
        && crate::alphacode_provider_core::model_route_metadata_is_recommended(
            model_name,
            &route.provider,
            &route.api_method,
            route.available,
...
fn model_picker_route_provider_matches_key(
    ...
    crate::alphacode_provider_core::model_route_provider_matches_key(
        route_provider_key,
...
impl App {
    pub(super) fn remote_model_catalog_snapshot(
        &self,
    ) -> crate::alphacode_provider_core::ModelCatalogSnapshot {
        crate::alphacode_provider_core::ModelCatalogSnapshot::new(
    ...
    fn append_alphacode_subscription_routes_static(
        ...
        let existing = routes
            ...
            .filter(|route| {
                route
                    .api_method
                    .eq_ignore_ascii_case(crate::subscription_catalog::ALPHACODE_ROUTE_API_METHOD)
            })
    ...
    fn extend_remote_routes_for_uncovered_models_static(
        ...
        // Alphacode subscription routes are a complete, server-managed catalog.
        ...
        let provider_is_alphacode_subscription = remote_provider_name.is_some_and(|name| {
            name.eq_ignore_ascii_case(crate::subscription_catalog::ALPHACODE_PROVIDER_DISPLAY_NAME)
        });
        if provider_is_alphacode_subscription {
            routes.clear();
            Self::append_alphacode_subscription_routes_static(
        ...
        let poisoned_by_alphacode_subscription = !routes.is_empty()
            && routes.iter().all(|route| {
                route
                    .api_method
                    .eq_ignore_ascii_case(crate::subscription_catalog::ALPHACODE_ROUTE_API_METHOD)
            });
        if poisoned_by_alphacode_subscription {
            // Version 1 could turn a mixed provider catalog into all-Alphacode rows
        ...
        // signed-in Alphacode subscriber still needs the managed route for each
    ...
    fn open_cached_model_picker_if_fresh(
        ...
        if std::env::var("ALPHACODE_LOG_MODEL_PICKER_TIMING").is_ok() {
    ...
    fn open_model_picker_with_routes(
        ...
        if total_ms >= 250 || std::env::var("ALPHACODE_LOG_MODEL_PICKER_TIMING").is_ok() {
            crate::logging::info(&format!(
    ...
    pub(in crate::alphacode_tui::tui::app) fn debug_model_picker_live_json(
        &mut self,
    ...
    fn handle_account_picker_selection(&mut self, action: AccountPickerAction) {
        match action {
            ...
            AccountPickerAction::Add { provider_id } => match provider_id.as_str() {
                "claude" => match crate::alphacode_base::auth::claude::next_account_label() {
                ...
                "openai" => match crate::alphacode_base::auth::codex::next_account_label() {
    ...
    pub(super) fn handle_session_picker_selection(&mut self, targets: &[ResumeTarget]) {
        ...
        if self.session_picker_mode == SessionPickerMode::CatchUp {
            ...
            for target in targets {
                let ResumeTarget::AlphacodeSession { session_id } = target else {
                    continue;
                };
        ...
        for target in targets {
            ...
            let name = match target {
                ResumeTarget::AlphacodeSession { session_id } => {
                    crate::id::extract_session_name(session_id)
                        .map(|s| s.to_string())
                        .unwrap_or_else(|| session_id.to_string())
                }
            ...
            match spawn_resume_target_in_new_terminal(&resolved_target, &cwd, socket.as_deref()) {
                ...
                Ok(false) | Err(_) => {
                    // No terminal emulator could be spawned. For a single alphacode
                    // session, fall back to resuming in the current terminal
                    // instead of dead-ending with a manual command (issue #203).
                    if targets.len() == 1
                        && spawned == 0
                        && matches!(resolved_target, ResumeTarget::AlphacodeSession { .. })
                    {
    ...
    pub(super) fn handle_session_picker_current_terminal_selection(
        ...
        let ResumeTarget::AlphacodeSession { session_id } = resolved_target else {
            self.push_display_message(DisplayMessage::error(format!(
    ...
    fn handle_live_claude_takeover(&mut self, target: &ResumeTarget) -> bool {
        ...
        let display_id = crate::alphacode_core::util::truncate_str(session_id, 8);
        ...
        let ResumeTarget::AlphacodeSession { session_id } = resolved else {
            self.push_display_message(DisplayMessage::error(
                "Claude takeover did not produce a Alphacode session.",
            ));
            return false;
        };
    ...
    pub(super) fn handle_batch_crash_restore(&mut self, session_ids: &[String]) {
        ...
        if spawned > 0 && failed.is_empty() {
            ...
            let manual: Vec<String> = failed
                .iter()
                .map(|id| format!("  alphacode --resume {}", id))
                .collect();
            self.push_display_message(DisplayMessage::system(format!(
            ...
            KeyCode::Enter => {
                ...
                match entry.action {
                    ...
                    PickerAction::Model => {
                        if !route.available {
                            self.push_display_message(DisplayMessage::error(
                                crate::alphacode_tui::tui::app::model_context::unavailable_model_route_message(
                                    &entry.name,
                                    &route.provider,
                                    &route.detail,
                                    self.is_remote,
    ...
    pub(super) fn picker_fuzzy_score(pattern: &str, text: &str) -> Option<i32> {
        crate::alphacode_fuzzy::fuzzy_score_tokens(pattern, text)
    }
    ...
    pub(super) fn apply_inline_interactive_filter(picker: &mut InlineInteractiveState) {
        if picker.filter.is_empty() {
            ...
            let prepared = crate::alphacode_fuzzy::PreparedTokenQuery::new(&picker.filter);
...
mod tests {
    ...
    fn model_picker_recommended_route_is_provider_aware() {
        ...
        // Current policy (see alphacode-provider-core): claude-opus-4-8 is the
    ...
    fn remote_model_catalog_cache_rejects_stale_and_future_timestamps() {
        let snapshot = crate::alphacode_provider_core::ModelCatalogSnapshot::new(
    ...
    fn remote_model_catalog_cache_rejects_forged_or_oversized_routes() {
        let safe_snapshot = crate::alphacode_provider_core::ModelCatalogSnapshot::new(

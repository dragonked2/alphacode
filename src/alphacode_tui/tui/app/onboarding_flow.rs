...
impl OnboardingFlow {
    ...
    /// choosing a model.
    #[allow(dead_code)] // retained for onboarding simulator + golden tests
...
/// `ALPHACODE_HOME`/external isolation, matching the import detectors.
...
mod tests {
    ...
    fn external_oauth_present_requires_nonempty_file() {
        let dir = std::env::temp_dir().join(format!("alphacode-onb-test-{}", std::process::id()));

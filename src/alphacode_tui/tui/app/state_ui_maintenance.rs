...
impl App {
            ...
            crate::bus::ClientMaintenanceAction::Rebuild => {
                ...
                self.set_client_maintenance_message(
                    ...
                    Self::client_maintenance_card_message(
                        ...
                        "Running in the background. alphacode will reload automatically after the rebuild succeeds.",
                    ),
                );
    ...
    pub(super) fn handle_update_status(&mut self, status: crate::bus::UpdateStatus) {
        use crate::alphacode_tui::bus::{ClientMaintenanceAction, UpdateStatus};
            ...
            UpdateStatus::Downloading {
                ...
                self.set_client_maintenance_message(
                    ...
                    Self::client_maintenance_card_message(
                        action,
                        format!("downloading {}\n{}", version, progress),
                        "alphacode will reload in place (input preserved) when the update is ready.",
                    ),
                );
            }
    ...
    pub(super) fn handle_session_update_status(&mut self, status: crate::bus::SessionUpdateStatus) {
        use crate::alphacode_tui::bus::{ClientMaintenanceAction, SessionUpdateStatus};
    ...
    /// user can hand the reconciliation to a fresh alphacode agent with one key.
    ///
    ...
    /// Accept the armed merge offer: spawn a fresh alphacode session pre-loaded with
    ...
    pub(super) fn accept_update_merge_offer(&mut self) -> bool {
        ...
        let repo_label = offer
            ...
            .unwrap_or_else(|| "the alphacode repository".to_string());
        let prompt = format!(
            "A alphacode self-update could not fast-forward because the local checkout and upstream have diverged.\n\n\
Repository: {repo}\n\
Update error: {detail}\n\n\
...
Do not force-push or discard local commits without confirming they are already upstream.",
        ...
        match self.launch_update_merge_agent(prompt, offer.repo_dir.as_deref()) {
            Ok(true) => {
                self.push_display_message(DisplayMessage::system(
                    "↗ Spawned a alphacode agent to merge the diverged update.",
                ));
                self.set_status_notice("Merge agent launched");
            }
            Ok(false) => {
                self.push_display_message(DisplayMessage::system(
                    "Could not open a new terminal for the merge agent. Run `git pull` / `git rebase` manually, or start `alphacode` in the repo and ask it to merge.",
                ));
    ...
    /// Spawn a fresh alphacode session, in the repo directory when known, with a

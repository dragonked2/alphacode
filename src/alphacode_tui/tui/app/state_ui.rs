...
use super::*;
use crate::alphacode_tui::tui::ui::tools_ui;
use crate::alphacode_tui::tui::{TuiState, backend};
...
pub(super) struct RestoredReloadInput {
    ...
    // These fields are part of the saved snapshot contract and are re-queued
    // (not auto-dispatched) by `apply_restored_reload_input` so a reconnect
    // cannot silently lose client-only user input.
...
impl App {
    ...
    pub(crate) fn unfocused_redraw_warranted(&self) -> bool {
        crate::alphacode_tui::tui::periodic_redraw_required(self)
    }
    ...
    pub(super) fn save_input_for_reload(&self, session_id: &str) {
            ...
            && !self.todo_confidence_spike_challenged
        {
            ...
            if let Ok(alphacode_dir) = crate::storage::alphacode_dir() {
                let path = alphacode_dir.join(format!("client-input-{}", session_id));
    ...
    fn prewarm_focused_side_panel(&self) {
        ...
        let has_protocol = crate::alphacode_tui::tui::mermaid::protocol_type().is_some();
        let _ = crate::alphacode_tui::tui::prewarm_focused_side_panel(
            &self.side_panel,
            terminal_width,
            terminal_height,
    ...
    pub fn start_debug_socket_listener(
        ...
        use crate::alphacode_tui::transport::Listener;
    ...
    pub fn debug_socket_path() -> std::path::PathBuf {
        crate::storage::runtime_dir().join("alphacode-debug.sock")
    }
}
...
fn format_cache_stats(app: &App) -> String {
    ...
    let effective_reported =
        crate::alphacode_tui::tui::info_widget::effective_prompt_tokens(reported, read, write);
...
/// plus the curated list of alphacode-endorsed skills (marking which are installed).
fn build_skills_report(app: &App) -> String {
    ...
    if app.is_remote && !app.remote_skills.is_empty() {
        ...
        if skills.is_empty() {
            out.push_str(
                "- none loaded\n  Add skills under ~/.alphacode/skills/<name>/SKILL.md or ./.alphacode/skills/<name>/SKILL.md\n",
            );
        } else {
            for skill in skills {
...
pub(super) fn handle_info_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed == "/version" {
        ...
        let mut content = format!("alphacode client: {}{}", version, is_canary);
        if app.is_remote {
    ...
    if trimmed == "/info" {
        ...
        if let Some(ref provider_id) = app.provider_session_id {
            info.push_str(&format!(
                "Provider Session: {}...\n",
                crate::alphacode_core::util::truncate_str(provider_id, 16)
            ));
        }
        ...
        let compaction_summary = if app.provider.supports_compaction() {
            ...
            if let Ok(manager) = manager.try_read() {
                ...
                format!(
                    "- supported: yes\n- mode: {}\n- alphacode-managed: {}\n- active summary: {} ({})\n- compacted messages: {}\n- active messages: {}\n- summary chars: {}\n- estimated tokens: {}\n- effective tokens: {}\n- observed tokens: {}\n- usage: {:.1}%\n- compacting now: {}\n- budget: {}",
                    mode,
                    if app.provider.uses_alphacode_compaction() {
                        "yes"
                    } else {
                        "no"
                    },

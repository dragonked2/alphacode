use super::*;
use crate::alphacode_tui::tui::{TuiState, detect_kv_cache_problem, ui};
...
impl App {
    ...
    pub(super) fn refresh_skills_snapshot(&mut self) {
        ...
        if let Ok(reloaded) = crate::skill::SkillRegistry::load_global() {
            ...
            crate::alphacode_tui::tui::ui::prepare::invalidate_header_prep_cache();
        }
    }
    ...
    pub(super) fn push_turn_footer(&mut self, duration: Option<f32>) {
        ...
        self.last_turn_input_tokens = {
            ...
            let input = crate::alphacode_tui::tui::info_widget::effective_prompt_tokens(
                self.streaming.streaming_input_tokens,

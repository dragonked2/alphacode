...
impl App {
    pub(super) fn command_help(&self, topic: &str) -> Option<String> {
        ...
        let help = match topic.as_str() {
            ...
            "model" | "set" => {
                "/model\nOpen model picker.\n\n/model <name>\nSwitch model.\n\n/model <name>@<provider>\nPin OpenRouter routing (@auto clears pin).\n\n/set model <name>\nAlias for /model <name>. Works with any provider-style model id\nincluding TokenRouter: /set model qwen/qwen3.8-max-free\n\nExamples:\n  /model claude-opus-4-6\n  /set model qwen/qwen3.8-max-free\n  /model gpt-5.5\n  /model deepseek/deepseek-chat"
            }
            "provider-test-coverage"
            | "provider test coverage"
            | "model-status"
            | "model status" => {
                "/provider-test-coverage\nShow alphacode live verification evidence for the current provider/model.\n\n/provider-test-coverage <provider> <model>\nLook up a specific provider/model pair in the live-test coverage ledger.\n\nThe report shows last-tested time, alphacode build, passed/missing checkpoints, readiness gaps, and a caveat that missing evidence is not a provider failure."
            }
            "refresh-model-list" => {
            ...
            "swarm-prompt" => {
                "/swarm-prompt\nOpen the active swarm routing prompt in $VISUAL or $EDITOR.\n\nAlphacode uses a nonblank project override at ./.alphacode/swarm-prompt.md when present, then ~/.alphacode/swarm-prompt.md, then the built-in default. If no editable override exists, this command creates the global file from the built-in default. Restart or reload Alphacode after editing because running agent tool registries cache the prompt."
            }
            "subagent" => {
            ...
            "triage" => {
                "/triage [focus]\nTriage open GitHub issues for the current repo, then autonomously fix the safe ones.\n\nThe agent lists untriaged issues with gh, classifies each (auto-fix, needs-info, needs-human, duplicate, question), applies existing labels, fixes and verifies the clear-cut bugs, and reports back with a summary table. Every public comment is clearly signed as the Alphacode agent, and issues are never closed as wontfix/invalid without your confirmation.\n\nOptional focus text narrows the triage, for example /triage only crash reports."
            }
            "catchup" => {
            ...
            "log" => {
                "/log mark [note]\nWrite a distinctive ALPHACODE_LOG_MARK line to ~/.alphacode/logs/alphacode-YYYY-MM-DD.log with the current session, provider, model, working directory, and optional note. Use this to mark a spot for agents to inspect later."
            }
            "goals" => {
            ...
            "poke" => {
                ...
                Set auto_poke = false under [features] in ~/.alphacode/config.toml to start with it disabled.\n\
            ...
            "restart" => {
                "/restart\nRestart alphacode with the current binary. Session is preserved.\nUseful after config changes, MCP server updates, or env var changes."
            }
            "rebuild" => {
                "/rebuild\nRun git pull --ff-only, cargo build --release, and release tests in the background. alphacode stays usable and reloads automatically when the build is ready."
            }
            "selfdev" => {
            ...
            "fork" | "split" => {
                "/fork\nFork the current session into a new terminal pane or window. Clones the full conversation history so both sessions continue from the same point. Inside tmux, alphacode automatically opens a right-side pane.\n\n/fork <prompt>\nFork the session and start the new pane/window by answering the prompt. The original session keeps working uninterrupted.\n\n/split\nAlias for /fork."
            }
            "resume" | "sessions" => {
            ...
            "subscription" => {
                "/subscription\nShow curated alphacode subscription status for this session, including router config, runtime mode, curated models, and planned tier budget scaffolding."
            }
            "subscribe" => {
                "/subscribe\nWhy subscribe to alphacode: more tokens on curated frontier models, one browser sign-in with no API keys, failover routing, and funding open-source development. Lists plans and prices, then start with /login alphacode."
            }
            "version" => "/version\nShow alphacode version/build details.",
        ...
        let help = help.replace(
            "Alt+",
            &format!("{}+", crate::alphacode_tui_core::keybind::alt_label()),
        );
        Some(help)
    }
}

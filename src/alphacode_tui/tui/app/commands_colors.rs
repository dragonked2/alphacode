...
//! `~/.alphacode/config.toml`. This command is the interactive front end for that:
...
//! [`crate::alphacode_tui_style::harmony`] so a user gets specific, actionable feedback
//! instead of trial and error.

use super::{App, DisplayMessage};
use crate::alphacode_tui_style::palette::{ALL_ROLES, Palette, Role, parse_hex, to_hex};
...
/// The palette the TUI is actually rendering with, preset included.
///
/// Delegates so `/colors` can never drift from what
/// [`crate::alphacode_tui::tui::theme_detect::init_palette`] installs. Problems
/// are dropped here because they were already logged when the palette was
/// installed; repeating them on every `/colors` would be noise.
fn configured_palette() -> Palette {
    crate::alphacode_tui::tui::theme_detect::configured_palette().0
}
...
fn active_background() -> (u8, u8, u8) {
    if crate::alphacode_tui_style::is_light_theme() {
        (255, 255, 255)
    } else {
        (18, 18, 18)
    }
...
fn show_harmony(app: &mut App) {
    ...
    let report = crate::alphacode_tui_style::analyze_harmony(&configured_palette(), background);
...
fn generate_palette(app: &mut App, seed: Option<&str>) {
    ...
    let background = active_background();
    let generated = crate::alphacode_tui_style::harmony::generate_from_seed(seed_rgb, background);
    let report = crate::alphacode_tui_style::analyze_harmony(&generated, background);
...
/// concurrent config edit by another alphacode session is not clobbered.
fn persist(

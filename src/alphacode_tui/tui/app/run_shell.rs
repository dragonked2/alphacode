...
pub(super) fn status_spinner_only_symbol(app: &App) -> Option<&'static str> {
    ...
    if status_uses_primary_spinner(&app.status) {
        Some(crate::alphacode_tui_style::theme::activity_indicator(
            status_spinner_elapsed(app),
            STATUS_SPINNER_FPS,
...
impl StatusSpinnerRenderer {
    ...
    pub(super) fn idle_animation_only_available(&self, app: &App) -> bool {
        let blocked = idle_animation_fast_path_blocked_reason(&IdleAnimationFastPathInputs {
            ...
            animation_active: crate::alphacode_tui::tui::idle_donut_active(app),
            has_animation_area: crate::alphacode_tui::tui::ui::last_idle_animation_area().is_some(),
        ...
        if let Some(reason) = blocked {
            crate::alphacode_tui::tui::ui::note_idle_animation_fast_path_blocked(reason);
            return false;
        }

        let allowed = idle_animation_partial_repaint_allowed(
            crate::alphacode_tui::tui::periodic_redraw_required_excluding_idle_animation(app),
    ...
    pub(super) fn draw_idle_animation_only(
...

        {
            ...
            crate::alphacode_tui::tui::ui::render_idle_animation_into(
                current_buffer,
                area,
                crate::alphacode_tui::tui::TuiState::animation_elapsed(app),
            );
        }
        ...
        crate::alphacode_tui::tui::ui::note_idle_animation_partial_repaint();
        Ok(true)
    }
    ...
    pub(super) fn draw_full(
        ...
        let completed = terminal.draw(|frame| {
            ...
            crate::alphacode_tui::tui::ui::draw(frame, app);
        ...
        crate::alphacode_tui::tui::ui::note_frame_painted();
        Ok(())
    }
    ...
    pub(super) fn draw_status_spinner_only(
        ...
        let status_area = crate::alphacode_tui::tui::ui::last_status_area();
...
fn render_status_spinner_into_buffer_mut(buffer: &mut Buffer, area: Rect, symbol: &str) {
    buffer.set_stringn(
        ...
        Style::default().fg(crate::alphacode_tui_style::adapt_color_for_theme(
...
impl App {
    ...
    pub async fn run(mut self, mut terminal: DefaultTerminal) -> Result<RunResult> {
        ...
        let mut redraw_period = crate::alphacode_tui::tui::redraw_interval(&self);
            ...
            if self.pending_turn {
                ...
                tokio::select! {
                    ...
                    _ = redraw_interval.tick() => {
                        ...
                        // Refresh the health monitor's "tui_render" timestamp
                        // each redraw tick so a hung UI is visible.
                        crate::health::heartbeat("tui_render");
                    }
                    command = async {
    ...
    pub async fn run_headless_replay(
        ...
        use crate::alphacode_tui::replay::ReplayEvent;
        ...
        let mut remote = crate::alphacode_tui::tui::backend::ReplayRemoteState::default();
        ...
        terminal.draw(|f| crate::alphacode_tui::tui::render_frame(f, &self))?;
...
mod tests {
    ...
    fn status_spinner_fast_path_uses_status_elapsed_clock() {
        ...
        let full_status_symbol = crate::alphacode_tui_style::theme::activity_indicator(
            full_status_elapsed,
            STATUS_SPINNER_FPS,
            true,
        );
        let old_app_lifetime_symbol = crate::alphacode_tui_style::theme::activity_indicator(
            app_lifetime_elapsed,
            STATUS_SPINNER_FPS,
            true,
        );
        let fast_path_symbol = crate::alphacode_tui_style::theme::activity_indicator(

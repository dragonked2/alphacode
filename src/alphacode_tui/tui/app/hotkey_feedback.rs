...
//!
//! 1. **Rare-hotkey feedback.** When the user presses a chord alphacode recognizes
...
use super::App;
use crate::alphacode_tui::tui::keybind::{
...
/// A chord alphacode responds to, with a human description of what it does.
#[derive(Debug, Clone)]
...
/// Resolve what a chord does, or `None` when alphacode has no binding for it.
pub(super) fn lookup(
...
impl App {
    ...
    pub(crate) fn observe_known_hotkey(
        ...
        let macos_option_char = cfg!(target_os = "macos")
            && modifiers.is_empty()
            && crate::alphacode_tui_core::keybind::macos_option_char_to_ascii_key(code).is_some();
...
mod tests {
    ...
    fn test_inputs_registry(remote: bool) -> Vec<KnownHotkey> {
        ...
        let toggles = crate::alphacode_tui::tui::keybind::load_toggle_keys();
    ...
    fn registry_covers_every_canonical_keybinding_default() {
        use crate::alphacode_tui::tui::keybind::KEYBINDING_DEFAULTS;

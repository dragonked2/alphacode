...
pub(super) fn is_auto_poke_connectivity_error(error: &str) -> bool {
    // Delegate to the shared connectivity classifier (alphacode-app-core's
...
pub(super) fn activate_auto_poke(app: &mut App) -> PokeActivation {
    ...
    app.auto_poke_incomplete_attempts = 0;
    app.todo_confidence_spike_challenged = false;
    app.todo_completion_gate_attempts = 0;
        ...
        Ok(result) => {
            ...
            match result {
                Ok(prepared) => {
                    ...
                    let socket = std::env::var("ALPHACODE_SOCKET").ok();
...
fn handle_subagent_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if app.is_remote {
        app.push_display_message(DisplayMessage::error(
            "/subagent requires a live alphacode server connection in remote mode.".to_string(),
        ));
        return true;
    }
...
/// and macOS shortcuts, and any conflicts with alphacode's own keybindings.
...
fn parse_diff_mode_name(value: &str) -> Option<crate::config::DiffDisplayMode> {
    use crate::alphacode_tui::config::DiffDisplayMode;
...
pub(super) fn handle_log_command(app: &mut App, trimmed: &str) -> bool {
    ...
    crate::logging::info(&format!(
        "ALPHACODE_LOG_MARK id={} session={} provider={} model={} cwd={} note={}",
        marker_id,
        app.session.id,
        app.provider_name(),
        app.provider_model(),
    ...
    let mut message = format!(
        "Log mark written: {}\n\nAgents can search ~/.alphacode/logs/ for ALPHACODE_LOG_MARK or this marker id.",
        marker_id
    );
    if !note.is_empty() {
...
Step 1/4: Tell Alphacode where to connect.
...
Security model
  - Alphacode stores this host/user target so you can run /ssh {} later.
  - Alphacode does not ask for or store your SSH password.
  - If a password is needed, it will be typed into your system ssh prompt, not into Alphacode.
...
Alphacode will ask for the SSH target, then use your system SSH client for authentication. Alphacode never stores SSH passwords."
                    .to_string(),
            ));
        }
        Ok(config) => {
            ...
            lines.push("".to_string());
            lines.push("Security: Alphacode stores targets only, never SSH passwords.".to_string());
...
Alphacode verified that {} is reachable through your system SSH client.

What this means:
  - Authentication is handled by OpenSSH / your SSH agent.
  - Alphacode did not see or store your password.
  - The SSH connection setup is ready for remote Alphacode tools.

Next implementation step: start the remote Alphacode server over this verified SSH connection.",
...
Alphacode could not connect without an interactive login, so it opened a separate terminal running your system ssh command.

What to expect in that terminal
  1. OpenSSH may ask for your password or two-factor prompt.
  2. You type credentials into OpenSSH, not into Alphacode.
  3. After login, SSH creates a temporary background control socket.
  4. The terminal verifies that socket before closing.

Security model
  - Alphacode cannot read what you type in the SSH terminal.
  - Alphacode stores only the target {}.
...
Alphacode could not open a terminal automatically. Run this command yourself:

  ssh -f -M -S {} -N {}

Type your password into that SSH prompt if asked. Alphacode will not see or store it.",
            profile.name,
            crate::ssh_remote::control_socket_path(&profile.name)
                .map(|p| p.display().to_string())
                .unwrap_or_else(|_| "~/.alphacode/ssh-control/remote.sock".to_string()),
            profile.ssh_target
        ))),
        Err(error) => app.push_display_message(DisplayMessage::error(format!(
...
fn git_command_repo_dir(app: &App) -> Result<PathBuf, String> {
    if let Some(path) = active_working_dir(app) {
        ...
        return Err(format!(
            "Unable to run /git: session working directory {} is not accessible from this alphacode client.",
            path.display()
        ));
    }
...
pub(super) fn build_triage_prompt(focus: &str) -> String {
    let mut prompt = String::from(
        ...
        Hard rules: every public comment, issue reply, or PR description you post MUST end with a clear agent attribution line like '--- *— Alphacode agent (automated triage), on behalf of @<repo-owner>*' so it can never be mistaken for the human. \
...
fn build_test_verification_prompt(claim: &str) -> String {
    ...
    format!(
        "Run Alphacode's /test verification orchestrator for: {target}\n\n\
...
pub(super) fn handle_dictation_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed.starts_with("/dictate ") || trimmed.starts_with("/dictation ") {
        app.push_display_message(DisplayMessage::error(
            "Usage: /dictate\nConfigure [dictation] in ~/.alphacode/config.toml to customize command, mode, hotkey, and timeout."
                .to_string(),
        ));
        return true;
    }
...
fn parse_agents_target(raw: &str) -> Option<crate::alphacode_tui::tui::AgentModelTarget> {
    match raw.trim().to_ascii_lowercase().as_str() {
        "swarm" | "agent" | "agents" | "subagent" | "subagents" => {
            Some(crate::alphacode_tui::tui::AgentModelTarget::Swarm)
        }
        "review" | "reviewer" | "code-review" | "codereview" => {
            Some(crate::alphacode_tui::tui::AgentModelTarget::Review)
        }
...
fn ensure_swarm_prompt_edit_path(
    ...
    alphacode_dir: &std::path::Path,
) -> std::io::Result<PathBuf> {
    let project_dir = match working_dir {
        Some(path) => PathBuf::from(path),
        None => std::env::current_dir()?,
    };
    let project_path = project_dir.join(".alphacode").join("swarm-prompt.md");
    if file_has_nonblank_content(&project_path) {
        return Ok(project_path);
    }

    let global_path = alphacode_dir.join("swarm-prompt.md");
    if file_has_nonblank_content(&global_path) {
        return Ok(global_path);
    }

    std::fs::create_dir_all(alphacode_dir)?;
...
pub(super) fn handle_swarm_prompt_command(app: &mut App, trimmed: &str) -> bool {
    ...
    let alphacode_dir = match crate::storage::alphacode_dir() {
        Ok(path) => path,
        Err(error) => {
            app.push_display_message(DisplayMessage::error(format!(
                "Failed to locate the Alphacode config directory: {}",
                error
            )));
            return true;
        }
        ...
        .spawn()
    {
        Ok(_) => {
            app.push_display_message(DisplayMessage::system(format!(
                "Opening the active swarm routing prompt in {}:\n{}\n\nChanges apply after restarting or reloading Alphacode because running agent tool registries cache the prompt.",
                editor,
                path.display()
            )));
...
fn handle_alignment_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if rest.is_empty() || matches!(rest, "show" | "status") {
        ...
        app.push_display_message(DisplayMessage::system(format!(
            ...
            alignment_label(saved), crate::alphacode_tui_core::keybind::alt_chord("C")
        )));
        return true;
    }
...
pub(super) fn handle_config_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed == "/subscription" || trimmed == "/subscription status" {
        app.show_alphacode_subscription_status();
        return true;
    }
    ...
    if trimmed == "/config edit" {
        ...
        if let Some(path) = Config::path() {
            ...
            app.push_display_message(DisplayMessage {
                role: "system".to_string(),
                content: format!(
                    "Opening config in editor...\n{} {}\n\n*Restart alphacode after editing for changes to take effect.*",
                    editor,
                    path.display()
                ),
                tool_calls: vec![],
...
pub(super) fn handle_telemetry_command(app: &mut App, trimmed: &str) -> bool {
    ...
    use crate::alphacode_tui::tui::app::onboarding_flow::TelemetryLevel;

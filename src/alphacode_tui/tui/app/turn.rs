use super::*;
use crate::alphacode_tui::message::ToolDefinition;
                        ...
                        match event {
                            Some(Ok(Event::Key(key))) => {
                                ...
                                if matches!(key.kind, KeyEventKind::Press | KeyEventKind::Repeat) {
                                    let _scroll_only = super::input::is_scroll_only_key(self, key.code, key.modifiers);
                                    ...
                                    // Always redraw on keyboard events so scroll
                            ...
                            Some(Ok(Event::Mouse(mouse))) => {
                                if !matches!(mouse.kind, MouseEventKind::Moved) {
                                    let _scroll_only = self.handle_mouse_event(mouse);
                                    // Always redraw on mouse events so scroll
                                    // updates are visible immediately.
                                    status_spinner_renderer.draw_full(self, terminal)?;
                                    super::run_shell::reset_status_spinner_interval(&mut status_spinner_interval, self);
                                }
                        ...
                        match event {
                            Some(Ok(Event::Key(key))) => {
                                ...
                                if matches!(key.kind, KeyEventKind::Press | KeyEventKind::Repeat) {
                                    ...
                                    // Always redraw on keyboard events during streaming
                                    // so scroll updates are visible immediately.
                                    status_spinner_renderer.draw_full(self, terminal)?;
                                }
                            }
                            Some(Ok(Event::Paste(text))) => {
                            ...
                            Some(Ok(Event::Mouse(mouse))) => {
                                if !matches!(mouse.kind, MouseEventKind::Moved) {
                                    let _scroll_only = self.handle_mouse_event(mouse);
                                    // Always redraw on mouse events during streaming
                                    // so scroll updates are visible immediately.
                                    status_spinner_renderer.draw_full(self, terminal)?;
                                }
                            }
                            ...
                            Some(Ok(event)) => {
                                ...
                                match event {
                                    StreamEvent::TextDelta(text) => {
                                        ...
                                        let revealed: Vec<String> = ops
                                            ...
                                            .filter_map(|op| match op {
                                                crate::alphacode_tui::tui::stream_buffer::StreamOp::Text(chunk) => {
                                                    Some(chunk.clone())
                                                }
                                                _ => None,
                                            })
                                        ...
                                        if self.apply_stream_ops(ops) {
                                            for chunk in revealed {
                                                self.broadcast_debug(crate::alphacode_tui::tui::backend::DebugEvent::TextDelta {
                                                    text: chunk
                                                });
                                            }
                                    ...
                                    StreamEvent::ToolUseStart { id, name } => {
                                        ...
                                        self.broadcast_debug(crate::alphacode_tui::tui::backend::DebugEvent::ToolStart {
                                            id: id.clone(),
                                            name: name.clone(),
                                        });
                                    ...
                                    StreamEvent::ToolInputDelta(delta) => {
                                        self.broadcast_debug(crate::alphacode_tui::tui::backend::DebugEvent::ToolInput {
                                            delta: delta.clone()
                                        });
                                    ...
                                    StreamEvent::ToolUseEnd => {
                                        ...
                                        if let Some(mut tool) = current_tool.take() {
                                            ...
                                            self.broadcast_debug(crate::alphacode_tui::tui::backend::DebugEvent::ToolExec {
                                                id: tool.id.clone(),
                                    ...
                                    StreamEvent::TokenUsage {
                                        ...
                                        self.broadcast_debug(crate::alphacode_tui::tui::backend::DebugEvent::TokenUsage {
                                    ...
                                    StreamEvent::ThinkingStart => {
                                        ...
                                        self.broadcast_debug(crate::alphacode_tui::tui::backend::DebugEvent::ThinkingStart);
                                        if eager_stream_redraw {
                                            status_spinner_renderer.draw_full(self, terminal)?;
                                        }
                                    }
                                    ...
                                    StreamEvent::ThinkingEnd => {
                                        ...
                                        self.thinking_buffer.clear();
                                        self.broadcast_debug(crate::alphacode_tui::tui::backend::DebugEvent::ThinkingEnd);
                                    }
                                    ...
                                    StreamEvent::ToolResult { tool_use_id, content, is_error } => {
                                        ...
                                        self.broadcast_debug(crate::alphacode_tui::tui::backend::DebugEvent::ToolDone {
                            ...
                            match event {
                                Some(Ok(Event::Key(key))) => {
                                    ...
                                    if matches!(key.kind, KeyEventKind::Press | KeyEventKind::Repeat) {
                                        ...
                                        // Always redraw on keyboard events during
                                        // tool execution so scroll updates are
                                        // visible immediately.
                                        status_spinner_renderer.draw_full(self, terminal)?;
                                    }
                                }
                                ...
                                Some(Ok(Event::Mouse(mouse))) => {
                                    if !matches!(mouse.kind, MouseEventKind::Moved) {
                                        let _scroll_only = self.handle_mouse_event(mouse);
                                        // Always redraw on mouse events during
                                        // tool execution so scroll updates are
                                        // visible immediately.
                                        status_spinner_renderer.draw_full(self, terminal)?;
                                    }

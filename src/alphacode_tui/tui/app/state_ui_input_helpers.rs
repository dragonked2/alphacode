use super::*;
use crate::alphacode_tui::tui::core;
...
const REGISTERED_COMMANDS: &[RegisteredCommand] = &[
    ...
    RegisteredCommand::public("/theme", "Pick a built-in color theme"),
    ...
    RegisteredCommand::public("/effort", crate::alphacode_tui::tui::keybind::EFFORT_HELP),
    ...
    RegisteredCommand::public(
        "/skills",
        "Show loaded skills and alphacode-endorsed recommendations",
    ),
    ...
    RegisteredCommand::public("/feedback", "Send feedback about alphacode"),
    RegisteredCommand::public("/telemetry", "Show or change what alphacode sends"),
    RegisteredCommand::public("/support", "Email support with diagnostics prefilled"),
    RegisteredCommand::public("/subscription", "Show alphacode subscription status"),
    RegisteredCommand::public("/subscribe", "Why and how to subscribe to alphacode"),
    RegisteredCommand::public("/config", "Show or edit configuration"),
    RegisteredCommand::public("/log", "Mark the current location in the alphacode logs"),
    RegisteredCommand::public(
        "/keys",
    ...
    RegisteredCommand::public("/selfdev", "Open a new self-dev alphacode session"),
    ...
    RegisteredCommand::public("/quit", "Exit alphacode"),
...
impl App {
    ...
    /// Delegates to the shared [`crate::alphacode_tui::tui::fuzzy`] matcher so slash-command
    /// ranking and highlight positions stay in sync.
    pub(super) fn fuzzy_score(needle: &str, haystack: &str) -> Option<i32> {
        crate::alphacode_tui::tui::fuzzy::fuzzy_score(needle, haystack)
    }
    ...
    pub(super) fn get_suggestions_for(&self, input: &str) -> Vec<(String, &'static str)> {
        ...
        if prefix.starts_with("/colors ") || prefix.starts_with("/color ") {
            ...
            suggestions.extend(
                crate::alphacode_tui_style::ALL_ROLES
                    .iter()
        ...
        if prefix.starts_with("/theme ") {
            // Descriptions come straight from the preset catalog so the picker
            // cannot drift from the themes that actually exist.
            let mut suggestions: Vec<(String, &'static str)> =
                vec![("/theme off".into(), "Restore the built-in palette")];
            suggestions.extend(
                crate::alphacode_tui_style::PRESETS
                    .iter()
                    .map(|preset| (format!("/theme {}", preset.id), preset.description)),
            );
            return self.rank_suggestions(input, suggestions);
        }
        ...
        if prefix.starts_with("/account ") || prefix.starts_with("/accounts ") {
            ...
            if let Ok(accounts) = crate::alphacode_base::auth::claude::list_accounts() {
            ...
            if let Ok(accounts) = crate::alphacode_base::auth::codex::list_accounts() {
                for account in accounts {
                    suggestions.push((
    ...
    pub(super) fn command_suggestions_uncached(
        ...
        if let Some(kind) = signature.inline_preview_kind {
            ...
            let suppress = match kind {
                crate::alphacode_tui::tui::PickerKind::Model => {
                    input.starts_with("/model") || input.starts_with("/models")
                }
                crate::alphacode_tui::tui::PickerKind::Login => input.starts_with("/login"),
                _ => false,
            };
            if suppress {
                return Vec::new();
            ...
            Some(OnboardingPhase::Login { import }) => {
                let prompt = import.as_ref().map(|review| {
                    let rows = review
                        ...
                        .iter()
                        .enumerate()
                        .map(|(i, candidate)| crate::alphacode_tui::tui::LoginImportRow {
                        ...
                        .collect();
                    crate::alphacode_tui::tui::LoginImportPrompt {
                        rows,
                ...
                OnboardingWelcomeKind::Login {
                    ...
                    repair_agent_label: self.onboarding_import_error.as_ref().and_then(|_| {
                        crate::alphacode_tui::tui::app::onboarding_repair::detect_preferred_repair_agent()
    ...
    ///
    /// Note: as of the "go straight to the session" UX change, the natural
    /// welcome screen is suppressed — the TUI now opens directly into an
    /// empty chat with a polished input prompt. Login still happens through
    /// the command palette (`/login`) or via the auth flow, not via a
    /// blocking welcome card. This hook stays in place so internal flows
    /// (onboarding simulator, guided flow phases) that probe the suggestion
    /// list still see a consistent answer.
...
mod external_cli_suggestion_tests {
    ...
    /// Run with:
    ///   cargo test -p alphacode-tui --lib onboarding_suggestion_scan_cost -- --ignored --nocapture
    #[test]
    #[ignore]
...
"#,
    ...
    fn parses_codex_input_text_blocks() {
        ...
        std::fs::write(
            &path,
            r#"{"type":"session_meta","payload":{"id":"sid","cwd":"/home/jeremy/alphacode"}}
{"type":"response_item","payload":{"type":"message","role":"user","content":[{"type":"input_text","text":"check in on alphacode"}]}}
"#,
        )
        .expect("write fixture");

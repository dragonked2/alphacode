...
impl App {
    fn open_auth_browser(url: &str) -> bool {
        // Honors --no-browser/NO_BROWSER/ALPHACODE_NO_BROWSER and never opens real
    ...
    fn record_oauth_preflight(
        ...
        if !browser_opened {
            crate::telemetry::record_auth_surface_blocked_reason(
                provider_id,
                "oauth",
                crate::alphacode_base::auth::login_diagnostics::AuthFailureReason::BrowserOpenFailed.label(),
            );
        ...
        if matches!(callback_available, Some(false)) {
            crate::telemetry::record_auth_surface_blocked_reason(
                provider_id,
                "oauth",
                crate::alphacode_base::auth::login_diagnostics::AuthFailureReason::CallbackPortUnavailable.label(),
            );
            if let Some(target) = callback_target {
                notices.push(format!(
                    "Local callback target {} is unavailable, so alphacode is using manual-safe paste completion instead.",
                    target
                ));
            } else {
                notices.push(
                    "The local callback listener is unavailable, so alphacode is using manual-safe paste completion instead."
                        .to_string(),
                );
            }
        }
        if !notices.is_empty() {
            notices.push(format!(
                "If login still fails, run alphacode auth doctor {} for a guided diagnosis.",
                provider_id
            ));
        }
        notices.join("\n")
    ...
    pub(super) fn show_alphacode_subscription_status(&mut self) {
        ...
        let configured_base = crate::subscription_catalog::configured_api_base()
            .unwrap_or_else(|| crate::subscription_catalog::DEFAULT_ALPHACODE_API_BASE.to_string());
        ...
        let mut message = String::from("Alphacode Subscription Status\n\n");
        message.push_str(&format!(
            "  - Credentials: {}\n",
            if configured_key {
                "configured"
            } else {
                "not configured (/login alphacode)"
            }
        ));
        message.push_str(&format!(
            "  - Router base: {}{}\n",
        ...
        if configured_key {
            ...
            message.push_str("\nLog in with /login alphacode to see account usage and tier.");
        }
                    ...
                    match crate::subscription_api::fetch_subscription_me().await {
                        Ok(me) => {
                            ...
                            crate::bus::Bus::global().publish(crate::bus::BusEvent::UiActivity(
                                crate::bus::UiActivity::background(
                                    Some(session_id),
                                    format!(
                                        "Alphacode Subscription Account\n\n  - Email: {}\n  - Tier: {} ({})\n  - Usage: ${:.2} of ${:.2}{}",
                                        me.email,
                                        tier_label,
                                        me.status,
                                        me.usage.used_usd,
                        ...
                        Err(error) => {
                                ...
                                == Some(&crate::subscription_api::AccountApiError::Unauthorized)
                            {
                                ...
                                "Alphacode Account Status\n\nThe saved account key was revoked or expired. Local credentials were cleared. Use /account alphacode login to sign in again."
                                    .to_string()
                            } else {
                                format!(
                                    "Alphacode Account Status\n\nCould not load /v1/me: {}\n\nThe local credential was retained. Retry /account alphacode status, open /account alphacode manage, or use /account alphacode logout.",
                                    error
                                )
                            };
                            crate::bus::Bus::global().publish(crate::bus::BusEvent::UiActivity(
                                crate::bus::UiActivity::background(
                                    Some(session_id),
                                    message,
                                    Some("Alphacode account status unavailable"),
                                ),
                            ));
                        }
                    }
    ...
    pub(super) fn show_auth_status(&mut self) {
        let status = crate::alphacode_base::auth::AuthStatus::check();
        let validation = crate::alphacode_base::auth::validation::load_all();
        let icon = |state: crate::alphacode_base::auth::AuthState| match state {
            crate::alphacode_base::auth::AuthState::Available => "ok",
            crate::alphacode_base::auth::AuthState::Expired => "needs attention",
            crate::alphacode_base::auth::AuthState::NotConfigured => "not configured",
        };
        ...
        message.push_str(
            "\nUse /login <provider> to authenticate. /login alphacode is for curated alphacode subscription access; /account opens the provider/account management center, /account <provider> settings shows provider-specific controls, and /auth doctor or /account <provider> doctor shows recovery steps.",
        );
        self.push_display_message(DisplayMessage::system(message));
    }
    ...
    pub(super) fn start_logout_provider(
        ...
        use crate::alphacode_tui::provider_catalog::LoginProviderTarget;

        if matches!(provider.target, LoginProviderTarget::Alphacode) {
            self.start_alphacode_account_logout();
            return;
        }
        ...
        let result: anyhow::Result<String> = (|| match provider.target {
            ...
            LoginProviderTarget::Azure => {
                Self::clear_api_key_login(
                    crate::alphacode_base::auth::azure::API_KEY_ENV,
                    crate::alphacode_base::auth::azure::ENV_FILE,
                )?;
                crate::provider_catalog::save_env_value_to_env_file(
                    crate::alphacode_base::auth::azure::USE_ENTRA_ENV,
                    crate::alphacode_base::auth::azure::ENV_FILE,
                    None,
                )?;
            ...
            LoginProviderTarget::Cursor => {
                crate::alphacode_base::auth::cursor::clear_api_key()?;
                Ok("Logged out of Cursor API key.".to_string())
            }
            LoginProviderTarget::Gemini => {
                crate::alphacode_base::auth::gemini::clear_tokens()?;
    ...
    pub(super) fn start_logout_all(&mut self) {
        ...
        Self::clear_api_key_logout_summary(
            &mut summary,
            &mut errors,
            "alphacode subscription API key",
            crate::subscription_catalog::ALPHACODE_API_KEY_ENV,
            crate::subscription_catalog::ALPHACODE_ENV_FILE,
        );
        for env_key in [
            crate::subscription_catalog::ALPHACODE_API_BASE_ENV,
            crate::subscription_catalog::ALPHACODE_ACCOUNT_ID_ENV,
            crate::subscription_catalog::ALPHACODE_ACCOUNT_EMAIL_ENV,
            crate::subscription_catalog::ALPHACODE_TIER_ENV,
        ] {
            if let Err(err) = crate::provider_catalog::save_env_value_to_env_file(
                ...
                errors.push(format!("alphacode subscription {}: {}", env_key, err));
            }
        }
        ...
        let cursor_configured = crate::alphacode_base::auth::cursor::load_api_key().is_ok();
        match crate::alphacode_base::auth::cursor::clear_api_key() {
            Ok(()) if cursor_configured => summary.push("Cursor API key".to_string()),
            Ok(()) => {}
            Err(err) => errors.push(format!("Cursor API key: {}", err)),
        }
        match crate::alphacode_base::auth::gemini::clear_tokens() {
    ...
    pub(super) fn start_login_provider(
        ...
        match provider.target {
            ...
            crate::provider_catalog::LoginProviderTarget::Alphacode => self.start_alphacode_login(),
            ...
            crate::provider_catalog::LoginProviderTarget::Google => {
                ...
                self.push_display_message(DisplayMessage::error(
                    "Google/Gmail login is only available from the CLI right now. Run alphacode login --provider google."
                        .to_string(),
                ));
            }
        }
    ...
    fn start_claude_login(&mut self) {
        let label = crate::alphacode_base::auth::claude::login_target_label(None)
            .unwrap_or_else(|_| crate::alphacode_base::auth::claude::primary_account_label());
        self.start_claude_login_for_account(&label);
    }

    fn start_alphacode_login(&mut self) {
        self.push_display_message(DisplayMessage::system(
            "Alphacode Account Login\n\nRequesting a secure browser approval flow. No email or API key will be requested in the terminal."
                .to_string(),
        ));
        self.set_status_notice("Alphacode account: requesting browser approval");
        let session_id = self.session.id.clone();
        let Ok(handle) = tokio::runtime::Handle::try_current() else {
            self.push_display_message(DisplayMessage::error(
                "Alphacode account login requires the async runtime.".to_string(),
            ));
            return;
        };
        handle.spawn(async move {
            use crate::alphacode_tui::subscription_api::{
            ...
            let device = match crate::subscription_api::request_device_authorization(
                &client,
                &api_base,
                Some(crate::subscription_catalog::AlphacodeTier::Pro),
            )
            .await
            {
                Ok(device) => device,
                Err(error) => {
                    publish(
                        format!(
                            "Alphacode Account Login\n\nCould not start browser approval: {}\n\nRetry /account alphacode login. No credential was saved.",
                            error
                        ),
                        "Alphacode account login failed",
                    );
                    return;
                }
            };
            ...
            publish(
                format!(
                    "Alphacode Account Login\n\n{}\n\nApprove the request in the same browser. Alphacode is waiting for the single-use exchange.{}",
                ...
                "Alphacode account: waiting for browser approval",
            );
                ...
                loop {
                    ...
                    .await
                    {
                        ...
                        Ok(TokenPollOutcome::Denied) => break Err(
                            "Alphacode account login was canceled or denied in the browser."
                                .to_string(),
                        ),
            ...
            let approved = match approved {
                ...
                Err(error) => {
                    publish(
                        format!("Alphacode Account Login\n\n{error}\n\nRetry /account alphacode login."),
                        "Alphacode account login stopped",
                    );
                    return;
                }
            };
            ...
            if let Err(error) = crate::subscription_catalog::persist_account_credentials(
                ...
                publish(
                    ...
                    "Alphacode account credential save failed",
                );
                return;
            }
            crate::alphacode_base::auth::AuthStatus::invalidate_cache();
            ...
            .await
            {
                ActivationOutcome::Active(me) => publish(
                    format!(
                        "Alphacode Account Ready\n\n{} is active for {}.\n\nStatus: /account alphacode status\nManage: /account alphacode manage\nLogout: /account alphacode logout",
                        me.parsed_tier()
                            .map(|tier| tier.display_name().to_string())
                            .unwrap_or(me.tier),
                        me.email
                    ),
                    "Alphacode account plan active",
                ),
                ActivationOutcome::Canceled(_) => publish(
                    "Alphacode Account Login\n\nCheckout was canceled. The valid account key remains saved, but no paid plan is active.\n\nStatus: /account alphacode status\nManage: /account alphacode manage\nLogout: /account alphacode logout".to_string(),
                    "Alphacode account plan not active",
                ),
                ActivationOutcome::TimedOut { last_error_was_offline } => publish(
                    ...
                    "Alphacode account activation pending",
                ),
                ActivationOutcome::Revoked | ActivationOutcome::Denied => {
                    let _ = crate::subscription_catalog::clear_account_credentials();
                    publish(
                        "Alphacode Account Login\n\nThe issued key was revoked or denied during activation checks. Local credentials were cleared. Retry /account alphacode login.".to_string(),
                        "Alphacode account key rejected",
                    );
                }
            }
        });
    ...
    pub(super) fn open_alphacode_account_management(&mut self) {
        let url = crate::subscription_catalog::ALPHACODE_ACCOUNT_URL;
        let opened = Self::open_auth_browser(url);
        self.push_display_message(DisplayMessage::system(format!(
            "Alphacode Account Management\n\n{}{}",
            url,
            if opened {
        ...
        self.set_status_notice("Alphacode account management");
    }

    pub(super) fn start_alphacode_account_logout(&mut self) {
        self.set_status_notice("Alphacode account: logging out");
        let session_id = self.session.id.clone();
        let Ok(handle) = tokio::runtime::Handle::try_current() else {
            let result = crate::subscription_catalog::clear_account_credentials();
            match result {
                Ok(()) => self.push_display_message(DisplayMessage::system(
                    "Alphacode account credentials and cache were cleared locally. Remote revocation could not be attempted without the async runtime."
                        .to_string(),
                )),
                Err(error) => self.push_display_message(DisplayMessage::error(format!(
                    "Failed to clear local Alphacode account credentials: {error}"
                ))),
            }
            return;
        };
        handle.spawn(async move {
            ...
            crate::bus::Bus::global().publish(crate::bus::BusEvent::UiActivity(
                crate::bus::UiActivity::background(
                    Some(session_id),
                    message,
                    Some("Alphacode account logout complete"),
                ),
            ));
        });
    }
    ...
    pub(super) fn start_claude_login_for_account(&mut self, label: &str) {
        ...
        let callback_listener = crate::alphacode_base::auth::oauth::bind_callback_listener(0).ok();
    ...
    pub(super) fn switch_account(&mut self, label: &str) {
        match crate::alphacode_base::auth::claude::set_active_account(label) {
            Ok(()) => {
                {
    ...
    pub(super) fn switch_account_by_label(&mut self, label: &str) {
        let has_anthropic = crate::alphacode_base::auth::claude::list_accounts()
            .unwrap_or_default()
            .iter()
            .any(|account| account.label == label);
        let has_openai = crate::alphacode_base::auth::codex::list_accounts()
            .unwrap_or_default()
            .iter()
            .any(|account| account.label == label);
    ...
    pub(super) fn remove_account(&mut self, label: &str) {
        match crate::alphacode_base::auth::claude::remove_account(label) {
            Ok(()) => {
                self.push_display_message(DisplayMessage::system(format!(
    ...
    pub(super) fn switch_openai_account(&mut self, label: &str) {
        match crate::alphacode_base::auth::codex::set_active_account(label) {
            Ok(()) => {
                {
                    let provider = self.provider.clone();
                    let label_owned = label.to_string();
    ...
    pub(super) fn remove_openai_account(&mut self, label: &str) {
        match crate::alphacode_base::auth::codex::remove_account(label) {
            Ok(()) => {
                self.push_display_message(DisplayMessage::system(format!(
    ...
    fn start_openai_login(&mut self) {
        let label = crate::alphacode_base::auth::codex::login_target_label(None)
            .unwrap_or_else(|_| crate::alphacode_base::auth::codex::primary_account_label());
    ...
    pub(super) fn start_openai_login_for_account(&mut self, label: &str) {
        ...
        let port = crate::alphacode_base::auth::oauth::openai::DEFAULT_PORT;
        let redirect_uri = crate::alphacode_base::auth::oauth::openai::redirect_uri(port);
        let auth_url = crate::alphacode_base::auth::oauth::openai_auth_url_with_prompt(
            &redirect_uri,
            &challenge,
            &state,
            Some("login"),
    ...
    async fn openai_token_exchange(
        ...
        let oauth_tokens = if let Some(expected_state) = expected_state {
            crate::alphacode_base::auth::oauth::exchange_openai_callback_input(
                &verifier,
                input.trim(),
                &expected_state,
                redirect_uri,
        ...
        crate::alphacode_base::auth::oauth::save_openai_tokens_for_account(&oauth_tokens, &label)
    ...
    fn start_gemini_login(&mut self) {
        ...
        if let (Some(listener), Some(expected_state)) = (callback_listener, pending_state.clone()) {
            ...
            tokio::spawn(async move {
                ...
                match code {
                    Ok(code) => {
                        match crate::alphacode_base::auth::gemini::exchange_callback_code(
                            &code,
                            &verifier_clone,
                            &redirect_clone,
                        )
    ...
    fn start_api_key_login(
        ...
        // Local-endpoint profiles (e.g. Dragonmeta/Alpha) point at a local
        // server the user runs themselves, so the external setup-docs URL is
        // noise there. Only API-key flows need the docs hint to find their key.
        let docs_hint = if api_key_optional {
            String::new()
        } else {
            format!("Setup docs: {}\n", docs_url)
        };
        ...
        let provider_id = openai_compatible_profile
            ...
            .unwrap_or_else(|| match key_name {
                crate::subscription_catalog::ALPHACODE_API_KEY_ENV => "alphacode".to_string(),
    ...
    fn start_azure_login(&mut self) {
        self.push_display_message(DisplayMessage::system(
            "Azure OpenAI Login\n\n\
             alphacode uses Azure OpenAI's /openai/v1 API with either Microsoft Entra ID or an API key.\n\n\
    ...
    fn start_cursor_login(&mut self) {
        ...
        self.push_display_message(DisplayMessage::system(
            "Cursor API Key\n\n\
             ...
             alphacode will save it securely and use the native Cursor HTTPS transport.\n\n\
    ...
    async fn antigravity_token_exchange(
        ...
        let tokens =
            if antigravity_input_requires_state_validation(trimmed, expected_state.as_deref()) {
                crate::alphacode_base::auth::antigravity::exchange_callback_input(
                    &verifier,
                    trimmed,
    ...
    pub(super) fn handle_login_input(&mut self, pending: PendingLogin, input: String) {
        ...
        match pending {
            ...
            PendingLogin::Gemini {
                ...
                let input_owned = input.clone();
                tokio::spawn(async move {
                    match crate::alphacode_base::auth::gemini::exchange_callback_input(
                        &verifier,
                        input_owned.trim(),
                        expected_state.as_deref(),
                        &redirect_uri,
            ...
            PendingLogin::ApiKeyProfile {
                ...
                // it proves the input was received and which env var/file alphacode
                ...
                let save_result: anyhow::Result<()> =
                    ...
                    } else if key_name == crate::subscription_catalog::ALPHACODE_API_KEY_ENV {
                        (|| {
                            ...
                            crate::alphacode_core::env::set_var(&key_name, &key);
                            Ok(())
                        })()
                ...
                match save_result {
                    Ok(()) => {
                            ...
                            == crate::subscription_catalog::ALPHACODE_API_KEY_ENV
                        {
                            ...
                            // The suggested default model is already shown in the
                            ...
                            "You can now use /model to switch to Bedrock models. TUI onboarding saved region us-east-2; for a different region, run alphacode login --provider bedrock from a terminal.".to_string()
            ...
            PendingLogin::CursorApiKey => {
                ...
                match crate::alphacode_base::auth::cursor::save_api_key(&key) {
                    Ok(()) => {
                        crate::alphacode_base::auth::AuthStatus::invalidate_cache();
                        Bus::global().publish(BusEvent::LoginCompleted(LoginCompleted {
                            ...
                            message: "Cursor API key saved.\n\n\
                             Stored at ~/.config/alphacode/cursor.env.\n\
                             alphacode will use it with the native Cursor HTTPS transport."
                                .to_string(),
                        }));
                    }
                    Err(e) => {
                        ...
                        if let Some(model) = selected {
                            ...
                            crate::bus::Bus::global().publish(crate::bus::BusEvent::UiActivity(
                                crate::bus::UiActivity::catalog(
                                    Some(session_id),
                                    format!(
                                        "{} Model Discovery Still Updating\n\nSaved credentials are active, but this local refresh pass did not find a selectable {} route yet. Alphacode is still processing the auth-change catalog refresh and will switch once provider routes are available. If the model list still looks stale after the auth catalog update, run /refresh-model-list.",
                    ...
                    Err(error) => {
                        ...
                        crate::bus::Bus::global().publish(crate::bus::BusEvent::UiActivity(
                            crate::bus::UiActivity::catalog(
                                Some(session_id),
                                format!(
                                    "{} Model Discovery Still Updating\n\nSaved credentials are active, but this local refresh pass failed before the server auth-change catalog refresh finished. Alphacode is still processing the auth-change catalog refresh and will switch once provider routes are available. If the model list still looks stale after the auth catalog update, run /refresh-model-list.\n\nLocal refresh error: {}",
    ...
    fn save_azure_config(
        ...
        use crate::alphacode_tui::auth::azure;

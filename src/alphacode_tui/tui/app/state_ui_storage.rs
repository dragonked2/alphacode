...
fn compact_tool_input_for_display(name: &str, input: &serde_json::Value) -> serde_json::Value {
    ...
    match crate::alphacode_tui::tui::ui::tools_ui::canonical_tool_name(name) {
        "bash" => obj(vec![(
            "command",
            input
                .get("command")
            ...
            let file_path = input.get("file_path").cloned().or_else(|| {
                input
                    ...
                    .and_then(|patch_text| {
                        match crate::alphacode_tui::tui::ui::tools_ui::canonical_tool_name(name) {
                            "apply_patch" => {
                                crate::alphacode_tui::tui::ui::tools_ui::extract_apply_patch_primary_file(
                                    patch_text,
                                )
                            }
                            "patch" => {
                                crate::alphacode_tui::tui::ui::tools_ui::extract_unified_patch_primary_file(
                                    patch_text,
                                )
                            }
                            _ => None,
...
mod tests {
    ...
    fn compaction_keeps_webfetch_url_for_transcript_summary() {
        ...
        let summary = crate::alphacode_tui::tui::ui::tools_ui::get_tool_summary(&tool);
        assert!(

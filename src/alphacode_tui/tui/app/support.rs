...
pub(super) const SUPPORT_EMAIL: &str = "support@alphacode.sh";
pub(super) const SUPPORT_SUBJECT: &str = "alphacode support";
...
fn build_channel() -> String {
    if std::env::var(crate::alphacode_selfdev_types::CLIENT_SELFDEV_ENV).is_ok() {
        return "selfdev".to_string();
    }
    if crate::alphacode_build_meta::is_release_build() {
...
fn read_telemetry_id() -> Option<String> {
    let path = crate::storage::alphacode_dir().ok()?.join("telemetry_id");
...
fn gather_diagnostics(app: &App) -> SupportDiagnostics {
    use crate::alphacode_tui::provider_catalog::load_env_value_from_env_or_config;
    use crate::subscription_catalog as cat;

    // Account identity is only present when a alphacode subscription is
    ...
    let (account_id, account_email, tier) = if has_subscription {
        (
            load_env_value_from_env_or_config(cat::ALPHACODE_ACCOUNT_ID_ENV, cat::ALPHACODE_ENV_FILE),
            load_env_value_from_env_or_config(cat::ALPHACODE_ACCOUNT_EMAIL_ENV, cat::ALPHACODE_ENV_FILE),
...
mod tests {
    ...
    fn support_mailto_url_is_correctly_encoded() {
        ...
        assert!(url.starts_with("mailto:support@alphacode.sh?subject=alphacode%20support&body="));

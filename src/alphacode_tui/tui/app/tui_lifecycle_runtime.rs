use super::*;
use crate::alphacode_tui::tui::connection_type_icon;
...
impl App {
    ...
    pub(super) fn update_terminal_title(&self) {
        ...
        let window_title = crate::alphacode_base::process_title::terminal_window_title(
            icon,
            display_title,
            Some(&fallback_label),
            is_canary,
    ...
    /// would `alphacode --resume <bogus-id>` and crash with "No session found
    ...
    pub fn restore_session(&mut self, session_id: &str) {
        ...
        if let Ok(session) = Session::load(session_id) {
            ...
            for item in crate::alphacode_tui_messages::display_messages_from_rendered_messages(
...
pub(super) fn handle_dev_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed == "/restart" {
        app.push_display_message(DisplayMessage {
            role: "system".to_string(),
            content: "Restarting alphacode (same binary, session preserved)...".to_string(),
            tool_calls: vec![],
            duration_secs: None,
            title: None,
            tool_data: None,
    ...
    if trimmed == "/z" || trimmed == "/zz" || trimmed == "/zzz" || trimmed == "/zstatus" {
        ...
        if trimmed == "/zstatus" {
            ...
            let env = std::env::var("ALPHACODE_COPILOT_PREMIUM").ok();

...
impl App {
    ...
    fn handle_provider_failover_prompt(&mut self, prompt: crate::provider::ProviderFailoverPrompt) {
            ...
            .cross_provider_failover
        {
            ...
            crate::config::CrossProviderFailoverMode::Countdown if !self.is_remote => {
                ...
                self.push_display_message(DisplayMessage::system(format!(
                    "⚠ {} became unavailable - alphacode will switch to {} in 3 seconds unless you cancel.\n\nReason: {}\n\nRetrying would send {}. Press Esc to cancel.\n\n{}",
                    prompt.from_label,
                    prompt.to_label,
                    prompt.reason,
                    input_summary,
    ...
    fn current_route_api_method(&self) -> Option<String> {
        ...
        match (provider_name.as_str(), credential) {
            ("claude", Some(crate::alphacode_provider_core::ResolvedCredential::Oauth)) => {
                Some("claude-oauth".to_string())
            }
            ("claude", Some(crate::alphacode_provider_core::ResolvedCredential::ApiKey)) => {
                Some("claude-api".to_string())
            }
            ("openai", Some(crate::alphacode_provider_core::ResolvedCredential::Oauth)) => {
                Some("openai-oauth".to_string())
            }
            ("openai", Some(crate::alphacode_provider_core::ResolvedCredential::ApiKey)) => {
    ...
    pub(super) fn effective_context_tokens_from_usage(
        ...
        // Shared heuristic (alphacode-compaction-core): keeps the sidebar figure
    ...
    pub(super) async fn try_auto_compact_and_retry(
        ...
        loop {
            ...
            let _ = terminal.draw(|frame| crate::alphacode_tui::tui::ui::draw(frame, self));
    ...
    fn clear_usage_transient_ui(&mut self) {
        ...
        if self
            ...
            .as_ref()
            .map(|picker| picker.kind == crate::alphacode_tui::tui::PickerKind::Usage)
            .unwrap_or(false)
        {
...
pub(super) fn handle_model_command(app: &mut App, trimmed: &str) -> bool {
   ...
   Or OpenRouter: `alphacode --provider openrouter --model {}`",
                        msg, model_name, model_name, model_name
                    );
                }
                app.push_display_message(DisplayMessage::error(msg));

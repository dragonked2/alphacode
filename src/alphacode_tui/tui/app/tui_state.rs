...
impl App {
    ...
    fn configured_remote_provider_hint(&self) -> Option<String> {
        std::env::var("ALPHACODE_PROVIDER")
            .ok()
    ...
    /// the header uses (session stub, `ALPHACODE_MODEL`, config default) so effort
    ...
    ///   [`AuthStatus`] snapshot (cached up to 60s) or a `ALPHACODE_RUNTIME_PROVIDER`
    ...
    fn dual_credential_active(
        ...
        if route.is_remote {
            ...
            return self
                ...
                .route_api_method
                .as_deref()
                .and_then(crate::alphacode_provider_core::AuthRoute::parse)
        ...
        crate::alphacode_base::auth::resolve_dual_credential_auth(
            provider,
            &auth_status,
        ...
        match route.provider {
            ...
            WidgetProviderKind::OpenRouter => {
                ...
                if transport_state.is_real_openrouter() {
                    crate::alphacode_tui::tui::info_widget::AuthMethod::OpenRouterApiKey
            ...
            WidgetProviderKind::Gemini => {
                ...
                let auth_status = crate::alphacode_base::auth::AuthStatus::check_fast_nonblocking();
                if auth_status.gemini == crate::alphacode_base::auth::AuthState::Available {
                    crate::alphacode_tui::tui::info_widget::AuthMethod::GeminiOAuth
                } else {
                    crate::alphacode_tui::tui::info_widget::AuthMethod::Unknown
                }
            }
    ...
    fn widget_usage_info(
        &self,
        route: WidgetRouteInfo,
        auth_method: crate::alphacode_tui::tui::info_widget::AuthMethod,
    ) -> Option<crate::alphacode_tui::tui::info_widget::UsageInfo> {
        ...
        let cost_based_usage = || crate::alphacode_tui::tui::info_widget::UsageInfo {
            provider: crate::alphacode_tui::tui::info_widget::UsageProvider::CostBased,
        ...
        match route.provider {
            ...
            WidgetProviderKind::Anthropic => {
                match auth_method {
                    ...
                    crate::alphacode_tui::tui::info_widget::AuthMethod::AnthropicOAuth => {}
                    _ => return None,
                }

                let usage = crate::usage::get_sync();
                Some(crate::alphacode_tui::tui::info_widget::UsageInfo {
                    provider: crate::alphacode_tui::tui::info_widget::UsageProvider::Anthropic,
            ...
            WidgetProviderKind::OpenAI => {
                match auth_method {
                    crate::alphacode_tui::tui::info_widget::AuthMethod::OpenAIApiKey => {
                        return Some(cost_based_usage());
                    }
                    crate::alphacode_tui::tui::info_widget::AuthMethod::OpenAIOAuth => {}
                    _ => return None,
                }

                let openai_usage = crate::usage::get_openai_usage_sync();
                Some(crate::alphacode_tui::tui::info_widget::UsageInfo {
                    provider: crate::alphacode_tui::tui::info_widget::UsageProvider::OpenAI,
...
impl crate::alphacode_tui::tui::TuiState for App {
    ...
    fn side_pane_images_signature(&self) -> (usize, u64) {
        ...
        for image in &images {
            crate::alphacode_tui::tui::hash_rendered_image_signature_fields(image, &mut hasher);
        }
    ...
    fn prompt_history_search(&self) -> Option<crate::alphacode_tui::tui::PromptHistorySearchView> {
    ...
    fn session_compaction_count(&self) -> usize {
        if self.is_remote || !self.provider.uses_alphacode_compaction() {
            return 0;
        }
        self.registry
            .compaction()
    ...
    fn status_notice(&self) -> Option<String> {
        if !self.is_remote
            && self.provider.uses_alphacode_compaction()
    ...
    fn context_snapshot(&self) -> crate::alphacode_tui::tui::ContextSnapshot {
        ...
        let (compaction_count, compaction_summary_chars, is_compacting, compaction_fresh) =
            if self.is_remote {
                (0, 0, false, true)
            } else if self.provider.uses_alphacode_compaction() {
        ...
        if !compaction_fresh {
            return crate::alphacode_tui::tui::ContextSnapshot {
                info: None,
        ...
        if self.is_remote {
            ...
            let skip = if self.provider.uses_alphacode_compaction() {
        ...
        if let Ok(mut cache) = CACHE.lock() {
            *cache = Some((
                ...
                CachedContextSnapshot {
                    ...
                    is_compacting,
                    snapshot: crate::alphacode_tui::tui::ContextSnapshot {
        ...
        crate::alphacode_tui::tui::ContextSnapshot {
            info: Some(info),
            revision: self.context_revision,
            fresh: true,
        }
    ...
    fn info_widget_data(&self) -> crate::alphacode_tui::tui::info_widget::InfoWidgetData {
        ...
        let (todos, todo_goals) = if todos_are_swarm_plan {
            (
                crate::alphacode_tui::tui::info_widget::swarm_plan_todos(&self.swarm_plan_items),
                Vec::new(),
            )
        } else {
        ...
        let swarm_info = if self.swarm_enabled {
                ...
                || !managed_members.is_empty()
            {
                ...
                Some(crate::alphacode_tui::tui::info_widget::SwarmInfo {
                    session_count,
                    subagent_status,
                    client_count,
                    session_names,
        ...
        let background_info = {
            ...
            if running_count > 0 {
                Some(crate::alphacode_tui::tui::info_widget::BackgroundInfo {
                    running_count,
                    running_tasks,
        ...
        let cache_hit_info =
            (self.token_accounting.total_cache_reported_input_tokens > 0).then(|| {
                crate::alphacode_tui::tui::info_widget::CacheHitInfo {
        ...
        let compaction_info = if !self.is_remote && self.provider.uses_alphacode_compaction() {
            let compaction = self.registry.compaction();
            compaction.try_read().ok().and_then(|manager| {
                ...
                (is_compacting || compacted_messages > 0 || summary_chars > 0).then(|| {
                    crate::alphacode_tui::tui::info_widget::CompactionInfo {
                        is_compacting,
                        compacted_messages,
        ...
        crate::alphacode_tui::tui::info_widget::InfoWidgetData {
            todos,
            todo_goals,
            ...
            cache_hit_info,
            compaction_info,
            is_compacting: if !self.is_remote && self.provider.uses_alphacode_compaction() {
                let compaction = self.registry.compaction();
                compaction
                    .try_read()
    ...
    fn auth_status(&self) -> crate::alphacode_base::auth::AuthStatus {
        ...
        crate::alphacode_base::auth::AuthStatus::check_fast_nonblocking()
    }
    ...
    fn image_expand_level(
        &self,
        image_id: u64,
    ) -> crate::alphacode_tui::tui::ui::inline_image_ui::ImageExpandLevel {
    ...
    fn account_picker_overlay(
        &self,
    ) -> Option<&RefCell<crate::alphacode_tui::tui::account_picker::AccountPicker>> {
    ...
    fn copy_badge_ui(&self) -> crate::alphacode_tui::tui::CopyBadgeUiState {
    ...
    fn copy_selection_range(&self) -> Option<crate::alphacode_tui::tui::CopySelectionRange> {
        self.normalized_copy_selection()
    }

    fn copy_selection_status(&self) -> Option<crate::alphacode_tui::tui::CopySelectionStatus> {
        ...
        let (selected_chars, selected_lines) = self
            ...
            .and_then(crate::alphacode_tui::tui::ui::copy_selection_metrics)
            .unwrap_or((0, 0));
        let has_selection = selected_chars > 0;
        Some(crate::alphacode_tui::tui::CopySelectionStatus {
            pane: self
                .current_copy_selection_pane()
                .unwrap_or(crate::alphacode_tui::tui::CopySelectionPane::Chat),
    ...
    fn onboarding_welcome_kind(&self) -> crate::alphacode_tui::tui::OnboardingWelcomeKind {

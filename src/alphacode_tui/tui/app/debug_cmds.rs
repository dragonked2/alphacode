...
impl App {
    pub(in crate::alphacode_tui::tui::app) fn debug_picker_state_json(
        &self,
    ...
    pub(in crate::alphacode_tui::tui::app) fn handle_debug_command(&mut self, cmd: &str) -> String {
        let cmd = cmd.trim();
        if cmd == "frame" {
        ...
        if cmd == "status" {
            let enabled = crate::alphacode_tui::tui::visual_debug::is_enabled();
            let overlay = crate::alphacode_tui::tui::visual_debug::overlay_enabled();
            return serde_json::json!({
        ...
        if cmd == "smoothness" {
            ...
            return crate::alphacode_tui::tui::ui::smoothness_report_json();
        }
        if cmd == "smoothness:reset" {
            crate::alphacode_tui::tui::ui::smoothness_reset();
        ...
        if cmd == "overlay:on" || cmd == "overlay:enable" {
            crate::alphacode_tui::tui::visual_debug::set_overlay(true);
            return "Visual debug overlay enabled.".to_string();
        }
        if cmd == "overlay:off" || cmd == "overlay:disable" {
            crate::alphacode_tui::tui::visual_debug::set_overlay(false);
        ...
        if cmd.starts_with("message:") {
            ...
            serde_json::json!({
                ...
                "diagram_count": crate::alphacode_tui::tui::mermaid::get_active_diagrams().len(),
            ...
            crate::alphacode_tui::tui::ui::debug_chat_image_regions_json()
            ...
            use crate::alphacode_tui::tui::visual_debug;
            ...
            use crate::alphacode_tui::tui::visual_debug;
            visual_debug::enable();
            let draw_calls = crate::alphacode_tui::tui::ui::debug_draw_call_history(16);
            match visual_debug::latest_frame() {
                Some(frame) => serde_json::to_string_pretty(&serde_json::json!({
                    "frame_id": frame.frame_id,
            ...
            let mut payload = crate::alphacode_tui::tui::ui::debug_draw_call_history(32);
            ...
            let mut payload = crate::alphacode_tui::tui::ui::debug_draw_call_history(limit);
            attach_redraw_schedule_debug(&mut payload, self);
            serde_json::to_string_pretty(&payload).unwrap_or_else(|_| "{}".to_string())
            ...
            let stats = crate::alphacode_tui::tui::mermaid::debug_stats();
            serde_json::to_string_pretty(&stats).unwrap_or_else(|_| "{}".to_string())
        } else if cmd == "mermaid:memory" {
            let profile = crate::alphacode_tui::tui::mermaid::debug_memory_profile();
            ...
            let payload = crate::alphacode_tui::tui::ui::debug_slow_frame_history(32);
            serde_json::to_string_pretty(&payload).unwrap_or_else(|_| "{}".to_string())
        } else if cmd.starts_with("slow-frames ") {
            let raw_limit = cmd.strip_prefix("slow-frames ").unwrap_or("32").trim();
            let limit = raw_limit.parse::<usize>().unwrap_or(32);
            let payload = crate::alphacode_tui::tui::ui::debug_slow_frame_history(limit);
            serde_json::to_string_pretty(&payload).unwrap_or_else(|_| "{}".to_string())
        } else if cmd == "flicker-frames" {
            let payload = crate::alphacode_tui::tui::ui::debug_flicker_frame_history(32);
            serde_json::to_string_pretty(&payload).unwrap_or_else(|_| "{}".to_string())
        } else if cmd.starts_with("flicker-frames ") {
            let raw_limit = cmd.strip_prefix("flicker-frames ").unwrap_or("32").trim();
            let limit = raw_limit.parse::<usize>().unwrap_or(32);
            let payload = crate::alphacode_tui::tui::ui::debug_flicker_frame_history(limit);
            serde_json::to_string_pretty(&payload).unwrap_or_else(|_| "{}".to_string())
        } else if cmd == "mermaid:memory-bench" {
            let result = crate::alphacode_tui::tui::mermaid::debug_memory_benchmark(40);
            serde_json::to_string_pretty(&result).unwrap_or_else(|_| "{}".to_string())
        } else if cmd == "mermaid:flicker-bench" {
            let result = crate::alphacode_tui::tui::mermaid::debug_flicker_benchmark(24);
            serde_json::to_string_pretty(&result).unwrap_or_else(|_| "{}".to_string())
            ...
            let result = crate::alphacode_tui::tui::mermaid::debug_flicker_benchmark(steps);
            serde_json::to_string_pretty(&result).unwrap_or_else(|_| "{}".to_string())
            ...
            let result = crate::alphacode_tui::tui::mermaid::debug_memory_benchmark(iterations);
            serde_json::to_string_pretty(&result).unwrap_or_else(|_| "{}".to_string())
        } else if cmd == "mermaid:cache" {
            let entries = crate::alphacode_tui::tui::mermaid::debug_cache();
            serde_json::to_string_pretty(&entries).unwrap_or_else(|_| "[]".to_string())
        } else if cmd == "mermaid:evict" || cmd == "mermaid:clear-cache" {
            match crate::alphacode_tui::tui::mermaid::clear_cache() {
                Ok(_) => "mermaid: cache cleared".to_string(),
                Err(e) => format!("mermaid: cache clear failed: {}", e),
            }
        } else if cmd == "markdown:stats" {
            let stats = crate::alphacode_tui::tui::markdown::debug_stats();
            serde_json::to_string_pretty(&stats).unwrap_or_else(|_| "{}".to_string())
        } else if cmd == "side-panel:stats" || cmd == "side-panel:debug" {
            crate::alphacode_tui::tui::side_panel_debug_json()
        ...
        {
            crate::alphacode_tui::tui::pinned_diagram_debug_json()
                .and_then(|value| serde_json::to_string_pretty(&value).ok())
                .unwrap_or_else(|| "null".to_string())
        } else if cmd == "markdown:memory" {
            let profile = crate::alphacode_tui::tui::markdown::debug_memory_profile();
            serde_json::to_string_pretty(&profile).unwrap_or_else(|_| "{}".to_string())
            ...
            use crate::alphacode_tui::tui::test_harness;
            ...
            crate::alphacode_core::env::set_var("ALPHACODE_TEST_BUNDLE", name);
            ...
            match serde_json::from_str::<crate::alphacode_tui::tui::test_harness::TestScript>(raw) {
                Ok(script) => self.handle_test_script(script),
                Err(e) => format!("script error: {}", e),
            }
        } else if cmd == "version" {
            format!("version: {}", crate::alphacode_build_meta::version())
        } else if cmd == "help" {
            "Debug commands:\n\
    ...
    pub(in crate::alphacode_tui::tui::app) fn check_stable_version(&mut self) -> bool {
    ...
    pub(in crate::alphacode_tui::tui::app) fn execute_migration(&mut self) -> bool {
        if let Some(ref version) = self.pending_migration.take() {
            ...
            crate::alphacode_core::env::set_var("ALPHACODE_MIGRATE_BINARY", stable_binary);
...
fn attach_redraw_schedule_debug(payload: &mut serde_json::Value, app: &App) {
    ...
    map.insert(
        ...
        serde_json::json!({
            "interval_ms": crate::alphacode_tui::tui::redraw_interval(app).as_millis() as u64,
            ...
            "idle_animation_active": crate::alphacode_tui::tui::idle_donut_active(app),
            "idle_animation_area": crate::alphacode_tui::tui::ui::last_idle_animation_area()
                .map(|a| serde_json::json!([a.x, a.y, a.width, a.height])),
            "client_focused": crate::alphacode_tui::tui::TuiState::client_focused(app),
            "periodic_redraw_required": crate::alphacode_tui::tui::periodic_redraw_required(app),
            ...
            "current_full_frame_reason": crate::alphacode_tui::tui::current_full_frame_redraw_reason(app),
            ...
            "key_to_paint": crate::alphacode_tui::tui::ui::key_to_paint_debug_json(),
            ...
            "status_notice": crate::alphacode_tui::tui::TuiState::status_notice(app),
            "learn_hint": crate::alphacode_tui::tui::TuiState::learn_hint(app),
            "has_notification": crate::alphacode_tui::tui::TuiState::has_notification(app),
            "is_processing": crate::alphacode_tui::tui::TuiState::is_processing(app),
        }),
    );
}

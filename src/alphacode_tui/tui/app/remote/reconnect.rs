...
use crate::alphacode_tui::tui::backend::{RemoteConnection, RemoteDisconnectReason};
use anyhow::Result;
...
pub(in crate::alphacode_tui::tui::app) enum ConnectOutcome {
    Connected(RemoteConnection),
    Retry,
    Quit,
}

pub(in crate::alphacode_tui::tui::app) enum PostConnectOutcome {
    Ready,
    Quit,
}
...
pub(in crate::alphacode_tui::tui::app) fn should_allow_reconnect_takeover(
    app: &App,
...
pub(super) fn reconnect_status_message(app: &App, state: &RemoteRunState, detail: &str) -> String {
    ...
    let resume_hint = if let Some(name) = &session_name {
        format!(" · resume: alphacode --resume {}", name)
    } else {
        String::new()
    };

    // Single canonical format so the TUI card parser (`parse_connection_retry_message`)
    // and the status line always agree — previously two formats ("attempt 1/3 · in 2s"
    // vs "attempt 5 · 6s") alternated and confused the retry display.
    format!(
...
fn disconnected_redraw_interval(initial_connect: bool) -> tokio::time::Interval {
    let period = if initial_connect {
        crate::alphacode_tui::tui::REDRAW_REMOTE_STARTUP
    } else {
...
pub(in crate::alphacode_tui::tui::app) fn reload_handoff_active(state: &RemoteRunState) -> bool {
    state.server_reload_in_progress || super::session_persistence::reload_marker_active()
}

pub(in crate::alphacode_tui::tui::app) fn should_use_same_session_fast_path(
...
async fn wait_for_reload_handoff_before_reconnect(
    ...
    .await
    {
        ...
        crate::alphacode_app_core::server::ReloadWaitStatus::Failed(detail) => {
        ...
        crate::alphacode_app_core::server::ReloadWaitStatus::Waiting { pid } => {
...
pub(in crate::alphacode_tui::tui::app) async fn connect_with_retry(
    app: &mut App,
    terminal: &mut DefaultTerminal,
    event_stream: &mut EventStream,
    state: &mut RemoteRunState,
    ...
    match loop {
        ...
        Err(e) => {
            ...
            // Fail-safe: after 10 silent retries the server is almost
            // certainly gone (crash during reload, killed daemon). Stop the
                ...
                || reload_recovery_exhausted
            {
                ...
                let hinted = format!(
                    ...
                    app.remote_session_id
                        .as_ref()
                        .and_then(|id| crate::id::extract_session_name(id))
                        .map(|n| format!("alphacode --resume {n}"))
                        .unwrap_or_else(|| "alphacode".to_string()),
                );
                set_disconnect_status_message(app, state, hinted);
                terminal.draw(|frame| crate::alphacode_tui::tui::ui::draw(frame, app))?;
                // Give the user a beat to read the message, then quit the
                // reconnect loop so the process exits instead of hanging.
                tokio::time::sleep(Duration::from_secs(2)).await;
                return Ok(ConnectOutcome::Quit);
            }
...
>(
    ...
    if state.reconnect_attempts > 0 {
        ...
        if must_reload_client {
            ...
            terminal
                .draw(|frame| crate::alphacode_tui::tui::ui::draw(frame, app))
            ...
            if (has_reload_ctx_for_session || !app.reload_info.is_empty())
                && let Ok(alphacode_dir) = crate::storage::alphacode_dir()
            {
                let marker = alphacode_dir.join(format!("client-reload-pending-{}", session_id));
...
pub(super) fn load_reload_reconnect_hints(
    ...
    let has_client_reload_marker = session_to_resume
        .and_then(|sid| {
            let alphacode_dir = crate::storage::alphacode_dir().ok()?;
            let marker = alphacode_dir.join(format!("client-reload-pending-{}", sid));
            if marker.exists() {
...
pub(in crate::alphacode_tui::tui::app) fn finalize_reload_reconnect(
    app: &mut App,
    session_to_resume: Option<&str>,

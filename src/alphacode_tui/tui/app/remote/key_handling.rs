use super::*;
use crate::alphacode_tui::tui::app as app_mod;
use crate::alphacode_tui::tui::app::PendingRemoteRewindNotice;
use crate::alphacode_tui::tui::core;

pub(in crate::alphacode_tui::tui::app) fn handle_remote_char_input(app: &mut App, c: char) {
    input::handle_text_input(app, &c.to_string());
}

pub(in crate::alphacode_tui::tui::app) async fn send_interleave_now(
    app: &mut App,
    content: String,
...
pub(in crate::alphacode_tui::tui::app) async fn handle_remote_update_command(
    app: &mut App,
    remote: &mut RemoteConnection,
) -> Result<()> {
...
pub(in crate::alphacode_tui::tui::app) async fn reload_stale_remote_server_before_update(
    app: &mut App,
    remote: &mut RemoteConnection,
) -> Result<bool> {
    ...
    pub(super) async fn handle_account_picker_command_remote(
        ...
        match command {
            ...
            crate::alphacode_tui::tui::account_picker::AccountPickerCommand::SubmitInput(input) => {
                crate::alphacode_tui::tui::app::auth::handle_account_command_remote(self, &input, remote).await?;
            }
            ...
            other => {
                if let Some(command) = crate::alphacode_tui::tui::app::auth::account_command_from_picker(&other) {
                    crate::alphacode_tui::tui::app::auth::execute_account_command_remote(self, command, remote)
                        .await?;
                }
            }
        }
...
pub(in crate::alphacode_tui::tui::app) async fn handle_remote_key(
    app: &mut App,
    code: KeyCode,
...
pub(in crate::alphacode_tui::tui::app) async fn handle_remote_key_event(
    app: &mut App,
    event: KeyEvent,
    remote: &mut RemoteConnection,
) -> Result<()> {
...
async fn handle_remote_key_internal(
    ...
    if crate::alphacode_tui::tui::app::input::newline::enter_inserts_newline(app, code, modifiers) {
        return Ok(());
    }
            ...
            if !app.input.is_empty() {
                ...
                if app_mod::model_context::is_refresh_model_list_command(trimmed) {
                    ...
                    super::super::local::handle_ui_activity(
                        ...
                        crate::bus::UiActivity::catalog(
                            ...
                            "Model List Refresh Started\n\nAsked the remote server to refresh the provider model catalog. Alphacode will show the discovered model and route changes when the server responds.",
                ...
                if trimmed == "/resume" || trimmed == "/sessions" || trimmed == "/session" {
                    ...
                    app.record_keybinding_slow(
                        crate::alphacode_tui::tui::app::shortcut_hints::LearnableAction::Resume,
                    );
                    return Ok(());
                }
                ...
                if trimmed == "/z" || trimmed == "/zz" || trimmed == "/zzz" {
                    use crate::alphacode_tui::provider::copilot::PremiumMode;

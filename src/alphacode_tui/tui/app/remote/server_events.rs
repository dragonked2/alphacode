...
use crate::alphacode_tui::tui::app::remote::swarm_plan_core::RemoteSwarmPlanSnapshot;
use crate::alphacode_tui::tui::app::remote::swarm_status_core::swarm_status_transition_notice;
...
fn client_release_version() -> String {
    if (cfg!(test) || cfg!(debug_assertions))
        && let Some(v) = std::env::var_os("ALPHACODE_TEST_CLIENT_VERSION_OVERRIDE")
    {
        return v.to_string_lossy().into_owned();
    }
    crate::alphacode_build_meta::version().to_string()
}
...
mod history_dedup_tests {
    ...
    use crate::alphacode_tui::protocol::HistoryMessage;
    use crate::alphacode_tui::session::{RenderedImage, RenderedImageSource};
...
pub(in crate::alphacode_tui::tui::app) fn handle_server_event(
    app: &mut App,
    event: ServerEvent,
    ...
    match event {
        ...
        ServerEvent::TokenUsage {
            ...
            if app.record_completed_stream_cache_usage() {
                ...
                let effective = crate::alphacode_tui::tui::info_widget::effective_prompt_tokens(
                    input,
                    app.streaming.streaming_cache_read_tokens.unwrap_or(0),
                    app.streaming.streaming_cache_creation_tokens.unwrap_or(0),
                );
        ...
        ServerEvent::Error { message, .. } => {
            ...
            let is_connectivity_error =
                crate::alphacode_tui::tui::app::commands::is_auto_poke_connectivity_error(&message)
        ...
        ServerEvent::History {
            ...
            if should_defer_history_for_runtime_identity(
                ...
                if client_detected_stale {
                    ...
                    app.push_display_message(DisplayMessage::system(format!(
                        "ℹ Connected server is running an older release ({}) than this client ({}). Reloading it before applying session state. If reload does not take, run `alphacode server stop` and relaunch. Set ALPHACODE_ALLOW_SERVER_VERSION_MISMATCH=1 only for intentional compatibility testing.",
                    ...
                    app.push_display_message(DisplayMessage::system(
                        "ℹ Connected server binary differs from the installed client channel. Reloading the server before applying remote session state. Set ALPHACODE_ALLOW_SERVER_VERSION_MISMATCH=1 only for intentional compatibility testing."
                            .to_string(),
                    ));
                }
                app.update_terminal_title();
            ...
            let model_catalog_snapshot = crate::alphacode_provider_core::ModelCatalogSnapshot::new(
                provider_name,
                provider_model,
                available_models,
                available_model_routes,

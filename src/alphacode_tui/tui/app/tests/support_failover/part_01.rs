...
fn cleanup_background_task_files(task_id: &str) {
    let task_dir = std::env::temp_dir().join("alphacode-bg-tasks");
...
/// Point `ALPHACODE_HOME` at a per-process scratch directory if nothing set one.
///
...
/// that scope their own `ALPHACODE_HOME` restore it by *removing* the variable, so
/// without serialization this function would observe the gap and repoint
/// `ALPHACODE_HOME` at the shared scratch home while that test was still running.
/// That is the race behind alphacode-tui's intermittent failures in unrelated
/// ambient, header, and model-picker tests, which all read files under
/// `ALPHACODE_HOME` mid-assertion.
///
...
fn clear_persisted_test_ui_state() {
    if let Ok(home) = crate::storage::alphacode_dir() {
    ...
    crate::alphacode_tui::tui::app::helpers::clear_ambient_info_cache_for_tests();
...
/// Run `f` in a hermetic `ALPHACODE_HOME` with reasoning display pinned to
/// `current`.
///
...
fn create_alphacode_repo_fixture() -> tempfile::TempDir {
...
fn auto_poke_config_sets_the_session_default_and_poke_on_still_overrides() {
    with_temp_alphacode_home(|| {
        ...
        commands::activate_auto_poke_local(&mut app);
        assert!(
            app.auto_poke_incomplete_todos,

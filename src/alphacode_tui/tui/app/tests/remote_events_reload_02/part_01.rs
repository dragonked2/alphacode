...
fn test_handle_remote_event_redraws_observe_tool_exec_immediately() {
    ...
    let (outcome, needs_redraw) = rt
        .block_on(remote::handle_remote_event(
            &mut app,
            &mut terminal,
            &mut remote,
            &mut state,
            crate::alphacode_tui::tui::backend::RemoteRead::Event(crate::protocol::ServerEvent::ToolStart {
                id: "tool_read".to_string(),
                name: "read".to_string(),
            }),
        ))
    ...
    let (outcome, needs_redraw) = rt
        .block_on(remote::handle_remote_event(
            ...
            &mut remote,
            &mut state,
            crate::alphacode_tui::tui::backend::RemoteRead::Event(crate::protocol::ServerEvent::ToolInput {
                delta: r#"{"file_path":"src/main.rs","start_line":1,"end_line":10}"#.to_string(),
            }),
        ))
    ...
    let (outcome, needs_redraw) = rt
        .block_on(remote::handle_remote_event(
            ...
            &mut remote,
            &mut state,
            crate::alphacode_tui::tui::backend::RemoteRead::Event(crate::protocol::ServerEvent::ToolExec {
                id: "tool_read".to_string(),
                name: "read".to_string(),
            }),
        ))
...
fn test_remote_protocol_error_stops_instead_of_reconnecting() {
    ...
    let (outcome, needs_redraw) = rt
        .block_on(remote::handle_remote_event(
            ...
            &mut remote,
            &mut state,
            crate::alphacode_tui::tui::backend::RemoteRead::Disconnected(
                crate::alphacode_tui::tui::backend::RemoteDisconnectReason::Protocol(
    ...
    assert!(matches!(outcome, remote::RemoteEventOutcome::Quit));
    assert!(needs_redraw);
    assert!(!app.is_processing);
...
fn test_handle_remote_event_redraws_observe_tool_done_immediately() {
    ...
    let (outcome, needs_redraw) = rt
        .block_on(remote::handle_remote_event(
            ...
            &mut remote,
            &mut state,
            crate::alphacode_tui::tui::backend::RemoteRead::Event(crate::protocol::ServerEvent::ToolDone {
                id: "tool_read".to_string(),
                name: "read".to_string(),
                output: "1 fn main() {}".to_string(),
                error: None,
...
fn test_swarm_completion_notification_inserts_agent_snapshot_without_report_prose() {
    ...
    app.handle_server_event(
        crate::protocol::ServerEvent::Notification {
            ...
            message: "Demo completed; README first heading is alphacode.".to_string(),
        },
        &mut remote,
    );
    ...
    assert_eq!(
        last.title.as_deref(),
        Some(crate::alphacode_tui::tui::ui::SWARM_AGENT_SNAPSHOT_TITLE)
    );
    assert!(!last.content.contains("Demo completed"));

    let rendered =
        crate::alphacode_tui::tui::ui::render_swarm_message(last, 100, crate::config::DiffDisplayMode::Off)
            .iter()
            .map(|line| {
                line.spans
                    .iter()
...
fn test_background_task_markdown_renders_card_even_if_role_was_lost() {
    ...
    app.push_display_message(DisplayMessage::user(
        "**Background task** `594967sj63` · `Run alphacode library tests afte` (`bash`) · ✗ failed · 1.0s · exit 124\n\n```text\n\n--- Command timed out after 1000ms ---\n```\n\n_Full output:_ `bg action=\"output\" task_id=\"594967sj63\"`",
    ));
    ...
    assert!(
        text.contains("✗ bg Run alphacode library tests afte failed · 594967sj63"),

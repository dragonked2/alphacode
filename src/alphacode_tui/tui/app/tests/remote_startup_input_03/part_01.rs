...
fn test_typed_absolute_image_path_promotes_before_slash_routing() {
    ...
    assert!(crate::alphacode_tui::tui::app::input::promote_dropped_images(&mut app));
    assert_eq!(app.input(), "[image 1]");
    assert_eq!(app.pending_images.len(), 1);
    assert_eq!(app.pending_images[0].0, "image/png");
}
...
fn test_incremental_terminal_drop_promotes_immediately_when_path_completes() {
    ...
    for ch in image.display().to_string().chars() {
        crate::alphacode_tui::tui::app::input::handle_text_input(&mut app, &ch.to_string());
    }
...
fn test_paste_expansion_on_submit() {
    ...
    assert_eq!(app.display_messages().len(), 1);
    assert_eq!(app.display_messages()[0].content, "A: [Pasted Content +5 lines] B");
...
fn test_recover_session_without_tools_preserves_debug_and_canary_flags() {
    ...
    app.session.working_dir = Some("/tmp/alphacode-test".to_string());
    ...
    assert_eq!(app.session.working_dir.as_deref(), Some("/tmp/alphacode-test"));
...
fn test_background_update_ready_waits_for_turn_to_finish() {
    ...
    crate::alphacode_tui::tui::app::local::handle_tick(&mut app);
...
fn test_selfdev_command_spawns_session_in_test_mode() {
    ...
    let prev_test = std::env::var_os("ALPHACODE_TEST_SESSION");
    crate::alphacode_core::env::set_var("ALPHACODE_HOME", temp_home.path());
    crate::alphacode_core::env::set_var("ALPHACODE_TEST_SESSION", "1");
    ...
    let sessions_dir = crate::storage::alphacode_dir().unwrap().join("sessions");
    ...
    if let Some(prev_test) = prev_test {
        crate::alphacode_core::env::set_var("ALPHACODE_TEST_SESSION", prev_test);
    } else {
        crate::alphacode_core::env::remove_var("ALPHACODE_TEST_SESSION");
    }
}
...
fn test_restore_reload_state_supports_legacy_input_format() {
    ...
    let alphacode_dir = crate::storage::alphacode_dir().unwrap();

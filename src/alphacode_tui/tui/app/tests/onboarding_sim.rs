...
// `/onboarding-sim`).
//
// `include!`d into `crate::alphacode_tui::tui::app::tests`, so it shares the `create_test_app`
...
fn alt_5_resets_onboarding_sim_to_a_pristine_first_screen() {
    ...
    app.onboarding_pending_model_validation = Some(
        crate::alphacode_tui::tui::app::onboarding_flow::OnboardingPendingValidation::new(
...
fn onboarding_sim_summary_arrows_preview_the_three_pills() {
    use crate::alphacode_tui::tui::app::onboarding_flow::SummaryPill;
    let mut app = create_test_app();
    app.start_onboarding_simulator();

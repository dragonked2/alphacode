...
fn input_pane_screen_points(
    width: u16,
    height: u16,
) -> Vec<(u16, u16, crate::alphacode_tui::tui::CopySelectionPoint)> {
...
fn test_input_composer_multiline_selection_preserves_newlines() {
    ...
    assert_eq!(
        crate::alphacode_tui::tui::ui::input_pane_line_text(1).as_deref(),
        Some("beta two")
    );
...
fn test_chat_drag_into_composer_clamps_to_chat_pane() {
    ...
    let (chat_col, chat_row, chat_point) = (0..24u16)
        ...
        .find_map(|(col, row)| {
            crate::alphacode_tui::tui::ui::copy_point_from_screen(col, row)
                .filter(|p| p.pane == crate::alphacode_tui::tui::CopySelectionPane::Chat)
                .map(|p| (col, row, p))
        })
    ...
    assert_eq!(
        ...
        Some(crate::alphacode_tui::tui::CopySelectionPane::Chat)
    );
    app.handle_copy_selection_mouse_with(
        MouseEvent {
            kind: MouseEventKind::Up(MouseButton::Left),

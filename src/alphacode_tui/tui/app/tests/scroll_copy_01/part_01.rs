...
fn create_error_copy_test_app() -> (App, ratatui::Terminal<ratatui::backend::TestBackend>) {
    ...
    app.display_messages = vec![
        ...
        DisplayMessage::error("permission denied while opening ~/.alphacode/config.toml"),
    ];
    app.bump_display_messages_version();
    app.scroll_offset = 0;
    app.auto_scroll_paused = false;
...
fn scroll_up_fallback_key(app: &App) -> (KeyCode, KeyModifiers) {
    app.scroll_keys
        .up_fallback
        .as_ref()
        .map(|binding| (binding.code, binding.modifiers))
...
fn prompt_up_key(app: &App) -> (KeyCode, KeyModifiers) {
    (
        app.scroll_keys.prompt_up.code,
...
fn scroll_render_test_lock() -> crate::alphacode_tui::tui::ui::RenderStateTestGuard {
    crate::alphacode_tui::tui::ui::render_state_test_lock()
}
...
impl CapturedClipboard {
    fn new() -> Self {
        crate::alphacode_tui::tui::app::helpers::capture_clipboard_for_tests();
        Self
    }

    /// The text most recently copied while this guard was active.
    fn text(&self) -> Option<String> {
        crate::alphacode_tui::tui::app::helpers::captured_clipboard_for_tests()
    }
}

impl Drop for CapturedClipboard {
    fn drop(&mut self) {
        crate::alphacode_tui::tui::app::helpers::stop_capturing_clipboard_for_tests();
    }
}
...
/// much as alphacode. On a loaded developer machine or a shared CI runner they fail
...
/// ones with `ALPHACODE_TEST_PERF_ASSERTIONS=1` on an idle machine (refs #592).
fn perf_assertions_enabled() -> bool {
    std::env::var("ALPHACODE_TEST_PERF_ASSERTIONS")
...
fn assert_perf_budget(within_budget: bool, message: impl FnOnce() -> String) {
    if perf_assertions_enabled() {
        ...
        eprintln!(
            "note: perf budget exceeded ({}); set ALPHACODE_TEST_PERF_ASSERTIONS=1 \
...
fn test_chat_native_scrollbar_hides_scroll_counters() {
    ...
    let text = render_and_snap(&app, &mut terminal);
    let scroll = app.scroll_offset.min(crate::alphacode_tui::tui::ui::last_max_scroll());
...
fn test_chat_mouse_scroll_requests_immediate_redraw_during_streaming() {
    ...
    assert!(
        crate::alphacode_tui::tui::ui::last_max_scroll() > 2,
...
fn test_chat_mouse_scroll_down_reaches_bottom_without_dead_zone() {
    ...
    let (mut app, mut terminal) = create_scroll_test_app(50, 12, 0, 36);
    render_and_snap(&app, &mut terminal);
    let bottom_scroll = crate::alphacode_tui::tui::ui::last_resolved_chat_scroll();
    ...
    render_and_snap(&app, &mut terminal);
    let scrolled_up_scroll = crate::alphacode_tui::tui::ui::last_resolved_chat_scroll();
    assert!(
    ...
    render_and_snap(&app, &mut terminal);
    let back_at_bottom_scroll = crate::alphacode_tui::tui::ui::last_resolved_chat_scroll();
...
fn test_queued_file_activity_repaint_does_not_leave_trailing_digit_artifact() {
    ...
    app.pending_soft_interrupts = vec![
        "⚠️ File activity: /home/jeremy/alphacode/src/lib.rs - amber previously read this file: read lines 1-9999"
            .to_string(),
    ];
    ...
    app.pending_soft_interrupts = vec![
        "⚠️ File activity: /home/jeremy/alphacode/src/lib.rs - amber previously read this file: read lines 1-9"
            .to_string(),
    ];
...
fn test_notification_file_activity_repaint_does_not_leave_trailing_digit_artifact() {
    ...
    app.status_notice = Some((
        "File activity · /home/jeremy/alphacode/src/lib.rs · read lines 1-9999".to_string(),
    ...
    app.status_notice = Some((
        "File activity · /home/jeremy/alphacode/src/lib.rs · read lines 1-9".to_string(),
        std::time::Instant::now(),
    ));
    let second = render_and_snap(&app, &mut terminal);
...
fn test_local_typing_snaps_rendered_viewport_to_bottom_in_one_frame() {
    ...
    let _ = render_and_snap(&app, &mut terminal);
    assert_eq!(
        crate::alphacode_tui::tui::ui::last_resolved_chat_scroll(),
        max_scroll - 8
    );
    ...
    assert_eq!(
        crate::alphacode_tui::tui::ui::last_resolved_chat_scroll(),
        crate::alphacode_tui::tui::ui::last_max_scroll(),
...
fn test_should_allow_reconnect_takeover_only_after_successful_attach() {
    ...
    assert!(!remote::should_allow_reconnect_takeover(
        &app,
        &state,

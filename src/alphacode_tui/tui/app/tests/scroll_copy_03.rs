...
fn scroll_arms_force_full_repaint_to_clear_wide_grapheme_ghosts() {
    ...
    for _ in 0..80 {
        app.force_full_redraw = false;
        app.force_full_repaint = false;
        let moved = app.handle_key(down_code, down_mods).is_ok();
        let _ = moved;
...
fn full_frame_invalidation_routes_hard_clear_over_soft_repaint() {
    use crate::alphacode_tui::tui::app::run_shell::{FullFrameInvalidation, full_frame_invalidation};
...
fn soft_full_repaint_re_emits_all_cells_without_backend_clear() {
    ...
    crate::alphacode_tui::tui::app::run_shell::invalidate_previous_terminal_buffer(&mut terminal);
...
fn test_scroll_down_past_bottom_does_not_accumulate_phantom_offset() {
    ...
    render_and_snap(&app, &mut terminal);
    let rendered_max = crate::alphacode_tui::tui::ui::last_max_scroll();
...
fn test_fast_flick_enqueues_more_lines_than_a_slow_notch() {
    ...
    assert_eq!(slow, 1, "a deliberate notch uses the base intent");
    // Even a maximum-velocity multiplier stays within the controllable cap.
    assert!(App::scroll_intent_lines(8) <= 3, "intent is capped");
}
...
fn test_queued_wheel_down_at_bottom_does_not_accumulate_phantom_scroll() {
    ...
    app.mouse_scroll_target = Some(MouseScrollTarget::Chat);
...
fn test_history_anchor_keeps_distance_from_bottom_after_prepend() {
    ...
    render_and_snap(&app, &mut terminal);
    let total_before = crate::alphacode_tui::tui::ui::last_total_wrapped_lines();
    ...
    let total_after = crate::alphacode_tui::tui::ui::last_total_wrapped_lines();
    assert!(
    ...
    let resolved = crate::alphacode_tui::tui::ui::last_resolved_chat_scroll();
    ...
    for &height in &[10u16, 16, 25] {
        for &padding in &[12usize, 24, 40] {
            for &streaming in &[false, true] {
                ...
                render_and_snap(&app, &mut terminal);
                if crate::alphacode_tui::tui::ui::last_max_scroll() == 0 {
                    continue;
                }
...
fn overscroll_reveal_does_not_relayout_transcript() {
    ...
    for pad in 4..30usize {
        ...
        let max_before = crate::alphacode_tui::tui::ui::last_max_scroll();
        ...
        let max_during = crate::alphacode_tui::tui::ui::last_max_scroll();
        ...
        let after = render_and_snap(&app, &mut terminal);
        let max_after = crate::alphacode_tui::tui::ui::last_max_scroll();
        assert_eq!(
...
fn test_click_on_swarm_expand_badge_toggles_tldr_collapse() {
    ...
    let content = crate::alphacode_tui_messages::encode_collapsible_swarm_content("fixed the flaky test", body);
    app.display_messages = vec![
    ...
    assert!(
        crate::alphacode_tui_messages::parse_collapsible_swarm_content(&app.display_messages[1].content)

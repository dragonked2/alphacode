...
//   cargo test -p alphacode-tui onboarding_golden -- --nocapture

// NOTE: This file is `include!`d into `crate::alphacode_tui::tui::app::tests`, which already
...
fn onboarding_golden_walks_every_phase() {
    ...
    // 5. Suggestions (resting state).
    {
        ...
        assert!(text.contains("Welcome to alphacode onboarding"), "{text}");
    }
}
...
fn onboarding_golden_telemetry_settings_page() {
    ...
    assert!(text.contains("Helps alphacode the most"), "caption 1: {text}");
    assert!(

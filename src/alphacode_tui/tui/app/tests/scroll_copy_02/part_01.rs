...
fn test_local_error_copy_badge_shortcut_supported() {
    ...
    assert_eq!(
        ...
        Some("permission denied while opening ~/.alphacode/config.toml"),
    );
...
fn test_copy_selection_metrics_match_built_selection_text() {
    ...
    let (chars, lines) =
        crate::alphacode_tui::tui::ui::copy_selection_metrics(range).expect("selection metrics");
...
fn test_copy_selection_full_user_prompt_line_skips_prompt_chrome() {
    ...
    let (visible_start, visible_end) =
        crate::alphacode_tui::tui::ui::copy_viewport_visible_range().expect("visible copy range");

    let (prompt_idx, prompt_text) = (visible_start..visible_end)
        .find_map(|abs_line| {
            let text = crate::alphacode_tui::tui::ui::copy_viewport_line_text(abs_line)?;
            text.contains("Show me some code")
                .then_some((abs_line, text))
        })
        .expect("expected visible user prompt line");

    app.copy_selection_anchor = Some(crate::alphacode_tui::tui::CopySelectionPoint {
        pane: crate::alphacode_tui::tui::CopySelectionPane::Chat,
        abs_line: prompt_idx,
        column: 0,
    });
    app.copy_selection_cursor = Some(crate::alphacode_tui::tui::CopySelectionPoint {
        pane: crate::alphacode_tui::tui::CopySelectionPane::Chat,
        abs_line: prompt_idx,
...
fn test_copy_selection_mouse_drag_extracts_expected_multiline_range() {
    ...
    for abs_line in visible_start..visible_end {
        let text = crate::alphacode_tui::tui::ui::copy_viewport_line_text(abs_line).unwrap_or_default();
    ...
    let start_x = (layout.messages_area.x..layout.messages_area.x + layout.messages_area.width)
        .find(|&column| {
            crate::alphacode_tui::tui::ui::copy_viewport_point_from_screen(column, start_row)
    ...
    let end_x = (layout.messages_area.x..layout.messages_area.x + layout.messages_area.width)
        .filter_map(|column| {
            crate::alphacode_tui::tui::ui::copy_viewport_point_from_screen(column, end_row)
...
fn test_copy_selection_mouse_click_does_not_enter_mode() {
    ...
    let col = (layout.messages_area.x..layout.messages_area.x + layout.messages_area.width)
        .find(|&column| {
            crate::alphacode_tui::tui::ui::copy_viewport_point_from_screen(column, row)
...
fn test_side_panel_mouse_drag_extracts_expected_text() {
    ...
    let (visible_start, visible_end) =
        crate::alphacode_tui::tui::ui::side_pane_visible_range().expect("side pane visible range");

    let (line_idx, _line_text) = (visible_start..visible_end)
        .find_map(|abs_line| {
            let text = crate::alphacode_tui::tui::ui::side_pane_line_text(abs_line)?;
    ...
    let (row, column) = (diff_area.y..diff_area.y + diff_area.height)
        .find_map(|screen_y| {
            (diff_area.x..diff_area.x + diff_area.width)
                .find(|&screen_x| {
                    crate::alphacode_tui::tui::ui::side_pane_point_from_screen(screen_x, screen_y)
    ...
    let end_column = (diff_area.x..diff_area.x + diff_area.width)
        .filter_map(|screen_x| {
            crate::alphacode_tui::tui::ui::side_pane_point_from_screen(screen_x, row)
    ...
    assert_eq!(
        ...
        Some(crate::alphacode_tui::tui::CopySelectionPane::SidePane)
    );
...
fn test_ctrl_a_copies_chat_viewport_with_context_when_input_empty() {
    ...
    let (visible_start, visible_end) =
        crate::alphacode_tui::tui::ui::copy_viewport_visible_range().expect("visible copy range");
    let line_count = crate::alphacode_tui::tui::ui::copy_viewport_line_count().expect("line count");
    let context = 4usize;
    ...
    assert_eq!(range.start.pane, crate::alphacode_tui::tui::CopySelectionPane::Chat);
    assert_eq!(range.end.pane, crate::alphacode_tui::tui::CopySelectionPane::Chat);
...
fn test_copy_selection_drag_to_top_edge_auto_scrolls_chat() {
    ...
    assert!(
        crate::alphacode_tui::tui::TuiState::copy_selection_edge_autoscroll_active(&app),
        "edge autoscroll should be reported active while drag is held at edge"
    );
    assert!(
        crate::alphacode_tui::tui::periodic_redraw_required(&app),
        "periodic redraw must be required while edge autoscroll is armed"
    );
    let policy = crate::perf::tui_policy();
    let interval = crate::alphacode_tui::tui::redraw_interval_with_policy(&app, &policy);
    assert!(
    ...
    assert!(!app.progress_copy_selection_edge_autoscroll());
    assert!(!crate::alphacode_tui::tui::TuiState::copy_selection_edge_autoscroll_active(
        &app
    ));
}
...
fn test_copy_selection_drag_below_last_line_fully_selects_last_line() {
    ...
    let last_line = (visible_start..visible_end)
        .rev()
        .find(|&ln| {
            crate::alphacode_tui::tui::ui::copy_viewport_line_text(ln)
                .map(|t| unicode_width::UnicodeWidthStr::width(t.as_str()) > 0)
                .unwrap_or(false)
        })
        .expect("a non-empty visible content line");
    let last_text = crate::alphacode_tui::tui::ui::copy_viewport_line_text(last_line).unwrap_or_default();
    ...
    let anchor_x = (area.x..area.x + area.width)
        .find(|&x| {
            crate::alphacode_tui::tui::ui::copy_viewport_point_from_screen(x, last_content_row)
...
fn test_alt_a_copies_chat_viewport_with_context_when_input_empty() {
    ...
    let handled = input::handle_alt_key(&mut app, KeyCode::Char('a'));
    assert!(handled);
    assert!(matches!(
...
fn test_changelog_overlay_supports_drag_select_and_copy() {
    ...
    let (visible_start, visible_end) = crate::alphacode_tui::tui::ui::copy_viewport_visible_range()
...
fn test_changelog_overlay_mouse_drag_release_copies_text() {
    ...
    for row in 0..24u16 {
        for col in 0..80u16 {
            if let Some(point) = crate::alphacode_tui::tui::ui::copy_point_from_screen(col, row)
                && point.pane == crate::alphacode_tui::tui::CopySelectionPane::Chat

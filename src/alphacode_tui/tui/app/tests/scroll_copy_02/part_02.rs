...
fn test_alt_shift_i_toggles_inline_images_and_persists() {
    ...
    assert!(!crate::alphacode_tui::tui::app::ui_prefs::inline_images_visible());
...
fn text_only_transcript_updates_keep_inline_image_signature_cached() {
    ...
    let signature = crate::alphacode_tui::tui::TuiState::side_pane_images_signature(&app);
    ...
    assert_eq!(app.side_pane_images_signature_cache.get(), Some(signature));
    assert_eq!(
        crate::alphacode_tui::tui::TuiState::side_pane_images_signature(&app),
        signature
    );
}
...
fn assert_rendered_expand_badge_shortcut_expands_to_full_diff(
    ...
    assert!(
        crate::alphacode_tui::tui::ui::visible_expand_edit_badge_line().is_some(),
...
fn test_try_open_link_at_opens_clicked_url_and_sets_notice() {
    let _render_lock = scroll_render_test_lock();
    let mut app = create_test_app();
    crate::alphacode_tui::tui::ui::clear_copy_viewport_snapshot();
    crate::alphacode_tui::tui::ui::record_copy_viewport_snapshot(
        std::sync::Arc::new(vec!["Docs: https://example.com/docs".to_string()]),
        std::sync::Arc::new(vec![0]),
        std::sync::Arc::new(vec!["Docs: https://example.com/docs".to_string()]),
        std::sync::Arc::new(vec![crate::alphacode_tui::tui::ui::WrappedLineMap {
            raw_line: 0,
            start_col: 0,
            end_col: 30,
        }]),
...
fn test_click_on_inline_image_label_line_cycles_level() {
    use crate::alphacode_tui::tui::ui::inline_image_ui::{
        AllFit, ImageExpandLevel, InlineImageItem, build_section,
    };
    use crate::alphacode_tui_messages::PreparedChatFrame;
    ...
    crate::alphacode_tui::tui::ui::clear_copy_viewport_snapshot();
    crate::alphacode_tui::tui::ui::record_copy_viewport_frame_snapshot_for_test(
        prepared,
        0,
        visible_end,
        content_area,
...
fn test_real_draw_click_on_body_anchored_image_label_cycles_level() {
    ...
    use crate::alphacode_tui::tui::ui::inline_image_ui::ImageExpandLevel;
    ...
    let images = <App as crate::alphacode_tui::tui::TuiState>::side_pane_images(&app);
    assert_eq!(
        images.len(),
        1,
    ...
    let image_id = {
        let img = &images[0];
        crate::alphacode_tui::tui::mermaid::inline_image_dims(&img.media_type, &img.data)

...
fn test_show_accounts_includes_masked_email_column() {
    ...
    let accounts = vec![crate::alphacode_base::auth::claude::AnthropicAccount {
...
fn test_account_openai_command_opens_account_picker() {
    with_temp_alphacode_home(|| {
        ...
        assert_eq!(picker.kind, crate::alphacode_tui::tui::PickerKind::Account);
        assert!(picker.entries.iter().any(|entry| {
            matches!(
                entry.action,
                crate::alphacode_tui::tui::PickerAction::Account(crate::alphacode_tui::tui::AccountPickerAction::Switch {
                    ref provider_id,
                    ..
...
fn test_account_command_uses_fast_auth_snapshot_without_running_cursor_status() {
    ...
    with_temp_alphacode_home(|| {
        let prev_cursor_cli_path = std::env::var_os("ALPHACODE_CURSOR_CLI_PATH");
        ...
        crate::alphacode_core::env::set_var("ALPHACODE_CURSOR_CLI_PATH", &script);
        ...
        match prev_cursor_cli_path {
            Some(value) => crate::alphacode_core::env::set_var("ALPHACODE_CURSOR_CLI_PATH", value),
            None => crate::alphacode_core::env::remove_var("ALPHACODE_CURSOR_CLI_PATH"),
        }
        crate::alphacode_base::auth::AuthStatus::invalidate_cache();
    });
}
...
fn test_account_switch_shorthand_switches_openai_account_by_label() {
    with_temp_alphacode_home(|| {
        ...
        rt.block_on(async {
            ...
            assert_eq!(
                crate::alphacode_base::auth::codex::active_account_label().as_deref(),
                Some("openai-1")
            );
        });
    });
...
fn test_account_picker_prompt_new_openai_label_cancel_clears_prompt() {
    let mut app = create_test_app();
    app.prompt_new_account_label(crate::alphacode_tui::tui::account_picker::AccountProviderKind::OpenAi);

    assert!(matches!(
        app.pending_account_input,
        Some(auth::PendingAccountInput::NewAccountLabel { ref provider_id, .. }) if provider_id == "openai"
    ));

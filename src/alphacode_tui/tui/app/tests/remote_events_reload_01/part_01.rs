...
fn test_local_bus_dictation_completion_ignores_other_session() {
    ...
    let handled = crate::alphacode_tui::tui::app::local::handle_bus_event(
        &mut app,
...
fn test_remote_bus_dictation_completion_ignores_other_session() {
    ...
    rt.block_on(crate::alphacode_tui::tui::app::remote::handle_bus_event(
        &mut app,
        &mut remote,
        Ok(crate::bus::BusEvent::DictationCompleted {
            dictation_id: "dictation_other".to_string(),
...
fn test_handle_server_event_history_session_change_clears_streaming_preview_diagram() {
    ...
    crate::alphacode_tui::tui::mermaid::set_streaming_preview_diagram(
        preview_hash,
        320,
        240,
...
fn test_handle_server_event_history_same_session_rewind_reapply_clears_streaming_preview_diagram() {
    ...
    app.pending_remote_rewind_notice =
        Some(crate::alphacode_tui::tui::app::PendingRemoteRewindNotice {
            undo: false,
...
fn test_handle_server_event_history_same_session_rewind_then_late_done_does_not_resurrect_content()
{
    ...
    // History redelivery arrives (remote/key_handling.rs).
    app.pending_remote_rewind_notice = Some(crate::alphacode_tui::tui::app::PendingRemoteRewindNotice {
        undo: false,
        message_index: Some(1),
        changed_messages: 2,
    });
...
fn test_handle_post_connect_marker_without_reload_context_does_not_queue_selfdev_continuation() {
    ...
    let alphacode_dir = crate::storage::alphacode_dir().expect("alphacode dir");
    std::fs::write(
        alphacode_dir.join(format!("client-reload-pending-{}", session_id)),
    ...
    rt.block_on(remote::handle_post_connect(
        &mut app,
        &mut terminal,
        &mut remote,
        &mut state,
...
fn test_handle_post_connect_defers_reload_followup_to_server_history_payload() {
    ...
    let outcome = rt
        .block_on(remote::handle_post_connect(
            &mut app,
            &mut terminal,
            &mut remote,
            &mut state,
    ...
    assert!(matches!(outcome, remote::PostConnectOutcome::Ready));
    assert!(app.hidden_queued_system_messages.is_empty());
...
fn test_handle_post_connect_requests_client_reload_after_server_reload_even_without_newer_binary() {
    ...
    assert!(matches!(outcome, remote::PostConnectOutcome::Quit));
    assert_eq!(
...
fn test_remote_done_waits_for_paced_backlog_and_one_live_frame() {
    ...
    rt.block_on(crate::alphacode_tui::tui::app::remote::handle_tick(&mut app, &mut remote));
    assert!(app.stream_buffer.is_empty());
    assert!(app.is_processing);
    assert_eq!(app.deferred_stream_done_id, Some(42));
...
fn test_connection_phase_elapsed_resets_per_attempt_not_per_turn() {
    ...
    assert!(crate::alphacode_tui::tui::TuiState::elapsed(&app).unwrap() > Duration::from_secs(60));
    ...
    let phase_elapsed = crate::alphacode_tui::tui::TuiState::connection_phase_elapsed(&app)
...
fn test_handle_server_event_side_pane_images_populates_pane_live() {
    ...
    assert!(<App as crate::alphacode_tui::tui::TuiState>::pin_images(&app));

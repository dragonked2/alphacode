...
// `crate::alphacode_tui::tui::markdown::mermaid_rendering_enabled()`, which supports a
// scoped thread-local test override
// (`with_mermaid_rendering_override`). Tests here must NOT mutate the
// process-global ALPHACODE_ENABLE_MERMAID env var: doing so races every other
...
impl DiagramModeOverrideGuard {
    fn pinned() -> Self {
        let prev = crate::alphacode_tui::tui::markdown::get_diagram_mode_override();
        crate::alphacode_tui::tui::markdown::set_diagram_mode_override(Some(
...
impl Drop for DiagramModeOverrideGuard {
    fn drop(&mut self) {
        crate::alphacode_tui::tui::markdown::set_diagram_mode_override(self.prev);
    }
}
...
fn test_upsert_in_place_plan_bump_accumulates_stale_active_diagrams() {
    ...
    let lines =
        crate::alphacode_tui::tui::ui::render_swarm_message(&v1_msg, 80, crate::config::DiffDisplayMode::Inline);
    assert!(
        !rendered_lines_to_text(&lines).is_empty(),
        "swarm plan message should render"
    );
    assert_eq!(
        crate::alphacode_tui::tui::mermaid::active_diagram_count(),
        1,
        "first plan render registers one active diagram"
    );
    let v1_hash = crate::alphacode_tui::tui::mermaid::get_active_diagrams()[0].hash;
    ...
    let _ =
        crate::alphacode_tui::tui::ui::render_swarm_message(&v2_msg, 80, crate::config::DiffDisplayMode::Inline);
    ...
    let diagrams = crate::alphacode_tui::tui::mermaid::get_active_diagrams();
    assert_eq!(
        diagrams.len(),
        2,
    ...
    let notice = crate::alphacode_tui::tui::TuiState::status_notice(&app);
    assert_eq!(
        notice.as_deref(),
        Some("Diagram 2/2"),
    ...
    let _ =
        crate::alphacode_tui::tui::ui::render_swarm_message(&v3_msg, 80, crate::config::DiffDisplayMode::Inline);
    assert_eq!(
        crate::alphacode_tui::tui::mermaid::active_diagram_count(),
        2,
...
fn test_new_registration_silently_shifts_parked_diagram_selection() {
    ...
    crate::alphacode_tui::tui::mermaid::register_active_diagram(0xC, 100, 80, None);

    // Newest-first: [C, B, A]. Park the user on B (index 1).
    let before = crate::alphacode_tui::tui::mermaid::get_active_diagrams();
    assert_eq!(
...
fn test_active_diagrams_cap_eviction_swaps_currently_shown_diagram() {
    ...
    assert_eq!(crate::alphacode_tui::tui::mermaid::active_diagram_count(), 128);
    ...
    crate::alphacode_tui::tui::mermaid::register_active_diagram(129, 100, 80, None);
    let diagrams = crate::alphacode_tui::tui::mermaid::get_active_diagrams();
...
fn test_offscreen_plan_graph_message_still_registers_active_diagram() {
    ...
    terminal
        .draw(|f| crate::alphacode_tui::tui::ui::draw(f, &app))
        .expect("draw failed");

    let deadline = std::time::Instant::now() + std::time::Duration::from_secs(10);
    let mut registered = crate::alphacode_tui::tui::mermaid::active_diagram_count();
    while registered == 0 && std::time::Instant::now() < deadline {
        ...
        terminal
            .draw(|f| crate::alphacode_tui::tui::ui::draw(f, &app))
            .expect("draw failed");
        registered = crate::alphacode_tui::tui::mermaid::active_diagram_count();
    }
    assert!(
        registered >= 1,
...
fn test_session_change_history_leaks_previous_session_active_diagram() {
    ...
    let plan_msg = app
        ...
        .clone();
    let _ =
        crate::alphacode_tui::tui::ui::render_swarm_message(&plan_msg, 80, crate::config::DiffDisplayMode::Inline);
    assert_eq!(
        crate::alphacode_tui::tui::mermaid::active_diagram_count(),
        1,
        "session A plan render registers one active diagram"
    );
    let stale_hash = crate::alphacode_tui::tui::mermaid::get_active_diagrams()[0].hash;
...
fn test_swarm_plan_pushes_no_plan_graph_message_when_mermaid_disabled() {
    ...
    // ALPHACODE_ENABLE_MERMAID=0) the SwarmPlan handler pushes no inline
    ...
    let count_before = app.display_messages().len();
    crate::alphacode_tui::tui::markdown::with_mermaid_rendering_override(Some(false), || {
        app.handle_server_event(
            swarm_plan_event(7, vec![swarm_plan_graph_item("haiku-1", "write a haiku")]),
            &mut remote,
        );
...
fn seed_rendered_plan_graph(
    ...
    crate::alphacode_tui::tui::mermaid::get_active_diagrams()[0].hash
}
...
fn assert_full_discard_clears_diagrams_but_keeps_plan_state(app: &mut App, path: &str) {
    ...
    assert!(
        crate::alphacode_tui::tui::mermaid::get_active_diagrams().is_empty(),
...
fn test_local_clear_command_clears_active_diagrams_but_keeps_swarm_plan_state() {
    ...
    assert!(commands::handle_session_command(&mut app, "/clear"));
...
fn test_local_clear_command_empties_margin_info_widget_diagram_list() {
    ...
    assert_eq!(
        crate::alphacode_tui::tui::TuiState::info_widget_data(&app)
            .diagrams
            .first()
            .map(|d| d.hash),
        Some(stale_hash),
    ...
    assert!(
        crate::alphacode_tui::tui::TuiState::info_widget_data(&app).diagrams.is_empty(),
...
fn test_remote_clear_command_clears_active_diagrams_but_keeps_swarm_plan_state() {
    ...
    assert_eq!(
        crate::alphacode_tui::tui::TuiState::status_notice(&app).as_deref(),
...
fn test_disconnected_ctrl_l_clears_view_but_keeps_queue_and_swarm_plan_state() {
    ...
    remote::handle_disconnected_key(&mut app, KeyCode::Char('l'), KeyModifiers::CONTROL)
...
fn test_info_widget_diagram_list_populated_only_in_margin_mode() {
    ...
    app.diagram_mode = crate::config::DiagramDisplayMode::Margin;
    let margin_data = crate::alphacode_tui::tui::TuiState::info_widget_data(&app);
    assert_eq!(
        margin_data
            .diagrams
            .iter()
    ...
    app.diagram_mode = crate::config::DiagramDisplayMode::Pinned;
    let pinned_data = crate::alphacode_tui::tui::TuiState::info_widget_data(&app);
    assert!(
...
fn test_margin_mode_plan_bump_accumulates_stale_diagram_in_info_widget_list() {
    ...
    // opts out, alphacode-tui-markdown/src/lib.rs mermaid_should_register_active),
    ...
    let _ =
        crate::alphacode_tui::tui::ui::render_swarm_message(&v1_msg, 80, crate::config::DiffDisplayMode::Inline);
    let v1_list = crate::alphacode_tui::tui::TuiState::info_widget_data(&app).diagrams;
    assert_eq!(
        v1_list.len(),
        1,
    ...
    let diagrams = crate::alphacode_tui::tui::TuiState::info_widget_data(&app).diagrams;
    assert_eq!(
        diagrams.len(),
        2,
    ...
    let _ =
        crate::alphacode_tui::tui::ui::render_swarm_message(&v3_msg, 80, crate::config::DiffDisplayMode::Inline);
    assert_eq!(
        crate::alphacode_tui::tui::TuiState::info_widget_data(&app).diagrams.len(),
        2,
...
fn test_margin_mode_has_no_diagram_selection_and_always_shows_newest() {
    ...
    crate::alphacode_tui::tui::mermaid::register_active_diagram(0xB3, 100, 80, None);
...
fn test_local_session_picker_switch_is_never_consumed_and_keeps_plan_graph_state() {
    ...
    app.session_picker_overlay = Some(RefCell::new(
        crate::alphacode_tui::tui::session_picker::SessionPicker::new(vec![
            crate::alphacode_tui::tui::session_picker::SessionInfo {
                ...
                server_name: None,
                server_icon: None,
                source: crate::alphacode_tui::tui::session_picker::SessionSource::Alphacode,
                resume_target: crate::alphacode_tui::tui::session_picker::ResumeTarget::AlphacodeSession {
    ...
    let _ = crate::alphacode_tui::tui::app::local::handle_tick(&mut app);
    let _ = crate::alphacode_tui::tui::app::local::handle_tick(&mut app);
    assert_eq!(
...
fn seed_streaming_preview(app: &mut App, hash: u64) {
    ...
    crate::alphacode_tui::tui::mermaid::set_streaming_preview_diagram(hash, 320, 240, Some("preview".to_string()));
    assert_eq!(
        crate::alphacode_tui::tui::mermaid::get_active_diagrams()
            .first()
            .map(|d| d.hash),
        Some(hash),

...
//   cargo test -p alphacode-tui onboarding_eval_scorecard -- --nocapture
//
// NOTE: `include!`d into `crate::alphacode_tui::tui::app::tests`, which already imports the
...
/// real app. `with_temp_alphacode_home` must already be active.
    ...
    let default_safe = {
        ...
        if let Some(flow) = app.onboarding_flow.as_mut() {
            ...
            let past = std::time::Instant::now()
                - (crate::alphacode_tui::tui::app::onboarding_flow::DECISION_TIMEOUT
...
fn looks_like_instruction(line: &str) -> bool {
    ...
    lower.starts_with("press ")
        ...
        // A "<command>" CTA: alphacode phrases these as both "run /login" and
...
fn tier9_metrics() -> Tier9Metrics {
    use crate::alphacode_tui::tui::app::onboarding_flow::DECISION_TIMEOUT;
...
fn selection_uses_noncolor_attribute() -> bool {
    ...
    fn render_modifiers_on_yesno_row(yes_highlighted: bool) -> Option<(Modifier, Modifier)> {
        ...
        terminal
            .draw(|frame| {
                let area = frame.area();
                crate::alphacode_tui::tui::ui::draw_onboarding_welcome_for_tests(frame, &app, area);
            })
            .ok()?;
...
fn action_row_follows_prose() -> bool {
    ...
    let prose_idx = lines.iter().position(|l| {
        ...
        lc.contains("welcome to alphacode") || lc.contains("log in")
    });
...
fn render_login_picker_overlay_text() -> String {
    ...
    let mut terminal = Terminal::new(backend).unwrap();
    terminal
        .draw(|frame| crate::alphacode_tui::tui::ui::draw(frame, &app as &dyn crate::alphacode_tui::tui::TuiState))
        .unwrap();

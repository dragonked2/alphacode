...
fn test_side_diagram_uses_left_splitter_instead_of_rounded_box() {
    ...
    let diagram_area = crate::alphacode_tui::tui::ui::last_layout_snapshot()
...
fn test_handterm_native_scroll_command_updates_chat_offset() {
    ...
    terminal
        ...
        .expect("draw failed");
    crate::alphacode_tui::tui::ui::record_layout_snapshot(Rect::new(0, 0, 50, 12), None, None, None);
    assert!(
        crate::alphacode_tui::tui::ui::last_max_scroll() > 7,
        "scroll test content should exceed the viewport"
    );

    app.apply_handterm_native_scroll(handterm_native_scroll::HostToApp::Scroll {
        pane: handterm_native_scroll::PaneKind::Chat,
        delta: -2,
    });
...
fn test_handterm_native_scroll_client_roundtrips_over_socket() {
    ...
    let mut client = handterm_native_scroll::HandtermNativeScrollClient::connect_from_env()
...
fn test_mouse_scroll_over_unfocused_diagram_scrolls_chat_without_resizing_pane() {
    ...
    let _ = render_and_snap(&app, &mut terminal);
    let max_scroll = crate::alphacode_tui::tui::ui::last_max_scroll();
...
fn test_is_scroll_only_key_detects_navigation_inputs() {
    ...
    assert!(input::is_scroll_only_key(&app, up_code, up_mods));

    let (down_code, down_mods) = scroll_down_key(&app);
    assert!(input::is_scroll_only_key(&app, down_code, down_mods));

    app.diff_pane_focus = true;
    assert!(input::is_scroll_only_key(
        &app,
        KeyCode::Char('j'),
        KeyModifiers::empty()
    ));
...
fn test_command_suggestion_navigation_moves_through_all_rows_and_allows_shift_arrow_noise() {
    ...
    assert!(suggestion_count > crate::alphacode_tui::tui::app::COMMAND_SUGGESTION_VISIBLE_LIMIT);

    for expected in 1..=crate::alphacode_tui::tui::app::COMMAND_SUGGESTION_VISIBLE_LIMIT {
        app.handle_key(KeyCode::Down, KeyModifiers::empty())
            .unwrap();
    ...
    assert_eq!(
        ...
        crate::alphacode_tui::tui::app::COMMAND_SUGGESTION_VISIBLE_LIMIT + 1
    );
    app.handle_key(KeyCode::Up, KeyModifiers::SHIFT).unwrap();
    assert_eq!(
        app.command_suggestion_selected,
        crate::alphacode_tui::tui::app::COMMAND_SUGGESTION_VISIBLE_LIMIT
    );
...
fn unmatched_command_fg(base: ratatui::style::Color) -> ratatui::style::Color {
    crate::alphacode_tui::tui::ui::input_ui::dim_command_color(Some(base))
}
...
fn matched_command_fg(base: ratatui::style::Color) -> ratatui::style::Color {
    crate::alphacode_tui::tui::ui::input_ui::brighten_command_color(Some(base))
}
...
fn test_command_suggestion_render_highlights_selected_row_by_color() {
    ...
    let selected_base = crate::alphacode_tui::tui::color_support::rgb(255, 213, 128);
    let unselected_base = crate::alphacode_tui::tui::color_support::rgb(128, 203, 196);
...
fn test_single_command_suggestion_uses_selected_color_only() {
    ...
    assert_command_match_recolored(
        &terminal,
        &command,
        crate::alphacode_tui::tui::color_support::rgb(255, 213, 128),
    );
}
...
fn test_command_suggestion_render_window_scrolls_with_selection() {
    ...
    app.cursor_pos = app.input.len();
    let suggestions = app.command_suggestions();
    let limit = crate::alphacode_tui::tui::app::COMMAND_SUGGESTION_VISIBLE_LIMIT;
    ...
    assert_eq!(
        ...
        Some(crate::alphacode_tui::tui::color_support::rgb(255, 213, 128))
    );
}
...
fn test_top_level_command_suggestions_include_project_local_skills() {
    ...
    // found under <working_dir>/.alphacode/skills, independent of the skills
...
fn test_logout_clear_anthropic_accounts_removes_all_accounts_once() {
    with_temp_alphacode_home(|| {
        ...
        crate::alphacode_base::auth::claude::set_active_account("claude-3").unwrap();

        let labels: Vec<_> = crate::alphacode_base::auth::claude::list_accounts()
            .unwrap()
            .into_iter()
        ...
        assert_eq!(crate::alphacode_base::auth::claude::clear_accounts().unwrap(), 3);
        assert!(crate::alphacode_base::auth::claude::list_accounts().unwrap().is_empty());
        assert!(crate::alphacode_base::auth::claude::active_account_label().is_none());
    });
}
...
fn test_agents_command_opens_agent_picker() {
    ...
    assert!(picker.entries.iter().any(|entry| matches!(
        entry.action,
        crate::alphacode_tui::tui::PickerAction::AgentTarget(crate::alphacode_tui::tui::AgentModelTarget::Swarm)
    )));
...
fn test_swarm_prompt_command_is_discoverable_in_suggestions_and_help() {
    ...
    assert!(help.contains(".alphacode/swarm-prompt.md"));
    assert!(help.contains("Restart or reload Alphacode"));
}
    ...
    with_temp_alphacode_home(|| {
        ...
        assert!(matches!(
            ...
            crate::alphacode_tui::tui::PickerAction::AgentModelChoice {
                target: crate::alphacode_tui::tui::AgentModelTarget::Swarm,
                clear_override: true,
            }
        ));
    });

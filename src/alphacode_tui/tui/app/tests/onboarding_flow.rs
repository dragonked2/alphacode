...
use crate::alphacode_tui::tui::app::onboarding_flow::{ExternalCli, OnboardingFlow, OnboardingPhase};
    ...
    fn model_routes(&self) -> Vec<crate::provider::ModelRoute> {
        vec![
            crate::provider::ModelRoute {
                model: crate::alphacode_provider_core::DEFAULT_CLAUDE_MODEL.to_string(),
            ...
            crate::provider::ModelRoute {
                model: crate::alphacode_provider_core::DEFAULT_OPENAI_MODEL.to_string(),
                provider: "OpenAI".to_string(),
                api_method: "openai-api-key".to_string(),
                available: true,
                detail: String::new(),
...
fn onboarding_strongest_model_only_runs_without_explicit_defaults() {
    with_temp_alphacode_home(|| {
        ...
        if let Some(value) = previous_explicit {
            crate::alphacode_core::env::set_var("ALPHACODE_INITIAL_PROVIDER_EXPLICIT", value);
        } else {
            crate::alphacode_core::env::remove_var("ALPHACODE_INITIAL_PROVIDER_EXPLICIT");
        }
    });
...
fn login_welcome_kind_shows_import_checkbox_list() {
    use crate::alphacode_tui::external_auth::ExternalAuthReviewCandidate;
    use crate::alphacode_tui::tui::OnboardingWelcomeKind;
    use crate::alphacode_tui::tui::app::onboarding_flow::ImportReview;
...
fn import_review_decision_timer_counts_down_and_times_out() {
    use crate::alphacode_tui::external_auth::ExternalAuthReviewCandidate;
    use crate::alphacode_tui::tui::app::onboarding_flow::{DECISION_TIMEOUT, ImportReview};
...
fn openrouter_key_typed_through_full_key_path_does_not_reopen_picker() {
    ...
    with_temp_alphacode_home(|| {
        ...
        // It is written to $ALPHACODE_HOME/config/alphacode/openrouter.env and exported
...
fn answering_no_on_continue_prompt_shows_suggestions() {
    ...
    impl Drop for AllowNoAuthGuard {
        fn drop(&mut self) {
            match self.0.take() {
                Some(value) => crate::alphacode_core::env::set_var("ALPHACODE_OPENROUTER_ALLOW_NO_AUTH", value),
                None => crate::alphacode_core::env::remove_var("ALPHACODE_OPENROUTER_ALLOW_NO_AUTH"),
            }
            crate::alphacode_base::auth::AuthStatus::invalidate_cache();
        }
    }
    with_temp_alphacode_home(|| {
        // Construct the guard inside the temp-home closure so the env
        // mutation below happens under `lock_test_env()` and cannot race with
        // parallel tests probing auth status.
        let _guard = AllowNoAuthGuard(std::env::var("ALPHACODE_OPENROUTER_ALLOW_NO_AUTH").ok());
...
fn startup_check_skips_selfdev_canary_session() {
    with_temp_alphacode_home(|| {
        ...
        app.onboarding_flow = None;
        app.onboarding_startup_checked = false;
        // Self-dev / canary sessions (e.g. the niri `alphacode self-dev` hotkey) take
...
fn remote_post_login_validation_waits_for_catalog_refresh() {
    use crate::alphacode_tui::tui::app::onboarding_flow::OnboardingPendingValidation;
    with_temp_alphacode_home(|| {
        let mut app = create_test_app();
        app.is_remote = true;
...
fn startup_check_skips_user_with_established_session_history() {
    with_temp_alphacode_home(|| {
        ...
        // user when their alphacode home has a substantial native session history
...
fn onboarding_state_is_escapable(app: &App) -> bool {
    use crate::alphacode_tui::tui::app::onboarding_flow::OnboardingPhase;
...
fn liveness_import_review_decline_all_then_enter_escapes() {
    use crate::alphacode_tui::external_auth::ExternalAuthReviewCandidate;
    use crate::alphacode_tui::tui::app::onboarding_flow::{ImportReview, OnboardingPhase};
    with_temp_alphacode_home(|| {
...
fn import_failure_h_key_prepares_agent_repair_brief() {
    ...
    with_temp_alphacode_home(|| {
        ...
        assert!(brief.contains("alphacode auth-test --provider openai --json"), "{brief}");
        ...
        let brief_path = crate::alphacode_tui::tui::app::onboarding_repair::repair_brief_path()
        ...
        assert!(on_disk.contains("alphacode auth-test --provider openai --json"), "{on_disk}");
...
fn import_continue_reaches_ready_quality_first_openai_model() {
    ...
    with_temp_alphacode_home(|| {
        let legacy_auth = crate::alphacode_base::auth::codex::legacy_auth_file_path().expect("legacy auth path");
        ...
        runtime.block_on(async {
            ...
            assert_eq!(model, crate::alphacode_provider_core::DEFAULT_OPENAI_MODEL);
...
fn import_summary_choose_pill_opens_checkbox_list() {
    ...
    with_temp_alphacode_home(|| {
        ...
        match app.onboarding_welcome_kind() {
            crate::alphacode_tui::tui::OnboardingWelcomeKind::Login {
                import: Some(prompt),
                ..
...
/// Point the session at a real Git checkout so the recent-project review
/// resolves a repository deterministically regardless of where the test binary
/// runs (the suite must not depend on the ambient cwd being inside a repo).
fn with_git_repo_working_dir(app: &mut App) -> tempfile::TempDir {
    let repo = create_alphacode_repo_fixture();
...
fn preparing_recent_project_review_finishes_onboarding_and_seeds_the_first_turn() {
    let mut app = onboarding_test_app();
    let _repo = with_git_repo_working_dir(&mut app);
    let repository = app
...
fn recent_project_review_falls_back_cleanly_when_no_repo_is_known() {
    // Isolate credentials: with a real login available, `suggestion_prompts`
    // returns nothing and the fallback lands on `Done` instead of the
    // suggestions screen, so the phase assertion below is environment-dependent
    // without the temp-home isolation. The temp home only isolates
    // file-based credentials; provider API keys exported in the process
    // environment survive it, so clear those too and restore them afterwards.
    ...
    impl CredentialEnvGuard {
        fn new() -> Self {
            ...
            // The default OpenAI-compatible profile advertises
            // `requires_api_key: false`, which makes openrouter look
            // "Available" even with no credentials configured, which in turn
            // empties `suggestion_prompts()` and lands the fallback on `None`
            // instead of the suggestions screen. Pin the no-auth gate off so
            // this test stays deterministic regardless of ambient config.
            crate::alphacode_core::env::set_var("ALPHACODE_OPENROUTER_ALLOW_NO_AUTH", "0");
            crate::alphacode_base::auth::AuthStatus::invalidate_cache();
            Self { saved }
        }
    ...
    with_temp_alphacode_home(|| {
        // Construct the guard inside the temp-home closure so the env
        // mutations below happen under `lock_test_env()` rather than racing
        // with parallel tests that may be probing auth status concurrently.
        let _credential_guard = CredentialEnvGuard::new();
        let mut app = onboarding_test_app();
        app.is_remote = true;
...
fn telemetry_pill_opens_settings_page_and_commits_choice() {
    use crate::alphacode_tui::external_auth::ExternalAuthReviewCandidate;
    use crate::alphacode_tui::tui::app::onboarding_flow::{ImportReview, TelemetryLevel};

...
fn test_remote_hydrated_catalog_restores_missing_direct_bedrock_route() {
    with_temp_alphacode_home(|| {
        let previous_enable = std::env::var_os("ALPHACODE_BEDROCK_ENABLE");
        ...
        match previous_enable {
            Some(value) => crate::alphacode_core::env::set_var("ALPHACODE_BEDROCK_ENABLE", value),
            None => crate::alphacode_core::env::remove_var("ALPHACODE_BEDROCK_ENABLE"),
        }
        crate::alphacode_base::auth::AuthStatus::invalidate_cache();
...
fn test_remote_current_fpt_live_model_uses_fpt_route_not_copilot_without_cache() {
    with_temp_alphacode_home(|| {
        crate::alphacode_core::env::set_var("FPT_API_KEY", "test-fpt-key");
        ...
        crate::alphacode_core::env::remove_var("FPT_API_KEY");
    });
}
...
fn test_remote_fallback_claude_model_gets_api_key_route_without_oauth() {
    ...
    with_temp_alphacode_home(|| {
        crate::alphacode_core::env::set_var("ANTHROPIC_API_KEY", "sk-ant-test-key");
        crate::alphacode_base::auth::AuthStatus::invalidate_cache();
    ...
    impl Drop for AnthropicApiKeyGuard {
        fn drop(&mut self) {
            if let Some(value) = self.0.take() {
                crate::alphacode_core::env::set_var("ANTHROPIC_API_KEY", value);
            } else {
                crate::alphacode_core::env::remove_var("ANTHROPIC_API_KEY");
            }
            crate::alphacode_base::auth::AuthStatus::invalidate_cache();
...
#[test]
fn test_remote_alphacode_subscription_catalog_is_not_augmented_with_local_auth_routes() {
    with_temp_alphacode_home(|| {
        ...
        app.remote_provider_name =
            Some(crate::subscription_catalog::ALPHACODE_PROVIDER_DISPLAY_NAME.to_string());
        ...
        match previous_anthropic_key {
            Some(value) => crate::alphacode_core::env::set_var("ANTHROPIC_API_KEY", value),
            None => crate::alphacode_core::env::remove_var("ANTHROPIC_API_KEY"),
        }
        crate::alphacode_base::auth::AuthStatus::invalidate_cache();
        ...
        let expected = crate::subscription_catalog::curated_models()
            .iter()
            .filter(|model| {
                crate::subscription_catalog::AlphacodeTier::Plus.allows(model.min_tier)
            })
            .map(|model| model.id)
            .collect::<std::collections::BTreeSet<_>>();
        assert_eq!(app.remote_model_options.len(), expected.len());
        assert!(app.remote_model_options.iter().all(|route| {
            route.provider == crate::subscription_catalog::ALPHACODE_PROVIDER_DISPLAY_NAME
                && route.api_method == crate::subscription_catalog::ALPHACODE_ROUTE_API_METHOD
                && route.available
        }));
        assert_eq!(
            app.remote_model_options
...
#[test]
fn test_remote_mixed_catalog_keeps_alphacode_subscription_separate_from_other_providers() {
    ...
    // `with_temp_alphacode_home` takes the shared env lock and points auth at an
    ...
    app.remote_model_options = vec![
        ...
        crate::provider::ModelRoute {
            model: "claude-opus-4-8".to_string(),
            provider: crate::subscription_catalog::ALPHACODE_PROVIDER_DISPLAY_NAME.to_string(),
            api_method: crate::subscription_catalog::ALPHACODE_ROUTE_API_METHOD.to_string(),
            available: true,
    ...
    let alphacode_routes = app
        ...
        .collect::<Vec<_>>();
    assert_eq!(alphacode_routes.len(), 3);
    assert_eq!(
        alphacode_routes
            .iter()
            .map(|route| route.model.as_str())
...
#[test]
fn test_remote_hydrated_catalog_adds_entitled_alphacode_subscription_routes() {
    with_temp_alphacode_home(|| {
        let previous_key = std::env::var_os(crate::subscription_catalog::ALPHACODE_API_KEY_ENV);
        let previous_tier = std::env::var_os(crate::subscription_catalog::ALPHACODE_TIER_ENV);
        crate::alphacode_core::env::set_var(
            crate::subscription_catalog::ALPHACODE_API_KEY_ENV,
            "alphacode_test_subscription_key",
        );
        crate::alphacode_core::env::set_var(crate::subscription_catalog::ALPHACODE_TIER_ENV, "plus");
        ...
        match previous_key {
            Some(value) => {
                crate::alphacode_core::env::set_var(crate::subscription_catalog::ALPHACODE_API_KEY_ENV, value)
            }
            None => crate::alphacode_core::env::remove_var(crate::subscription_catalog::ALPHACODE_API_KEY_ENV),
        }
        match previous_tier {
            Some(value) => crate::alphacode_core::env::set_var(crate::subscription_catalog::ALPHACODE_TIER_ENV, value),
            None => crate::alphacode_core::env::remove_var(crate::subscription_catalog::ALPHACODE_TIER_ENV),
        }
        ...
        let expected = crate::subscription_catalog::curated_models()
            ...
            .map(|model| model.id)
            .collect::<std::collections::BTreeSet<_>>();
        assert_eq!(alphacode_routes.len(), expected.len());
        assert_eq!(
            alphacode_routes
                .iter()
                .map(|route| route.model.as_str())
...
#[test]
fn test_remote_non_alphacode_catalog_repairs_poisoned_all_alphacode_routes() {
    with_temp_alphacode_home(|| {
        let previous_tier = std::env::var_os(crate::subscription_catalog::ALPHACODE_TIER_ENV);
        crate::alphacode_core::env::set_var(crate::subscription_catalog::ALPHACODE_TIER_ENV, "plus");
...
fn test_transfer_command_queues_pause_while_processing_locally() {
    ...
    commands::handle_transfer_command_local(&mut app);

    assert!(app.pending_transfer_request);
    let pause_message = commands::transfer_pause_message();
    assert_eq!(
...
fn test_create_transfer_session_from_parent_copies_todos_and_uses_compacted_context_only() {
    with_temp_alphacode_home(|| {
        ...
        let (child_id, _) = commands::create_transfer_session_from_parent(
            &app.session.id,
            &app.session,
...
fn test_ctrl_x_cuts_entire_input_line_to_clipboard() {
    ...
    let cut = input::cut_input_line_to_clipboard_with(&mut app, |text| {
...
fn test_ctrl_x_preserves_input_when_clipboard_copy_fails() {
    ...
    let cut = input::cut_input_line_to_clipboard_with(&mut app, |_text| false);
...
fn test_handle_input_shell_completed_renders_markdown_blocks() {
    ...
    local::handle_bus_event(&mut app, Ok(event));

...
fn test_help_topic_shows_log_command_details() {
    ...
    assert!(msg.content.contains("ALPHACODE_LOG_MARK"));
}
...
fn session_picker_preview_wheel_uses_shared_scroll_momentum() {
    use crate::alphacode_tui::tui::session_picker::{PreviewMessage, SessionInfo, SessionSource};
    ...
    let session = SessionInfo {
        ...
        server_name: None,
        server_icon: None,
        source: SessionSource::Alphacode,
        resume_target: crate::alphacode_tui::tui::session_picker::ResumeTarget::AlphacodeSession {
    ...
    let mut picker = crate::alphacode_tui::tui::session_picker::SessionPicker::new(vec![session]);
...
fn test_triage_command_includes_focus_in_prompt() {
    let prompt = crate::alphacode_tui::tui::app::commands::build_triage_prompt(" only crash reports");
...
fn test_fast_release_prompt_uses_selfdev_cache() {
    let fast_prompt = commands::build_fast_release_prompt();
...
fn test_remote_release_command_uses_tag_only_ci_path() {
    ...
    let prompt = commands::build_remote_release_prompt();
...
fn test_git_command_works_in_remote_mode_with_accessible_working_directory() {
    ...
    assert!(
        !msg.content
            .contains("currently only available in a local alphacode TUI session")
    );
}
...
fn submit_git_command_and_wait_for_response(app: &mut App) {
    ...
    rt.block_on(async {
        loop {
            ...
            local::handle_bus_event(app, Ok(event));

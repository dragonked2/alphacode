...
fn test_remote_connectivity_error_waits_for_network_without_retry_budget() {
    ...
    app.handle_server_event(
        crate::protocol::ServerEvent::Error {
            id: 15,
            message: "Failed to send OpenAI-compatible chat request\n  endpoint: https://api.groq.com/openai/v1/chat/completions\n  model: llama-3.1-8b-instant\n  auth: GROQ_API_KEY\nHint: check network connectivity, DNS/TLS, that the base URL includes the API version (usually /v1), and that the model exists on the provider.: error sending request for url (https://api.groq.com/openai/v1/chat/completions): client error (Connect): dns error: failed to lookup address information: Name or service not known".to_string(),
            retry_after_secs: None,
        },
        &mut remote,
    );
...
fn test_remote_fallback_resend_dropped_when_switch_fails() {
    ...
    app.is_remote = true;
    app.pending_fallback_resend = Some(crate::alphacode_tui::tui::app::FallbackResendPayload {
        content: "hi".to_string(),
        images: vec![],
        is_system: false,
        auto_retry: false,
...
fn test_info_widget_data_includes_connection_type() {
    ...
    let data = crate::alphacode_tui::tui::TuiState::info_widget_data(&app);
...
fn test_remote_tui_state_prefers_cached_model_during_brief_connecting_phase() {
    ...
    assert_eq!(crate::alphacode_tui::tui::TuiState::provider_model(&app), "gpt-5.4");
    assert_eq!(crate::alphacode_tui::tui::TuiState::provider_name(&app), "openai");
    assert_eq!(
        crate::alphacode_tui::tui::TuiState::session_display_name(&app).as_deref(),
        Some("otter")
    );
...
/// Run `f` with the shared env lock held and `ALPHACODE_MODEL`/`ALPHACODE_PROVIDER`
...
/// config `default_model` (shared test home) or exported `ALPHACODE_MODEL` would
...
fn test_remote_tui_state_shows_connected_after_startup_phase_clears_without_model() {
    with_neutral_remote_model_hints(|| {
        ...
        assert_eq!(crate::alphacode_tui::tui::TuiState::provider_model(&app), "connected");
        assert_eq!(crate::alphacode_tui::tui::TuiState::provider_name(&app), "");
    });
}
...
fn test_remote_tui_state_hides_brief_connecting_phase_without_cached_model() {
    ...
    assert_eq!(
        crate::alphacode_tui::tui::TuiState::provider_model(&app),
...
fn test_remote_tui_state_shows_startup_elapsed_in_header() {
    with_neutral_remote_model_hints(|| {
        let mut app = App::new_for_remote(None);
        app.set_remote_startup_phase(crate::alphacode_tui::tui::app::RemoteStartupPhase::Connecting);
...
fn test_remote_tui_state_shows_reconnecting_phase_in_header() {
    let mut app = App::new_for_remote(None);
    app.set_remote_startup_phase(crate::alphacode_tui::tui::app::RemoteStartupPhase::Reconnecting { attempt: 3 });
...
fn test_remote_header_keeps_known_model_during_brief_loading_session_phase() {
    ...
    assert_eq!(crate::alphacode_tui::tui::TuiState::provider_model(&app), "claude-fable-5");
...
fn test_remote_effort_identity_falls_back_to_session_model_before_history() {
    ...
    let efforts =
        crate::alphacode_tui::tui::app::inferred_reasoning_efforts(provider.as_deref(), model.as_deref());
    assert!(
        !efforts.is_empty(),
...
fn test_openai_compatible_login_preserves_profile_for_runtime_activation() {
    ...
    match app.pending_login {
        Some(crate::alphacode_tui::tui::app::PendingLogin::ApiKeyProfile {
            provider,
...
fn test_info_widget_remote_openai_uses_remote_provider_for_usage_and_context() {
    ...
    app.remote_resolved_credential = Some(crate::alphacode_provider_core::ResolvedCredential::Oauth);
    ...
    assert_eq!(
        ...
        Some(crate::alphacode_tui::tui::info_widget::UsageProvider::OpenAI)
    );
}
...
fn test_info_widget_remote_opencode_shows_cost_based_usage() {
    ...
    assert_eq!(
        usage.provider,
        crate::alphacode_tui::tui::info_widget::UsageProvider::CostBased
    );
...
fn test_info_widget_remote_anthropic_api_key_shows_cost_based_usage() {
    ...
    app.remote_resolved_credential = Some(crate::alphacode_provider_core::ResolvedCredential::ApiKey);
    app.token_accounting.total_input_tokens = 12_000;
    app.token_accounting.total_output_tokens = 3_400;
    ...
    assert_eq!(
        data.usage_info.as_ref().map(|info| info.provider),
        Some(crate::alphacode_tui::tui::info_widget::UsageProvider::Anthropic)
    );
}
...
fn test_info_widget_remote_openai_uses_explicit_route_when_credential_is_missing() {
    ...
    assert_eq!(
        data.usage_info.as_ref().map(|usage| usage.provider),
        Some(crate::alphacode_tui::tui::info_widget::UsageProvider::CostBased)
    );
...
fn test_info_widget_local_direct_api_runtime_shows_cost_based_usage() {
    ...
    crate::alphacode_core::env::set_var("ALPHACODE_RUNTIME_PROVIDER", "alphacode");
    crate::alphacode_core::env::remove_var("ALPHACODE_OPENROUTER_ALLOW_NO_AUTH");
    ...
    crate::alphacode_core::env::set_var("ALPHACODE_RUNTIME_PROVIDER", "openai-compatible");
...
fn test_anthropic_api_cost_accounts_for_split_cache_tokens() {
    ...
    let _guard = crate::storage::lock_test_env();
    let saved_runtime = std::env::var_os("ALPHACODE_RUNTIME_PROVIDER");
...
fn test_remote_anthropic_api_key_accrues_cost_from_token_usage() {
    ...
    oauth_app.remote_resolved_credential = Some(crate::alphacode_provider_core::ResolvedCredential::Oauth);
...
fn test_info_widget_local_gemini_shows_oauth_auth_method() {
    ...
    let path = crate::alphacode_base::auth::gemini::tokens_path().expect("gemini tokens path");
...
fn test_remote_transcript_send_uses_remote_submission_path() {
    ...
    rt.block_on(async {
        let mut remote = crate::alphacode_tui::tui::backend::RemoteConnection::dummy();
        remote::apply_remote_transcript_event(
            &mut app,
            &mut remote,
...
fn drain_paced_stream_and_replay_deferred_done(
    app: &mut crate::alphacode_tui::tui::app::App,

...
//!      login (`alphacode auth-test --json`, `alphacode provider add --api-key-stdin`,
//!      `alphacode login`). The agent can add the key/login for the user itself.
//!
...
/// An external coding agent alphacode can hand a repair task to. These map to the
...
/// `ALPHACODE_HOME`/external isolation, matching the onboarding import detectors.
...
pub(crate) fn build_repair_brief(
    ...
    let log_line = crate::alphacode_logging::log_path()
        .map(|p| format!("Logs:    {}", p.display()))
        .unwrap_or_else(|| "Logs:    ~/.alphacode/logs/alphacode-<date>.log".to_string());

    let mut brief = String::new();
    brief.push_str(
        "alphacode (a terminal coding agent) could not import or validate a login \
...
\u{20}     --base-url https://api.example.com/v1 --model <model> --api-key-stdin\n\n",
    ...
    brief.push_str(
        "When auth-test reports success, tell the user to restart alphacode (or press Enter on \
...
/// to paste anything. Lives under the alphacode home so it honors `ALPHACODE_HOME`.
...
mod tests {
    ...
    fn brief_without_provider_or_agent_is_still_actionable() {
        ...
        assert!(
            brief.contains("alphacode auth-test --all-configured --json"),
            "{brief}"
        );
        assert!(brief.contains("alphacode auth doctor"), "{brief}");
        // No agent label, but still tells the user what to do.
        assert!(brief.contains("restart alphacode"), "{brief}");
    }

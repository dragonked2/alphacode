...
use crate::alphacode_tui::util::truncate_str;
use anyhow::Result;
use base64::Engine;
...
/// (see `crate::alphacode_tui_markdown::reasoning_line_markup`). Trailing blank lines left
/// behind are trimmed so the remaining answer renders cleanly.
pub(super) fn strip_reasoning_lines(content: &str) -> String {
    let sentinel = crate::alphacode_tui_markdown::REASONING_SENTINEL;
...
pub(super) fn handle_paste(app: &mut App, text: String) {
    ...
    // Count newlines so the paste guard can scale its suppression window for
    // very long pastes that arrive slowly over SSH.
    paste_guard::count_paste_newlines(text.matches('\n').count());
...
pub(super) fn parse_dropped_paths(text: &str) -> Option<Vec<PathBuf>> {
    ...
    while let Some(ch) = chars.next() {
        if escaped {
            ...
            // A backslash is only an escape when it escapes a meaningful char
            // (space, backslash, or a quote). On Windows a backslash is also a
            // path separator (`C:\Users\...`), so `\U` must pass through as a
            // literal backslash, not be consumed as an escape.
...
impl App {
    pub(in crate::alphacode_tui::tui::app) fn handle_clipboard_paste_completed(
        &mut self,
...
/// the reporting modes alphacode enables, so `array[0]` and `[TODO]` are left alone
/// while `[<65;50;24M` and `[200~` are recognized.
///
/// Exception: bare focus reports `[I` (focus-in) and `[O` (focus-out) carry
/// *no* parameter bytes by design (they are `ESC[I` / `ESC[O` with the ESC
/// torn off by a split read or a multiplexer). Those two are stripped even
/// with empty params because they are virtually never intentional user input
/// and otherwise leak into the draft as `[I[O[I[O...` when switching terminal
/// tabs/windows.
fn bare_terminal_report_length(bytes: &[u8]) -> Option<usize> {
    ...
    // Focus reports have no params by spec — strip them bare.
    if params.is_empty() {
        return matches!(final_byte, b'I' | b'O').then_some(len);
    }
...
pub(super) fn insert_input_text(app: &mut App, text: &str) {
    ...
    // Feed the rapid-insertion detector so the *first* Enter of a
    // multi-line paste (whose bracketed-paste markers were stripped
    // by the terminal) gets classified as a paste-newline instead
    // of submitting "line1" as a queued message.
    note_text_inserted(text.len());
...
pub(super) fn handle_text_input(app: &mut App, text: &str) -> bool {
    ...
    let onboarding_suggestions = matches!(
        ...
        Some(crate::alphacode_tui::tui::app::onboarding_flow::OnboardingPhase::Suggestions)
    );
    if app.input.is_empty()
        && !app.is_processing
...
pub(super) fn clear_input_for_escape(app: &mut App) {
    ...
    app.reset_tab_completion();
    app.sync_model_picker_preview_from_input();
    // Escape always resets the paste-burst detector; the user has
    // committed to a non-paste action.
    reset_burst();
    if had_input {
...
impl App {
    ...
    /// Spawn a brand-new alphacode session in a new terminal window.
    pub(crate) fn handle_new_terminal_hotkey(&mut self) {
        ...
        match super::spawn_fresh_session_in_new_terminal(&cwd) {
            ...
            Ok(false) => {
                self.set_status_notice("No supported terminal found; run `alphacode` manually")
            }
...
pub(super) fn handle_pre_control_shortcuts(
    ...
    if app.toggle_keys.info_widget.matches(code, modifiers) {
        crate::alphacode_tui::tui::info_widget::toggle_enabled();
        let status = if crate::alphacode_tui::tui::info_widget::is_enabled() {
            "Info widget: ON"
        } else {
            "Info widget: OFF"
        };
    ...
    {
        use crate::alphacode_tui::tui::TuiState as _;
...
pub(super) fn handle_visible_copy_shortcut(
    ...
    if let Some(target) = crate::alphacode_tui::tui::ui::recent_flicker_copy_target_for_key(c)
        .or_else(|| crate::alphacode_tui::tui::ui::visible_copy_target_for_key(c))
    {
...
fn handle_expand_edit_badge_shortcut(app: &mut App, key: char) -> bool {
    ...
    let visible_expand_badge = crate::alphacode_tui::tui::ui::visible_expand_edit_badge();
    let has_edit_tool_message = app.display_edit_tool_message_count > 0
        || app.display_messages.iter().any(|message| {
            message
                .tool_data
                .as_ref()
                .map(|tool| crate::alphacode_tui::tui::ui::tools_ui::is_edit_tool_name(&tool.name))
                .unwrap_or(false)
        });
...
pub(super) fn handle_modal_key(
    ...
    if app.copy_selection_mode {
            ...
            && matches!(code, KeyCode::Char('c') | KeyCode::Char('d'))
        {
            ...
            // closed alphacode and lost the error (issue #497). Only fall
...
pub(super) fn handle_enter(app: &mut App) -> bool {
    ...
    // Line-burst detector: if the user is mid-paste (rapid Enters
    ...
    // and any Enter that fires now is a paste-newline, not a submit.
    // This catches the *first* Enter of a non-bracketed paste, which
    // the line-burst detector below cannot (no prior Enter exists).
    if rapid_insertion_active() {
        // Treat as a fresh burst-start: line count = 1 (this Enter),
        // but we're already "in flight" so collapse at end.
        app.remember_input_undo_state();
        insert_paste_newline(&mut app.input, &mut app.cursor_pos);
        app.follow_chat_bottom_for_typing();
        return true;
    }
    match check_enter_burst(input_len) {
        ...
        PasteBurstState::JustEnded => {
            let lines = paste_buffer::burst_line_count();
            collapse_recent_burst_into_placeholder(app, lines);
            return true;
        }
...
fn collapse_recent_burst_into_placeholder(app: &mut App, lines: usize) {
    ...
    for (idx, _ch) in input.char_indices().rev() {
    ...
    app.cursor_pos = app.input.len();
    app.pasted_contents.push(content.clone());
    app.reset_tab_completion();
    app.sync_model_picker_preview_from_input();
    app.follow_chat_bottom_for_typing();
    ...
    let color = output_enhance::rgb_escape(r, g, b, true);
    let reset = output_enhance::RESET;
    ...
    let note = format!(
        "{color}Pasted {line_count} {line_label} ({size_label}) collapsed into {placeholder}{reset}",
    );
    app.set_status_notice(note);
    reset_burst();
    // Suppress the unused-mut warning on `app` if the borrow checker
    // would otherwise complain about the unused `app` reference above
    // (the status notice call is a `&mut self` borrow that consumes
    // the app ref before reset_burst()).
    let _ = &content;
}
...
impl App {
    pub(super) fn handle_key_event(&mut self, event: crossterm::event::KeyEvent) {
        ...
        use crate::alphacode_tui::tui::test_harness::{TestEvent, record_event};
    ...
    fn handle_key_core(
        ...
        // Accept an armed "merge the diverged update" offer: spawn a alphacode agent
        ...
        if code == KeyCode::Enter {
            ...
            if self.input.trim().is_empty()
                && matches!(
                    ...
                    Some(crate::alphacode_tui::tui::app::onboarding_flow::OnboardingPhase::ModelSelect)
                )
            {
    ...
    pub(super) fn gc_offscreen_reasoning_traces(&mut self) -> bool {
        ...
        let total = crate::alphacode_tui::tui::ui::last_total_wrapped_lines();
        let viewport = crate::alphacode_tui::tui::ui::last_layout_snapshot()
    ...
    /// [`StreamBuffer`](crate::alphacode_tui::tui::stream_buffer::StreamBuffer) to the live
    ...
    pub(super) fn apply_stream_ops(
        &mut self,
        ops: Vec<crate::alphacode_tui::tui::stream_buffer::StreamOp>,
    ) -> bool {
        use crate::alphacode_tui::tui::stream_buffer::StreamOp;
    ...
    pub(super) fn submit_input(&mut self) {
        ...
        if let Some(command) = extract_input_shell_command(&input) {
            ...
            if self.is_remote {
                self.push_display_message(DisplayMessage::system(
                    "Input-line ! shell commands are only available in a local alphacode TUI session.",
                ));
        ...
        if let Some(invocation) = skill_invocation {
            ...
            // as .alphacode/skills/optimization work immediately after reload/build.
            if skill.is_none() {
            ...
            if let Some(skill) = skill {
                ...
                let endorsed_hint = crate::skill::endorsed_skills()
                    ...
                    .map(|endorsed| match endorsed.install {
                        ...
                        None => format!(
                            "Skill /{} is endorsed but not installed (source: {}). Install it into ~/.alphacode/skills/{}/SKILL.md.",

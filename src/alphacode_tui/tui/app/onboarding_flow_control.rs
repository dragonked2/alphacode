...
use crate::alphacode_tui::import::repo_ranking::{self, SessionLocation};
use crate::alphacode_tui::tui::session_picker::{SessionPicker, load_sessions};
...
impl App {
    ...
    pub(super) fn maybe_begin_onboarding_flow_after_login(&mut self) {
        ...
        // DISABLED: first-run onboarding is intentionally suppressed. A fresh
        // install that successfully logs in (via `/login` or any other path)
        // should land directly on the normal new-session screen instead of
        // being walked through the post-login "Ready to use / Needs attention"
        // readiness block. Returning here keeps an existing flow advancing
        // (e.g. simulator-driven) but never auto-starts the flow from scratch.
    }
    ...
    ///
    /// DISABLED: first-run onboarding is intentionally suppressed. The guided
    /// "How would you like to begin?" prompt and the post-login "Ready to use /
    /// Needs attention" readiness block are no longer shown; the TUI simply
    /// opens a normal new-session screen. Users can still trigger login and
    /// model validation manually via `/login` and `/model`. We still commit
    /// `onboarding_startup_checked` so subsequent ticks stay a no-op.
    ...
    #[cfg_attr(not(test), allow(dead_code))] // exercised by `mod tests` in onboarding_flow.rs
    ...
    pub(super) fn begin_onboarding_flow_at_login(&mut self) {
        ...
        if had_imports {
            self.set_status_notice(
                "Welcome to alphacode: review detected logins (arrows/hl to move, Enter to choose)",
            );
        } else {
            self.set_status_notice(
    ...
    fn onboarding_finish_import_review(&mut self) {
        ...
        // target the agent repair brief at the right `alphacode auth-test --provider`.
    ...
    fn onboarding_start_choice_banner_lines() -> Vec<ratatui::text::Line<'static>> {
        ...
        let accent = crate::alphacode_tui::tui::color_support::rgb(186, 139, 255);
        vec![
    ...
    /// with a large `~/.alphacode/sessions` directory. Doing it inline on Enter made
    ...
    pub(super) fn onboarding_prepare_recent_project_review(&mut self) -> bool {
        let Some(repository) = self.onboarding_recent_project_path() else {
            self.onboarding_show_suggestions();
            self.set_status_notice(
                "No recent Git repository found. Start alphacode inside a project to review it.",
            );
            return false;
        };
        self.onboarding_finish();
    ...
    pub(super) fn onboarding_validate_default_model(&mut self) {
        if !crate::alphacode_base::auth::AuthStatus::check_fast().has_any_available() {
            return;
        }
    ...
    fn onboarding_spawn_model_validation(&mut self) {
        ...
        let verify_copilot = provider_key.as_deref() != Some("copilot")
            && crate::alphacode_base::auth::copilot::has_copilot_credentials_fast();
    ...
    /// 1. A alphacode login that exists but expired (they clearly use it).
    ...
    fn onboarding_login_suggestion_provider() -> Option<&'static str> {
        ...
        // If a custom OpenAI-compatible base URL is configured via env var,
        // suggest that provider — the user has already set up a local/custom endpoint.
        if std::env::var("ALPHACODE_OPENAI_COMPAT_API_BASE").is_ok() {
    ...
    fn onboarding_other_provider_rows(skip: Option<&str>) -> (Vec<String>, Vec<String>) {
        ...
        let providers: [(&str, &str, AuthState); 8] = [
            ...
            ("Alphacode subscription", "alphacode", status.alphacode),
    ...
    /// commands the agent can run (`alphacode auth-test --json`, `alphacode login
    /// --api-key-stdin`, `alphacode provider add`), copy it to the clipboard, and
    /// surface it as a system message so the user can paste it into that agent.
    fn onboarding_prepare_agent_repair_brief(&mut self) {
        use crate::alphacode_tui::tui::app::onboarding_repair;

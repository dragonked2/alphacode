...
fn parse_account_command(trimmed: &str) -> Option<Result<AccountCommand, String>> {
    ...
    if let Some(provider) = resolve_account_provider_descriptor(first) {
        ...
        let parsed = match subcommand {
            ...
            "status" if provider.id == "alphacode" => AccountCommand::AlphacodeStatus,
            "manage" if provider.id == "alphacode" => AccountCommand::AlphacodeManage,
            "logout" if provider.id == "alphacode" => AccountCommand::AlphacodeLogout,
...
pub(crate) fn account_command_from_picker(
    ...
    use crate::alphacode_tui::tui::account_picker::{AccountPickerCommand, AccountProviderKind};
...
pub(crate) fn execute_account_command_local(app: &mut App, command: AccountCommand) {
    match command {
        ...
        AccountCommand::AlphacodeStatus => app.show_alphacode_subscription_status(),
        AccountCommand::AlphacodeManage => app.open_alphacode_account_management(),
        AccountCommand::AlphacodeLogout => app.start_alphacode_account_logout(),
    ...
    match provider_id {
        "claude" => {
            let target = match label.map(str::trim).filter(|label| !label.is_empty()) {
                ...
                None => match crate::alphacode_base::auth::claude::next_account_label() {
                    Ok(label) => label,
                    Err(e) => {
                        app.push_display_message(DisplayMessage::error(format!(
        ...
        "openai" => {
            let target = match label.map(str::trim).filter(|label| !label.is_empty()) {
                Some(existing) => existing.to_string(),
                None => match crate::alphacode_base::auth::codex::next_account_label() {
                    Ok(label) => label,
                    Err(e) => {
                        app.push_display_message(DisplayMessage::error(format!(
...
fn save_openai_compat_setting(app: &mut App, setting: OpenAiCompatSetting, value: Option<&str>) {
    ...
    let (env_key, normalized_value) = match setting {
        OpenAiCompatSetting::ApiBase => {
            ...
            ("ALPHACODE_OPENAI_COMPAT_API_BASE", normalized)
        }
...
mod tests {
    ...
    #[test]
    fn parse_native_alphacode_account_actions() {
        assert!(matches!(
            parse_account_command("/account alphacode login"),
            Some(Ok(AccountCommand::Login { provider_id })) if provider_id == "alphacode"
        ));
        assert!(matches!(
            parse_account_command("/account alphacode status"),
            Some(Ok(AccountCommand::AlphacodeStatus))
        ));
        assert!(matches!(
            parse_account_command("/account alphacode manage"),
            Some(Ok(AccountCommand::AlphacodeManage))
        ));
        assert!(matches!(
            parse_account_command("/account alphacode logout"),
            Some(Ok(AccountCommand::AlphacodeLogout))
        ));
    }
    ...
    fn render_auth_doctor_markdown_includes_recovery_steps() {
        ...
        assert!(markdown.contains("alphacode login --provider openai"));

use crate::alphacode_tui::tui::{TuiState, color_support::rgb};
use ratatui::prelude::*;
use std::cell::RefCell;
use std::collections::{HashSet, hash_map::DefaultHasher};
use std::hash::{Hash, Hasher};
use std::sync::OnceLock;
/// The default idle decoration is a calm, stable wordmark. The 3D ASCII
/// animations (`donut`, `orbit_rings`, `gyroscope`, `black_hole`) are still
/// available but only re-enabled when the user explicitly opts in via
/// `display.idle_animation = true` (or by adding the variant name to
/// `display.disabled_animations`' complement, which is what
/// `choose_animation_variant` actually consults).
///
/// Why: the 3D samplers redraw a `3 * width * 3 * height` subpixel grid every
/// animation tick using ASCII shading characters (`.`, `:`, `-`, `=`, `+`,
/// `*`, `#`, `@`, `\`, `/`, ...). At 30+ FPS on a 160x48 terminal that is ~57k
/// glyph/colour updates a second, which read as fuzzy / glitchy in many
/// real-world terminals and dominated idle CPU. The wordmark is a single
...
pub(crate) fn idle_animation_debug_json() -> serde_json::Value {
    serde_json::json!({
        ...
        "full_repaints": IDLE_ANIMATION_FULL_REPAINTS
            .load(std::sync::atomic::Ordering::Relaxed),
        "last_full_frame_reason": crate::alphacode_tui::tui::last_full_frame_redraw_reason(),
...
// dependency-free `alphacode-tui-anim` crate, which is pinned to opt-level = 3 in
...
pub(crate) fn render_idle_animation(buf: &mut Buffer, area: Rect, elapsed: f32) {
    ...
    // The default decoration is a calm, stable wordmark. Skipping the 3D
    // samplers entirely is the whole point: the historical decoration redrew
    // a subpixel glyph grid every animation tick and read as fuzzy / glitchy
    ...
    IDLE_BUF.with(|cell| {
        ...
        match variant {
            ...
            // Any unrecognised name (including the historical `three_rings`
            // alias) falls back to the calm wordmark instead of redrawing the
            // subpixel grid. Users who explicitly opted into an old animation
            // and did not pin a working variant name no longer get a fuzzy 3D
            // donut, and the cost is bounded.
            _ => {
                render_idle_wordmark(buf, area);
                return;
            }
...
/// Calm, stable idle decoration.
///
/// Writes two centred text lines into the reservation: a wordmark and a
/// short prompt hint. No flicker, no per-frame cost beyond what the rest of
/// the frame already pays. The `elapsed` argument is intentionally unused:
/// the decoration is *static* on purpose, so the run loop does not need to
...
fn render_idle_wordmark(buf: &mut Buffer, area: Rect) {
    ...
    // Pull the role colours out of the active theme so the wordmark matches
    // the rest of the UI without hard-coding palette numbers. The fallback
    // colours (gray / dark gray) keep the decoration readable even before a
...
mod tests {
    ...
    fn idle_variants_exclude_retired_variants() {
        ...
        // The historical 3D variants are intentionally NOT in the default
        // pool: the calm wordmark is the default decoration.
    ...
    #[test]
    fn idle_variants_default_to_calm_static_logo() {
        // The default decoration is a calm, static wordmark. The historical
        // 3D variants are reachable only by name (via `disabled_animations`
        // filtering against the full set, or by future variants the user
        // explicitly adds), never by being the default.
        assert!(IDLE_VARIANTS.contains(&"static_logo"));
        assert!(!IDLE_VARIANTS.contains(&"donut"));
    ...
    #[test]
    /// The calm default decoration must be byte-identical across two arbitrary
    /// `elapsed` values. If a future change re-introduces a per-frame delta
    /// (e.g. a fade-in, a slow shimmer, anything that depends on `elapsed`),
    /// the screens will diverge and the test will fail.
    fn static_logo_decoration_is_static_across_frames() {
        let area = Rect::new(0, 0, 80, 14);
        let buf_area = Rect::new(0, 0, 80, 14);
        let mut a = Buffer::empty(buf_area);
        let mut b = Buffer::empty(buf_area);
        render_idle_animation(&mut a, area, 0.0);
        render_idle_animation(&mut b, area, 12.5);
        assert_eq!(
            a, b,
            "the calm default decoration must not vary with elapsed time"
        );
    }
    ...
    #[test]
    fn static_logo_decoration_handles_narrow_reservations() {
        for w in [4u16, 8, 12, 16, 20, 40, 80, 160] {
            for h in [2u16, 3, 5, 14] {
                let area = Rect::new(0, 0, w, h);
                let buf_area = Rect::new(0, 0, w, h);
                let mut buf = Buffer::empty(buf_area);
                render_idle_animation(&mut buf, area, 0.5);
                // Reservations narrower than the wordmark ("alphacode" = 9
                // chars) cannot render anything; that is a documented
                // no-op, not a failure. Wider reservations must draw at
                // least one glyph.
                if w < 9 {
                    continue;
                }
    ...
    /// animation-only repaint a still decoration) but the gating policy in
    /// `idle_donut_paces_redraws` must keep the loop at the slow idle
    /// cadence when the variant is the calm wordmark. We do not re-test the
    /// cadence here (it lives in `redraw_schedule.rs`); we only test that
    /// `render_idle_animation` is a no-op for sub-minimum areas, which is
    /// what the rest of the pipeline keys off.
    #[test]
    fn static_logo_decoration_noops_on_tiny_areas() {
        let area = Rect::new(0, 0, 3, 1);
        let buf_area = Rect::new(0, 0, 4, 2);
        let mut buf = Buffer::empty(buf_area);
        let before = buf.clone();
        render_idle_animation(&mut buf, area, 0.0);
        // Nothing changed outside the requested sub-area, and nothing was
        // written inside it either (the helper returns early for < 4 wide).
        assert_eq!(buf, before, "tiny areas must not be touched");
    }

    #[test]
    fn variant_selection_avoids_disabled_entries_when_possible() {
        // With only `static_logo` in the default pool, disabling it (which
        // is what the empty disabled-set would let through) should fall
        // back to the only available entry, not panic.
        let disabled = expand_disabled_animation_names(["static_logo"]);
        let variant = choose_animation_variant_from_disabled(IDLE_VARIANTS, 7, &disabled);
        assert_eq!(variant, "static_logo");
    }

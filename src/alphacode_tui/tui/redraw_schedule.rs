...
pub(crate) const REDRAW_SWARM_SPINNER: Duration =
    Duration::from_millis(crate::alphacode_tui_render::swarm_gallery::STRIP_SPINNER_FRAME_MS);
...
fn swarm_spinner_redraw_active(state: &dyn TuiState) -> bool {
    state.client_focused()
        && state
            ...
            .iter()
            .any(|m| crate::alphacode_tui_render::swarm_gallery::is_active_status(&m.status))
}

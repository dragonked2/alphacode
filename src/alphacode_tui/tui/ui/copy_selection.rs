...
pub(super) fn copy_selection_text_from_raw_lines(
    snapshot: &CopyViewportSnapshot,
    start: crate::alphacode_tui::tui::CopySelectionPoint,
    end: crate::alphacode_tui::tui::CopySelectionPoint,
) -> Option<String> {
...
fn copy_selection_text_with_math_targets(
    ...
    let math_targets: Vec<_> = prepared
        ...
        .filter(|target| {
            matches!(
                &target.kind,
                crate::alphacode_tui_markdown::CopyTargetKind::Math { .. }
            )
    ...
    for target in math_targets {
        if cursor.abs_line < target.start_line {
            ...
            let last_col = snapshot
                ...
                .unwrap_or(0);
            let normal_end = crate::alphacode_tui::tui::CopySelectionPoint {
                pane: cursor.pane,
                abs_line: last_line,
                column: last_col,
            };
        ...
        cursor = crate::alphacode_tui::tui::CopySelectionPoint {
            pane: cursor.pane,
...
mod tests {
    ...
    fn point(line: usize, column: usize) -> crate::alphacode_tui::tui::CopySelectionPoint {

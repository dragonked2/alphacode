...
//! alphacode already requests it at startup, and crossterm decodes it, so on any
...
/// A terminal whose Shift+Enter behavior alphacode can configure.
...
impl SetupTarget {
    ...
    pub fn activation_note(self) -> &'static str {
        match self {
            ...
            Self::Tmux => "Run `tmux source ~/.tmux.conf`, then restart alphacode.",
...
///
/// `extended-keys on` only forwards when the app asks (alphacode does), which is
...
# alphacode: let Shift+Enter reach the application instead of collapsing to Enter.
...
pub const MANAGED_MARKER: &str = "# alphacode: let Shift+Enter reach the application";

/// Whether `config` already contains alphacode's managed tmux block.
...
/// Configure tmux so a capable outer terminal's Shift+Enter reaches alphacode.
        ...
        std::fs::write(
            ...
            format!(
                "local wezterm = require 'wezterm'\n\
                 ...
                 -- alphacode: report Shift+Enter distinctly from Enter.\n\
                 {WEZTERM_CONFIG_LINE}\n\
                 return config\n"
            ),
        )?;

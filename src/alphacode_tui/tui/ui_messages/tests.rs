...
fn render_cold_cache_warning_is_always_one_width_bounded_line() {
    let saved = crate::alphacode_tui::tui::markdown::center_code_blocks();
    let msg = DisplayMessage::system(
        "🧊 Prompt cache went cold · next turn may resend ~96K tok · /cache extends",
    );

    for centered in [false, true] {
        crate::alphacode_tui::tui::markdown::set_center_code_blocks(centered);
    ...
    crate::alphacode_tui::tui::markdown::set_center_code_blocks(saved);
}
...
fn render_compact_launch_and_divergence_notices_as_one_line() {
    ...
    let notices = [
        DisplayMessage::system(
            "Configured Alphacode launch hotkeys (niri):\nSuper+; → alphacode (/home/user/project)\n\nBound system-wide.",
        )
        .with_title("Launch hotkeys"),
        DisplayMessage::system(
            "Update diverged. Press Ctrl+Y to let a alphacode agent merge local and upstream (or run `git pull` / `git rebase` yourself).",
        )
        .with_title("Update"),
    ];
...
fn render_system_message_centered_mode_left_aligns_with_padding() {
    let saved = crate::alphacode_tui::tui::markdown::center_code_blocks();
    crate::alphacode_tui::tui::markdown::set_center_code_blocks(true);
    let msg = DisplayMessage::system("Reload complete - continuing.");
...
fn render_system_message_uses_width_stable_titles_on_kitty() {
    ...
    crate::alphacode_core::env::set_var("TERM_PROGRAM", "kitty");
    crate::alphacode_core::env::set_var("TERM", "xterm-kitty");
    ...
    match prev_term_program {
        Some(value) => crate::alphacode_core::env::set_var("TERM_PROGRAM", value),
        None => crate::alphacode_core::env::remove_var("TERM_PROGRAM"),
    }
    match prev_term {
        Some(value) => crate::alphacode_core::env::set_var("TERM", value),
        None => crate::alphacode_core::env::remove_var("TERM"),
    }
}
...
fn render_background_task_message_uses_swarm_flavor_for_swarm_tool() {
    crate::alphacode_tui::tui::markdown::set_center_code_blocks(false);
    let msg = DisplayMessage::background_task(
...
fn render_assistant_message_plan_card_keeps_nested_fences_inside() {
    ...
    let bash_line = lines
        .iter()
        .map(extract_line_text)
        .find(|line| line.contains("cargo test -p alphacode-tui"))
...
fn render_system_message_uses_connection_card_for_reconnect_status() {
    let msg = DisplayMessage::system(
        "⚡ Connection lost - retrying (attempt 2, 7s) - connection reset by server · resume: alphacode --resume koala",
    )
    .with_title("Connection");
    ...
    assert!(plain.contains("alphacode --resume koala"));
}
...
fn render_swarm_message_centered_mode_caps_wrap_width_for_long_notifications() {
    ...
    let msg = DisplayMessage::swarm(
        "File activity",
        "/home/jeremy/alphacode/src/tui/ui_messages.rs - moss just edited this file while you were working nearby, so the notification should still read as centered in wide layouts.",
    );
...
fn render_swarm_message_collapsed_shows_tldr_and_expand_badge_only() {
    let saved = crate::alphacode_tui::tui::markdown::center_code_blocks();
    crate::alphacode_tui::tui::markdown::set_center_code_blocks(false);
    let content = crate::alphacode_tui_messages::encode_collapsible_swarm_content(
...
fn render_swarm_message_expanded_shows_body_and_collapse_badge() {
    let saved = crate::alphacode_tui::tui::markdown::center_code_blocks();
    crate::alphacode_tui::tui::markdown::set_center_code_blocks(false);
    let collapsed = crate::alphacode_tui_messages::encode_collapsible_swarm_content(
        "fixed the flaky test",
        "The flaky test was caused by a race in the setup helper.",
    );
...
fn render_tool_message_shows_intent_and_technical_preview_on_one_line() {
    crate::alphacode_tui::tui::ui::tools_ui::tests_tool_call_details_override::set(true);
    let msg = DisplayMessage {
        role: "tool".to_string(),
        ...
        tool_data: Some(crate::message::ToolCall {
            ...
            name: "bash".to_string(),
            input: serde_json::json!({
                "command": "cargo test -p alphacode render_background_task --lib",
...
fn render_tool_message_shows_discovery_browse_results_and_rationale() {
    let msg = discovery_message(
        "Discoverable tools in 'payments' (Alphacode tool directory; recommendations must be based only on fit; details: https://alphacode.sh/discovery-tools):\n\n- agentcard: prepaid virtual Visa cards for AI agents (https://agentcard.sh/?via=alphacode-discovery)\n\nBrowse request ID: `11111111-2222-4333-8444-555555555555`",
        serde_json::json!({
            "action": "browse",
            "category": "payments",
...
fn render_tool_message_shows_catalog_suggestion_receipt_and_trust_line() {
    ...
    assert!(plain.contains("Alphacode maintainers only"), "{plain}");
...
fn discovery_cards_wrap_within_narrow_transcript_width() {
    let msg = discovery_message(
        "Catalog suggestion submitted.\n\nStatus: received for Alphacode maintainer review.",
        serde_json::json!({
            "action": "suggest",
...
#[test]
fn theme_preview_renders_truecolor_swatches_and_active_marker() {
        ...
        "rows": [
            {
                "id": "nord",
                "dark": true,
                "description": "Arctic calm blues",
                "swatch": [[46, 52, 64], [59, 66, 82], [136, 192, 208]]
            },
            {
                "id": "paper",
                "dark": false,
                "description": "Warm paper",
                "swatch": [[250, 248, 240], [40, 40, 40], [176, 56, 56]]
            }
        ]
    });
    let msg = DisplayMessage {
    ...
    // Active marker: exactly one `●` (nord) and one blank marker (paper).
    assert_eq!(text.matches('●').count(), 1, "one active marker: {text}");

    // The swatch cells carry truecolor foregrounds, not the forced system
    // color: [46,52,64] and [136,192,208] must both appear as fg RGB spans.
    let mut saw_nord_rgb = false;
    let mut saw_accent_rgb = false;
    for line in &lines {
        for span in &line.spans {
            if span.style.fg == Some(Color::Rgb(46, 52, 64)) {
                saw_nord_rgb = true;
            }
            if span.style.fg == Some(Color::Rgb(136, 192, 208)) {
                saw_accent_rgb = true;
            }
        }
    }
...
#[test]
fn theme_preview_falls_back_to_plaintext_on_unparseable_payload() {
    let msg = DisplayMessage {
        role: "system".to_string(),
        content: "just some old plain text".to_string(),
        tool_calls: Vec::new(),
        duration_secs: None,
        title: Some(THEME_PREVIEW_TITLE.to_string()),
        tool_data: None,
    ...
    // Fallback lines are forced back to the system color.
    for line in &lines {
        for span in &line.spans {
            assert_eq!(span.style.fg, Some(system_message_color()));
        }

...
fn session_scan_limit() -> usize {
    std::env::var("ALPHACODE_SESSION_PICKER_MAX_SESSIONS")
        .ok()
...
/// for users who only ever want alphacode's own sessions, so `[display]
/// external_sessions = false` (or `ALPHACODE_EXTERNAL_SESSIONS=0`) opts out
...
fn include_old_saved_sessions_on_initial_load() -> bool {
    std::env::var("ALPHACODE_SESSION_PICKER_INCLUDE_OLD_SAVED")
        .ok()
...
struct GroupedSessionListDiskCache {
    ...
    #[serde(default = "crate::alphacode_tui::tui::session_picker::loading::default_true")]
    external_sessions: bool,
...
fn session_list_disk_cache_path() -> Result<PathBuf> {
    Ok(storage::alphacode_dir()?.join("cache/session-picker-list-v1.json"))
}
...
pub fn load_cached_sessions_grouped() -> Option<(Vec<ServerGroup>, Vec<SessionInfo>)> {
    let sessions_dir = storage::alphacode_dir().ok()?.join("sessions");
...
/// Parse a single alphacode session snapshot (+ journal) into a [`SessionInfo`],
...
fn parse_alphacode_session_info(
    sessions_dir: &Path,
    stem: &str,
    ...
    Some(SessionInfo {
        ...
        server_icon: None,
        source,
        resume_target: ResumeTarget::AlphacodeSession {
...
pub fn load_sessions() -> Result<Vec<SessionInfo>> {
    ...
    let (mut sessions, external_sessions) = std::thread::scope(|scope| {
        ...
        // alphacode session parsing below.
        let external_handle =
        ...
        // default list appear to jump from a handful of Alphacode rows straight to old
        ...
        'fill: while start < candidates.len() {
            ...
            let parsed = parallel_map(batch, move |stem| {
                parse_alphacode_session_info(sessions_dir_ref, &stem, catchup_ref)
            });

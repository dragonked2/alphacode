...
mod cache_support;
use crate::alphacode_tui::message::{
...
/// Title the `/theme` command attaches to its listing message. The renderer
/// matches it to draw truecolor preview bars; the command module reads this
/// same constant so the two never drift apart.
pub(crate) const THEME_PREVIEW_TITLE: &str = "Themes";
...
/// Deserialized shape of the `/theme` listing payload (see
/// `commands_theme::ThemePreviewRow` for the emitting side).
#[derive(serde::Deserialize)]
struct ThemePreviewPayload {
    active: Option<String>,
    rows: Vec<ThemePreviewRow>,
}

#[derive(serde::Deserialize)]
struct ThemePreviewRow {
    id: String,
    dark: bool,
    description: String,
...
/// Render the `/theme` listing with a truecolor preview bar per theme.
///
/// The command pushes the listing as JSON with `title == THEME_PREVIEW_TITLE`;
/// regular system messages are forced to the system color, so this is the only
/// place real theme colors can reach the screen. Falls back to a plain-text
/// table (like the pre-preview listing) when the payload cannot be parsed,
/// e.g. a persisted message that predates the payload format.
fn render_theme_preview(msg: &DisplayMessage, width: u16) -> Vec<Line<'static>> {
    let centered = markdown::center_code_blocks();
    let wrap_width = centered_wrap_width(width.saturating_sub(4), centered, 96);

    let Ok(payload) = serde_json::from_str::<ThemePreviewPayload>(&msg.content) else {
        // Unparseable (e.g. a persisted message from before the payload
        // format): degrade to the plaintext table at the system color, exactly
        // like any other system message, so the raw JSON never shows.
        let mut lines = render_plaintext_lines(&msg.content, wrap_width);
    ...
    for row in &payload.rows {
        let marker = if payload.active.as_deref() == Some(row.id.as_str()) {
            "●"
        } else {
            " "
        };
        let mode = if row.dark { "dark" } else { "light" };

        // One solid bar per theme: two-cell blocks tinted with each swatch
        // color, so the terminal's own palette is never involved.
        let mut swatch_spans = Vec::new();
        for (r, g, b) in &row.swatch {
            swatch_spans.push(Span::styled(
                "██",
                Style::default().fg(Color::Rgb(*r, *g, *b)),
            ));
        }
        let bar: Line<'static> = Line::from(swatch_spans);

        let label = format!(
            "{marker} {:<id_width$}  {mode:<5}  ",
            row.id,
            id_width = id_width
        );
        let label_style = Style::default().fg(system_message_color());
        let mut spans = vec![Span::styled(label, label_style)];
        spans.extend(bar.spans);
        spans.push(Span::styled(format!("  {}", row.description), label_style));
    ...
    lines.push(Line::from(""));
    match payload.active.as_deref() {
        Some(id) => lines.push(Line::from(Span::styled(
            format!("Active: {id}. `/theme off` restores the built-in palette."),
            Style::default().fg(system_message_color()),
        ))),
        None => lines.push(Line::from(Span::styled(
            "Active: alphacode's built-in palette. Per-role tweaks live in `/colors`.",
            Style::default().fg(system_message_color()),
        ))),
    }
...
pub(crate) fn render_system_message(
    ...
    if let Some(title) = msg.title.as_deref() {
        ...
        if title == THEME_PREVIEW_TITLE {
            return render_theme_preview(msg, width);
        }
    }
...
pub(crate) fn render_todos_message(
    ...
    const MAX_INLINE_TODOS: usize = 8;
    if todos.is_empty() {
        ...
        if has_groups {
            ...
            for (group, items) in &groups {
                if shown_total >= MAX_INLINE_TODOS {
                    break;
                }
                ...
                let remaining = MAX_INLINE_TODOS.saturating_sub(shown_total);
                for todo in items.iter().take(remaining) {
                    lines.push(render_todo_card_item_line(todo, base_indent, inner_width));
                }
                shown_total += items.len().min(remaining);
            }
            let hidden = todos.len().saturating_sub(shown_total);
            if hidden > 0 {
                lines.push(todo_card_line(
                    vec![Span::styled(
                        format!("  … +{} more tasks", hidden),
                        Style::default().fg(dim_color()),
                    )],
                    base_indent,
                    inner_width,
            ...
            let shown = todos.len().min(MAX_INLINE_TODOS);
            for todo in &todos[..shown] {
                lines.push(render_todo_card_item_line(todo, base_indent, inner_width));
            }
            let hidden = todos.len().saturating_sub(shown);
            if hidden > 0 {
                lines.push(todo_card_line(
                    vec![Span::styled(
                        format!("  … +{} more tasks", hidden),
...
fn parse_connection_retry_message(content: &str) -> Option<(String, String, Option<String>)> {
    // Accept both legacy "⚡ Connection lost - retrying (attempt ..." and the
    // canonical "Connection lost — retrying (attempt ..." (em-dash, no emoji).
    let rest = content
        .strip_prefix("⚡ Connection lost - retrying (attempt ")
        .or_else(|| content.strip_prefix("Connection lost — retrying (attempt "))
        .or_else(|| content.strip_prefix("Connection lost - retrying (attempt "))?;
    let (attempt_and_elapsed, detail) = rest
        .split_once(") — ")
...
fn render_compact_agent_notification(
    ...
    let collapsible = crate::alphacode_tui_messages::parse_collapsible_swarm_content(content);
...
pub(crate) fn render_swarm_message(
    ...
    {
        return crate::alphacode_tui::tui::info_widget::swarm_gallery::render_swarm_chat_card_lines(
            &[member],
            width as usize,
        );
    }
    ...
    let collapsible = crate::alphacode_tui_messages::parse_collapsible_swarm_content(&msg.content);
    ...
    match action {
        "suggest" => {
            ...
            push_compact_discovery_kv(
                &mut content,
                "review",
                "Alphacode maintainers only; not approval or availability",
                block_width,
                label_style,
                muted_style,
                MAX_DISCOVERY_DETAIL_LINES,
...
pub(crate) fn render_tool_message(
        ...
        && let Some(parsed) = parse_todo_tool_output(&msg.content)
    {
        ...
        .to_string();
        crate::alphacode_tui::tui::ui_todo_suppress::note_todo_card_visible();
    ...
    let block_width = if centered {
        super::centered_content_block_width(width, super::MAX_CENTERED_CONTENT_WIDTH)
    } else {
        width as usize
    };
    ...
    // Show short successful bash outputs inline so users see realtime results
    // (e.g. `count`, `print(...)`) without opening anything. Long outputs stay
    // collapsed to keep the transcript scannable; errors already render via
    // the error summary path above.
    if tools_ui::canonical_tool_name(&tc.name) == "bash"
        && !is_error
        && !msg.content.trim().is_empty()
    {
        let trimmed = msg.content.trim();
        // Skip progress scaffolding / empty markers.
        let body = trimmed;
        let line_count = body.lines().count();
        if body.chars().count() <= 800 && line_count <= 12 {
            for line in body.lines().take(12) {
                let short = line
                    .chars()
                    .take(row_width.saturating_sub(6))
                    .collect::<String>();
                lines.push(Line::from(vec![
                    Span::raw("    "),
                    Span::styled(short, Style::default().fg(rgb(160, 170, 185))),
                ]));
            }
        }
    }

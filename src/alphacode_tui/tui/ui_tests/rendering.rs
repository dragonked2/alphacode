...
fn test_render_swarm_message_centered_mode_keeps_file_activity_preview_centered_when_diff_wraps() {
    ...
    let msg = DisplayMessage::swarm(
        "File activity · rose",
        "`…/alphacode/src/server/comm_sync.rs`
...
fn test_render_memory_tiles_allows_boxes_below_other_boxes() {
    let tiles = group_into_tiles(vec![
        ...
        (
            "preference".to_string(),
            "User prefers quick verification that alphacode is up-to-date.".to_string(),
        ),
        ("fact".to_string(), "Jeremy".to_string()),
        (
            "entity".to_string(),
...
fn test_light_theme_adapted_frame_has_readable_contrast() {
    ...
    terminal
        .draw(|frame| {
            crate::alphacode_tui::tui::ui::draw(frame, &state);
            crate::alphacode_tui_style::theme_mode::adapt_buffer(
                frame.buffer_mut(),
                crate::alphacode_tui_style::ThemeMode::Light,
            );
        })
        .expect("draw");
...
///
/// The unit tests in `alphacode-tui-style` cover the substitution in isolation, but
...
fn test_configured_palette_recolors_a_real_rendered_frame() {
    ...
    impl Drop for Restore {
        fn drop(&mut self) {
            crate::alphacode_tui_style::set_palette(crate::alphacode_tui_style::Palette::default());
        }
    }
    ...
    let mut palette = crate::alphacode_tui_style::Palette::default();
    palette.set(crate::alphacode_tui_style::Role::User, (250, 40, 200));

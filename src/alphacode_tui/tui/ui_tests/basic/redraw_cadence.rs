...
fn a_status_notice_does_not_force_animation_cadence() {
    ...
    let quiet = crate::alphacode_tui::tui::redraw_interval_with_policy(&static_chrome_state(None), &policy);
    let with_notice = crate::alphacode_tui::tui::redraw_interval_with_policy(
...
fn streaming_output_keeps_the_fast_cadence_even_with_a_notice() {
    ...
    assert_eq!(
        crate::alphacode_tui::tui::redraw_interval_with_policy(&state, &policy),
        fast_interval,
...
fn a_status_notice_still_requires_periodic_frames() {
    assert!(
        crate::alphacode_tui::tui::periodic_redraw_required(&static_chrome_state(Some("Swarm plan synced"))),
...
fn an_idle_empty_session_still_animates_smoothly() {
    ...
    let interval = crate::alphacode_tui::tui::redraw_interval_with_policy(&idle, &policy);
    assert!(
...
fn active_typing_backs_the_decorative_animation_off() {
    ...
    let interval = crate::alphacode_tui::tui::redraw_interval_with_policy(&typing, &policy);
    assert!(
        interval > fast,
...
fn a_paused_draft_lets_the_animation_recover() {
    ...
    let interval = crate::alphacode_tui::tui::redraw_interval_with_policy(&paused, &policy);
    assert!(
        interval < crate::alphacode_tui::tui::REDRAW_IDLE && interval <= Duration::from_millis(40),
...
fn a_lower_configured_animation_rate_is_respected() {
    ...
    assert_eq!(
        crate::alphacode_tui::tui::redraw_interval_with_policy(&idle, &policy),

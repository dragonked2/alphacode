...
impl IdleAnimationEnvGuard {
    fn enable() -> Self {
        ...
        crate::alphacode_core::env::set_var("ALPHACODE_IDLE_ANIMATION", "1");
...
impl Drop for IdleAnimationEnvGuard {
    fn drop(&mut self) {
        crate::alphacode_core::env::remove_var("ALPHACODE_IDLE_ANIMATION");
        crate::config::invalidate_config_cache();
    }
}
...
fn draw_publishes_the_animated_rows_only_when_the_animation_rendered() {
    ...
    let area = crate::alphacode_tui::tui::ui::last_idle_animation_area()
    ...
    assert_eq!(
        crate::alphacode_tui::tui::ui::last_idle_animation_area(),
        None,
...
fn partial_repaint_matches_a_full_frame_at_the_same_animation_time() {
    ...
    let area = crate::alphacode_tui::tui::ui::last_idle_animation_area().expect("animated rectangle");
    ...
    crate::alphacode_tui::tui::ui::render_idle_animation_into(&mut patched, area, 1.5);
...
fn idle_animation_is_excluded_from_the_full_frame_redraw_signal() {
    ...
    assert!(
        crate::alphacode_tui::tui::periodic_redraw_required(&idle),
        "the animation must keep the redraw loop ticking"
    );
    assert!(
        !crate::alphacode_tui::tui::periodic_redraw_required_excluding_idle_animation(&idle),
    ...
    assert!(
        crate::alphacode_tui::tui::periodic_redraw_required_excluding_idle_animation(&busy),
...
fn partial_repaint_does_no_layout_or_transcript_work() {
    ...
    let area = crate::alphacode_tui::tui::ui::last_idle_animation_area().expect("animated rectangle");
    let layout_before = crate::alphacode_tui::tui::ui::last_layout_snapshot().expect("layout snapshot");
    let status_before = crate::alphacode_tui::tui::ui::last_status_area().expect("status area");
    let mut buffer = terminal.backend().buffer().clone();

    crate::alphacode_tui::tui::ui::render_idle_animation_into(&mut buffer, area, 1.5);
    ...
    assert_eq!(
        crate::alphacode_tui::tui::ui::last_status_area(),
        Some(status_before),
        "a partial repaint must not re-run status layout"
    );
    let layout_after = crate::alphacode_tui::tui::ui::last_layout_snapshot().expect("layout snapshot");
    assert_eq!(
...
fn published_rectangle_round_trips_through_the_shared_slot() {
    ...
    for area in [
        ...
        crate::alphacode_tui::tui::ui::record_idle_animation_area(Some(area));
        assert_eq!(crate::alphacode_tui::tui::ui::last_idle_animation_area(), Some(area));
    }

    crate::alphacode_tui::tui::ui::record_idle_animation_area(None);
    assert_eq!(crate::alphacode_tui::tui::ui::last_idle_animation_area(), None);
}
...
fn a_full_screen_overlay_stops_the_decorative_animation_cadence() {
    ...
    let animation_interval = crate::alphacode_tui::tui::redraw_interval(&idle);
    assert!(
        animation_interval < crate::alphacode_tui::tui::REDRAW_IDLE,
    ...
    for (name, state) in overlays {
        ...
        let interval = crate::alphacode_tui::tui::redraw_interval(&state);
        assert!(
            interval >= crate::alphacode_tui::tui::REDRAW_IDLE,
            "with the {name} overlay up there is no visible animation, so the \
             loop must not run at animation cadence; got {interval:?}"
        );
        assert!(
            !crate::alphacode_tui::tui::periodic_redraw_required(&state),
...
fn animation_cadence_implies_the_renderer_published_animated_rows() {
    ...
    for (name, state) in screens {
        for (width, height) in [(160u16, 48u16), (100, 40), (80, 24), (60, 12), (40, 8)] {
            ...
            let published = crate::alphacode_tui::tui::ui::last_idle_animation_area().is_some();
            let paces_animation = crate::alphacode_tui::tui::redraw_interval(&state) < crate::alphacode_tui::tui::REDRAW_IDLE;
            assert!(
...
fn the_animation_can_still_start_from_a_state_with_nothing_published() {
    ...
    let idle = idle_animation_state(1.0);
    assert!(
        crate::alphacode_tui::tui::idle_donut_active(&idle),
    ...
    let _ = render_full(&idle, 160, 48);
    assert!(
        crate::alphacode_tui::tui::ui::last_idle_animation_area().is_some(),
...
fn copying_only_the_animated_rows_matches_cloning_the_whole_frame() {
    ...
    for (width, height) in [(160u16, 48u16), (100, 40), (80, 24)] {
        ...
        let Some(area) = crate::alphacode_tui::tui::ui::last_idle_animation_area() else {
            continue;
        };
        ...
        for step in 0..8 {
            ...
            if seeded {
                ...
                crate::alphacode_tui::tui::app::idle_animation_repaint::copy_cells_in(
                    &base,
                    &mut optimized,
                    area,
                );
            ...
            crate::alphacode_tui::tui::ui::render_idle_animation_into(&mut optimized, area, elapsed);

            naive = base.clone();
            crate::alphacode_tui::tui::ui::render_idle_animation_into(&mut naive, area, elapsed);
        ...
        crate::alphacode_tui::tui::ui::render_idle_animation_into(
            &mut edge,
...
fn copy_cells_in_copies_exactly_the_rectangle_and_nothing_else() {
    ...
    crate::alphacode_tui::tui::app::idle_animation_repaint::copy_cells_in(&src, &mut dst, area);

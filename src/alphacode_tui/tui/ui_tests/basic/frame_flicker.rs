...
fn test_redraw_interval_uses_low_frequency_during_remote_startup_phase() {
    let idle = TestState {
        ...
        time_since_activity: Some(crate::alphacode_tui::tui::REDRAW_DEEP_IDLE_AFTER + Duration::from_secs(1)),
        ..Default::default()
    };
    ...
    let idle_interval = crate::alphacode_tui::tui::redraw_interval(&idle);
    let startup_interval = crate::alphacode_tui::tui::redraw_interval(&startup);

    assert_eq!(idle_interval, crate::alphacode_tui::tui::REDRAW_DEEP_IDLE);
    assert_eq!(startup_interval, crate::alphacode_tui::tui::REDRAW_REMOTE_STARTUP);
}
...
fn test_active_overscroll_keeps_redrawing_at_deep_idle() {
    ...
    let deep_idle = crate::alphacode_tui::tui::REDRAW_DEEP_IDLE_AFTER + Duration::from_secs(1);
    ...
    assert!(
        crate::alphacode_tui::tui::periodic_redraw_required(&overscrolling),
    ...
    assert_eq!(
        crate::alphacode_tui::tui::redraw_interval(&idle),
        crate::alphacode_tui::tui::REDRAW_DEEP_IDLE
    );
    assert_ne!(
        crate::alphacode_tui::tui::redraw_interval(&overscrolling),
        crate::alphacode_tui::tui::REDRAW_DEEP_IDLE,
...
fn test_cold_cache_warning_keeps_redrawing_at_deep_idle() {
    ...
    let cold = TestState {
        display_messages: vec![DisplayMessage::system("seed".to_string())],
        time_since_activity: Some(deep_idle),
        cache_ttl_status: Some(crate::alphacode_tui::tui::CacheTtlInfo {
            remaining_secs: 0,
            ttl_secs: 300,
            is_cold: true,
            cold_for_secs: 90,
    ...
    assert!(
        crate::alphacode_tui::tui::periodic_redraw_required(&cold),
        "a cold prompt cache must keep driving redraws so the warning appears at deep idle"
    );
    assert_ne!(
        crate::alphacode_tui::tui::redraw_interval(&cold),
        crate::alphacode_tui::tui::REDRAW_DEEP_IDLE,
    ...
    assert!(
        crate::alphacode_tui::tui::periodic_redraw_required(&warm_countdown),
    ...
    assert!(
        crate::alphacode_tui::tui::periodic_redraw_required(&warm_countdown_1h),
    ...
    assert!(
        !crate::alphacode_tui::tui::periodic_redraw_required(&warm),
...
fn test_active_swarm_spinner_keeps_redrawing_at_deep_idle() {
    ...
    assert!(
        !crate::alphacode_tui::tui::periodic_redraw_required(&quiet),
        "a quiet deep-idle session should not require periodic redraws"
    );
    ...
    for status in ["running", "streaming", "thinking"] {
        ...
        assert!(
            crate::alphacode_tui::tui::periodic_redraw_required(&animating),
            "an {status} swarm agent must keep driving redraws at deep idle"
        );
        assert_eq!(
            crate::alphacode_tui::tui::redraw_interval(&animating),
            crate::alphacode_tui::tui::REDRAW_SWARM_SPINNER,
    ...
    for status in ["completed", "failed", "stopped", "ready", "blocked"] {
        ...
        assert!(
            !crate::alphacode_tui::tui::periodic_redraw_required(&settled),
            "a {status} swarm agent renders a static glyph and should stay deep-idle"
        );
        assert_eq!(
            crate::alphacode_tui::tui::redraw_interval(&settled),
            crate::alphacode_tui::tui::REDRAW_DEEP_IDLE,
...
fn test_inline_ui_gap_height_only_when_inline_ui_visible() {
    ...
    let inline_interactive_state = crate::alphacode_tui::tui::InlineInteractiveState {
        kind: crate::alphacode_tui::tui::PickerKind::Model,
        entries: vec![],
        filtered: vec![],
        selected: 0,
        column: 0,
    ...
    let state_with_inline_view = TestState {
        inline_view_state: Some(crate::alphacode_tui::tui::InlineViewState {

...
use crate::alphacode_tui::tui::ui::clear_flicker_frame_history_for_tests;
...
fn fact_test_state(input: String, scheduled: bool) -> TestState {
    ...
    TestState {
        ...
        working_dir: Some(format!("{home}/alphacode")),
        info_widget_data,
...
fn right_fact_stack_uses_transcript_status_notification_and_input_rows_in_order() {
    ...
    let status = crate::alphacode_tui::tui::ui::last_status_area().expect("status area");
...
fn right_fact_stack_uses_neutral_gray_except_for_context_usage() {
    ...
    let _lock = viewport_snapshot_test_lock();
    clear_flicker_frame_history_for_tests();
    // This test asserts exact RGB values on the rendered buffer; pin truecolor
    // so it is not quantized to the 256-color palette on hosts without
    // COLORTERM=truecolor (mirrors palette_topology.rs).
    ...
    for needle in ["OpenAI · OAuth", "GPT-5.6 Sol high", "~/alphacode"] {
...
fn right_fact_stack_leaves_fully_used_input_rows_untouched_and_moves_up() {
    ...
    assert!(input_rows.iter().all(|row| !row.contains("~/alphacode")));
...
fn swarm_strip_full_draw_writes_chips_row_above_status_line() {
    ...
    crate::alphacode_tui::tui::info_widget::clear_widget_placements_for_tests();
    let state = TestState {
...
fn swarm_strip_stands_down_through_dock_blinks() {
    ...
    coordinator.role = Some("coordinator".to_string());
    let data = crate::alphacode_tui::tui::info_widget::InfoWidgetData {
        swarm_info: Some(crate::alphacode_tui::tui::info_widget::SwarmInfo {
    ...
    let wide_margins = crate::alphacode_tui::tui::info_widget::Margins {
    ...
    let covered_margins = crate::alphacode_tui::tui::info_widget::Margins {
    ...
    crate::alphacode_tui::tui::info_widget::note_widget_pass_skipped();
    assert!(
        !swarm_strip_stands_down_for_dock(),
...
fn swarm_dock_widget_full_render_writes_agent_rows_in_margin() {
    ...
    let mut terminal = Terminal::new(backend).expect("test terminal");
    let messages_area = Rect::new(0, 0, 120, 26);
    let margins = crate::alphacode_tui::tui::info_widget::Margins {
        right_widths: vec![44; 26],
        left_widths: Vec::new(),
        centered: false,
        ..Default::default()
    ...
    terminal
        .draw(|frame| {
            ...
            dock_rect = placements
                ...
                .map(|p| p.rect);
            crate::alphacode_tui::tui::info_widget::render_all(frame, &placements, &data);
        })

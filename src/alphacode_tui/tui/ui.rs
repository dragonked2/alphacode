...
pub(crate) use messages::{
    SWARM_AGENT_SNAPSHOT_TITLE, THEME_PREVIEW_TITLE, compact_swarm_await_summary,
    encode_swarm_agent_snapshot, render_assistant_message, render_background_task_message,
...
#[cfg(test)]
use theme_support::{success_color, warning_color};

pub(crate) use crate::alphacode_tui_markdown::{CopyTargetKind, RawCopyTarget};
pub(crate) use crate::alphacode_tui_messages::{
...
/// alphacode-tui's intermittent layout failures.
///
...
pub(crate) fn draw_onboarding_welcome_for_tests(
    ...
    app: &dyn crate::alphacode_tui::tui::TuiState,
...
impl CopyViewportSnapshot {
    ...
    fn inline_image_id_for_label_line(&self, abs_line: usize) -> Option<u64> {
        ...
        prepared
            ...
            .iter()
            .find(|region| {
                region.render == crate::alphacode_tui_messages::ImageRegionRender::Fit
...
fn copy_snapshot_slot_mut(
    ...
    match pane {
        crate::alphacode_tui::tui::CopySelectionPane::Chat => &mut snapshots.chat,
        crate::alphacode_tui::tui::CopySelectionPane::SidePane => &mut snapshots.side,
        crate::alphacode_tui::tui::CopySelectionPane::Input => &mut snapshots.input,
    }
}
...
fn copy_snapshot_for_pane(pane: crate::alphacode_tui::tui::CopySelectionPane) -> Option<CopyViewportSnapshot> {
    #[cfg(test)]
    {
        TEST_COPY_VIEWPORT.with(|snapshots| {
            ...
            match pane {
                crate::alphacode_tui::tui::CopySelectionPane::Chat => snapshots.chat,
                crate::alphacode_tui::tui::CopySelectionPane::SidePane => snapshots.side,
                crate::alphacode_tui::tui::CopySelectionPane::Input => snapshots.input,
            }
        })
    }
    #[cfg(not(test))]
...
pub(crate) fn copy_point_from_screen(
    column: u16,
    row: u16,
) -> Option<crate::alphacode_tui::tui::CopySelectionPoint> {
    #[cfg(test)]
    {
        TEST_COPY_VIEWPORT.with(|snapshots| {
            let snapshots = snapshots.borrow().clone();
...
pub(crate) fn copy_pane_vertical_edge_point(
    ...
    column: u16,
    row: u16,
) -> Option<(crate::alphacode_tui::tui::CopySelectionPoint, bool)> {
    ...
    if pane == crate::alphacode_tui::tui::CopySelectionPane::Input {
        return None;
    }
...
pub(crate) fn copy_pane_drag_point(
    ...
    if row >= last_row {
        ...
        return Some(crate::alphacode_tui::tui::CopySelectionPoint {
            pane,
    ...
    copy_point_from_snapshot(&snapshot, clamped_col, row.clamp(area.y, last_row)).or(Some(
        crate::alphacode_tui::tui::CopySelectionPoint {
            pane,
            abs_line: last_visible_line,
...
pub(crate) fn copy_viewport_point_from_screen(
    ...
    (point.pane == crate::alphacode_tui::tui::CopySelectionPane::Chat).then_some(point)
}
...
pub(crate) fn copy_viewport_line_text(abs_line: usize) -> Option<String> {
    copy_pane_line_text(crate::alphacode_tui::tui::CopySelectionPane::Chat, abs_line)
}
...
pub(crate) fn copy_viewport_line_count() -> Option<usize> {
    copy_pane_line_count(crate::alphacode_tui::tui::CopySelectionPane::Chat)
}

pub(crate) fn side_pane_line_count() -> Option<usize> {
    copy_pane_line_count(crate::alphacode_tui::tui::CopySelectionPane::SidePane)
}

pub(crate) fn input_pane_line_count() -> Option<usize> {
    copy_pane_line_count(crate::alphacode_tui::tui::CopySelectionPane::Input)
}

pub(crate) fn copy_viewport_visible_range() -> Option<(usize, usize)> {
    let snapshot = copy_snapshot_for_pane(crate::alphacode_tui::tui::CopySelectionPane::Chat)?;
    Some((snapshot.scroll, snapshot.visible_end))
}

#[cfg(test)]
pub(crate) fn side_pane_visible_range() -> Option<(usize, usize)> {
    let snapshot = copy_snapshot_for_pane(crate::alphacode_tui::tui::CopySelectionPane::SidePane)?;
    Some((snapshot.scroll, snapshot.visible_end))
}
...
pub(crate) fn copy_pane_first_visible_point(
    ...
    Some(crate::alphacode_tui::tui::CopySelectionPoint {
        pane,
...
pub(crate) fn link_target_from_screen(column: u16, row: u16) -> Option<String> {
    ...
    if point.pane == crate::alphacode_tui::tui::CopySelectionPane::Input {
        return None;
    }
...
pub(crate) fn swarm_expand_target_from_screen(column: u16, row: u16) -> Option<usize> {
    let point = copy_point_from_screen(column, row)?;
    if point.pane != crate::alphacode_tui::tui::CopySelectionPane::Chat {
        return None;
    }
    let snapshot = copy_snapshot_for_pane(point.pane)?;
...
pub fn draw(frame: &mut Frame, app: &dyn TuiState) {
    ...
    crate::alphacode_tui_style::palette::adapt_buffer_for_palette(frame.buffer_mut());
    ...
    crate::alphacode_tui::tui::mermaid::render_pending_terminal_image_cleanup(frame.buffer_mut());
}
fn draw_inner(frame: &mut Frame, app: &dyn TuiState) {
    ...
    // Reset the todo-card-visible flag so the side-panel Todos widget can
    // detect whether a todo card is rendered in the chat viewport this frame.
    super::ui_todo_suppress::reset_frame();
        ...
        && (app.swarm_panel_focused() || !super::info_widget::swarm_strip_stands_down_for_dock())
    {
        ...
        if chat_area.width >= 24 {
            let focus_key = crate::alphacode_tui::tui::keybind::swarm_panel_focus_key_label();
    ...
    let onboarding_takes_over = onboarding_welcome
        && !matches!(
            ...
            crate::alphacode_tui::tui::OnboardingWelcomeKind::Suggestions
        );
    ...
    crate::alphacode_tui::tui::reset_pinned_diagram_debug_snapshot();
    ...
    crate::alphacode_tui::tui::clear_side_panel_debug_snapshot();
    ...
    if let Some(capture) = debug_capture {
        ...
        capture.mermaid = crate::alphacode_tui::tui::mermaid::debug_stats_json();
        capture.side_panel = crate::alphacode_tui::tui::side_panel_debug_json();
        capture.markdown = crate::alphacode_tui::tui::markdown::debug_stats_json();

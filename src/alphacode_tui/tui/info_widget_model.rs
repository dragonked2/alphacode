...
pub(super) fn render_model_widget(data: &InfoWidgetData, inner: Rect) -> Vec<Line<'static>> {
    ...
    let short_name = crate::alphacode_tui::tui::session_facts::pretty_model(model);
    let max_len = inner.width.saturating_sub(2) as usize;

    // Model name with contextual icon based on provider
    let (model_icon, model_color) = match data.provider_name.as_deref() {
        ...
        .filter(|s| !s.is_empty())
    {
            ...
            .filter(|b| !b.is_empty())
        {
            dir_spans.push(Span::styled(
                format!("  \u{2442} {}", truncate_chars(branch, 24)),
                Style::default().fg(rgb(108, 198, 118)),
            ));
        }
    ...
    if data.auth_method != AuthMethod::Unknown {
        let (icon, label, color) = match data.auth_method {
            ...
            AuthMethod::Unknown => ("❓", "Unknown", rgb(160, 160, 170)),
        };
    ...
    if let Some(tps) = data.tokens_per_second
        ...
        && tps > 0.1
    {
        let tps_color = if tps >= 100.0 {
            rgb(110, 210, 130) // green for fast
        } else if tps >= 30.0 {
            rgb(180, 200, 130) // yellow for medium
        } else {
            rgb(200, 160, 130) // orange for slow
        };
        lines.push(Line::from(vec![
            Span::styled("⚡ ", Style::default().fg(rgb(140, 180, 255))),
...
pub(super) fn render_model_info(data: &InfoWidgetData, inner: Rect) -> Vec<Line<'static>> {
    ...
    let model_display = if short_name.chars().count() > max_len {
        format!(
            "{}...",
    ...
    let mut spans = vec![
        ...
        Span::styled(
            model_display,
            Style::default().fg(rgb(220, 170, 255)).bold(),
        ),
    ];
    ...
    if has_provider || has_auth {
            ...
            .filter(|s| !s.is_empty())
        {
            detail_spans.push(Span::styled(
                provider.to_lowercase(),
                Style::default().fg(rgb(100, 200, 180)),
            ));
        }
        ...
        if has_auth {
            ...
            if !detail_spans.is_empty() {
                detail_spans.push(Span::styled(" · ", Style::default().fg(rgb(70, 70, 80))));
            }
            detail_spans.push(Span::styled(
                format!("{} {}", icon, label),
...
fn short_reasoning_effort(effort: &str) -> Option<&str> {
    ...
    if effort.is_empty() || effort == "low" || effort == "none" {
        return None;
    }
    Some(match effort {
        "max" => "max",
...
fn home_relative_dir(path: &str) -> String {
    crate::alphacode_tui::tui::session_facts::dir_label(path)
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::alphacode_tui::tui::info_widget::InfoWidgetData;
    ...
    fn data() -> InfoWidgetData {
        InfoWidgetData {
            ...
            provider_name: None,
            auth_method: crate::alphacode_tui::tui::info_widget::AuthMethod::Unknown,

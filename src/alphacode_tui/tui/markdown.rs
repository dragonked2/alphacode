pub use crate::alphacode_tui_markdown::{
...
) -> crate::alphacode_tui_markdown::MarkdownSpacingMode {
    match mode {
        crate::config::MarkdownSpacingMode::Compact => {
            crate::alphacode_tui_markdown::MarkdownSpacingMode::Compact
        }
        crate::config::MarkdownSpacingMode::Document => {
            crate::alphacode_tui_markdown::MarkdownSpacingMode::Document
        }
    }
}
...
pub fn install_alphacode_markdown_hooks() {
    crate::alphacode_tui_markdown::set_latex_log_hook(|error| {
        crate::logging::warn(&format!(
            "LaTeX image rendering fell back to Unicode: {error}"
        ));
    });
    crate::alphacode_tui_markdown::set_config_snapshot_hook(|| {
        let cfg = crate::config::config();
        crate::alphacode_tui_markdown::MarkdownConfigSnapshot {
    ...
    crate::alphacode_tui_markdown::set_memory_snapshot_hook(|| {
        let snapshot = crate::process_memory::snapshot_with_source("client:markdown:memory");
        crate::alphacode_tui_markdown::ProcessMemorySnapshot {
...
pub fn set_diagram_mode_override(mode: Option<crate::config::DiagramDisplayMode>) {
    crate::alphacode_tui_markdown::set_diagram_mode_override(mode.map(to_markdown_diagram_mode));
}

pub fn get_diagram_mode_override() -> Option<crate::config::DiagramDisplayMode> {
    crate::alphacode_tui_markdown::get_diagram_mode_override().map(from_markdown_diagram_mode)
}
...
pub fn with_diagram_mode_scope<T>(
    ...
    crate::alphacode_tui_markdown::with_diagram_mode_scope(to_markdown_diagram_mode(mode), f)
}

pub fn with_deferred_mermaid_render_context<T>(f: impl FnOnce() -> T) -> T {
    crate::alphacode_tui_markdown::with_deferred_mermaid_render_context(f)
}

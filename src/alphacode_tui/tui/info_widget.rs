...
pub use crate::alphacode_tui::memory_types::{
...
pub use crate::alphacode_tui_mermaid::DiagramInfo;
...
impl InfoWidgetData {
    ...
    pub fn has_data_for(&self, kind: WidgetKind) -> bool {
        ...
        match kind {
            ...
            WidgetKind::Todos => {
                // Suppress the side-panel Todos widget when a todo card is
                // already rendered inline in the chat viewport. This eliminates
                // the duplicate todo display (header "1/6" + right panel
                // "Todos 0/11") that the user sees.
                !self.todos.is_empty() && !super::ui_todo_suppress::is_todo_card_visible()
            }
            WidgetKind::ContextUsage => {
...
pub(crate) fn calculate_widget_height(
    ...
    let content_height = match kind {
        ...
        WidgetKind::Todos => {
            ...
            // Header (with inline pip meter) + up to 5 items + progress bar (2 lines)
            let items = data.todos.len().min(5) as u16;
            let extra_lines = if data.todos.len() > 5 { 1 } else { 0 };
            // Progress bar needs at least 5 total lines to render
            let progress_bar_lines = if max_height >= 5 { 2 } else { 0 };
...
fn render_single_widget(frame: &mut Frame, placement: &WidgetPlacement, data: &InfoWidgetData) {
    ...
    // Unified border style using the refreshed palette for consistency
    // across all widget frames. Uses a subtle, professional border that
    // blends with the dark theme.
    let border_color = rgb(48, 54, 76);
    let title_color = rgb(88, 95, 118);
    ...
    match placement.kind {
        ...
        WidgetKind::Todos => {
            block = block.title(Span::styled(
                " \u{2714} Todos ",
                Style::default().fg(rgb(108, 230, 158)),
            ));
        }
        WidgetKind::ModelInfo => {
            block = block.title(Span::styled(
        ...
        WidgetKind::ContextUsage => {
            block = block.title(Span::styled(
                " \u{2637} Context ",
                Style::default().fg(rgb(148, 188, 255)),
            ));
        }
        WidgetKind::UsageLimits => {
            block = block.title(Span::styled(
                " \u{1f4b0} Usage ",
                Style::default().fg(rgb(255, 195, 88)),
            ));
        }
        WidgetKind::KvCache => {
            block = block.title(Span::styled(
                " \u{26a1} Cache ",
                Style::default().fg(rgb(255, 215, 78)),
            ));
        }
        WidgetKind::MemoryActivity => {
            block = block.title(Span::styled(
        ...
        WidgetKind::SwarmStatus => {
            block = block.title(Span::styled(
                " \u{1f916} Agents ",
                Style::default().fg(rgb(108, 215, 255)),
            ));
        }
        WidgetKind::Compaction => {
            block = block.title(Span::styled(
                " \u{1f504} Compact ",
                Style::default().fg(rgb(138, 235, 188)),
            ));
        }
        WidgetKind::BackgroundTasks => {
            block = block.title(Span::styled(
                " \u{23f3} Tasks ",
                Style::default().fg(rgb(138, 142, 165)),
            ));
        }
        WidgetKind::AmbientMode => {
            block = block.title(Span::styled(

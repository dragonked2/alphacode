...
//!
//! 1. `ALPHACODE_THEME=dark|light` env override (also accepts `auto`).
...
//! 4. Fallback: dark (alphacode's native palette).
//!
//! The result is stored in `crate::alphacode_tui_style::theme_mode` where the renderer
//! adapts colors for light backgrounds at frame time.

use crate::alphacode_tui_style::ThemeMode;
...
pub fn prewarm_theme_mode() {
    ...
    if let Ok(handle) = std::thread::Builder::new()
        .name("alphacode-theme-detect".to_string())
...
pub fn init_theme_mode() -> ThemeMode {
    ...
    crate::alphacode_tui_style::set_theme_mode(mode);
    init_palette();
    mode
}
...
/// Resolve the effective palette from `[display.preset]` and `[display.colors]`.
///
/// The preset supplies a complete, internally consistent base and individual
/// `colors` entries are layered on top, so a user can pick a theme and still
/// retune single roles without having to restate the other 38.
///
/// Problems are returned rather than logged so the caller chooses how to surface
/// them. This is the single definition of "the configured palette": `/colors`
/// reads it too, so what that command lists and scores is always what the TUI
/// actually renders.
pub fn configured_palette() -> (crate::alphacode_tui_style::Palette, Vec<String>) {
    let display = &crate::config::config().display;
    let mut problems = Vec::new();

    let preset = display.preset.trim();
    let base = if preset.is_empty() {
        crate::alphacode_tui_style::Palette::default()
    } else {
        match crate::alphacode_tui_style::preset_palette(preset) {
            Some(palette) => palette,
            None => {
                problems.push(format!(
                    "display.preset: unknown theme '{preset}' (run /theme to list presets)"
                ));
                crate::alphacode_tui_style::Palette::default()
            }
        }
    ...
    let (palette, errors) = base.with_pairs(
        display
            .colors
            .iter()
...
/// Install the user's configured color palette.
///
/// Invalid entries are logged and skipped rather than failing the palette, so
/// one typo can never leave the TUI unstyled. An unknown preset name is likewise
/// a warning that falls back to the built-in palette — a mistyped theme should
/// cost the user their colors, not their session. Safe to call repeatedly; the
/// TUI calls it again after `/colors` and `/theme` edits so changes apply
/// without a restart.
pub fn init_palette() {
    let (palette, problems) = configured_palette();
    for problem in problems {
        crate::logging::warn(&problem);
    }
    crate::alphacode_tui_style::set_palette(palette);
}

pub fn current_theme_label() -> &'static str {
    match crate::alphacode_tui_style::theme_mode() {
        ThemeMode::Dark => "dark",
...
fn resolve_configured_theme(query_terminal: bool) -> ThemeMode {
    let configured = std::env::var("ALPHACODE_THEME")
        .ok()

...
pub use crate::alphacode_tui_session_picker::{
...
pub enum PickerResult {
    ...
    /// to Alphacode. This is never emitted by ordinary Enter/resume behavior.
...
impl SessionPicker {
    ...
    #[cfg(test)]
    #[allow(dead_code)] // test-only helper kept for parity with sibling for_test accessors
    ...
    fn ensure_selected_preview_loading(&mut self) {
        ...
        let _ = std::thread::Builder::new()
            .name("alphacode-session-preview-loader".to_string())
            .spawn(move || {
    ...
    fn render_onboarding_band(&self, frame: &mut Frame, area: Rect) {
        ...
        let action_line = |label: &'static str, selected: bool| {
            let (cap_style, body_style) = if selected {
                ...
                (
                    Style::default().fg(rgb(55, 60, 75)),
                    Style::default().fg(rgb(160, 165, 180)).bg(rgb(55, 60, 75)),
                )
            };
            Line::from(vec![
    ...
    pub fn render(&mut self, frame: &mut Frame) {
        ...
        if has_search {
            ...
            let search_line = Line::from(vec![
                Span::styled(" 🔍 ", Style::default().fg(rgb(130, 224, 215))),
                Span::styled(
                    &self.search_query,
                    Style::default()
                        .fg(rgb(220, 230, 240))
                ...
                Span::styled(cursor_char, Style::default().fg(rgb(130, 224, 215))),
                if self.search_active {
                    Span::styled("  Esc to clear", Style::default().fg(rgb(80, 85, 100)))
                } else {
                    Span::styled("  / to edit", Style::default().fg(rgb(80, 85, 100)))
                },
            ]);
            let search_widget =
                Paragraph::new(search_line).style(Style::default().bg(rgb(22, 26, 38)));
        ...
        let body = vec![
            Line::from(Span::styled(
                format!(
                    ...
                    crate::alphacode_core::util::truncate_str(session_id, 12)
                ),
                Style::default()
                    .fg(Color::White)
                    .add_modifier(Modifier::BOLD),
            ...
            Line::from(""),
            Line::from(Span::styled(
                "Alphacode will prepare the transcript first, then ask Claude to exit.",

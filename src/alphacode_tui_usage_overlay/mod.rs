...
pub struct UsageOverlayItem {
    ...
    /// Highest utilization percentage across this source's limits (0.0-100.0),
    /// used to draw a quick-read meter in the source list. `None` hides it.
...
impl UsageOverlayItem {
    ...
    /// Attach the peak utilization percentage for the source list meter.
    pub fn with_usage_percent(mut self, percent: f32) -> Self {
        self.usage_percent = Some(percent);
        self
    }
}
...
impl UsageOverlay {
    ...
    pub fn from_progress(progress: &crate::alphacode_usage_types::ProviderUsageProgress) -> Self {
    ...
    pub fn from_provider_reports(
        reports: &[crate::alphacode_usage_types::ProviderUsage],
        refreshing: bool,
        completed: usize,
        total: usize,
        from_cache: bool,
        ...
        let mut overlay = Self::new(title, items, summary);
        overlay.refreshing = refreshing;
        overlay
    }
    ...
    pub fn render(&self, frame: &mut Frame) {
        ...
        let title = if self.refreshing {
            format!(" {} {} ", refresh_spinner(), self.title)
        } else {
            format!(" {} ", self.title)
        };
    ...
    fn render_item_list(&self, frame: &mut Frame, area: Rect) {
        ...
        for (visible_idx, item_idx) in self.filtered.iter().enumerate() {
            ...
            if let Some(percent) = item.usage_percent {
                let mut bar_line = vec![Span::styled("   ", subtitle_style)];
                bar_line.extend(usage_percent_bar_spans(
                    percent,
...
/// Time-based refresh spinner for the overlay title. Pure wall-clock so the
/// panel animates without needing a render tick passed in.
fn refresh_spinner() -> &'static str {
    const FRAMES: [&str; 4] = ["◐", "◓", "◑", "◒"];
...
/// Compact utilization meter for a source row: filled blocks colored by
/// headroom (green < 70%, amber < 90%, red at/over 90%), plus a percent label.
fn usage_percent_bar_spans(percent: f32, width: usize, selected: bool) -> Vec<Span<'static>> {
    let width = width.clamp(4, 22);
    ...
    vec![
        Span::styled("▰".repeat(filled), Style::default().fg(color).bg(bg)),
        Span::styled(
            "▱".repeat(empty),
            Style::default().fg(Color::Rgb(60, 66, 80)).bg(bg),
        ),
        Span::styled(
            format!(" {:.0}%", pct),
            Style::default().fg(color).bold().bg(bg),
        ),
    ]
}
...
fn provider_item(report: &crate::alphacode_usage_types::ProviderUsage) -> UsageOverlayItem {
    ...
    let max_percent = if report.hard_limit_reached {
        100.0
    } else {
        report
            .limits
    ...
    if max_percent > 0.0 {
        item.with_usage_percent(max_percent)
    } else {
        item
    }
}

fn provider_status(report: &crate::alphacode_usage_types::ProviderUsage) -> UsageOverlayStatus {
...
fn provider_subtitle(report: &crate::alphacode_usage_types::ProviderUsage) -> String {
...
fn provider_detail_lines(report: &crate::alphacode_usage_types::ProviderUsage) -> Vec<String> {
...
mod tests {
    ...
    #[test]
    fn provider_items_carry_peak_utilization_percent() {
        let report = crate::alphacode_usage_types::ProviderUsage {
            provider_name: "Claude".to_string(),
            limits: vec![
                crate::alphacode_usage_types::UsageLimit {
                    name: "5h".to_string(),
                    usage_percent: 34.0,
                    resets_at: None,
                },
        ...
        let item = overlay.selected_item().expect("selected item");
        assert_eq!(item.usage_percent, Some(82.0));
    }

    #[test]
    fn hard_limit_reports_full_meter() {
        let report = crate::alphacode_usage_types::ProviderUsage {
    ...
    #[test]
    fn usage_bar_spans_never_exceed_width() {
        for width in 0..=40usize {
            for pct in [0.0, 42.0, 71.0, 92.0, 100.0] {
                let spans = usage_percent_bar_spans(pct, width, false);
                let drawn: usize = spans
                    .iter()
                    .map(|span| span.content.as_ref().chars().count())
                    .sum();
                let budget = width.clamp(4, 22) + " 100%".len();
                assert!(
                    drawn <= budget,
                    "width {width} pct {pct} drew {drawn} > budget {budget}"
                );
            }
        }
    }
    ...
    fn provider_reports_build_searchable_overlay_items() {
        let overlay = UsageOverlay::from_provider_reports(
            &[crate::alphacode_usage_types::ProviderUsage {
                provider_name: "Claude".to_string(),
                limits: vec![crate::alphacode_usage_types::UsageLimit {
                    name: "5h".to_string(),
                    usage_percent: 92.0,

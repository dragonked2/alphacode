...
pub fn estimate_complexity(objective: &str, file_count: usize) -> TaskComplexity {
    ...
    // "update ... dependency" (e.g. "update docker dependency version") is a
    // mechanical edit: the wording varies, so match it structurally rather
    // than as a fixed phrase. Infrastructure named inside such a phrase
    // ("docker") is incidental and must not raise the estimate on its own.
    let mechanical_dep_update = lower.contains("update") && lower.contains("dependenc");

    let raises = COMPLEXITY_RAISERS
        .iter()
        .filter(|sig| {
            if mechanical_dep_update && **sig == "docker" {
                return false;
            }
            lower.contains(**sig)
        })
    ...
    let lowers = COMPLEXITY_LOWERERS
        ...
        .count()
        + usize::from(mechanical_dep_update);

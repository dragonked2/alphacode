...
use crate::alphacode_app_core::background;
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        match action.as_str() {
            ...
            "output" | "tail" => {
                ...
                let output = match manager.output(&task_id).await {
                    Some(text) => text,
                    None => {
                        // The on-disk output file is missing. Distinguish the
                        // common cases so the agent can recover without
                        // blocking on the user:
                        //   - Task is still running in this process: tell the
                        //     model to use `bg wait` first; the file only
                        //     exists once the task finishes.
                        //   - Task status is known (started/finished) but the
                        //     on-disk file was cleaned up: tell the model
                        //     what the task's final status was, so it can
                        //     decide whether to inspect status.json or move
                        //     on.
                        //   - Task is genuinely unknown: surface the original
                        //     "task may not exist" message.
                        if manager.is_live_task(&task_id) {
                            return Err(anyhow::anyhow!(
                                "Output for task {task_id} is not yet on disk: the \
                                 task is still running in this process. Use \
                                 `bg action=wait task_id={task_id}` to block on \
                                 completion, then re-issue `bg action=output`."
                            ));
                        }
                        match manager.status(&task_id).await {
                            Some(status) => {
                                return Err(anyhow::anyhow!(
                                    "Output for task {task_id} is not on disk (final \
                                     status: {:?}, exit: {:?}). The output file was \
                                     probably cleaned up after the task finished. \
                                     Read the status with \
                                     `bg action=status task_id={task_id}` for the \
                                     final report, or start a new task.",
                                    status.status,
                                    status.exit_code,
                                ));
                            }
                            None => {
                                return Err(anyhow::anyhow!(
                                    "Output not found for task: {task_id}. Task may \
                                     not exist or output file was deleted. Use \
                                     `bg action=list` to see what tasks this \
                                     session has."
                                ));
                            }
                        }

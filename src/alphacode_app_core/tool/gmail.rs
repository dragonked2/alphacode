...
use crate::alphacode_app_core::gmail::{self, GmailClient, MessageFormat};
            ...
            if !self.client.supports_connect() {
                return Ok(ToolOutput::new(
                    "The 'connect' action is only available with the Composio Gmail backend. \
                     Set ALPHACODE_GMAIL_BACKEND=composio and COMPOSIO_API_KEY, then retry. \
                     For the default backend, run `alphacode login google` instead.",
                ));
            }
            let no_browser = crate::alphacode_base::auth::browser_suppressed(false);
                ...
                if !self.client.can_send() {
                    return Ok(ToolOutput::new(
                        "Send is not available. Your Gmail access is configured as Read & Draft Only (API-level restriction).\n\
                         ...
                         To enable sending, rerun `alphacode login google --google-access-tier full`.",
                    ));
                }
                ...
                if !self.client.can_delete() {
                    return Ok(ToolOutput::new(
                        "Trash is not available. Your Gmail access is configured as Read & Draft Only (API-level restriction).\n\
                         To enable delete, rerun `alphacode login google --google-access-tier full`.",
                    ));
                }

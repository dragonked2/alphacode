...
impl RunPlanReporter {
    ...
    /// Record an explicit checkpoint (a ALPHACODE_CHECKPOINT-style milestone) on
...
/// The `alphacode login` invocation most likely to fix a credential wave for
...
fn credential_login_fix_hint(provider: Option<&str>) -> String {
    ...
    format!("`alphacode login --provider {target}`")
}
...
fn format_swarm_model_list(
    ...
    model_routes: &[crate::alphacode_provider_core::ModelRoute],
) -> String {
...
impl CommunicateTool {
    pub fn new() -> Self {
        ...
        let description = if swarm_prompt.is_empty() {
            ...
            format!(
                "{BASE_DESCRIPTION}\n\nSwarm prompt (user-tunable via ~/.alphacode/swarm-prompt.md):\n{swarm_prompt}"
            )
        };
        Self { description }
    }

...
use crate::alphacode_app_core::message::{Message, Role};
...
fn search_messages(messages: &[Message], query: &str) -> Vec<SearchResult> {
    // Build a case-insensitive regex from the query, escaping special chars.
    let re = match build_search_regex(query) {
        Some(re) => re,
        None => return Vec::new(),
    };
    ...
    for (idx, msg) in messages.iter().enumerate() {
        ...
        if re.is_match(&text) {
            let snippet = extract_snippet_regex(&text, &re);
...
fn extract_snippet_regex(text: &str, re: &regex::Regex) -> String {
...
mod tests {
    ...
    fn setup_session(messages: Vec<Message>) -> (ToolContext, std::path::PathBuf, Option<String>) {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let base = std::env::temp_dir().join(format!("alphacode-test-{}", nonce));
        let _ = std::fs::create_dir_all(base.join("sessions"));

        let previous_home = std::env::var("ALPHACODE_HOME").ok();
        crate::alphacode_core::env::set_var("ALPHACODE_HOME", &base);

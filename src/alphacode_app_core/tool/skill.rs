...
#[derive(Deserialize)]
struct SkillInput {
    /// Action to perform: load (default), list, search, reload, reload_all, read, info.
    /// `list` shows both loaded skills and the alphacode-endorsed catalog.
    /// `search` finds skills by keyword in name, description, or content.
    /// `info` shows detailed skill info including reference files.
    #[serde(default = "default_action")]
    action: String,
    /// Skill name (required for load, reload, read, info)
    #[serde(alias = "skill")]
    #[serde(default)]
    name: Option<String>,
    ...
    #[serde(default)]
    query: Option<String>,
    /// Reference file name (for 'read' action with a skill)
    #[serde(default)]
...
impl Tool for SkillTool {
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "action": {
                    "type": "string",
                    "enum": ["load", "list", "search", "reload", "reload_all", "read", "info"],
                    "description": "Action. 'search' finds skills by keyword. 'info' shows skill details including reference files."
                },
                "query": {
                    "type": "string",
                ...
                "reference": {
                    "type": "string",
                    "description": "Reference file name (for 'read' action with a skill)."
                },
                "name": {
                    "type": "string",
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        match params.action.as_str() {
            ...
            "read" => {
                self.read_skill(params.name, params.reference, ctx.working_dir.as_deref())
                    .await
            }
            "search" => {
                self.search_skills(params.query, ctx.working_dir.as_deref())
                    .await
            }
            "info" => {
                self.info_skill(params.name, ctx.working_dir.as_deref())
                    .await
            }
            _ => Ok(ToolOutput::new(format!(
                "Unknown action: {}. Use 'load', 'list', 'search', 'reload', 'reload_all', 'read', or 'info'.",
                params.action
            ))),
        }
        .map_err(|err| {
...
impl SkillTool {
        ...
        let skill = registry.get(&name).ok_or_else(|| {
                ...
                .find(|endorsed| endorsed.name == name)
            {
                match endorsed.install {
                    ...
                    None => anyhow::anyhow!(
                        "Skill '{}' is endorsed but not installed (source: {}). Install it into ~/.alphacode/skills/{}/SKILL.md, then run skill_manage reload_all.",
                        name,
                        endorsed.source,
                        name
                    ),
    ...
    async fn read_skill(
        ...
        if let Some(skill) = registry.get(&name) {
            // If a reference file is requested, return that specifically
            if let Some(ref_name) = reference {
                if let Some(content) = skill.get_reference(&ref_name) {
                    return Ok(ToolOutput::new(format!(
                        "# {} / {}\n\n{}",
                        skill.name, ref_name, content
                    ))
                    .with_title(format!("Skills: {} / {}", skill.name, ref_name)));
                } else {
                    let available: Vec<&str> = skill.list_references();
                    return Ok(ToolOutput::new(format!(
            ...
            // Show reference files if any
            let refs = skill.list_references();
            if !refs.is_empty() {
                output.push_str(&format!("**Reference files:** {}\n", refs.join(", ")));
            }
            output.push_str("\n---\n\n");
    ...
    async fn search_skills(
        ...
        let query = match query {
            ...
            _ => {
                return Ok(ToolOutput::new(
                    "Provide a 'query' parameter to search skills.\n\nExample: search for 'security' or 'api'"
                        .to_string(),
                )
                .with_title("Skills: Search"));
            }
        };
        ...
        let mut matches: Vec<(&str, &str, u32)> = all_skills
            .iter()
            .map(|skill| {
                ...
                score += content_lower.matches(&*query).count() as u32;

                (skill.name.as_str(), skill.description.as_str(), score)
            })
    ...
    async fn info_skill(
        ...
        let name = normalize_skill_name(name, "info")?;
        ...
        if let Some(skill) = registry.get(&name) {
            ...
            Ok(ToolOutput::new(output).with_title(format!("Skills: {} info", name)))
        } else {
            Ok(ToolOutput::new(format!(
                "Skill '{}' not found.\n\nUse 'list' to see available skills.",
                name
...
/// Append the curated alphacode-endorsed skill catalog to `output`, grouped by
...
fn append_endorsed_skills(output: &mut String, installed: &std::collections::HashSet<&str>) {
    ...
    output.push_str("\nEndorsed skills (recommended by alphacode)\n");
...
mod tests {
    ...
    fn create_test_tool_with_skill(name: &str) -> (SkillTool, tempfile::TempDir) {
        ...
        let skill_dir = temp_dir.path().join(".alphacode").join("skills").join(name);

...
/// Cached path to Git Bash on Windows, resolved once at startup.
/// `None` when Git Bash is not installed; falls back to `cmd.exe`.
#[cfg(windows)]
static GIT_BASH_PATH: LazyLock<Option<String>> = LazyLock::new(|| {
    // Check the three standard Git for Windows install locations.
    let candidates = [
...
fn build_shell_command(cmd_str: &str) -> TokioCommand {
    #[cfg(windows)]
    {
        // Prefer Git Bash when available so POSIX commands (ls, grep, cat, etc.)
        // work as documented. Only fall back to cmd.exe when Git Bash is not
        // installed, in which case the model receives cmd.exe-compatible output
        // and should use Windows-native commands.
        if let Some(ref bash_path) = *GIT_BASH_PATH {
            ...
            // Tokio's `raw_arg` is specifically provided for `cmd.exe /C`. Wrap
            // the full command in the outer quotes expected by cmd so its inner
            // quotes reach child programs intact. `/D` disables AutoRun hooks and
...
fn build_detached_shell_wrapper(command: &str) -> StdCommand {
    ...
    cmd.arg("-lc")
        .arg(
            r#"eval "$ALPHACODE_RELOAD_DETACH_COMMAND"; status=$?; printf '\n--- Command finished with exit code: %s ---\n' "$status"; exit "$status""#,
        )
        .env("ALPHACODE_RELOAD_DETACH_COMMAND", command);
    if let Some(dir) = tool_scratch_dir() {
        cmd.env("TMPDIR", &dir).env("ALPHACODE_SCRATCH_DIR", dir);
    }
    cmd
}

fn format_command_output(mut output: String, exit_code: Option<i32>) -> String {
    // Apply smart stream filtering to reduce context bloat
    let filter_result = smart_stream::filter_output("bash", &output);
    if filter_result.was_filtered {
        let was_significant = filter_result.significant_reduction();
    ...
    if output.trim().is_empty() {
        // Short marker. The TUI still draws a frame around the tool
...
mod utf8_truncation_tests {
    ...
    async fn build_shell_command_uses_cmd_and_executes_command() {
        ...
        let probe_path = std::env::temp_dir().join(format!(
            "alphacode-cmd-quoting-probe-{}.cmd",
            std::process::id()
        ));
        std::fs::write(
            &probe_path,
...
impl Tool for BashTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        if crate::browser::is_browser_command(&params.command) {
            ...
            // Start/attach a browser session for this alphacode session.

...
async fn communicate_assign_task_can_spawn_fallback_agent() {
    ...
    let socket_path = runtime_dir.path().join("alphacode.sock");
    let _runtime = EnvGuard::set("ALPHACODE_RUNTIME_DIR", runtime_dir.path());
    let _socket = EnvGuard::set("ALPHACODE_SOCKET", &socket_path);
    let _debug = EnvGuard::set("ALPHACODE_DEBUG_CONTROL", "1");

...
//! Self-development tool - manage canary builds when working on alphacode itself

use crate::alphacode_app_core::background::{self, TaskResult};
use crate::alphacode_app_core::build;
use crate::alphacode_app_core::bus::BackgroundTaskStatus;
use crate::alphacode_app_core::protocol::{ServerEvent, TranscriptMode};
use crate::alphacode_app_core::server;
use crate::alphacode_app_core::session;
use crate::alphacode_app_core::session_launch;
use crate::alphacode_app_core::storage;
use crate::alphacode_app_core::tool::{Tool, ToolContext, ToolExecutionMode, ToolOutput};
use anyhow::Result;
...
/// Public GitHub source used when cloning the alphacode repository for self-dev.
pub const ALPHACODE_REPO_URL: &str = "https://github.com/dragonked2/alphacode.git";
...
impl BuildRequest {
    ...
    fn requests_dir() -> Result<PathBuf> {
        let dir = storage::alphacode_dir()?.join("selfdev-build-requests");
    ...
    fn terminal_history_limit() -> usize {
        std::env::var("ALPHACODE_SELFDEV_REQUEST_HISTORY_LIMIT")
            .ok()
...
impl SelfDevTool {
    ...
    pub fn description_for(is_selfdev: bool) -> &'static str {
        if is_selfdev {
            "Manage self-dev builds, tests, and reloads while working on alphacode itself."
        } else {
            "Enter self-dev mode to work on alphacode itself: setup, reload, find config/paths."
        }
    }
            ...
            json!({
                ...
                "properties": {
                    ...
                    "target": {
                        ...
                        "description": "Build target for action=build. auto chooses from changed paths; tui builds alphacode; desktop2 builds alphacode-desktop2; all builds every binary."
                    },
                    "command": {
                        "type": "string",
            ...
            json!({
                ...
                "properties": {
                    ...
                    "action": {
                        ...
                        "description": "Action. `enter` starts a self-dev session; `setup` installs prerequisites; `reload` restarts alphacode."
                    },
                    "prompt": {
                        "type": "string",
...
impl Tool for SelfDevTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        let result = match action.as_str() {
            ...
            // access only make sense once you are working on alphacode itself.
            "build" => {
                self.do_build(
                    params.reason,
                    params.target,
...
impl SelfDevTool {
    fn is_test_session() -> bool {
        std::env::var("ALPHACODE_TEST_SESSION")
            .map(|value| {
    ...
    fn reload_timeout_secs() -> u64 {
        std::env::var("ALPHACODE_SELFDEV_RELOAD_TIMEOUT_SECS")
            .ok()
    ...
    fn build_reload_wait_secs() -> u64 {
        std::env::var("ALPHACODE_SELFDEV_BUILD_WAIT_SECS")
            .ok()
            .and_then(|raw| raw.trim().parse::<u64>().ok())
            .filter(|secs| *secs > 0)
            .unwrap_or(1800)
    ...
    fn resolve_repo_dir(working_dir: Option<&std::path::Path>) -> Option<std::path::PathBuf> {
        if let Some(dir) = working_dir {
            for ancestor in dir.ancestors() {
                if build::is_alphacode_repo(ancestor) {
    ...
    fn launch_binary() -> Result<std::path::PathBuf> {
        build::client_update_candidate(true)
            ...
            .ok_or_else(|| anyhow::anyhow!("Could not resolve alphacode executable to launch"))
    }
    ...
    fn build_lock_path(worktree_scope: &str) -> Result<PathBuf> {
        let dir = storage::alphacode_dir()?.join("selfdev-build-locks");
        storage::ensure_dir(&dir)?;

...
impl SelfDevTool {
  ...
  case "${{1:-}}" in
    test|check|build|clippy|bench)
      if [[ "${{ALPHACODE_IN_DEV_CARGO:-0}}" == "1" || -z "${{ALPHACODE_DEV_CARGO_SCRIPT:-}}" ]]; then
        command cargo "$@"
      else
        ALPHACODE_IN_DEV_CARGO=1 "${{ALPHACODE_DEV_CARGO_SCRIPT}}" "$@"
      fi
      ;;
    *) command cargo "$@" ;;
  esac
...
{command}"#
    ...
    fn desktop_binary_name(command: &SelfDevBuildCommand) -> Option<&'static str> {
        if command.display.contains("-p alphacode ") {
            return None;
        }
        if command.display.contains("-p alphacode-desktop2") {
            return Some(if cfg!(windows) {
                "alphacode-desktop2.exe"
            } else {
                "alphacode-desktop2"
            });
        }
    ...
    fn validate_desktop_selfdev_binary(
        ...
        let output = std::process::Command::new(&binary)
            .arg("--version")
            .env("ALPHACODE_NON_INTERACTIVE", "1")
            .output()?;
    ...
    pub(super) async fn do_build(
        ...
        let repo_dir =
            SelfDevTool::resolve_repo_dir(ctx.working_dir.as_deref()).ok_or_else(|| {
                anyhow::anyhow!("Could not find the alphacode repository directory for selfdev build")
            })?;
    ...
    pub(super) async fn do_test(
        ...
        let repo_dir =
            SelfDevTool::resolve_repo_dir(ctx.working_dir.as_deref()).ok_or_else(|| {
                anyhow::anyhow!("Could not find the alphacode repository directory for selfdev test")
            })?;
        let requested_source = SelfDevTool::requested_source_state(&repo_dir)?;
...
mod desktop_binary_tests {
    ...
    fn each_desktop_build_validates_its_own_binary() {
        let desktop2 = SelfDevTool::desktop_binary_name(&command(
            "scripts/dev_cargo.sh build --profile selfdev -p alphacode-desktop2 --bin alphacode-desktop2",
        ));
        assert!(
            desktop2.is_some_and(|name| name.starts_with("alphacode-desktop2")),
    ...
    fn tui_and_combined_builds_are_not_desktop_only() {
        for display in [
            "scripts/dev_cargo.sh build --profile selfdev -p alphacode --bin alphacode",
            "scripts/dev_cargo.sh build --profile selfdev -p alphacode --bin alphacode -p alphacode-desktop2",
        ] {
            assert_eq!(

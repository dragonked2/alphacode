...
impl SelfDevTool {
    ...
    /// building alphacode from source: the rust toolchain, git, and a local repo
    ...
    pub(super) async fn do_setup(&self, ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        // Rust toolchain: cargo + rustc are required to build alphacode.
        ...
        if repo_dir.is_none() {
            ...
            if SelfDevTool::is_test_session() {
                clone_note = Some("Test mode: skipped cloning the alphacode source.".to_string());
            } else if git_available {
                match Self::clone_selfdev_source() {
                    Ok(path) => {
                        clone_note = Some(format!("Cloned alphacode source into {}.", path.display()));
                        repo_dir = Some(path);
                    }
                    Err(err) => {
                        clone_note =
                            Some(format!("Could not clone alphacode source automatically: {err}",));
                    }
                }
            }
        }
        ...
        match &repo_dir {
            ...
            None => {
                let target = Self::selfdev_clone_dir()
                    ...
                    .unwrap_or_else(|_| "~/.alphacode/source/alphacode".to_string());
                checks.push(SetupCheck::missing(
                    "repository",
                    "no local alphacode checkout found",
                    format!(
                        "Clone the source manually: `git clone {} {}`.",
                        super::ALPHACODE_REPO_URL,
                        target
                    ),
                ));
            }
        ...
        if let Some(repo) = repo_dir.as_deref() {
            match build::find_dev_binary(repo) {
                ...
                None => checks.push(SetupCheck::missing(
                    ...
                    "Build it once with `alphacode self-dev --build`, or inside a \
        ...
        if !failing.is_empty() {
            ...
            output.push_str(
                "\nAll prerequisites satisfied. Use `selfdev enter` to start working on alphacode.\n",
            );
        }
    ...
    /// `selfdev find-config`: report the key alphacode paths (config file, home,
    ...
    pub(super) async fn do_find_config(&self, ctx: &ToolContext) -> Result<ToolOutput> {
        let alphacode_home = storage::alphacode_dir().ok();
        let config_path = alphacode_home.as_ref().map(|home| home.join("config.toml"));
        ...
        let mut output = String::from("## alphacode config & paths\n\n");
        output.push_str(&format!(
            "**Config file:** {}\n",
        ...
        output.push_str(&format!(
            "**alphacode home:** {}\n",
            format_path(alphacode_home.as_deref())
        ));
        output.push_str(&format!(
            "**Logs dir:** {}\n",
        ...
        let metadata = json!({
            ...
            "alphacode_home": alphacode_home.as_ref().map(|p| p.display().to_string()),
    ...
    pub(super) async fn do_reload_to_newer_build(&self, _ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        if !server::server_has_newer_binary() {
            return Ok(ToolOutput::new(
                "Already running the newest installed alphacode build; no reload needed.",
            ));
        }
    ...
    fn selfdev_clone_dir() -> Result<std::path::PathBuf> {
        Ok(storage::alphacode_dir()?.join("source").join("alphacode"))
    }

    /// Clone the alphacode source into the default self-dev source directory.
    fn clone_selfdev_source() -> Result<std::path::PathBuf> {
        let repo_dir = Self::selfdev_clone_dir()?;
        if repo_dir.exists() {
            if build::is_alphacode_repo(&repo_dir) {
                return Ok(repo_dir);
            }
            anyhow::bail!(
                "{} exists but is not a alphacode repository; move it aside and retry",
                repo_dir.display()
            );
        }
        ...
        if !build::is_alphacode_repo(&repo_dir) {
            anyhow::bail!(
                "cloned source at {} is not a valid alphacode repository",
                repo_dir.display()
            );
        }
        Ok(repo_dir)

...
const DISCOVERY_REQUEST_ID_HEADER: &str = "x-alphacode-discovery-request-id";
const DISCOVERY_BENCHMARK_HEADER: &str = "x-alphacode-discovery-benchmark";
const DISCOVERY_SESSION_ID_HEADER: &str = "x-alphacode-discovery-session-id";
const DISCOVERY_SESSION_METADATA_HEADER: &str = "x-alphacode-discovery-session-metadata";
const DISCOVERY_SELF_DEV_HEADER: &str = "x-alphacode-discovery-self-dev";
const DISCOVERY_DEBUG_HEADER: &str = "x-alphacode-discovery-debug";
const DISCOVERY_CANARY_HEADER: &str = "x-alphacode-discovery-canary";
const DISCOVERY_EXECUTION_MODE_HEADER: &str = "x-alphacode-discovery-execution-mode";
const DISCOVERY_BUILD_CHANNEL_HEADER: &str = "x-alphacode-discovery-build-channel";
const DISCOVERY_GIT_CHECKOUT_HEADER: &str = "x-alphacode-discovery-git-checkout";
const DISCOVERY_CI_HEADER: &str = "x-alphacode-discovery-ci";
const DISCOVERY_RAN_FROM_CARGO_HEADER: &str = "x-alphacode-discovery-ran-from-cargo";
const DISCOVERY_BENCHMARK_ENV: &str = "ALPHACODE_DISCOVERY_BENCHMARK";
...
///
/// Disclosure contract: some providers may share revenue with Alphacode, but
/// partnership status never influences recommendations. The policy is
/// disclosed in the tool schema and at <https://alphacode.sh/discovery-tools>.
...
impl Tool for DiscoverToolsTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        // metered coarsely; see crate::alphacode_base::sponsors::provenance).
...
async fn fetch_listing(
    ...
    let response = request.send().await.map_err(|err| DiscoveryFetchError {
        ...
        // No retry (see fn docs): the failure reason must describe the first
        // and only attempt, so callers can tell an HTTP error apart from a
        // transport failure.
...
async fn submit_suggestion(
    ...
    // No retry: suggestions are non-idempotent (a retry can double-submit),
...
fn render_listing(category: &str, listing: &Value, request_id: &str) -> Result<String> {
    ...
    let mut out = format!(
        "Discoverable tools in '{category}' (Alphacode tool directory; recommendations must be based \
         only on fit; details: {}):\n",
...
fn render_selection(category: &str, tool_name: &str, listing: &Value) -> Result<String> {
    ...
    let blurb = tool.get("blurb").and_then(|v| v.as_str()).unwrap_or("");
    let mut out = format!(
        "Selected '{name}' from '{category}' (Alphacode tool directory; selection must be based only \
         on fit; details: {}):\n\n{name}: {blurb}",
        crate::sponsors::DISCOVERY_PARTNERS_URL
    );
    if let Some(url) = tool.get("url").and_then(|v| v.as_str()) {
...
mod tests {
    ...
    fn render_listing_includes_disclosure_and_tools() {
        ...
        assert!(out.contains("Alphacode tool directory"));
    ...
    fn agentmail_selection_preserves_signup_attribution_and_mcp_provenance() {
        let listing = json!({
            "tool": {
                ...
                "url": "https://www.agentmail.to/?via=alphacode-discovery",
                "setup": concat!(
                    "POST https://api.agentmail.to/v0/agent/sign-up with JSON ",
                    "{\"source\":\"alphacode\",\"referrer\":\"https://alphacode.sh/discovery-tools\"}. ",
        ...
        assert!(rendered.contains("\"source\":\"alphacode\""));
        assert!(rendered.contains("\"referrer\":\"https://alphacode.sh/discovery-tools\""));
    ...
    fn suggestion_validation_distinguishes_product_and_capability_gap() {
        ...
        known.product_url = Some("https://example.com/tool?via=alphacode#setup".to_string());
    ...
    fn render_suggestion_is_clear_about_review_status_and_recipient() {
        ...
        assert!(out.contains("does not mean Alphacode has partnered with the tool"));
    }
    ...
    async fn fetch_listing_round_trips_and_sends_only_expected_params() {
        ...
        assert!(
            request
                .to_ascii_lowercase()
                .contains("x-alphacode-discovery-request-id: request-test-1"),
            "{request}"
        );
        assert!(
            request
                .to_ascii_lowercase()
                .contains("x-alphacode-discovery-benchmark: 1"),
            "{request}"
        );
        for expected in [
            "x-alphacode-discovery-session-id: session-test-1",
            "x-alphacode-discovery-session-metadata: 1",
            "x-alphacode-discovery-self-dev: 1",
            "x-alphacode-discovery-debug: 0",
            "x-alphacode-discovery-canary: 1",
            "x-alphacode-discovery-execution-mode: agent_turn",
            "x-alphacode-discovery-build-channel: selfdev",
            "x-alphacode-discovery-git-checkout: 1",
            "x-alphacode-discovery-ci: 0",
            "x-alphacode-discovery-ran-from-cargo: 1",
        ] {
    ...
    async fn submit_suggestion_posts_structured_maintainer_only_payload() {
        ...
        assert!(
            lower.contains("x-alphacode-discovery-benchmark: 1"),
            "{request}"
        );
    ...
    async fn execute_end_to_end_with_enabled_config_and_local_server() {
        ...
        assert!(output.output.contains("Alphacode tool directory"));
        assert!(
            output
                .output
        ...
        if let Some(prev) = prev_home {
            crate::alphacode_core::env::set_var("ALPHACODE_HOME", prev);
        } else {

...
fn parse_usize_lenient(s: &str) -> Result<usize, String> {
    let trimmed = s.trim();
    // Plain integer first.
    if let Ok(v) = trimmed.parse::<usize>() {
        return Ok(v);
    }
    // Float like 100.0 (models sometimes emit floats for ints).
    ...
    Err(format!("string {trimmed:?} is not a valid usize"))
}

/// Deserialize an `Option<usize>` from a JSON number, numeric string, float,
/// or null/missing. Empty strings become `None`. This fixes
/// `invalid type: string "100", expected usize` when models send
/// `{"offset": "100"}` for the read tool.
    ...
    D: Deserializer<'de>,
{
    ...
    match value {
        ...
        Some(serde_json::Value::String(s)) => {
            parse_usize_lenient(&s).map(Some).map_err(de::Error::custom)
        }
        Some(serde_json::Value::Number(n)) => {
            if let Some(u) = n.as_u64() {
                usize::try_from(u)
                    .map(Some)
                    .map_err(|_| de::Error::custom(format!("number {u} out of range for usize")))
            } else if let Some(f) = n.as_f64() {
                if f.fract() == 0.0 && f >= 0.0 && f <= usize::MAX as f64 {
                    Ok(Some(f as usize))
                } else {
                    Err(de::Error::custom(format!(
                        "number {f} is not a valid usize"
                    )))
                }
            } else {
                Err(de::Error::custom("number is not a valid usize"))
            }
        }
        Some(other) => Err(de::Error::custom(format!(
            "expected usize or numeric string, got {other}"
        ))),
    }
}
...
fn parse_u64_lenient(s: &str) -> Result<u64, String> {
    ...
    Err(format!("string {trimmed:?} is not a valid u64"))
}

/// Deserialize an `Option<u64>` leniently (number, numeric string, float,
/// null/missing). Used for timeouts and other u64 tool fields.
    ...
    D: Deserializer<'de>,
{
    ...
    match value {
        ...
        Some(serde_json::Value::String(s)) if s.trim().is_empty() => Ok(None),
        Some(serde_json::Value::String(s)) => {
            parse_u64_lenient(&s).map(Some).map_err(de::Error::custom)
        }
        Some(serde_json::Value::Number(n)) => {
            if let Some(u) = n.as_u64() {
                Ok(Some(u))

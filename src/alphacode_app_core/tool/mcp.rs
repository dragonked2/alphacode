...
use crate::alphacode_app_core::mcp::{McpManager, McpServerConfig};
use crate::alphacode_app_core::tool::{Tool, ToolContext, ToolOutput};
use anyhow::Result;
    ...
    async fn list_servers(&self) -> Result<ToolOutput> {
        ...
        if servers.is_empty() && configured.is_empty() {
            return Ok(ToolOutput::new(
                ...
                Or add servers to ~/.alphacode/mcp.json or .alphacode/mcp.json and use {\"action\": \"reload\"}.\n\
    ...
    async fn reload_config(&self, session_id: &str) -> Result<ToolOutput> {
        ...
        if config.servers.is_empty() {
            ...
            return Ok(ToolOutput::new(
                ...
                Add servers to ~/.alphacode/mcp.json (global) or .alphacode/mcp.json (project):\n\
...
mod tests {
    ...
    impl LocalMcpConfigGuard {
        fn new(content: &str) -> std::io::Result<Self> {
            let path = PathBuf::from(".alphacode/mcp.json");
            let dir = path
                .parent()
    ...
    async fn test_reload_empty_config() {
        let _guard =
            LocalMcpConfigGuard::new("{\"servers\":{}}").expect("create temporary .alphacode/mcp.json");
        let tool = create_test_tool();
        let ctx = create_test_context();

...
use crate::alphacode_app_core::runtime_memory_log::{
...
const SERVER_NAME_ENV: &str = "ALPHACODE_SERVER_NAME";
const SERVER_DISPLAY_NAME_ENV: &str = "ALPHACODE_SERVER_DISPLAY_NAME";
...
impl Server {
    ...
    pub fn new_with_name(provider: Arc<dyn Provider>, server_name: Option<String>) -> Self {
        use crate::alphacode_app_core::id::{new_id, new_memorable_server_id, server_icon};
        ...
        crate::alphacode_base::process_title::set_server_title(&identity.name);
    ...
    fn spawn_registry_metadata_publisher(&self, registry_info: crate::registry::ServerInfo) {
        ...
        tokio::spawn(async move {
            ...
            let _ = std::fs::write(&hash_path, crate::alphacode_build_meta::git_hash());
    ...
    fn spawn_power_inhibitor(swarm_members: Arc<RwLock<HashMap<String, SwarmMember>>>) {
        ...
        if !inhibitor.is_available() {
            ...
            crate::logging::info(
                "power_inhibit: unavailable (unsupported platform or ALPHACODE_DISABLE_POWER_INHIBIT set); not monitoring",
            );
            return;
        }
    ...
    pub async fn run(&self) -> Result<()> {
        // Ensure socket directory exists (for named sockets like /run/user/1000/alphacode/)

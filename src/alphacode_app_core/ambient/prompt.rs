...
pub fn gather_recent_sessions(since: Option<DateTime<Utc>>) -> Vec<RecentSessionInfo> {
    let sessions_dir = match crate::storage::alphacode_dir() {
...
pub fn build_ambient_system_prompt(
    ...
    prompt.push_str(
        "You are the ambient agent for alphacode. You operate autonomously without \
    ...
    prompt.push_str(
        "## Instructions\n\n\
         ...
         without opening alphacode. You can optionally target a specific channel \

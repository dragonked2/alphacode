...
use crate::alphacode_app_core::ambient_scheduler::{AdaptiveScheduler, AmbientSchedulerConfig};
use crate::alphacode_app_core::config::config;
use crate::alphacode_app_core::logging;
use crate::alphacode_app_core::memory::MemoryManager;
use crate::alphacode_app_core::notifications::NotificationDispatcher;
use crate::alphacode_app_core::provider::Provider;
use crate::alphacode_app_core::safety::SafetySystem;
...
impl AmbientRunnerHandle {
    ...
    pub async fn log_json(&self) -> String {
        let transcripts_dir = match crate::storage::alphacode_dir() {
    ...
    async fn wait_for_request_done(
        client: &mut crate::alphacode_app_core::server::Client,
        request_id: u64,
    ) -> anyhow::Result<()> {
        loop {
    ...
    async fn run_cycle_visible(
        ...
        // Find the alphacode binary
        let alphacode_bin =
            std::env::current_exe().unwrap_or_else(|_| std::path::PathBuf::from("alphacode"));

        // Spawn kitty with `alphacode ambient run-visible`
        logging::info("Ambient visible: spawning kitty with alphacode TUI");
        let child = std::process::Command::new("kitty")
            .args([
                "--title",
                "🤖 alphacode ambient cycle",
                "-e",
                &alphacode_bin.to_string_lossy(),
                "ambient",
                "run-visible",
            ])

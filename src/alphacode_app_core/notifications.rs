...
//! - Message channels (Telegram, Discord, etc.)
//!
//! All sends are fire-and-forget: errors are logged, never block.

use crate::alphacode_app_core::config::{SafetyConfig, config};
use crate::alphacode_app_core::logging;
use crate::alphacode_app_core::safety::AmbientTranscript;

// Email transport/reply parsing lives in the neutral `alphacode-notify-email`
// module. Re-exported here because `channel.rs` reaches the permission-reply
// helpers through `crate::notifications::` (the historical path).
#[cfg(feature = "email")]
use crate::alphacode_notify_email::{ReplyAction, SendEmailRequest, poll_imap_once, send_email};
pub use crate::alphacode_notify_email::{extract_permission_id, parse_permission_reply};
...
impl NotificationDispatcher {
    ...
    pub fn dispatch_permission_request(&self, action: &str, description: &str, request_id: &str) {
        let title = format!("alphacode: permission needed ({})", action);
        let safe_body = "An ambient action needs your approval. Open alphacode to review.".to_string();
        let detailed_body = format!(
            "Action: {}\n{}\n\nRequest ID: {}\nReview in alphacode to approve or deny.",
    ...
    fn send_all_with_email_override(
        ...
        detailed_body: &str,
        priority: Priority,
        #[allow(unused_variables)] cycle_id: Option<&str>,
        #[allow(unused_variables)] email_html_override: Option<&str>,
    ) {
...
fn format_cycle_body_safe(transcript: &AmbientTranscript) -> String {
    ...
    lines.push("Check alphacode for full details.".to_string());
    lines.join("\n")
}
...
fn format_cycle_body_detailed(transcript: &AmbientTranscript) -> String {
    ...
    if transcript.pending_permissions > 0 {
        lines.push(String::new());
        lines.push(format!(
            "**⚠ {} permission request(s) pending** — review in alphacode",
            transcript.pending_permissions
        ));
    }
...
mod tests {
    ...
    fn test_format_cycle_body_safe() {
        ...
        assert!(body.contains("Check alphacode for full details"));
    ...
    fn test_format_cycle_body_with_pending_permissions() {
        ...
        assert!(safe.contains("Check alphacode for full details"));

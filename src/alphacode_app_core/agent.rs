...
use crate::alphacode_app_core::bus::{Bus, BusEvent, SubagentStatus, ToolEvent, ToolStatus};
use crate::alphacode_app_core::cache_tracker::CacheTracker;
use crate::alphacode_app_core::compaction::CompactionEvent;
...
impl Agent {
    fn should_track_client_cache(&self) -> bool {
        match std::env::var("ALPHACODE_TRACK_CLIENT_CACHE") {
            Ok(value) => {
    ...
    fn record_client_cache_request(&mut self, messages: &[Message]) {
        ...
        let fast_snapshot =
            if !self.provider.uses_alphacode_compaction() && self.session.compaction.is_none() {

...
use crate::alphacode_app_core::{build, update};
...
pub fn hot_rebuild(session_id: &str) -> Result<()> {
    ...
    let is_selfdev = crate::alphacode_selfdev_types::client_selfdev_requested();
...
fn run_background_session_rebuild(session_id: String) {
    ...
    let Some(repo_dir) = build::get_repo_dir() else {
        publisher.error("Rebuild failed: could not find the alphacode repository.");
        return;
    };

//! Launching alphacode sessions in new terminal windows.
//!
//! These helpers spawn a fresh `alphacode` process (resume or self-dev) inside a
...
use crate::alphacode_app_core::{id, server};
...
/// spawn hooks and spawned terminals as `ALPHACODE_SPAWN_*` env vars so external
...
pub struct SessionSpawnContext {
    ...
    /// Extra `ALPHACODE_SPAWN_*` env entries (e.g. swarm/coordinator ids).
...
/// `ALPHACODE_FOCUS_SESSION_ID` and `ALPHACODE_FOCUS_TITLE` env vars.
...
/// var is exported natively and under a `ALPHACODE_CLIENT_<NAME>` alias.
pub fn focus_session_via_hook_with_env(
    ...
    cmd.args(args)
        .env("ALPHACODE_FOCUS_SESSION_ID", session_id)
        .env("ALPHACODE_FOCUS_TITLE", title)
        .stdin(std::process::Stdio::null())
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null());
    for (key, value) in client_terminal_env {
        cmd.env(key, value);
        cmd.env(format!("ALPHACODE_CLIENT_{key}"), value);
    }
...
fn focus_title_best_effort(title: &str) {
    ...
    cmd.arg("-c")
        .arg(
            "sleep 0.4; \
             if command -v wmctrl >/dev/null 2>&1; then wmctrl -a \"$ALPHACODE_WINDOW_TITLE\" >/dev/null 2>&1 && exit 0; fi; \
             if command -v xdotool >/dev/null 2>&1; then xdotool search --name \"$ALPHACODE_WINDOW_TITLE\" windowactivate >/dev/null 2>&1 && exit 0; fi; \
             exit 0",
        )
        .env("ALPHACODE_WINDOW_TITLE", title)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
...
fn resume_terminal_candidates_windows() -> Vec<String> {
    std::env::var("ALPHACODE_RESUME_TERMINAL")
        .ok()
        .map(|value| {
            value
                .split(',')
...
pub fn spawn_resume_in_new_terminal_with_context(
    ...
    let mut alphacode_args: Vec<String> = Vec::new();
    if let Some(provider_arg) = resume_provider_arg(provider_key) {
        alphacode_args.push("--provider".to_string());
        alphacode_args.push(provider_arg.to_string());
    }
    alphacode_args.push("--resume".to_string());
    alphacode_args.push(session_id.to_string());

    let hook_command = crate::terminal_launch::TerminalCommand::new(exe, alphacode_args.clone())

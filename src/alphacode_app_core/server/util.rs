...
pub(crate) fn debug_control_allowed() -> bool {
    ...
    if std::env::var("ALPHACODE_DEBUG_CONTROL")
        .ok()
    ...
    if let Ok(alphacode_dir) = crate::storage::alphacode_dir()
        && alphacode_dir.join("debug_control").exists()
    {
        return true;
    }
    false
...
pub(crate) fn embedding_idle_unload_secs() -> u64 {
    parse_embedding_idle_unload_secs(
        std::env::var("ALPHACODE_EMBEDDING_IDLE_UNLOAD_SECS")
            .ok()
            .as_deref(),
    )
}
...
pub(crate) fn swarm_id_for_dir(dir: Option<PathBuf>) -> Option<String> {
    if let Ok(sw_id) = std::env::var("ALPHACODE_SWARM_ID") {
...
pub(crate) fn startup_headless_recovery_test_delay() -> Option<std::time::Duration> {
    let raw = std::env::var("ALPHACODE_TEST_HEADLESS_STARTUP_RECOVERY_DELAY_MS").ok()?;
...
mod newer_binary_tests {
    ...
    fn reports_update_when_candidate_is_strictly_newer() {
        let candidates = vec![(PathBuf::from("/x/stable/alphacode"), Some(t(200)))];
        assert!(newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/alphacode")),
            candidates,
        ));
    }
    ...
    fn ignores_candidate_that_is_not_newer() {
        let candidates = vec![(PathBuf::from("/x/stable/alphacode"), Some(t(100)))];
        assert!(!newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/alphacode")),
            candidates,
    ...
    fn never_reloads_into_self_even_if_paths_were_equal() {
        ...
        let candidates = vec![(PathBuf::from("/x/current/alphacode"), Some(t(999)))];
        assert!(!newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/alphacode")),
            candidates,
    ...
    fn suppresses_update_when_candidate_mtime_unavailable() {
        ...
        let candidates = vec![(PathBuf::from("/x/stable/alphacode"), None)];
        assert!(!newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/alphacode")),
            candidates,
    ...
    fn reports_update_if_any_candidate_is_newer() {
        let candidates = vec![
            (PathBuf::from("/x/stable/alphacode"), None),
            (PathBuf::from("/x/shared/alphacode"), Some(t(300))),
        ];
        assert!(newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/alphacode")),
    ...
    fn newer_server_is_not_outdated_by_older_channel_binary() {
        ...
        assert!(!newer_binary_available(
            Some(t(300)),
            Some(std::path::Path::new("/x/builds/versions/dev/alphacode")),
            candidates,
        ));
    }
...
mod reload_target_tests {
    ...
    fn same_binary_is_always_used() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/current/alphacode"),
            Path::new("/x/current/alphacode"),
            Some(Path::new("/x/current/alphacode")),
            Some(Path::new("/x/current/alphacode")),
            Some(t(200)),
            Some(t(100)),
        );
    ...
    fn newer_candidate_is_used() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/shared-server/alphacode"),
            Path::new("/x/builds/versions/new/alphacode"),
            Some(Path::new("/x/builds/versions/old/alphacode")),
            Some(Path::new("/x/builds/versions/old/alphacode")),
            Some(t(100)),
            Some(t(200)),
        );
        match decision {
            ReloadTargetDecision::UseCandidate((path, _)) => {
                assert_eq!(path, PathBuf::from("/x/shared-server/alphacode"));
            }
    ...
    fn equal_mtime_candidate_is_used() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/shared-server/alphacode"),
            Path::new("/x/builds/versions/same/alphacode"),
            Some(Path::new("/x/builds/versions/current/alphacode")),
            Some(Path::new("/x/builds/versions/current/alphacode")),
            Some(t(100)),
            Some(t(100)),
        );
    ...
    fn strictly_older_candidate_is_blocked_and_uses_current_exe() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/shared-server/alphacode"),
            Path::new("/x/builds/versions/old-0.14.3/alphacode"),
            Some(Path::new("/x/builds/versions/new/alphacode")),
            Some(Path::new("/x/builds/versions/new/alphacode")),
            Some(t(300)),
            Some(t(100)),
        );
        match decision {
            ReloadTargetDecision::DowngradeBlockedUseCurrent((path, _)) => {
                assert_eq!(path, PathBuf::from("/x/builds/versions/new/alphacode"));
            }
    ...
    fn unreadable_candidate_mtime_is_treated_as_downgrade() {
        let decision = guarded_reload_target(
            candidate("/x/shared-server/alphacode"),
            Path::new("/x/builds/versions/unknown/alphacode"),
            Some(Path::new("/x/builds/versions/new/alphacode")),
            Some(Path::new("/x/builds/versions/new/alphacode")),
            Some(t(300)),
            None,
    ...
    fn downgrade_without_current_exe_falls_back_to_candidate() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/shared-server/alphacode"),
            Path::new("/x/builds/versions/old/alphacode"),
            None,
            None,
            None,
            Some(t(100)),
...
mod pick_newest_candidate_tests {
    ...
    fn other_flavor_wins_when_strictly_newer() {
        ...
        let chosen = pick_newest_candidate([
            entry(
                "/x/versions/old-selfdev/alphacode",
                "shared-server",
                Some(t(100)),
            ),
            entry("/x/versions/new-release/alphacode", "stable", Some(t(200))),
        ])
        .expect("a candidate");
        assert_eq!(chosen.0, PathBuf::from("/x/versions/new-release/alphacode"));
    }
    ...
    fn own_flavor_wins_on_tie() {
        ...
        .expect("a candidate");
        assert_eq!(chosen.0, PathBuf::from("/x/versions/selfdev/alphacode"));
    }
    ...
    fn own_flavor_wins_when_strictly_newer() {
        let chosen = pick_newest_candidate([
            entry(
                "/x/versions/fresh-selfdev/alphacode",
                "shared-server",
                Some(t(300)),
            ),
            entry("/x/versions/release/alphacode", "stable", Some(t(200))),
    ...
    fn duplicate_canonical_paths_collapse() {
        ...
        let chosen = pick_newest_candidate([
            entry("/x/versions/same/alphacode", "shared-server", Some(t(100))),
            entry("/x/versions/same/alphacode", "stable", Some(t(999))),
        ])
        .expect("a candidate");
...
mod newest_reload_candidate_integration_tests {
    ...
    //! a temp `ALPHACODE_HOME`. This reproduces the field "/update -> new client,
    ...
    fn candidate_version_for(is_selfdev: bool) -> Option<String> {
        let (_path, label) = newest_reload_candidate(is_selfdev)?;
        channel_label_version(label)
    }

    /// Resolve the version a candidate channel label points at from the version
    /// marker file written by `update_*_symlink`. The marker is the source of
    /// truth on every platform: on Windows the channel entry is a copy rather
    /// than a symlink, so the version cannot be recovered from the canonicalized
    /// path (its parent dir is the channel name, not the version).
    fn channel_label_version(label: &str) -> Option<String> {
        match label {
            "shared-server" => build::read_shared_server_version().ok().flatten(),
            "stable" => build::read_stable_version().ok().flatten(),
            "current" => build::read_current_version().ok().flatten(),
            _ => None,
        }
    }
    ...
    /// Install a release-archive-style version dir: a tiny `alphacode` wrapper
    /// script plus the real `alphacode-linux-x86_64.bin` payload, with independently
    ...
    fn install_release_style_binary(
        ...
        std::fs::write(
            &wrapper,
            "#!/usr/bin/env sh\nexec ./alphacode-linux-x86_64.bin \"$@\"\n",
        )
    ...
    fn freshly_updated_release_daemon_reports_no_phantom_update() {
        // This models the Linux tar.gz release layout: a tiny shell wrapper
        // (`#!`) next to an `alphacode-linux-x86_64.bin` payload in the same
        // version dir. Channel symlinks keep the wrapper next to its payload
        // only on Unix; on Windows the channel entry is a standalone copy with
        // no payload sibling, so `resolve_binary_payload` cannot see the shared
        // install and the premise of this regression test does not apply.
        if cfg!(windows) {
            eprintln!("skipping: wrapper+payload release layout is Unix-only");
            return;
        }
...
mod deleted_suffix_tests {
    ...
    fn strips_linux_deleted_marker() {
        let p = PathBuf::from("/home/u/.alphacode/builds/versions/abc/alphacode (deleted)");
        assert_eq!(
            strip_deleted_suffix(p),
            PathBuf::from("/home/u/.alphacode/builds/versions/abc/alphacode")
        );
    }

    #[test]
    fn leaves_normal_paths_untouched() {
        let p = PathBuf::from("/home/u/.alphacode/builds/versions/abc/alphacode");
    ...
    fn only_strips_trailing_marker() {
        ...
        let p = PathBuf::from("/home/u/alphacode (deleted)/alphacode");
        assert_eq!(strip_deleted_suffix(p.clone()), p);
    }
}

...
fn format_auth_catalog_refresh_complete(
    ...
    let catalog_status = if has_warning {
        ...
        format!(
            "{provider_label} catalog ready: {} models, {} routes.",
...
fn premium_mode_label(mode: crate::provider::copilot::PremiumMode) -> &'static str {
    use crate::alphacode_app_core::provider::copilot::PremiumMode;
    match mode {
...
pub(super) async fn handle_notify_auth_changed(
    ...
    tokio::spawn(async move {
        ...
        crate::alphacode_base::auth::AuthStatus::check_fast().log_snapshot("auth_changed");
...
mod tests {
    ...
    use crate::alphacode_app_core::provider::EventStream;
    ...
    impl IsolatedRuntimeDir {
        fn new() -> Self {
            ...
            let prev_runtime = std::env::var_os("ALPHACODE_RUNTIME_DIR");
            crate::alphacode_core::env::set_var("ALPHACODE_RUNTIME_DIR", temp.path());
            Self {
    ...
    impl Drop for IsolatedRuntimeDir {
        fn drop(&mut self) {
            if let Some(prev_runtime) = self._prev_runtime.take() {
                crate::alphacode_core::env::set_var("ALPHACODE_RUNTIME_DIR", prev_runtime);
            } else {

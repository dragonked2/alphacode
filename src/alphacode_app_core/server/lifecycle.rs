...
const TEMP_SERVER_ENV: &str = "ALPHACODE_TEMP_SERVER";
const SERVER_SCOPE_ENV: &str = "ALPHACODE_SERVER_SCOPE";
const OWNER_PID_ENV: &str = "ALPHACODE_SERVER_OWNER_PID";
const TEMP_IDLE_SECS_ENV: &str = "ALPHACODE_TEMP_SERVER_IDLE_SECS";
...
pub fn configure_temporary_server(owner_pid: Option<u32>, idle_timeout_secs: Option<u64>) {
    crate::alphacode_core::env::set_var(TEMP_SERVER_ENV, "1");
    crate::alphacode_core::env::set_var(SERVER_SCOPE_ENV, "temporary");
    if let Some(owner_pid) = owner_pid {
        crate::alphacode_core::env::set_var(OWNER_PID_ENV, owner_pid.to_string());
    }
    if let Some(idle_timeout_secs) = idle_timeout_secs {
        crate::alphacode_core::env::set_var(TEMP_IDLE_SECS_ENV, idle_timeout_secs.to_string());
    }
}
...
mod tests {
        ...
        fn drop(&mut self) {
            for (name, value) in &self.entries {
                if let Some(value) = value {
                    crate::alphacode_core::env::set_var(name, value);
                } else {
                    crate::alphacode_core::env::remove_var(name);
                }
            }
        }
    }
    ...
    fn temporary_policy_requires_explicit_marker() {
        ...
        crate::alphacode_core::env::remove_var(TEMP_SERVER_ENV);
        crate::alphacode_core::env::remove_var(SERVER_SCOPE_ENV);
        crate::alphacode_core::env::set_var(OWNER_PID_ENV, "123");
    ...
    fn temporary_policy_reads_owner_and_timeout() {
        ...
        crate::alphacode_core::env::set_var(SERVER_SCOPE_ENV, "temporary");
        crate::alphacode_core::env::set_var(OWNER_PID_ENV, "123");
        crate::alphacode_core::env::set_var(TEMP_IDLE_SECS_ENV, "42");
    ...
    fn temporary_metadata_path_is_socket_scoped() {
        assert_eq!(
            metadata_path(Path::new("/tmp/example/alphacode.sock")),
            PathBuf::from("/tmp/example/alphacode.sock.server.json")
        );
    }

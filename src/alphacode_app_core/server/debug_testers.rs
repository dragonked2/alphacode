...
fn load_testers() -> Result<Vec<serde_json::Value>> {
    let path = crate::storage::alphacode_dir()?.join("testers.json");
    if path.exists() {
...
async fn spawn_tester(opts: serde_json::Value) -> Result<String> {
    ...
    let debug_cmd = std::env::temp_dir().join(format!("alphacode_debug_cmd_{}", id));
    let debug_resp = std::env::temp_dir().join(format!("alphacode_debug_response_{}", id));
    let stdout_path = std::env::temp_dir().join(format!("alphacode_tester_stdout_{}", id));
    let stderr_path = std::env::temp_dir().join(format!("alphacode_tester_stderr_{}", id));
    ...
    cmd.current_dir(cwd);
    cmd.env(crate::alphacode_selfdev_types::CLIENT_SELFDEV_ENV, "1");
    cmd.env(
        "ALPHACODE_DEBUG_CMD_PATH",
        debug_cmd.to_string_lossy().to_string(),
    );
    cmd.env(
        "ALPHACODE_DEBUG_RESPONSE_PATH",

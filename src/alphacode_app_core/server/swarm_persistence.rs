...
use crate::alphacode_swarm_core::{SwarmLifecycleStatus, SwarmMemberRecord};
...
/// Directory name under the durable state dir (`~/.alphacode/state`).
const SWARM_STATE_DIR: &str = "swarm";
/// Pre-0.36 location under the runtime dir (tmpfs on Linux, wiped on reboot).
const LEGACY_SWARM_STATE_DIR: &str = "alphacode-swarm-state";
...
fn dormant_plan_retention() -> Duration {
    Duration::from_secs(
        std::env::var("ALPHACODE_SWARM_DORMANT_PLAN_RETENTION_SECS")
            .ok()
...
/// `ALPHACODE_RUNTIME_DIR`. Never let those tests fall through to the real
/// `~/.alphacode/state/swarm`: that leaked synthetic `swarm-1` plans/members into
/// live user state during ordinary `cargo test` runs. Tests that need an
/// isolated explicit location still set `ALPHACODE_RUNTIME_DIR` and use the normal
/// resolver; otherwise use a process-local temp directory.
#[cfg(test)]
fn state_dir() -> PathBuf {
    if std::env::var_os("ALPHACODE_RUNTIME_DIR").is_some() {
        storage::durable_state_dir().join(SWARM_STATE_DIR)
    } else {
        std::env::temp_dir()
            .join(format!("alphacode-test-state-{}", std::process::id()))

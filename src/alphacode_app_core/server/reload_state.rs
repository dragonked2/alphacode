...
pub fn reload_marker_path() -> PathBuf {
    crate::storage::runtime_dir().join("alphacode.reload")
}
...
mod tests {
    ...
    impl EnvGuard {
        fn set_runtime_dir(path: &std::path::Path) -> Self {
            let key = "ALPHACODE_RUNTIME_DIR";
            let old = std::env::var_os(key);
            crate::alphacode_core::env::set_var(key, path);
            Self { key, old }
        }
    }
    ...
    impl Drop for EnvGuard {
        fn drop(&mut self) {
            if let Some(old) = &self.old {
                crate::alphacode_core::env::set_var(self.key, old);
            } else {
    ...
    async fn inspect_reload_wait_status_handles_repeated_ready_markers() {
        ...
        let temp = tempfile::tempdir().expect("tempdir");
        let _guard = EnvGuard::set_runtime_dir(temp.path());
        let socket_path = temp.path().join("alphacode.sock");

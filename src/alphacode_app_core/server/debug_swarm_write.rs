                ...
                Some(proposal) => {
                    if let Ok(items) = serde_json::from_str::<Vec<PlanItem>>(&proposal) {
                        let version = {
                            ...
                            if merged_count > crate::alphacode_plan::MAX_PLAN_ITEMS {
                                return Err(anyhow::anyhow!(
                                    "Plan approval would contain {} items, exceeding the per-swarm limit of {}",
                                    merged_count,
                                    crate::alphacode_plan::MAX_PLAN_ITEMS
                                ));
                            }
...
fn debug_specs(specs: Vec<DebugNodeSpec>) -> Vec<crate::alphacode_plan::dag::NodeSpec> {
    specs
        .into_iter()
        .map(|s| crate::alphacode_plan::dag::NodeSpec {
            id: Some(s.id),
            content: s.content,
            kind: crate::alphacode_plan::bridge::parse_kind(s.kind.as_deref()),
...
async fn handle_debug_graph_op(arg: &str, ctx: &DebugSwarmWriteContext<'_>) -> String {
    use crate::alphacode_plan::bridge::{apply_task_graph, to_task_graph};
...
mod tests {
    ...
    fn isolated_runtime(dir: &tempfile::TempDir) -> EnvGuard {
        ...
        let runtime = std::env::var_os("ALPHACODE_RUNTIME_DIR");
        crate::alphacode_core::env::set_var("ALPHACODE_RUNTIME_DIR", dir.path());
        EnvGuard {
            _lock: lock,
            runtime,
        }

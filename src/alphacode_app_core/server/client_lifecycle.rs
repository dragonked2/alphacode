...
use crate::alphacode_app_core::protocol::{Request, ServerEvent, decode_request, encode_event};
...
fn server_reload_starting() -> bool {
    matches!(
        crate::alphacode_app_core::server::recent_reload_state(RELOAD_STARTING_GUARD_MAX_AGE),
        Some(state) if state.phase == crate::alphacode_app_core::server::ReloadPhase::Starting
    )
}

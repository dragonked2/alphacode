use crate::alphacode_app_core::tool::selfdev::ReloadRecoveryDirective;
use anyhow::Result;
...
fn recovery_dir() -> Result<PathBuf> {
    Ok(crate::storage::alphacode_dir()?.join("reload-recovery"))
}
...
mod tests {
    ...
    impl IsolatedHome {
        fn new() -> Self {
            let temp = tempfile::TempDir::new().expect("alphacode home");

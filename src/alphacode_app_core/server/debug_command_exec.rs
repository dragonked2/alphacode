...
pub(super) fn debug_message_timeout_secs() -> Option<u64> {
    let raw = std::env::var("ALPHACODE_DEBUG_MESSAGE_TIMEOUT_SECS").ok()?;
...
pub(super) async fn execute_debug_command(
    ...
    if trimmed == "mcp" || trimmed == "mcp:servers" {
        ...
        let config_path = if let Ok(alphacode_dir) = crate::storage::alphacode_dir() {
            let path = alphacode_dir.join("mcp.json");
            if path.exists() {
        ...
        let default_model = match provider.as_str() {
            "claude" | "anthropic" => {
                if claude_usage_exhausted {
                    ...
                    crate::alphacode_provider_core::DEFAULT_CLAUDE_MODEL
                }
            }
            "openai" | "codex" => crate::alphacode_provider_core::DEFAULT_OPENAI_MODEL,
    ...
    if trimmed == "reload" {
        let repo_dir = crate::build::get_repo_dir()
            .ok_or_else(|| anyhow::anyhow!("Could not find alphacode repository directory"))?;

        let target_binary = crate::build::find_dev_binary(&repo_dir)
            .unwrap_or_else(|| build::release_binary_path(&repo_dir));
        if !target_binary.exists() {
            return Err(anyhow::anyhow!(format!(
                "No binary found at {}. Run 'alphacode self-dev --build' first, or build with 'scripts/dev_cargo.sh build --profile selfdev -p alphacode --bin alphacode' and publish current.",
        ...
        let info_path = alphacode_dir.join("reload-info");
...
mod tests {
    ...
    /// which set `ALPHACODE_HOME` under `storage::lock_test_env`). Two mutexes
    ...
    async fn debug_tool_selfdev_reload_returns_promptly_for_direct_execution() {
        // The reload path runs real `git` plumbing against the source checkout
        // (source state hashing, dev-binary smoke tests), so it can only run
        // where a git working tree is present. Copy-only workspaces (e.g. a
        // synced folder without `.git`) must skip instead of failing.
        if crate::build::get_repo_dir().is_none() {
            eprintln!("skipping: no alphacode git checkout available");
            return;
        }

        let _env_lock = lock_env();
        let _test_session = EnvGuard::set("ALPHACODE_TEST_SESSION", "1");
        let _debug_control = EnvGuard::set("ALPHACODE_DEBUG_CONTROL", "1");

        let mut reload_rx = crate::alphacode_app_core::server::subscribe_reload_signal_for_tests();
        ...
        let ack_task = tokio::spawn(async move {
            loop {
                if let Some(signal) = reload_rx.borrow_and_update().clone() {
                    crate::alphacode_app_core::server::acknowledge_reload_signal(&signal);
                    return;
                }
                reload_rx
                    .changed()

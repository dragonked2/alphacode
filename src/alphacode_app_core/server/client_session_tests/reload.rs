...
-> Result<()> {
    ...
    crate::alphacode_core::env::set_var("ALPHACODE_RUNTIME_DIR", runtime.path());
    ...
    crate::alphacode_app_core::server::clear_reload_marker();
...
fn mark_remote_reload_started_writes_starting_marker() -> Result<()> {
    ...
    let state = crate::alphacode_app_core::server::recent_reload_state(std::time::Duration::from_secs(5))
        .ok_or_else(|| anyhow!("reload state should exist"))?;
    assert_eq!(state.request_id, "reload-test");
    assert_eq!(state.phase, crate::alphacode_app_core::server::ReloadPhase::Starting);
...
fn handle_reload_queues_signal_for_canary_session() -> Result<()> {
    ...
    rt.block_on(async {
        let mut rx = crate::alphacode_app_core::server::subscribe_reload_signal_for_tests();
        ...
        assert_eq!(signal.hash, crate::alphacode_build_meta::git_hash());

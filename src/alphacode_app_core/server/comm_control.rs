...
fn deep_mode_assignment_content(
    ...
    if is_gate {
        ...
        let low_confidence_siblings: Vec<String> =
            crate::alphacode_plan::bridge::low_confidence_completed_ids(plan)
                .into_iter()
                .filter(|id| audited.contains(id.as_str()))
                .collect();
        crate::alphacode_swarm_core::append_deep_gate_instructions(
            content,
            item_id,
            &audited_ids,
        ...
        crate::alphacode_swarm_core::append_deep_node_instructions(content, item_id)
    }
}
...
async fn task_snapshot_for(
    ...
    let hydrated = crate::alphacode_plan::bridge::hydrate_assignment(plan, task_id, &item.content);
...
async fn next_runnable_task_id_reclaiming_stranded(
...
) -> Option<String> {
    {
        ...
        if let Some(plan) = plans.get(swarm_id)
            && let Some(task_id) = crate::plan::next_unassigned_runnable_item_id(plan)
        {
            return Some(task_id);
        }
    }

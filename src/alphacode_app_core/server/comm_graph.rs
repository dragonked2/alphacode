...
//! `crate::alphacode_plan::dag`. Each handler lifts the swarm's current `VersionedPlan` into
//! a `TaskGraph` (via `crate::alphacode_plan::bridge`), applies the engine op (which enforces
...
use crate::alphacode_plan::MAX_PLAN_ITEMS;
use crate::alphacode_plan::bridge::{apply_task_graph, parse_kind, to_task_graph};
use crate::alphacode_plan::dag::{self, HandoffArtifact, NodeSpec, NodeStatus, TaskGraph};

...
fn prepare_server_exec(cmd: &mut std::process::Command, socket_path: &std::path::Path) {
    ...
    crate::alphacode_app_core::server::cleanup_socket_pair(socket_path);
...
) -> Option<crate::alphacode_app_core::server::ReloadSignal> {
...
pub(super) async fn await_reload_signal(
    ...
    loop {
        ...
        if std::env::var("ALPHACODE_TEST_SESSION")
            .map(|value| {
        ...
        {
            crate::logging::info(
                "Server: ALPHACODE_TEST_SESSION set, skipping process exec for reload test",
            );
            continue;
        }
        ...
        super::reload_trace::record_value(
            ...
            serde_json::json!({
                ...
                "state": crate::alphacode_app_core::server::reload_state_summary(std::time::Duration::from_secs(60)),
            }),
        );

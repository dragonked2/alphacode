...
async fn ensure_client_swarm_member(
    ...
    if inserted && let Some(ref swarm_id_ref) = derived_swarm_id {
        ...
        super::record_swarm_event(
            ...
            crate::alphacode_app_core::server::SwarmEventType::MemberChange {
...
pub(super) async fn handle_reload(
    ...
    // A non-forced reload (e.g. `alphacode server reload`) is a graceful upgrade
    ...
    if !force && !super::server_has_newer_binary() {
        ...
        // so callers like `alphacode server reload` can report "already up to date"

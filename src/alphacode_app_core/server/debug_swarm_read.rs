...
use crate::alphacode_app_core::plan::{next_runnable_item_ids, summarize_plan_graph};
use anyhow::Result;
...
pub(super) async fn maybe_handle_swarm_read_command(
    ...
    if cmd.starts_with("swarm:id:") {
        ...
        let env_override = std::env::var("ALPHACODE_SWARM_ID")
            .ok()
        ...
        return Ok(Some(
            serde_json::json!({
                "path": path_str,
                "swarm_id": swarm_id,
                "source": if env_override.is_some() { "env:ALPHACODE_SWARM_ID" }

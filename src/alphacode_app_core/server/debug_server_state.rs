...
pub(super) async fn maybe_handle_server_state_command(
    ...
    shutdown_signals: &Arc<RwLock<HashMap<String, crate::alphacode_agent_runtime::InterruptSignal>>>,
    ...
    if cmd == "memory-judge" || cmd == "memory:judge" || cmd == "server:memory-judge" {
        ...
        return Ok(Some(
            serde_json::to_string_pretty(&crate::alphacode_base::memory_judge_metrics::snapshot())
                .unwrap_or_else(|_| "{}".to_string()),
        ));
    }
    ...
    let (summary, actions) = match decision.primary_cause {
        "runaway_live_session_population" => (
            ...
            vec![
                serde_json::json!({
                    ...
                    "commands": ["alphacode debug 'swarm:list'", "alphacode debug 'server:memory-incident'"]
                }),
                serde_json::json!({
                    "priority": 2,
                ...
                serde_json::json!({
                    ...
                    "commands": ["alphacode debug 'server:memory-incident'", "python scripts/analyze_runtime_memory_log.py --days 1"]
                }),
                serde_json::json!({
                    "priority": 4,
                    ...
                    "commands": ["alphacode debug 'allocator:purge'"]
                }),
            ],
        ),
        "allocator_retention" => (
            ...
            vec![
                serde_json::json!({
                    ...
                    "commands": ["alphacode debug 'allocator:purge'", "alphacode debug 'server:memory-incident'"]
                }),
                serde_json::json!({
                    "priority": 2,
                    "action": "If retention repeatedly regrows, inspect allocation churn and allocator decay settings.",
                    "commands": ["alphacode debug 'allocator'", "alphacode debug 'allocator:decay:1000'"]
                }),
            ],
        ),
        "unattributed_live_heap" => (
            ...
            vec![
                serde_json::json!({
                    ...
                    "commands": ["alphacode debug 'server:memory'", "python scripts/analyze_runtime_memory_log.py --days 1"]
                }),
                serde_json::json!({
                    "priority": 2,
                    "action": "Escalate to a jemalloc-prof build and heap dump if attribution remains below 50% of live heap.",
                    "commands": ["alphacode debug 'allocator:profile:on'", "alphacode debug 'allocator:profile:dump /tmp/alphacode-server.heap'"]
                }),
            ],
        ),
        ...
        _ => (
            ...
            vec![serde_json::json!({
                ...
                "commands": ["alphacode debug 'server:memory-incident'"]
            })],
        ),
    };

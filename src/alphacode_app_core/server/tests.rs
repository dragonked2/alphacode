...
use crate::alphacode_app_core::message::{Message, Role, StreamEvent, ToolDefinition};
...
async fn removing_server_session_clears_active_pid_marker() {
    ...
    let home = tempfile::tempdir().expect("create temporary ALPHACODE_HOME");
    let _home_guard = ScopedEnvVar::set("ALPHACODE_HOME", home.path());
...
fn server_identity_reads_name_from_env() {
    let _guard = crate::storage::lock_test_env();
    let _server_name_guard = ScopedEnvVar::set("ALPHACODE_SERVER_NAME", "mount-cloud/john");
    let _server_display_name_guard = ScopedEnvVar::set("ALPHACODE_SERVER_DISPLAY_NAME", "ignored");
...
impl Drop for EnvGuard {
    fn drop(&mut self) {
        if let Some(value) = &self.prev_home {
            crate::alphacode_core::env::set_var("ALPHACODE_HOME", value);
        } else {
            crate::alphacode_core::env::remove_var("ALPHACODE_HOME");
        }
        if let Some(value) = &self.prev_runtime_dir {
            crate::alphacode_core::env::set_var("ALPHACODE_RUNTIME_DIR", value);
        } else {
            crate::alphacode_core::env::remove_var("ALPHACODE_RUNTIME_DIR");
        }
        if let Some(value) = &self.prev_socket {
            crate::alphacode_core::env::set_var("ALPHACODE_SOCKET", value);
        } else {
            crate::alphacode_core::env::remove_var("ALPHACODE_SOCKET");
        }
    }
}
...
fn configure_test_env(root: &tempfile::TempDir) -> EnvGuard {
    ...
    let prev_runtime_dir = std::env::var_os("ALPHACODE_RUNTIME_DIR");
    let prev_socket = std::env::var_os("ALPHACODE_SOCKET");
    let home_dir = root.path().join("home");
    let runtime_dir = root.path().join("runtime");
    std::fs::create_dir_all(&home_dir).expect("create home dir");
    std::fs::create_dir_all(&runtime_dir).expect("create runtime dir");
    crate::alphacode_core::env::set_var("ALPHACODE_HOME", &home_dir);
    crate::alphacode_core::env::set_var("ALPHACODE_RUNTIME_DIR", &runtime_dir);
    crate::alphacode_core::env::remove_var("ALPHACODE_SOCKET");
    EnvGuard {
        prev_home,
        prev_runtime_dir,
...
#[allow(
    ...
    reason = "test intentionally serializes process-wide ALPHACODE_HOME/env state across async recovery assertions"
)]
...
#[allow(
    clippy::await_holding_lock,
    reason = "test intentionally serializes process-wide ALPHACODE_HOME/env state across async startup assertions"
)]
async fn startup_ready_signal_is_not_blocked_by_headless_recovery_delay() -> Result<()> {
    ...
    let _ready_fd_guard = ScopedEnvVar::set("ALPHACODE_READY_FD", write_fd.to_string());

...
impl Agent {
    pub(super) async fn run_turn_streaming_mpsc(
        ...
        // Health monitor: the agent loop is the heartbeat's owner, so it
        // must refresh the subsystem timestamp whenever a turn starts. This
        // is one atomic store per turn (the streaming loop also touches
        // it on every event via `health::heartbeat("agent_loop")`).
        crate::alphacode_app_core::health::heartbeat("agent_loop");
        ...
        // Cache static system prompt and tool definitions across turn-loop
        ...
        loop {
                        ...
                        result = &mut complete_future => {
                            match result {
                                ...
                                Err(e) => {
                                    ...
                                    // injection so the agent keeps working
                                    // instead of stalling on a single blip.
                                    // The helper enforces the retry budget.
            ...
            loop {
                ...
                if activity_mark_last.elapsed() >= std::time::Duration::from_secs(2) {
                    ...
                    // Refresh the health monitor's "agent_loop" timestamp at
                    // the same cadence as the existing activity marker so a
                    // hung stream is visible in the next health snapshot.
                    crate::alphacode_app_core::health::heartbeat("agent_loop");
                }
            ...
            for tool_index in 0..tool_count {
                ...
                // Normalize before validation (see turn_loops.rs): stringified
                // JSON, wrapped arrays, bare URLs, null/empty all become objects
                // so recoverable calls execute instead of hitting validation.
                {
                    let normalized =
                        ToolCall::normalize_input_to_object(tool_calls[tool_index].input.clone());
                    tool_calls[tool_index].input = normalized;
                    tool_calls[tool_index].refresh_intent_from_input();
                }

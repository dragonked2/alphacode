...
impl Agent {
    ...
    ///
    /// Returns `Ok(None)` (i.e. *do not* retry) for non-transient errors
    ...
    /// Returns `Ok(Some(delay_secs))` when the error is transient and the
    /// caller should sleep `delay_secs` then retry the exact same request.
    /// The delay is computed exactly once here so the system-reminder text,
    /// the status line, and the actual sleep always agree (previously the
    /// caller recomputed with fresh jitter and showed 10s while sleeping
    /// 11s, and duplicate attempt counters appeared across loops).

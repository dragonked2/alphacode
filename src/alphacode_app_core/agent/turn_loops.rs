...
impl Agent {
    ...
    /// tool results. The counter is per turn-loop, so a genuinely finished
    /// agent still exits promptly.
    pub(crate) const MAX_EMPTY_POST_TOOL_CONTINUATION_ATTEMPTS: u32 = 5;
    /// Interval (in tool iterations) between periodic session auto-saves.
    /// During long-running turns with many tool calls, this ensures progress
    ...
    pub(crate) const PROACTIVE_COMPACTION_THRESHOLD: f32 = 0.85;
    ...
    pub(super) async fn run_turn(&mut self, print_output: bool) -> Result<String> {
        ...
        // Counter for retries injected after a transient provider / transport
        // error (5xx, 429 with rate-limit words, network EOF, ...). Bounded
        // by Agent::MAX_PROVIDER_ERROR_CONTINUATION_ATTEMPTS so a permanently
        // broken provider cannot keep an autonomous session running forever.
        let mut provider_error_continuations = 0u32;
        // Cache the static system prompt and tool definitions across turn-loop
        // iterations. Within a single user turn, the model receives tool results
        ...
        loop {
            ...
            // a crash or power loss during a long turn loses at most N tool
            // results instead of the entire turn. This is critical for
            ...
            // hitting the hard context limit mid-turn. This prevents the
            // dreaded "context limit exceeded" error during critical work.
            if self.provider.uses_alphacode_compaction() && iteration_count > 1 {
                self.maybe_proactive_compact();
            }
            ...
            if let Some(event) = compaction_event {
                ...
                // Compaction changes the message history, so invalidate cached prompt/tools
                cached_static_prompt = None;
                cached_tools = None;
                if print_output {
                ...
                .await
            {
                ...
                Err(e) => {
                    ...
                    // the helper itself enforces the retry budget. Non-
                    // transient errors fall through to the context-limit and
                    // final-return paths below exactly as before.
                    if let Some(delay) = self.maybe_continue_after_provider_error(
                        &err_text,
            ...
            for mut tc in tool_calls {
                // Normalize provider quirks (stringified JSON, single-wrapped
                // array, bare URL, null/empty) *before* validation so a
                // recoverable httpflow/webfetch/browser call executes instead
                // of failing with "arguments must be a JSON object".
                tc.input = ToolCall::normalize_input_to_object(tc.input);
                ...
                let is_native_tool = ALPHACODE_NATIVE_TOOLS.contains(&tc.name.as_str());
            ...
            // Rate-limit protection: add a minimum delay between consecutive API
            // calls within the same turn. Free models on OpenRouter have strict
            // per-minute limits (~10 RPM). Without this delay, the agent burns
            // through the quota in seconds by calling the API immediately after
            // each tool execution. The delay is short enough to be unnoticeable
            // to the user but prevents hammering the provider.

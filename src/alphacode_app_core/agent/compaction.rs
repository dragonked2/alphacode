...
impl Agent {
    ...
    fn effective_context_tokens_from_usage(
        ...
        // Shared heuristic (alphacode-compaction-core): keeps the compaction
    ...
    pub(super) fn update_compaction_usage_from_stream(
        ...
        if !self.provider.uses_alphacode_compaction() || input_tokens == 0 {
            return;
        }
    ...
    /// Proactively compact context when usage exceeds the threshold.
    /// Called during long turns to prevent hitting the hard context limit.
    /// Returns true if compaction was triggered.
    pub(super) fn maybe_proactive_compact(&mut self) -> bool {
        let budget = self.provider.context_window();
        if budget == 0 {
            return false;
        }
        let compaction = self.registry.compaction();
        let should_compact = match compaction.try_read() {
            Ok(manager) => {
                let usage = manager.context_usage_with(self.session.provider_messages());
                usage >= Self::PROACTIVE_COMPACTION_THRESHOLD
            }
            Err(_) => return false,
        };
        if !should_compact {
        ...
        // Trigger background compaction to keep context healthy.
        let provider = self.provider.fork();
        let messages = self.session.messages_for_provider();
        if let Ok(mut manager) = compaction.try_write() {
            match manager.force_compact_with(&messages, provider) {
                Ok(()) => {
                    logging::info("Proactive compaction triggered: context usage above threshold");
                    true
                }
                Err(reason) => {
    ...
    pub(super) fn push_embedding_snapshot_if_semantic(&mut self, text: &str) {
        use crate::alphacode_app_core::config::CompactionMode;
        let is_semantic = {
            let compaction = self.registry.compaction();
            compaction
                .try_read()

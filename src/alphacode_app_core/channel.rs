...
impl MessageChannel for JadeRelayChannel {
    ...
    fn is_reply_enabled(&self) -> bool {
        ...
        // Alphacode session directly. Keep this channel for outbound notifications
    ...
    async fn reply_loop(&self, runner: AmbientRunnerHandle) {
        ...
        let device_id = format!("alphacode-{}", host);
        logging::info(&format!(
...
mod tests {
    ...
    ///   cargo test -p alphacode-app-core relay_live -- --ignored --nocapture
    #[tokio::test]
    ...
    async fn test_relay_live_roundtrip() {
        ...
        ch.heartbeat("alphacode-test-device").await;

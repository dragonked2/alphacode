...
//!   notice that it renders alphacode poorly and suggest a modern terminal (Ghostty).
//! - Linux: create a .desktop launcher file.
//!
//! Each nudge can be dismissed permanently with "Don't ask again".
//! State is persisted in `~/.alphacode/setup_hints.json`.
...
///   connection.
/// - 3: register three launch hotkeys instead of one. `Cmd+;` opens alphacode in
///   `$HOME`, `Cmd+'` opens it in the last project directory, and `Cmd+Shift+'`
///   opens a self-dev session in the last alphacode repo. Existing users are
...
///   `alphacode-terminal-launch`, avoiding the generated shell-script hop on hotkey
///   press. Scripts/plan are still written for compatibility and diagnostics.
/// - 6: direct launches pass `--spawn-hotkey` into the new Alphacode process so
...
impl SetupHintsState {
    fn path() -> Result<PathBuf> {
        Ok(storage::alphacode_dir()?.join("setup_hints.json"))
    }
    ...
    #[cfg_attr(
        ...
        allow(
            dead_code,
            reason = "only used by nudge code paths we no longer call on first run"
        )
    )]
    ...
    #[cfg_attr(
        ...
        allow(
            dead_code,
            reason = "nudge code paths no longer called from first-run flow"
        )
    )]
...
fn mac_hotkey_support_dir() -> Result<PathBuf> {
    Ok(storage::alphacode_dir()?.join("hotkey"))
}

/// File holding the last project directory alphacode was launched from. The `Cmd+'`
/// global hotkey reads this at fire time to reopen alphacode there.
#[cfg(any(test, target_os = "macos", target_os = "linux", windows))]
fn mac_hotkey_last_dir_file() -> Result<PathBuf> {
    Ok(mac_hotkey_support_dir()?.join("last_dir"))
}

/// File holding the last alphacode *repository* directory the user worked in. The
...
/// Load the `[launch_hotkeys]` table from `~/.alphacode/config.toml`.
///
...
fn load_launch_hotkeys_config() -> crate::alphacode_config_types::LaunchHotkeysConfig {
    #[derive(serde::Deserialize, Default)]
    struct Wrapper {
        #[serde(default)]
        launch_hotkeys: crate::alphacode_config_types::LaunchHotkeysConfig,
    }
    let Ok(dir) = storage::alphacode_dir() else {
...
/// its own `Cmd+;` hotkey). When `dir` is inside a alphacode repo, the repo root is
...
fn mac_hotkey_launch_agent_path() -> Result<PathBuf> {
    ...
    Ok(home
        .join("Library")
        .join("LaunchAgents")
        .join("com.alphacode.hotkey.plist"))
}
...
<dict>
    <key>Label</key>
    <string>com.alphacode.hotkey</string>
    ...
    <dict>
        <key>ALPHACODE_PREFERRED_TERMINAL</key>
...
/// Launch a new alphacode window in the user's preferred macOS terminal, passing
/// `extra_args` (e.g. `["--resume", "<session-id>"]`) to the alphacode invocation.
///
...
#[cfg(target_os = "macos")]
pub fn launch_alphacode_in_macos_terminal(extra_args: &[String]) -> Result<()> {
    ...
    let command = match macos_terminal::no_automation_launch(terminal, &shell_command) {
        ...
        macos_terminal::NoAutomationLaunch::CommandFile { app } => {
            let dir = storage::alphacode_dir()?.join("launcher");
    ...
    let status = std::process::Command::new("sh")
        ...
        .arg(&command)
        .status()
        .context("failed to launch terminal for alphacode")?;
    if !status.success() {
        anyhow::bail!(
...
fn install_macos_hotkey_listener(
    ...
    let status = std::process::Command::new("launchctl")
        ...
        .status()
        .context("failed to load alphacode LaunchAgent")?;
    if !status.success() {
...
fn startup_hints_for_launch(_state: &SetupHintsState) -> Option<StartupHints> {
    ...
    let spawn_notice = if !_state.hotkey_configured || _state.startup_spawn_hint_dismissed {
        ...
        Some(format!(
            "Cmd+; launches a new alphacode in your home directory from anywhere, system-wide (opens in {}). Cmd+' reopens your last project; Cmd+Shift+' opens a self-dev session.",
...
/// Manual `alphacode setup-hotkey` command.
///
...
pub fn run_setup_hotkey(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let terminal = effective_macos_terminal();
        eprintln!("\x1b[1malphacode setup-hotkey\x1b[0m");
        eprintln!();
        eprintln!("  Preferred terminal: {}", terminal.label());
        eprintln!("  Installing a LaunchAgent with three system-wide alphacode launch hotkeys.");
        eprintln!();
        ...
        match install_macos_hotkey_listener(Some(terminal)) {
            Ok(installed_terminal) => {
                ...
                let _ = state.save();
                eprintln!(
                    "  \x1b[32m✓\x1b[0m Created launch hotkeys → {} + alphacode",
        ...
        if let Some(comp) = detect_linux_compositor() {
            ...
            match install_linux_launch_hotkeys(comp) {
                Ok(changed) => {
                    if changed {
                        eprintln!(
                            "  \x1b[32m✓\x1b[0m Installed alphacode launch hotkeys into your {} config.",
                            comp.name()
                        );
                    } else {
                        eprintln!(
                            "  \x1b[32m✓\x1b[0m alphacode launch hotkeys already up to date in your {} config.",
                            comp.name()
                        );
                    }
                    state.hotkey_configured = true;
...
pub(crate) fn active_primary_launch_hotkey() -> Option<(String, String)> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let exe_path = std::env::current_exe()
            ...
            .unwrap_or_else(|_| "alphacode".to_string());
...
fn run_macos_hotkey_listener() -> Result<()> {
    ...
    // launches alphacode on key-down, then hand the thread to the event loop so the
    ...
    // standard alphacode log plus the plist's StandardOut/ErrorPath.
    crate::alphacode_logging::init();
    macos_hotkey_log("starting macOS alphacode launch hotkey listener");
    ...
    // to a cwd + alphacode argv. Older versions dispatched through generated shell
    ...
    if launch_for_id.is_empty() {
        anyhow::bail!("failed to register any alphacode launch hotkey");
    }
    ...
    GlobalHotKeyEvent::set_event_handler(Some(move |event: GlobalHotKeyEvent| {
        ...
        if let Some(launch) = launch_for_id.get(&event.id) {
            ...
            let command = TerminalCommand::new(&exe_path, args)
                .fresh_spawn()
                .kind("hotkey")
                .spawn_env("ALPHACODE_SPAWN_LABEL", launch.label.clone());
            match spawn_command_in_new_terminal_with(&command, &cwd, |cmd| cmd.spawn().map(|_| ()))
            {
                Ok(true) => {}
                Ok(false) => {
                    macos_hotkey_log("failed to launch alphacode: no terminal candidate worked")
                }
                Err(err) => macos_hotkey_log(&format!("failed to launch alphacode: {err}")),
            }
        }
    }));

    macos_hotkey_log("macOS alphacode launch hotkeys registered; entering event loop");
    ...
    macos_hotkey_log("macOS alphacode launch hotkey event loop exited");
    Ok(())
}
...
/// Log a hotkey-listener diagnostic to both the alphacode log and stderr.
///
...
fn macos_hotkey_log(message: &str) {
    crate::alphacode_logging::info(message);
    eprintln!("[alphacode hotkey] {message}");
}
...
pub fn maybe_show_setup_hints() -> Option<StartupHints> {
                ...
                match install_linux_launch_hotkeys(comp) {
                    Ok(_) => {
                        ...
                        if action == LinuxHotkeySetupAction::Install {
                            crate::alphacode_logging::info(&format!(
    ...
    // `alphacode setup-launcher` explicitly.
...
fn macos_launch_hotkeys_notice(state: &SetupHintsState) -> Option<StartupHints> {
    ...
    Some(StartupHints::with_status_and_display(
        ...
        "Launch hotkeys",
        format!("Configured Alphacode launch hotkeys:\n{}", lines.join("\n")),
    ))
}
...
/// Config file alphacode manages for a flat (`#`-commented) compositor config.
...
/// Directory for alphacode's hidden KDE launcher desktop files.
#[cfg(target_os = "linux")]
...
/// The config file alphacode would manage for the *current* session's compositor.
#[cfg(target_os = "linux")]
...
/// The sentinel that marks alphacode's managed region in `path` for `comp`.
#[cfg(target_os = "linux")]
...
/// Whether alphacode's launch hotkeys are already installed for `comp`.
#[cfg(target_os = "linux")]
fn linux_hotkeys_installed(comp: linux_env::LinuxCompositor) -> bool {
    ...
    match comp {
        ...
        LinuxCompositor::Xfce => xfce_shortcut_commands_text().contains("/launch_alphacode_"),
        LinuxCompositor::Kde => kde_globalshortcutsrc_path()
            .and_then(|p| std::fs::read_to_string(p).ok())
            .map(|text| text.contains("[services][alphacode-launch-"))
            .unwrap_or(false),
...
/// Pick a terminal emulator to launch alphacode in on Linux. Honors `$TERMINAL`,
...
/// scripts written under `~/.alphacode/hotkey/`, so the config never embeds shell
...
/// xfconf-query. Stale alphacode entries bound to accelerators we no longer use
...
fn install_xfce_launch_hotkeys() -> Result<bool> {
    ...
    // Remove stale alphacode entries bound to accelerators we no longer use.
    for line in existing.lines() {
        let Some((prop, value)) = line.split_once(char::is_whitespace) else {
            continue;
        };
        if value.trim().contains("/launch_alphacode_") && !wanted.iter().any(|(p, _)| p == prop) {
...
/// `~/.alphacode/hotkey/` and return the chord -> script binds. The scripts `cd`
/// into the target (with `$HOME` fallback for stale/dynamic dirs) and exec the
/// user's terminal running alphacode, so compositor bind lines stay trivial.
#[cfg(target_os = "linux")]
fn write_linux_launch_scripts() -> Result<Vec<linux_env::ScriptBind>> {
    ...
    for entry in resolved {
        ...
        let script_body = format!(
            "#!/bin/sh\n# Auto-generated by alphacode setup-hotkey; re-run it to refresh.\n{cd}exec {exec}\n",
            cd = entry.cd_prefix,
        );
        let script_path = hotkey_dir.join(&entry.script_file_name);
...
/// Timestamped backup matching the `.bak-alphacode-*` convention, taken before
...
fn backup_compositor_config(config_path: &std::path::Path) {
    ...
    let backup = config_path.with_file_name(format!("{file_name}.bak-alphacode-hotkeys-{ts}"));
...
fn reload_compositor_config(comp: linux_env::LinuxCompositor) {
        ...
        .status()
    {
        ...
        Ok(status) => crate::alphacode_logging::warn(&format!(
...
fn linux_launch_hotkeys_notice(state: &SetupHintsState) -> Option<StartupHints> {
    ...
    Some(StartupHints::with_status_and_display(
        ...
        "Launch hotkeys",
        format!(
            "Configured Alphacode launch hotkeys ({}): {} {}",
            comp.name(),
            lines.join("; "),
            footer
        ),
...
/// - Once the user has learned at least one binding and has launched alphacode at
...
/// Check whether alphacode's keybindings conflict with shortcuts owned by the
...
pub fn maybe_show_keymap_conflict_hint(
    keybindings: &crate::alphacode_config_types::KeybindingsConfig,
) -> Option<StartupHints> {
    if !io::stdin().is_terminal() || !io::stderr().is_terminal() {
        return None;
    }
...
/// Whether the current terminal triggers alphacode's glyph-safe color quantization
/// (macOS VS Code integrated terminal / Apple Terminal). Mirrors the detection
/// in `alphacode-tui-style`'s color module and `alphacode-app-core::perf` so the
/// disclosure fires exactly when the behavior is active. Overridable with
/// `ALPHACODE_GLYPH_SAFE_MODE=on|off`.
...
/// shown the first time alphacode launches in a fragile-glyph terminal. Discloses
...
its glyph cache under alphacode's full-color animations, rendering letters as boxes. \
alphacode automatically quantizes colors to the 256-palette here to keep text readable; \
the only tradeoff is slightly reduced color fidelity. Animations still run. \
For full color, use Ghostty, iTerm2, kitty, or WezTerm, or set ALPHACODE_GLYPH_SAFE_MODE=off."
        .to_string();
    (
        Some(StartupHints::with_status_and_display(
...
/// Manual `alphacode setup-launcher` command.
pub fn run_setup_launcher() -> Result<()> {
    #[cfg(target_os = "macos")]
    {
        let mut state = SetupHintsState::load();
        eprintln!("\x1b[1malphacode setup-launcher\x1b[0m");
        eprintln!();
        ...
        match install_macos_app_launcher() {
            Ok((app_dir, terminal)) => {
                ...
                eprintln!(
                    "  \x1b[32m✓\x1b[0m Spotlight/Launchpad/Dock will launch alphacode in {}",
                    terminal.label()
                );
                eprintln!();
                eprintln!("  Tip: pin Alphacode.app to your Dock or launch it with Cmd+Space.");
                return Ok(());
            }
            Err(e) => {
                eprintln!("  \x1b[31m✗\x1b[0m Failed: {}", e);
    ...
    #[cfg(windows)]
    {
        ...
        match create_windows_desktop_shortcut(&mut state) {
            Ok(()) => {
                eprintln!("  \x1b[32m✓\x1b[0m Created desktop shortcut: alphacode.lnk");
                Ok(())
            }
            Err(e) => {
                eprintln!("  \x1b[31m✗\x1b[0m Failed: {}", e);
...
/// Create a desktop shortcut/launcher for alphacode.
///
/// - macOS: creates a alphacode.app bundle in ~/Applications/
/// - Windows uses [`windows_setup::create_windows_desktop_shortcut`] via
///   `alphacode setup-launcher` instead (PowerShell/COM is too slow for the
///   startup path).
...
fn create_desktop_shortcut(state: &mut SetupHintsState) -> Result<()> {
    #[cfg(any(test, target_os = "macos"))]
    {
        ...
        crate::alphacode_logging::info(&format!("Created macOS app bundle: {}", app_dir.display()));
    }
...
pub fn reinstall_launch_hotkeys_after_config_change() {
    #[cfg(target_os = "macos")]
    {
        ...
        match install_macos_hotkey_listener(preferred) {
            Ok(terminal) => crate::alphacode_logging::info(&format!(
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        match install_linux_launch_hotkeys(comp) {
            Ok(true) => crate::alphacode_logging::info(&format!(

use super::{
    ...
    escape_shell_single_quotes, launch_command_for_macos_terminal, paused_alphacode_shell_command,
    save_preferred_macos_terminal,
};
use anyhow::{Context, Result};
use std::path::{Path, PathBuf};

const MACOS_APP_ICON_FILE_NAME: &str = "Alphacode.icns";
#[cfg(target_os = "macos")]
...
pub(super) fn install_macos_app_launcher() -> Result<(PathBuf, MacTerminalKind)> {
    ...
    let launcher_path = macos_dir.join("alphacode-launcher");
...
<dict>
    ...
    <string>com.alphacode.launcher</string>
    <key>CFBundleVersion</key>
    <string>{version}</string>
    <key>CFBundleShortVersionString</key>
    <string>{version}</string>
    <key>CFBundleExecutable</key>
    <string>alphacode-launcher</string>
...
fn macos_app_launcher_dir() -> Result<PathBuf> {
    ...
    Ok(home.join("Applications").join("Alphacode.app"))
}
...
/// `Path::exists()` on `alphacode.app` also matches `Alphacode.app`. The legacy-bundle
...
fn macos_launcher_script(terminal: MacTerminalKind, exe_path: &str, app_dir: &Path) -> String {
    ...
    let shell_command = paused_alphacode_shell_command(exe_path);
    ...
    let missing_message = escape_applescript_text(&format!(
        "Alphacode could not launch because the executable was not found.\n\nExpected path:\n{}\n\nTry reinstalling alphacode or rerun:\nalphacode setup-launcher",
        exe_path
    ));
    let terminal_failure_message = escape_applescript_text(&format!(
        "Alphacode could not open {}.\n\nTry rerunning:\nalphacode setup-launcher\n\nLauncher log:\n~/.alphacode/launcher/macos-launcher.log",
        terminal.label()
    ));
...
LOG_DIR="$HOME/.alphacode/launcher"
LOG_FILE="$LOG_DIR/macos-launcher.log"
mkdir -p "$LOG_DIR" >/dev/null 2>&1 || true

show_missing_executable() {{
  /usr/bin/osascript <<'APPLESCRIPT' >/dev/null 2>&1 || true
display alert "Alphacode launch failed" message "{missing_message}" as critical
APPLESCRIPT
}}

show_terminal_launch_failure() {{
  /usr/bin/osascript <<'APPLESCRIPT' >/dev/null 2>&1 || true
display alert "Alphacode launch failed" message "{terminal_failure_message}" as critical
APPLESCRIPT
}}

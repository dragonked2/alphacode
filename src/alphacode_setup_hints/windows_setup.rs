...
fn default_windows_launch_entries() -> Vec<LaunchHotkeyEntry> {
    let mut entries = crate::alphacode_setup_hints::launch_hotkeys::default_launch_entries();
...
Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
  Where-Object {{
    ($_.ProcessId -ne $current) -and ($_.ProcessId -ne $PID) -and (
      (($_.Name -eq 'powershell.exe' -or $_.Name -eq 'pwsh.exe') -and $_.CommandLine -like '*alphacode-hotkey*') -or
      (($_.Name -eq 'alphacode.exe') -and $_.CommandLine -like '*--listen-windows-hotkey*')
    )
  }} |
...
$shortcut.Description = 'alphacode global launch hotkey listener'
...
pub(super) fn uninstall_windows_hotkey_listener() -> Result<()> {
    ...
    state.save()?;
    eprintln!("  \x1b[32m✓\x1b[0m Removed alphacode Windows launch-hotkey listener");
    Ok(())
}
...
fn create_hotkey_shortcut(_use_alacritty: bool) -> Result<()> {
    ...
    let hotkey_dir = storage::alphacode_dir()?.join("hotkey");
...
fn launch_windows_hotkey(entry: &WindowsHotkey) -> Result<()> {
    ...
    let command = crate::alphacode_terminal_launch::TerminalCommand::new(exe, args)
        .title("alphacode")
        .fresh_spawn()
        .kind("launch-hotkey");
...
fn windows_native_hotkey_loop(entries: Vec<WindowsHotkey>) -> Result<()> {
    ...
    const MUTEX_NAME: &str = "Local\\AlphacodeLaunchHotkeyListener";
            ...
            if let Some(entry) = entry.cloned() {
                std::thread::spawn(move || {
                    if let Err(err) = launch_windows_hotkey(&entry) {
                        crate::alphacode_logging::warn(&format!(
                            "failed to launch alphacode from Windows hotkey: {err}"
                        ));
                    }
                });
            }
...
/// interactive `alphacode setup-hotkey` flow to install it.
...
fn nudge_hotkey(state: &mut SetupHintsState) -> bool {
    ...
    eprintln!(
        "\x1b[36m│\x1b[0m \x1b[1m💡 Set up global keys to launch alphacode?\x1b[0m                  \x1b[36m│\x1b[0m"
    );
    eprintln!(
    ...
    eprintln!(
        "\x1b[36m│\x1b[0m    Opens alphacode in {:<39}    \x1b[36m│\x1b[0m",
        ...
        "y" | "yes" => {
            ...
            match create_hotkey_shortcut(using_alacritty) {
                Ok(()) => {
                    ...
                    let _ = state.save();
                    eprintln!(
                        "  \x1b[32m✓\x1b[0m Created hotkeys (\x1b[1mAlt+;\x1b[0m and \x1b[1mCopilot\x1b[0m) → {} + alphacode",
                        terminal_name
                    );
                    eprintln!();
                    true
                ...
                Err(e) => {
                    ...
                    eprintln!(
                        "    You can set it up manually later with: \x1b[1malphacode setup-hotkey\x1b[0m"
                    );
                    eprintln!();
                    false
                }
...
fn nudge_alacritty(state: &mut SetupHintsState) -> bool {
    ...
    eprintln!("\x1b[36m┌─────────────────────────────────────────────────────────────┐\x1b[0m");
    eprintln!(
        "\x1b[36m│\x1b[0m \x1b[1m💡 Alacritty: the fastest terminal for alphacode\x1b[0m               \x1b[36m│\x1b[0m"
    );
    eprintln!(
        "\x1b[36m│\x1b[0m                                                             \x1b[36m│\x1b[0m"
    );
                ...
                Ok(()) => {
                    ...
                    if state.hotkey_configured {
                        ...
                        match create_hotkey_shortcut(true) {
                            Ok(()) => {
                                eprintln!(
                                    "  \x1b[32m✓\x1b[0m Hotkeys updated: \x1b[1mAlt+;\x1b[0m and \x1b[1mCopilot\x1b[0m → Alacritty + alphacode"
                                );
                            }
                            Err(e) => {
...
fn prompt_try_it_out(installed_alacritty: bool) {
    ...
    eprintln!(
        "\x1b[32m│\x1b[0m    Press \x1b[1mAlt+;\x1b[0m or the \x1b[1mCopilot key\x1b[0m to launch alphacode.       \x1b[32m│\x1b[0m"
    );
    eprintln!(
        "\x1b[32m│\x1b[0m    The listener is native Alphacode, no AutoHotkey required.          \x1b[32m│\x1b[0m"
    );
    ...
    eprintln!(
        "\x1b[32m│\x1b[0m    \x1b[90m(Starting alphacode normally in 3 seconds...)\x1b[0m                 \x1b[32m│\x1b[0m"
    );
    eprintln!("\x1b[32m└─────────────────────────────────────────────────────────────┘\x1b[0m");
    eprintln!();
...
pub(super) fn run_setup_hotkey_windows() -> Result<()> {
    ...
    eprintln!(
        "  Setting up global launch hotkeys → {} + alphacode...",
        terminal_name
    );
...
pub(super) fn create_windows_desktop_shortcut(state: &mut SetupHintsState) -> Result<()> {
    ...
    let shortcut_path = format!("{}\\Desktop\\alphacode.lnk", desktop_dir);
...
$shortcut.Description = "alphacode - AI coding agent"
$shortcut.Save()
Write-Output "OK"
"#,
    ...
    if output.status.success() {
        ...
        if stdout.contains("OK") {
            ...
            let _ = state.save();
            crate::alphacode_logging::info(&format!("Created desktop shortcut: {}", shortcut_path));
        }
    }
...
mod tests {
    ...
    fn default_windows_hotkeys_are_shared_defaults_plus_copilot_key() {
        ...
        let shared = crate::alphacode_setup_hints::launch_hotkeys::default_launch_entries();
    ...
    #[test]
    fn listener_stop_sweep_excludes_alphacode_and_powershell_processes_running_it() {
    ...
    fn startup_shortcut_uses_native_listener_without_vbscript_or_bypass() {
        let script = render_startup_shortcut_script(
            Path::new(r"C:\Users\O'Hara\Startup\alphacode-hotkey.lnk"),
            Path::new(r"C:\Program Files\Alphacode O'Hara\alphacode.exe"),
        );

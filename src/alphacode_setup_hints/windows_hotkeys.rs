...
//! low-level keyboard hook in the first-party Rust listener (`alphacode setup-hotkey
...
/// Returns `None` for keys with no stable VK code. alphacode `cmd` maps to Alt
...
/// Translate a canonical alphacode key token into a Win32 virtual-key code.
...
/// User-facing chord rendering for Windows notices: alphacode's `cmd` modifier
...
fn ps_dir_expr(dir: &str, last_dir_file: &str, last_repo_file: &str) -> String {
    match dir {
        "$HOME" => "(Resolve-AlphacodeDir $null)".to_string(),
        "$LAST_DIR" => format!("(Resolve-AlphacodeDir {})", ps_quote(last_dir_file)),
        "$LAST_REPO" => format!("(Resolve-AlphacodeDir {})", ps_quote(last_repo_file)),
        path => format!("(Resolve-AlphacodeFixedDir {})", ps_quote(path)),
    }
}

/// Render the full hotkey-listener PowerShell script.
///
/// `launch_exe`/`launch_args` describe the terminal command that opens alphacode
/// (e.g. `wt.exe` + profile args, or `alacritty -e alphacode`); `{DIR}` never
...
pub(crate) fn render_windows_listener_ps1(
    ...
    for (index, entry) in entries.iter().enumerate() {
        ...
        registrations.push_str(&format!(
            "Register-AlphacodeHotkey -Id 0x{id:X} -Mods 0x{mods:X} -Vk 0x{vk:X} -Label {label}\n",
    ...
    Some(format!(
        r#"# alphacode global launch hotkey listener
# Auto-generated by alphacode setup-hotkey from [launch_hotkeys] config. Runs at
# login via a startup shortcut. Do not edit; re-run `alphacode setup-hotkey`.
...
function Register-AlphacodeHotkey {{
    ...
    if ([HotKeyHelper]::RegisterHotKey([IntPtr]::Zero, $Id, $Mods -bor $MOD_NOREPEAT, $Vk)) {{
        ...
        Write-Warning "alphacode: failed to register hotkey $Label (already claimed?)"
    }}
}}

# Resolve a dynamic launch dir from a tracking file, falling back to $HOME.
function Resolve-AlphacodeDir {{
    param([string]$File)
    if ($File) {{
        try {{
...
function Resolve-AlphacodeFixedDir {{
    param([string]$Dir)
    if ($Dir -and (Test-Path -LiteralPath $Dir -PathType Container)) {{ return $Dir }}
    return $env:USERPROFILE
}}
...
if ($script:RegisteredIds.Count -eq 0) {{
    Write-Error "alphacode: no launch hotkeys could be registered"
    exit 1
}}
...
mod tests {
    ...
    fn args_for(entry: &WindowsHotkey) -> String {
        ...
        if entry.self_dev {
            format!(r#"-e "C:\alphacode.exe" --spawn-hotkey "{hotkey}" self-dev"#)
        } else {
            format!(r#"-e "C:\alphacode.exe" --spawn-hotkey "{hotkey}""#)
        }
    }
    ...
    fn listener_script_registers_each_entry_and_dispatches_dirs() {
        ...
        let script = render_windows_listener_ps1(
            ...
            "wt.exe",
            args_for,
            "C:\\Users\\u\\.alphacode\\hotkey\\last_dir",
            "C:\\Users\\u\\.alphacode\\hotkey\\last_repo",
        )
        .unwrap();

        // Three registrations with distinct ids.
        assert_eq!(script.matches("Register-AlphacodeHotkey").count(), 3 + 1); // 3 calls + fn def

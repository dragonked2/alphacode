...
/// counts, and any conflicts with alphacode's configured bindings.
pub fn render_report(cfg: &KeybindingsConfig, snapshot: &KeymapSnapshot) -> String {
    ...
    if term_count == 0 && sys_count == 0 && app_count == 0 {
        out.push_str(
            "\nNo machine bindings were discovered. alphacode can read Ghostty bindings, macOS\n\
    ...
    if conflicts.is_empty() {
        out.push_str("No conflicts found between your alphacode keybindings and the machine.\n");
    } else {
        out.push_str(&format!(
        ...
        out.push_str(
            "These keys may be captured by your terminal, macOS, or another app (window\n\
             manager, launcher) before alphacode sees them.\n\
             To fix: rebind the alphacode action in ~/.alphacode/config.toml under [keybindings],\n\
...
fn render_conflict_block(c: &Conflict) -> String {
    ...
    format!(
        "  ⚠ {key}\n      alphacode: {action} ({field} = \"{raw}\")\n      taken by {interceptor}\n",
        key = c.alphacode.chord.display(),
        action = c.alphacode.action,
        field = c.alphacode.field,
        raw = c.alphacode.raw,
...
pub fn render_status_line(cfg: &KeybindingsConfig, snapshot: &KeymapSnapshot) -> Option<String> {
    ...
    let keys: Vec<String> = conflicts
        .iter()
        .map(|c| c.alphacode.chord.display())
        .collect::<std::collections::BTreeSet<_>>()
        .into_iter()
        .collect();
    Some(format!(
...
mod tests {
    ...
    use crate::alphacode_setup_hints::keymap::source::DiscoveredBinding;

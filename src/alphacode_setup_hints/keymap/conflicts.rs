//! Detect conflicts between alphacode's own key bindings and the bindings
//! discovered on the machine (terminal emulator + macOS system shortcuts).
//!
//! The flow is:
//!   1. enumerate alphacode's configured bindings as `(field, label, KeyChord)`
//!      from [`crate::alphacode_config_types::KeybindingsConfig`] ([`alphacode_bindings`]),
...
use crate::alphacode_config_types::KeybindingsConfig;

use super::KeymapSnapshot;
use super::chord::KeyChord;
use super::source::{DiscoveredBinding, KeySource};

/// One configured alphacode binding, tied back to its config field.
#[derive(Debug, Clone)]
pub struct AlphacodeBinding {
...
/// A detected conflict between a alphacode binding and something on the machine.
#[derive(Debug, Clone)]
pub struct Conflict {
    /// The alphacode binding that may not reach the app.
    pub alphacode: AlphacodeBinding,
...
impl Conflict {
    ...
    pub fn summary(&self) -> String {
        format!(
            ...
            self.alphacode.action,
            self.alphacode.field,
            self.alphacode.raw,
...
/// Enumerate alphacode's configured bindings as comparable chords. Bindings that are
...
/// chord.
pub fn alphacode_bindings(cfg: &KeybindingsConfig) -> Vec<AlphacodeBinding> {
...
/// Find conflicts between alphacode's configured bindings and the discovered
/// machine bindings in `snapshot`.
///
/// Conflicts are deduplicated per `(alphacode field, interceptor chord, source)` so
...
pub fn detect_conflicts(cfg: &KeybindingsConfig, snapshot: &KeymapSnapshot) -> Vec<Conflict> {
    ...
    for alphacode in alphacode_bindings(cfg) {
        let Some(interceptors) = by_chord.get(&alphacode.chord) else {
            continue;
        };
        for interceptor in interceptors {
            let dedup_key = (
                alphacode.field.clone(),
...
mod tests {
    use super::*;
    use crate::alphacode_setup_hints::keymap::source::KeySource;
    ...
    fn enumerates_default_bindings() {
        ...
        let binds = alphacode_bindings(&cfg);
    ...
    fn detects_ghostty_ctrl_tab_conflict() {
        ...
        let fields: Vec<&str> = conflicts.iter().map(|c| c.alphacode.field.as_str()).collect();
    ...
    fn detects_window_manager_prompt_jump_conflict() {
        ...
        // focus, shadowing alphacode's built-in prompt navigation fallback.
        let cfg = KeybindingsConfig::default();
        let snapshot = snapshot_with(vec![
        ...
        assert!(
            conflicts
                .iter()
                .all(|c| c.alphacode.field.contains("built-in fallback")),
    ...
    fn no_conflict_when_chords_differ() {
        ...
        // alphacode has no cmd+t binding by default.
    ...
    fn disabled_binding_is_not_reported() {
        ...
        assert!(
            !conflicts
                .iter()
                .any(|c| c.alphacode.field == "keybindings.model_switch_next")
        );
    }

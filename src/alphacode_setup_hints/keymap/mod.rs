...
//! (macOS system shortcuts + terminal emulator bindings) so alphacode can detect
//! when one of them intercepts a key alphacode wants to use.
//!
...
//!
//! Conflict detection against alphacode's own bindings is built on top of this in a
//! later layer.
...
pub use conflicts::{Conflict, AlphacodeBinding, conflict_signature, detect_conflicts, alphacode_bindings};
...
pub fn snapshot_path() -> anyhow::Result<std::path::PathBuf> {
    Ok(crate::alphacode_storage::alphacode_dir()?.join("keymap-snapshot.json"))
}

/// Collect a fresh snapshot and persist it to `~/.alphacode/keymap-snapshot.json`.
...
pub fn refresh_and_save() -> KeymapSnapshot {
    ...
    if let Ok(path) = snapshot_path()
        ...
        && let Err(err) = crate::alphacode_storage::write_json_fast(&path, &snapshot)
    {
        crate::alphacode_logging::warn(&format!("keymap snapshot write failed: {err}"));
    }
    snapshot
}

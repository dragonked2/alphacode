...
pub struct TerminalCommand {
    ...
    /// Exported as `ALPHACODE_SPAWN_KIND` to spawn hooks and spawned terminals.
    pub kind: Option<String>,
    /// The alphacode session this terminal will run, when known.
    /// Exported as `ALPHACODE_SPAWN_SESSION_ID`.
    pub session_id: Option<String>,
    /// Extra metadata env entries (e.g. `ALPHACODE_SPAWN_SWARM_ID`) exported to
    /// spawn hooks and spawned terminals. Applied after the first-class
    /// `ALPHACODE_SPAWN_*` keys, so entries here win on key collisions.
    ...
    /// and are also exported under a `ALPHACODE_CLIENT_*` prefix so spawn/focus
...
///
/// The alphacode server process is long-lived and captures these at *its* startup,
...
fn resume_terminal_candidates_with_client_env(
    ...
    // ALPHACODE_TERMINAL and configured spawn hooks still take precedence.
...
pub fn resume_terminal_candidates() -> Vec<String> {
    let configured_terminal = std::env::var("ALPHACODE_TERMINAL").ok();
...
pub fn resume_terminal_candidates() -> Vec<String> {
    let mut candidates = Vec::new();
    if let Ok(term) = std::env::var("ALPHACODE_TERMINAL") {
        push_unique_terminal(&mut candidates, term);
    }
...
/// The `ALPHACODE_SPAWN_*` metadata env exported to spawn hooks and to terminals
/// launched by the built-in fallback:
///
/// - `ALPHACODE_SPAWN_KIND`: why this spawn happened ("resume", "selfdev",
///   "swarm-agent", ...), when known.
/// - `ALPHACODE_SPAWN_SESSION_ID`: the alphacode session the window will run.
/// - `ALPHACODE_SPAWN_TITLE`: the suggested window/tab title.
/// - `ALPHACODE_SPAWN_CWD`: the working directory for the session.
/// - `ALPHACODE_SPAWN_PROGRAM`: path of the alphacode binary to execute.
/// - `ALPHACODE_SPAWN_COMMAND`: the full command line, shell-escaped, for hooks
///   (like tmux) that take a single shell-command string.
///
/// `TerminalCommand::extra_env` entries (e.g. `ALPHACODE_SPAWN_SWARM_ID`,
/// `ALPHACODE_SPAWN_COORDINATOR_SESSION_ID`) are appended last and win collisions.
fn spawn_metadata_env(command: &TerminalCommand, cwd: &Path) -> Vec<(String, String)> {
    let mut env: Vec<(String, String)> = Vec::new();
    if let Some(kind) = &command.kind {
        env.push(("ALPHACODE_SPAWN_KIND".to_string(), kind.clone()));
    }
    if let Some(session_id) = &command.session_id {
        env.push(("ALPHACODE_SPAWN_SESSION_ID".to_string(), session_id.clone()));
    }
    if let Some(title) = &command.title {
        env.push(("ALPHACODE_SPAWN_TITLE".to_string(), title.clone()));
    }
    env.push((
        "ALPHACODE_SPAWN_CWD".to_string(),
        cwd.to_string_lossy().into_owned(),
    ));
    env.push((
        "ALPHACODE_SPAWN_PROGRAM".to_string(),
        command.program.to_string_lossy().into_owned(),
    ));
    env.push((
        "ALPHACODE_SPAWN_COMMAND".to_string(),
    ...
    // `ALPHACODE_CLIENT_<NAME>` alias so hooks can explicitly distinguish the
    // client's terminal from the server's.
    for (key, value) in &command.client_terminal_env {
        env.push((key.clone(), value.clone()));
        env.push((format!("ALPHACODE_CLIENT_{key}"), value.clone()));
    }
...
/// convention), so `hook --flag` becomes `hook --flag <alphacode> <args...>`.
/// The hook runs in the session working directory with the full
/// `ALPHACODE_SPAWN_*` metadata env set (see [`spawn_metadata_env`]); hooks that
/// need a single shell-command string (tmux, kitty `@ launch`) can use
/// `$ALPHACODE_SPAWN_COMMAND` instead of the appended argv.
pub fn build_hook_spawn_command(
    ...
    if command.fresh_spawn {
        cmd.env("ALPHACODE_FRESH_SPAWN", "1");
    }
...
fn build_spawn_command(term: &str, command: &TerminalCommand, cwd: &Path) -> Option<Command> {
    let title = command.title.as_deref().unwrap_or("alphacode");
    ...
    match term {
        ...
        "terminal" => {
            ...
            // means split/resume spawns silently fail to launch alphacode. Use AppleScript's
    ...
    // not inherit this env, matching the existing ALPHACODE_FRESH_SPAWN caveat.
    for (key, value) in spawn_metadata_env(command, cwd) {
        cmd.env(key, value);
    }
...
mod tests {
    ...
    fn spawn_metadata_env_reexports_client_terminal_env_with_native_and_client_keys() {
        ...
        assert_eq!(
            lookup("ALPHACODE_CLIENT_ZELLIJ_SESSION_NAME").as_deref(),
            Some("sessionB")
        );
    ...
    fn spawn_metadata_env_without_client_env_has_no_client_keys() {
        ...
        assert!(
            ...
            "no client terminal env should produce no ALPHACODE_CLIENT_* keys"
        );
    }
    ...
    fn missing_tmux_binary_falls_back_to_detected_emulator() {
        let _guard = ENV_LOCK.lock().unwrap();
        let previous_terminal = std::env::var_os("ALPHACODE_TERMINAL");
        unsafe {
            std::env::remove_var("ALPHACODE_TERMINAL");
        }
        let command =
            TerminalCommand::new("/usr/local/bin/alphacode", vec![]).client_terminal_env(vec![
                (
                    "TMUX".to_string(),
                    "/tmp/tmux-1000/default,123,0".to_string(),
                ),
        ...
        match previous_terminal {
            Some(value) => unsafe { std::env::set_var("ALPHACODE_TERMINAL", value) },
            None => unsafe { std::env::remove_var("ALPHACODE_TERMINAL") },
        }
    ...
    #[test]
    #[cfg(not(unix))]
    fn windows_cmd_fallback_runs_alphacode_under_cmd_k() {
        let command = TerminalCommand::new(
            std::path::PathBuf::from(r"C:\Program Files\alphacode\alphacode.exe"),
    ...
    #[test]
    #[cfg(target_os = "macos")]
    fn macos_terminal_inner_script_runs_alphacode() {
        let command = TerminalCommand::new(
            std::path::PathBuf::from("/usr/local/bin/alphacode"),
            vec!["--resume".to_string(), "abc-123".to_string()],
        );
        let script = macos_terminal_inner_script(&command, Path::new("/work/dir"));
        assert_eq!(
            script,
            "cd '/work/dir' && exec '/usr/local/bin/alphacode' '--resume' 'abc-123'"
        );
        // Must actually exec alphacode, not the broken `open -a Terminal <file>` form.
        assert!(script.contains("exec '/usr/local/bin/alphacode'"));
    }
    ...
    fn macos_terminal_inner_script_injects_fresh_spawn() {
        let command =
            TerminalCommand::new(std::path::PathBuf::from("/usr/local/bin/alphacode"), vec![])
                .fresh_spawn();
        let script = macos_terminal_inner_script(&command, Path::new("/tmp"));
        assert_eq!(
            script,
            "cd '/tmp' && exec env ALPHACODE_FRESH_SPAWN=1 '/usr/local/bin/alphacode'"
        );
    }
    ...
    fn macos_terminal_applescript_uses_do_script() {
        ...
        assert!(applescript.contains("'/usr/local/bin/alphacode'"));
    }
    ...
    fn hook_spawn_command_appends_program_args_and_exports_metadata() {
        ...
        .title("🦊 alphacode ses_abc")
        .kind("swarm-agent")
        .session_id("ses_abc")
        .spawn_env("ALPHACODE_SPAWN_SWARM_ID", "swarm-1")
        .fresh_spawn();
        ...
        assert_eq!(
            args,
            vec!["--flag", "/usr/local/bin/alphacode", "--resume", "ses_abc"]
        );
        assert_eq!(
            cmd.get_current_dir(),
        ...
        assert_eq!(
            env_value(&cmd, "ALPHACODE_SPAWN_KIND").as_deref(),
            Some("swarm-agent")
        );
        assert_eq!(
        ...
        assert_eq!(
            env_value(&cmd, "ALPHACODE_SPAWN_TITLE").as_deref(),
            Some("🦊 alphacode ses_abc")
        );
        assert_eq!(
            env_value(&cmd, "ALPHACODE_SPAWN_CWD").as_deref(),
            Some("/work/dir")
        );
        assert_eq!(
            env_value(&cmd, "ALPHACODE_SPAWN_PROGRAM").as_deref(),
            Some("/usr/local/bin/alphacode")
        );
        #[cfg(unix)]
        assert_eq!(
            env_value(&cmd, "ALPHACODE_SPAWN_COMMAND").as_deref(),
            Some("'/usr/local/bin/alphacode' '--resume' 'ses_abc'")
        );
        assert_eq!(

...
//!
//! Lives in `alphacode-plan` (rather than the TUI) so every consumer renders the
//! same graph from the same logic: the TUI's inline plan-graph message, and
//! the renderer stress probe in `alphacode-tui-mermaid`
...
use crate::alphacode_plan::{PlanItem, summarize_plan_graph};

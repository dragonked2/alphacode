...
pub fn to_task_graph(plan: &VersionedPlan) -> TaskGraph {
    ...
    for item in &plan.items {
        ...
        graph.push_node(TaskNode {
            ...
            priority: crate::alphacode_plan::priority_rank(&item.priority),
            output: artifact,
...
pub fn upstream_context(plan: &VersionedPlan, task_id: &str) -> Option<String> {
    ...
    for dep_id in &item.blocked_by {
        ...
        if !crate::alphacode_plan::is_completed_status(&dep.status) {
            continue;
        }
...
pub fn low_confidence_completed_ids(plan: &VersionedPlan) -> Vec<String> {
    plan.items
        .iter()
        .filter(|item| crate::alphacode_plan::is_completed_status(&item.status))
        .filter(|item| {
            ...
            meta.artifact_json
                ...
                == Some(crate::alphacode_plan::dag::ConfidenceLevel::Low)
        })
...
mod tests {
    use super::*;
    use crate::alphacode_plan::dag::{NodeSpec, complete_node, dispatch, expand_node, seed};

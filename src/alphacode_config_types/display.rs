...
pub struct DisplayConfig {
    ...
    #[serde(deserialize_with = "crate::alphacode_config_types::serde_lenient::lenient_enum")]
    ...
    #[serde(
        default,
        deserialize_with = "crate::alphacode_config_types::serde_lenient::lenient_optional_enum"
    )]
    ...
    #[serde(default)]
    pub pin_todos: bool,
    /// Show a small decorative idle decoration before the first prompt.
    ///
    /// The decoration is intentionally *calm*: a stable wordmark with optional
    /// slow fade-in, not the historical 3D ASCII donut (which redrew ~60 frames
    /// a second of subpixel shading characters and read as fuzzy / glitchy in
    /// many terminals). Flip this back on with `idle_animation = true` to
    /// restore the 3D donut/orbit-rings animation. Default: false.
    ...
    /// adapts alphacode's palette for light backgrounds. Default: auto.
    #[serde(default)]
    pub theme: String,
    /// Built-in theme preset, e.g. `tokyo-night`. Supplies a complete, coherent
    /// base palette so a full theme does not have to be hand-written as 39
    /// `colors` entries. Individual `colors` overrides still win over the
    /// preset. Empty (the default) keeps alphacode's built-in palette. Run
    /// `/theme` to list the available presets.
    #[serde(default)]
    pub preset: String,
    ...
    /// resumed or imported (default: true). Set false to show only alphacode's own
    /// sessions (issue #674).
    #[serde(default = "default_true")]

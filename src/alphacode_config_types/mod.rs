...
pub struct NamedProviderConfig {
    ...
    /// Must be a JSON object; keys here override alphacode-generated body fields.
    ...
    /// `reasoning_effort` request field (`/effort` support). When unset, alphacode
...
pub struct AuthConfig {
    /// External auth source ids that the user has approved alphacode to read/use.
...
pub struct AgentsConfig {
    ...
    /// local. Env override: `ALPHACODE_MEMORY_EMBEDDING_BACKEND`.
    ...
    /// Unset = `text-embedding-3-small`. Env: `ALPHACODE_MEMORY_EMBEDDING_MODEL`.
    #[serde(default)]
    ...
    /// Env: `ALPHACODE_MEMORY_EMBEDDING_BASE_URL`.
    #[serde(default)]
...
/// pane by the built-in launcher. `ALPHACODE_TERMINAL` explicitly selects a terminal
/// emulator instead.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
#[serde(default)]
pub struct TerminalConfig {
    ...
    /// opening a terminal emulator itself, with `ALPHACODE_SPAWN_*` metadata env
    ...
    /// tmux window in the current server. If the hook fails to launch, alphacode
    ...
    ///
    /// When set, alphacode runs the hook (instead of wmctrl/xdotool) whenever it
    /// wants to bring a session's window to the foreground, with
    /// `ALPHACODE_FOCUS_SESSION_ID` and `ALPHACODE_FOCUS_TITLE` env vars. Pair this
    ...
    /// which terminal alphacode launches into and is preferred over the legacy
    /// `~/.alphacode/preferred_terminal.json` file. Re-run `alphacode setup-hotkey`
...
/// Lifecycle hooks: external commands alphacode runs at well-defined points.
///
/// Hook commands are parsed shell-style (quotes work) but executed directly,
/// with `ALPHACODE_HOOK_*` env vars describing the event (`ALPHACODE_HOOK_EVENT`,
/// `ALPHACODE_HOOK_SESSION_ID`, `ALPHACODE_HOOK_CWD`, event-specific fields, and a
/// `ALPHACODE_HOOK_PAYLOAD` JSON mirror). Hook processes get
/// `ALPHACODE_HOOKS_DISABLED=1` so nested alphacode invocations don't recurse.
///
/// All hooks except `pre_tool` are observers: detached, fire-and-forget,
/// failures only logged. `pre_tool` is a gate: alphacode waits for it and exit
...
pub struct HooksConfig {
    ...
    /// ("chat"/"resume"/"ambient"). Env override: ALPHACODE_HOOK_TURN_START.
...
/// providers may share revenue with Alphacode when a referred user becomes a
...
pub struct SponsorsConfig {
    ...
    /// `discover_tools` tool is not registered, and alphacode never contacts the
    /// discovery endpoint.
    pub enabled: bool,
...
impl Default for SponsorsConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            endpoint: "https://api.alphacode.sh/v1/discovery".to_string(),
        }
    }
}
...
pub struct KeybindingsConfig {
    ...
    /// Spawn a fresh alphacode session in a new terminal window (default: unbound).
    /// Example: "alt+enter".
...
pub enum WebSearchEngine {
    ...
    /// `ALPHACODE_SEARXNG_URL` env var) to point at a SearXNG instance. Useful on
...
impl Default for WebSearchConfig {
    fn default() -> Self {
        Self {
            ...
            bing_api_key: None,
            bing_api_key_env: "ALPHACODE_BING_API_KEY".to_string(),
            bing_market: "en-US".to_string(),
            searxng_url: None,
            searxng_url_env: "ALPHACODE_SEARXNG_URL".to_string(),
        }
    }
}
...
pub struct ProviderConfig {
    ...
    /// Whether alphacode should automatically try another account on the same provider
    ...
    /// automatically (see `crate::alphacode_base::provider::stream_idle_timeout_for_effort`).
    /// Default: 180. Overridable via `ALPHACODE_STREAM_IDLE_TIMEOUT_SECS`.
...
pub struct SafetyConfig {
    ...
    /// SMTP password (prefer ALPHACODE_SMTP_PASSWORD env var)
    ...
    /// Jade relay bearer token (prefer ALPHACODE_JADE_RELAY_TOKEN env var)
...
pub struct PowerConfig {
    /// Prevent automatic system sleep while any alphacode session is actively
    ...
    ///
    /// Honored by the shared `alphacode serve` daemon. The `ALPHACODE_DISABLE_POWER_INHIBIT`
...
/// A single global launch hotkey: a chord plus the directory it opens alphacode in.
///
...
/// - `$LAST_DIR` -> the most recent non-home project directory alphacode ran in.
/// - `$LAST_REPO` -> the most recent alphacode repo (for self-dev).
///
...
pub struct LaunchHotkeyEntry {
    /// alphacode-style chord string, e.g. `cmd+;`, `cmd+[`, `cmd+shift+'`.
    pub chord: String,
...
/// Configuration for the global "launch a new alphacode" hotkeys (macOS).
///
/// When `entries` is empty, alphacode uses its built-in defaults (`Cmd+;` -> home,

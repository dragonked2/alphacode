...
//! conversion). It is kept separate from the rest of the TUI for two reasons:
//!
//! 1. It is isolated enough to reason about and profile on its own. Note that
//!    the per-package `opt-level` override this module once relied on no longer
//!    applies: the former workspace crates are now modules of a single crate,
//!    and `[profile.dev.package."*"]` can only target dependencies, not internal
//!    modules. In `dev` these loops therefore build at the crate-wide
//!    `opt-level`; the lookup tables below are what keep them cheap regardless.

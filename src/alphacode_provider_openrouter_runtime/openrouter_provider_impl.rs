...
impl Provider for OpenRouterProvider {
    ...
    async fn complete(
        ...
        let api_messages = crate::alphacode_provider_openrouter::request::build_chat_messages(
            &effective_messages,
            system,
            allow_reasoning,
        ...
        if let Some(effort) = reasoning_effort.as_deref() {
            if self.supports_deepseek_reasoning_effort() {
                ...
                let effort = if crate::alphacode_base::prompt::is_swarm_effort(effort) {
                    "max"
                } else {
                    effort
                };
        ...
        // alphacode-generated field with the same key (issue #341).
        ...
        crate::alphacode_base::logging::info("OpenRouter transport: HTTPS (SSE)");
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        if let Some(stored_effort) = stored_effort
            && !crate::alphacode_base::prompt::is_swarm_effort(&stored_effort)
    ...
    fn available_efforts(&self) -> Vec<&'static str> {
        if self.supports_deepseek_reasoning_effort() {
            crate::alphacode_provider_core::DEEPSEEK_SELECTABLE_EFFORTS.to_vec()
        } else if self.supports_openai_reasoning_effort() {
            ...
            crate::alphacode_provider_core::OPENROUTER_SELECTABLE_EFFORTS.to_vec()
        } else {
            vec![]
        }
    }
    ...
    fn model_routes(&self) -> Vec<crate::alphacode_provider_core::ModelRoute> {
        ...
        self.available_models_display()
            .into_iter()
            .filter(|model| crate::alphacode_base::provider::is_listable_model_name(model))
            .map(|model| {
    ...
    fn context_window(&self) -> usize {
        ...
        if let Some(profile_id) = self.profile_id.as_deref()
            && let Some(limit) =
                crate::alphacode_base::provider_catalog::openai_compatible_profile_context_limit(
                    profile_id, &model_id,
                )
        {
            return limit;
...
impl OpenRouterProvider {
    ...
    /// `ALPHACODE_OPENROUTER_CACHE_NAMESPACE` env var, so with several named
    ...
    pub(crate) fn is_user_named_profile(&self) -> bool {
        ...
        match crate::alphacode_base::provider_catalog::openai_compatible_profile_by_id(id) {

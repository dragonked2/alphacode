...
//! NVIDIA NIM), moved out of `alphacode-base` so provider edits compile only this
...
//! with `crate::alphacode_base::provider::external::register_openrouter_factory`.
...
use crate::alphacode_provider_openrouter::{
...
const OPENROUTER_TRANSPORT_STATE_ENV: &str = "ALPHACODE_OPENROUTER_TRANSPORT_STATE";
...
///
/// Increased from 15min to 60min: free-tier providers (GMI Cloud, OpenRouter
/// free) have very tight rate limits, and the old 15min interval meant the
/// background catalog sweeper alone could burn a significant portion of the
/// rate-limit budget just on /models calls. 60min still catches new models
/// within a reasonable window while cutting background request volume by 75%.
const MODEL_CATALOG_SOFT_REFRESH_SECS: u64 = 60 * 60;
/// Minimum delay between background refresh attempts.
///
/// Increased from 60s to 300s (5min): after a failed refresh (e.g. 429 from
/// a free-tier provider), waiting only 60s before retrying is almost certain
/// to hit the same rate-limit window. 5 minutes gives providers meaningful
/// breathing room.
const MODEL_CATALOG_REFRESH_RETRY_SECS: u64 = 300;
...
/// Endpoints cache TTL (2 hours) - per-model provider endpoint data.
///
/// Increased from 1h to 2h: endpoint data rarely changes, and the refresh
/// hits OpenRouter's /api/v1/models/<id>/endpoints endpoint which shares
/// rate-limit buckets with other requests. Doubling the TTL halves the
/// endpoint-related request volume.
const ENDPOINTS_CACHE_TTL_SECS: u64 = 2 * 60 * 60;
...
-> Option<crate::alphacode_base::provider_catalog::ResolvedOpenAiCompatibleProfile> {
    ...
    // Prefer a specific configured profile (Chutes, Cerebras, DeepSeek, ...)
    // over the generic keyless OpenAI-compatible fallback. The generic profile
    // declares `requires_api_key: false` (it serves local/keyless endpoints), so
    // the old ordering short-circuited on `!compat.requires_api_key` before the
    // specific-profile scan below ever ran, making direct-profile autodetection
    // dead code: a user with only `CHUTES_API_KEY` set was mislabeled as the
    // generic "OpenAI-compatible" profile in the model picker.
    ...
    // No specific profile configured: fall back to the generic keyless
    // OpenAI-compatible endpoint (local servers, relay endpoints, ...).
...
fn configured_api_base() -> String {
    let raw = std::env::var("ALPHACODE_OPENROUTER_API_BASE")
        .ok()
    ...
    normalize_api_base(&raw).unwrap_or_else(|| {
        crate::alphacode_base::logging::warn(&format!(
            "Ignoring invalid ALPHACODE_OPENROUTER_API_BASE '{}'; using {}",
...
fn configured_api_key_name() -> String {
    let raw = std::env::var("ALPHACODE_OPENROUTER_API_KEY_NAME")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
    ...
    if is_safe_env_key_name(&raw) {
        ...
        crate::alphacode_base::logging::warn(&format!(
            "Ignoring invalid ALPHACODE_OPENROUTER_API_KEY_NAME '{}'; using {}",
...
fn configured_env_file_name() -> String {
    let raw = std::env::var("ALPHACODE_OPENROUTER_ENV_FILE")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
    ...
    if is_safe_env_file_name(&raw) {
        ...
        crate::alphacode_base::logging::warn(&format!(
            "Ignoring invalid ALPHACODE_OPENROUTER_ENV_FILE '{}'; using {}",
...
fn load_named_profile_api_key(
    env_key: &str,
    profile: &crate::alphacode_base::config::NamedProviderConfig,
) -> Option<String> {
...
fn provider_features_enabled(api_base: &str) -> bool {
    if let Ok(raw) = std::env::var("ALPHACODE_OPENROUTER_PROVIDER_FEATURES") {
        if let Some(value) = parse_env_bool(&raw) {
            return value;
        }
        crate::alphacode_base::logging::warn(&format!(
            "Ignoring invalid ALPHACODE_OPENROUTER_PROVIDER_FEATURES '{}'; expected true/false",
            raw
        ));
    }
    api_base.contains("openrouter.ai") || api_base.contains("agentrouter.org")
}

fn model_catalog_enabled() -> bool {
    if let Ok(raw) = std::env::var("ALPHACODE_OPENROUTER_MODEL_CATALOG") {
        if let Some(value) = parse_env_bool(&raw) {
            return value;
        }
        crate::alphacode_base::logging::warn(&format!(
            "Ignoring invalid ALPHACODE_OPENROUTER_MODEL_CATALOG '{}'; expected true/false",
            raw
        ));
    }
    true
...
fn configured_auth_header_mode() -> AuthHeaderMode {
    let Some(raw) = std::env::var("ALPHACODE_OPENROUTER_AUTH_HEADER")
        .ok()
    ...
    match raw.as_str() {
        ...
        other => {
            crate::alphacode_base::logging::warn(&format!(
                "Ignoring invalid ALPHACODE_OPENROUTER_AUTH_HEADER '{}'; expected authorization-bearer or api-key",
                other
            ));
...
fn configured_auth_header_name() -> HeaderName {
    let raw = std::env::var("ALPHACODE_OPENROUTER_AUTH_HEADER_NAME")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
    ...
    HeaderName::from_bytes(raw.as_bytes()).unwrap_or_else(|_| {
        crate::alphacode_base::logging::warn(&format!(
            "Ignoring invalid ALPHACODE_OPENROUTER_AUTH_HEADER_NAME '{}'; using api-key",
            raw
        ));
...
fn configured_dynamic_bearer_provider() -> Option<String> {
    std::env::var("ALPHACODE_OPENROUTER_DYNAMIC_BEARER_PROVIDER")
        .ok()
        .map(|v| v.trim().to_ascii_lowercase())
        .filter(|v| !v.is_empty())
}
...
fn configured_allow_no_auth() -> bool {
    std::env::var("ALPHACODE_OPENROUTER_ALLOW_NO_AUTH")
        .ok()
...
pub enum OpenRouterTransportState {
    ...
    OpenRouterApiKey,
    /// Alphacode subscription access currently reuses the OpenRouter HTTP slot, but is
    /// not user BYOK/OpenRouter billing.
    AlphacodeSubscription,
...
impl OpenRouterTransportState {
    pub fn from_current_env(runtime_provider: Option<&str>) -> Self {
        ...
        if matches!(runtime_provider.as_deref(), Some("alphacode")) {
            return Self::AlphacodeSubscription;
        }
    ...
    fn from_env_marker() -> Option<Self> {
        ...
        match value.as_str() {
            ...
            other => {
                crate::alphacode_base::logging::warn(&format!(
                    "Ignoring invalid {} '{}'; expected openrouter-api-key, alphacode-subscription, direct-api-key, or direct-no-auth",
    ...
    fn runtime_provider_is_direct_compatible(runtime_provider: Option<&str>) -> bool {
        matches!(runtime_provider, Some("openai-compatible" | "azure-openai"))
            || runtime_provider
                .and_then(crate::alphacode_base::provider_catalog::openai_compatible_profile_by_id)
                .is_some()
    }
...
impl ProviderAuth {
    async fn apply(&self, req: reqwest::RequestBuilder) -> Result<reqwest::RequestBuilder> {
        match self {
            ...
            Self::AzureEntra { .. } => {
                let token = crate::alphacode_base::auth::azure::get_bearer_token().await?;
...
async fn fetch_models_from_api(
    ...
    let models = parse_openai_compatible_models_response(&raw_body).with_context(|| {
            format!(
                ...
                url,
                auth.label(),
                crate::alphacode_base::util::truncate_str(&raw_body.trim().replace('\n', "\\n"), 1200)
            )
        })?;
...
struct ProfileCatalogRefreshTracker {
    ...
    /// Consecutive rate-limit (429) failures per profile. These get a much
    /// longer backoff than regular failures because retrying too soon wastes
    /// rate-limit budget on non-essential /models calls.
    consecutive_rate_limit_failures: HashMap<String, u32>,
}
...
/// Exponential backoff for repeatedly failing catalog refreshes.
///
/// Rate-limit (429) failures get a much longer backoff because retrying too
/// soon is almost guaranteed to fail again and wastes the rate-limit budget
/// on non-essential /models calls. Transient errors (network, 5xx) use the
/// standard exponential backoff.
///
/// Base schedule: 300s, 600s (10m), 1200s (20m), 2400s (40m), ... capped at 2h.
/// Rate-limit schedule: 600s (10m), 1800s (30m), 3600s (1h), ... capped at 6h.
/// A single success resets the profile to the base retry interval.
fn profile_catalog_retry_delay_secs(consecutive_failures: u32) -> u64 {
    const MAX_RETRY_SECS: u64 = 2 * 60 * 60;
    MODEL_CATALOG_REFRESH_RETRY_SECS
        .saturating_mul(1u64 << consecutive_failures.min(8))
        .min(MAX_RETRY_SECS)
}

/// Aggressive backoff specifically for rate-limit (429) catalog refresh failures.
/// Free-tier providers like GMI Cloud and OpenRouter free have very tight per-key
/// rate limits; retrying after only a few minutes is almost guaranteed to fail
/// again. This gives providers 10-30+ minutes of breathing room.
fn profile_catalog_rate_limit_retry_delay_secs(consecutive_rate_limit_failures: u32) -> u64 {
    const MAX_RATE_LIMIT_RETRY_SECS: u64 = 6 * 60 * 60;
    const RATE_LIMIT_BASE_SECS: u64 = 600; // 10 minutes
    RATE_LIMIT_BASE_SECS
        .saturating_mul(1u64 << consecutive_rate_limit_failures.min(6))
...
fn finish_profile_catalog_refresh_with_outcome(profile_id: &str, succeeded: bool, error: Option<&str>) {
    if let Ok(mut state) = global_profile_catalog_refresh().lock() {
        ...
        if succeeded {
            ...
            state.consecutive_rate_limit_failures.remove(profile_id);
        } else {
            let entry = state
                .consecutive_failures
            ...
            *entry = entry.saturating_add(1);
            ...
            // Track rate-limit failures separately so they get longer backoff.
...
pub fn maybe_schedule_openai_compatible_profile_catalog_refresh(
    ...
    let previous_fingerprint =
        crate::alphacode_provider_openrouter::load_disk_cache_entry_for_namespace(&profile_id)
    ...
    handle.spawn(async move {
        ...
        let result = fetch_models_from_api(
            crate::alphacode_provider_core::shared_http_client(),
            api_base,
            auth,
            models_cache,
        ...
        match result {
            Ok(models) => {
                ...
                if updated {
                    ...
                    crate::alphacode_base::bus::Bus::global().publish_models_updated();
                } else {
                    crate::alphacode_base::logging::info(&format!(
            ...
            Err(error) => crate::alphacode_base::logging::info(&format!(
                "Failed to refresh OpenAI-compatible profile model catalog in background ({}): {} ({})",
                context, display_name, error
            )),
        }
        finish_profile_catalog_refresh_with_outcome(&profile_id, succeeded, error_string.as_deref());
    });
...
pub fn maybe_schedule_standard_openrouter_catalog_refresh(context: &'static str) -> bool {
    ...
    // when ALPHACODE_OPENROUTER_* env vars are set by an active named profile
    // (e.g. NVIDIA NIM via `[providers.mynvidia]`, which sets
    // ALPHACODE_OPENROUTER_API_BASE to the NVIDIA endpoint): that profile owns the
    ...
    let cache_is_fresh = current_unix_secs()
        .zip(crate::alphacode_provider_openrouter::load_disk_cache_entry_for_namespace(namespace))
        .map(|(now, cache)| {
    ...
    handle.spawn(async move {
        ...
        finish_profile_catalog_refresh_with_outcome(namespace, succeeded, error_string.as_deref());
    });
...
pub struct OpenRouterProvider {
    ...
    /// `ALPHACODE_OPENAI_EXTRA_BODY` env/env-file value.
...
impl OpenRouterProvider {
    fn profile_supports_reasoning_effort(profile_id: Option<&str>) -> bool {
        matches!(profile_id, Some(id) if id.eq_ignore_ascii_case("deepseek")
            // The Alpha local endpoint (DragonMeta) serves a GLM-5.2-based
            // reasoning model and accepts the standard top-level
            // `reasoning_effort` request field, so the DeepSeek-style effort
            // ladder applies to it too.
            || id.eq_ignore_ascii_case("dragonmeta"))
    }
    ...
    fn initial_reasoning_effort(
        ...
        crate::alphacode_base::config::config()
            .provider
    ...
    fn configured_max_tokens(profile_id: Option<&str>) -> Option<u32> {
        if let Ok(raw) = std::env::var("ALPHACODE_OPENROUTER_MAX_TOKENS") {
            ...
            match trimmed.parse::<u32>() {
                ...
                Err(_) => crate::alphacode_base::logging::warn(&format!(
                    "Ignoring invalid ALPHACODE_OPENROUTER_MAX_TOKENS '{}'; expected a positive integer or auto",
                    raw
                )),
            }
        }
    ...
    /// 2. The `ALPHACODE_OPENAI_EXTRA_BODY` env var (or the same key inside the
    ...
    fn resolve_extra_body(
        ...
        if let Some(value) = config {
            match value.as_object() {
                ...
                None => crate::alphacode_base::logging::warn(
        ...
        if let Some(raw) = load_env_value_from_env_or_config("ALPHACODE_OPENAI_EXTRA_BODY", env_file) {
            match serde_json::from_str::<Value>(&raw) {
                ...
                Ok(_) => crate::alphacode_base::logging::warn(
                    "Ignoring ALPHACODE_OPENAI_EXTRA_BODY: expected a JSON object string, e.g. {\"chat_template_kwargs\":{\"thinking\":true}}",
                ),
                Err(err) => crate::alphacode_base::logging::warn(&format!(
                    "Ignoring invalid ALPHACODE_OPENAI_EXTRA_BODY JSON: {err}"
                )),
            }
        }
    ...
    ///
    /// Unlike the env-var based [`crate::alphacode_base::provider_catalog::runtime_provider_display_name`],
    ...
    pub fn runtime_display_name(&self) -> String {
        ...
        if !self.supports_provider_features {
            if let Some(profile_id) =
                crate::alphacode_base::provider_catalog::openai_compatible_profile_id_for_api_base(
                    &self.api_base,
                )
            ...
            if std::env::var("ALPHACODE_RUNTIME_PROVIDER")
                .ok()
    ...
    /// scoped `ALPHACODE_API_KEY`, then routes through the OpenAI-compatible
    ...
    pub fn new_named_openai_compatible(
        ...
        crate::alphacode_base::env::set_var("ALPHACODE_OPENROUTER_CACHE_NAMESPACE", profile_name);
        ...
        Ok(Self {
            ...
            supports_provider_features: matches!(
                ...
                crate::alphacode_base::config::NamedProviderType::OpenRouter
    ...
    fn is_kimi_model(model: &str) -> bool {
        crate::alphacode_provider_openrouter::is_kimi_model(model)
    }
    ...
    fn strip_session_profile_prefix<'a>(&self, model: &'a str) -> &'a str {
        ...
        if crate::alphacode_provider_core::explicit_model_provider_prefix(model).is_some() {
            return model;
        }
        let rest = rest.trim();
        if rest.is_empty() {
    ...
    fn thinking_override() -> Option<bool> {
        let raw = std::env::var("ALPHACODE_OPENROUTER_THINKING").ok()?;
        ...
        match value.as_str() {
            ...
            other => {
                crate::alphacode_base::logging::info(&format!(
                    "Warning: Unsupported ALPHACODE_OPENROUTER_THINKING '{}'; expected enabled/disabled/auto",
                    other
                ));
                None
            }
    ...
    pub fn new() -> Result<Self> {
        ...
        let profile_id = std::env::var("ALPHACODE_OPENROUTER_CACHE_NAMESPACE")
            .ok()
            .map(|value| value.trim().to_ascii_lowercase())
            .filter(|value| !value.is_empty())
        ...
        let static_context_limits = profile_id
            ...
            .unwrap_or_default();
        let static_models = std::env::var("ALPHACODE_OPENROUTER_STATIC_MODELS")
            .ok()
            .map(|raw| {
                raw.lines()
                    .map(str::trim)
    ...
    pub fn new_openai_compatible_profile_runtime(
        ...
        let auth = match load_api_key_from_env_or_config(&resolved.api_key_env, &resolved.env_file)
        {
            ...
            None => {
                ...
                anyhow::bail!(
                    "{} credentials not available. {} not found in environment or {}. Run `alphacode login --provider {}` first.",
    ...
    pub(crate) fn filter_profile_chat_supported_models(&self, models: Vec<String>) -> Vec<String> {
        ...
        models
            .into_iter()
            .filter(|model| {
                crate::alphacode_base::provider_catalog::openai_compatible_profile_model_supports_chat(
                    profile_id, model,
                )
            })
            .collect()
    ...
    fn model_disk_cache_source_matches(
        &self,
        cache_entry: &crate::alphacode_provider_openrouter::DiskCache,
    ) -> bool {
    ...
    fn maybe_schedule_model_catalog_refresh(&self, cache_age_secs: u64, context: &'static str) {
        ...
        handle.spawn(async move {
            match fetch_models_from_api(client, api_base, auth, models_cache, ns).await {
                ...
                Err(e) => crate::alphacode_base::logging::info(&format!(
    ...
    fn parse_provider_routing() -> ProviderRouting {
        crate::alphacode_provider_openrouter::parse_provider_routing_from_env()
    }
    ...
    fn rank_providers_from_endpoints(endpoints: &[EndpointInfo]) -> Vec<String> {
        crate::alphacode_provider_openrouter::rank_providers_from_endpoints(endpoints)
    }
    ...
    pub fn has_credentials() -> bool {
        if matches!(
            ...
            return crate::alphacode_base::auth::azure::has_configuration();
        }
        if configured_allow_no_auth() {
            return true;
        }
    ...
    fn resolve_auth() -> Result<ProviderAuth> {
        if let Some(provider) = configured_dynamic_bearer_provider() {
            return match provider.as_str() {
                "azure" => {
                    if crate::alphacode_base::auth::azure::has_configuration() {
                        Ok(ProviderAuth::AzureEntra {
                            label: "Azure OpenAI Entra ID".to_string(),
                        })
                    } else {
                        anyhow::bail!(
                            "Azure OpenAI is configured for Entra ID, but Azure settings are incomplete. Run `alphacode login --provider azure`."
                        )
                    }
                }
                other => anyhow::bail!(
                    "Unsupported ALPHACODE_OPENROUTER_DYNAMIC_BEARER_PROVIDER '{}'.",
                    other
                ),
            };
        }
...
#[cfg(test)]
mod reasoning_effort_profile_tests {

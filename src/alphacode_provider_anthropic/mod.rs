...
pub fn format_messages(messages: &[Message], is_oauth: bool) -> Vec<ApiMessage> {
    ...
    // alphacode never intends to prefill, so a trailing assistant turn here is always
...
pub fn add_message_cache_breakpoint(messages: &mut [ApiMessage], cache_ttl_1h: bool) {
    ...
    if messages.len() < 3 {
        ...
        crate::alphacode_logging::info("Conversation caching: too few messages, skipping");
        return;
    }
    ...
    if assistant_indices.is_empty() {
        crate::alphacode_logging::info("Conversation caching: no assistant message found");
        return;
    }

use crate::alphacode_logging as logging;
...
const DEFAULT_DISCOVERY_ENDPOINT: &str = "https://api.alphacode.sh/v1/discovery";
...
pub fn is_enabled() -> bool {
    if std::env::var("ALPHACODE_NO_TELEMETRY").is_ok() || std::env::var("DO_NOT_TRACK").is_ok() {
...
/// Its presence is equivalent to setting `ALPHACODE_NO_TELEMETRY=1`, but persists
...
pub fn opt_out_forced_by_env() -> bool {
    std::env::var("ALPHACODE_NO_TELEMETRY").is_ok() || std::env::var("DO_NOT_TRACK").is_ok()
}
...
fn detect_project_profile() -> ProjectProfile {
    ...
    profile.repo_present = root.join(".git").exists() || is_alphacode_repo_dir(root);
    ...
    F: FnMut(Value) + Send + 'static,
{
    ...
    std::thread::Builder::new()
        .name("alphacode-telemetry".to_string())
        .spawn(move || {
...
pub fn record_install_if_first_run() {
    ...
    // fresh ~/.alphacode (so a new telemetry_id) on every job, which would otherwise
...
pub fn record_auth_started(provider: &str, method: &str) {
    crate::alphacode_logging::auth_event("auth_started", provider, &[("method", method)]);
...
pub fn record_auth_failed_reason(provider: &str, method: &str, reason: &str) {
    crate::alphacode_logging::auth_event(
        "auth_failed",
        provider,
...
pub fn record_auth_cancelled(provider: &str, method: &str) {
    crate::alphacode_logging::auth_event("auth_cancelled", provider, &[("method", method)]);
...
pub fn record_auth_surface_blocked(provider: &str, method: &str) {
    crate::alphacode_logging::auth_event("auth_surface_blocked", provider, &[("method", method)]);
...
pub fn record_auth_success(provider: &str, method: &str) {
    crate::alphacode_logging::auth_event("auth_success", provider, &[("method", method)]);
    if !is_enabled() {
        return;
    }
    let Some(id) = get_or_create_id() else {

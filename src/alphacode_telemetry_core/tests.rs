...
// flip `ALPHACODE_NO_TELEMETRY` / `DO_NOT_TRACK`, while the session tests drive the
...
fn telemetry_endpoint_uses_production_custom_domain() {
    assert_eq!(TELEMETRY_ENDPOINT, "https://telemetry.alphacode.sh/v1/event");
}
...
fn test_opt_out_env_var() {
    ...
    crate::alphacode_core::env::set_var("ALPHACODE_NO_TELEMETRY", "1");
    assert!(!is_enabled());
...
fn test_do_not_track() {
    let _guard = lock_test_env();
    crate::alphacode_core::env::set_var("DO_NOT_TRACK", "1");
    assert!(!is_enabled());
...
fn test_is_ci_detects_ci_env() {
    ...
    crate::alphacode_core::env::set_var("CI", "true");
    assert!(
        is_ci(),
        "CI env var should mark the run as CI (gates install skip)"
    );
    crate::alphacode_core::env::remove_var("CI");
    assert!(!is_ci());
}
...
fn test_onboarding_step_milestone_key_includes_provider_and_method() {
    assert_eq!(
        onboarding_step_milestone_key("auth_success", Some("alphacode"), Some("API key")),
        "auth_success_alphacode_api_key"
    );
    assert_eq!(

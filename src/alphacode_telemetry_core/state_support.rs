...
pub(super) fn telemetry_state_path(name: &str) -> Option<PathBuf> {
    storage::alphacode_dir().ok().map(|d| d.join(name))
}
...
pub(super) fn version() -> String {
    crate::alphacode_build_meta::pkg_version().to_string()
}
...
pub(super) fn is_alphacode_repo_dir(dir: &Path) -> bool {
    ...
    std::fs::read_to_string(cargo_toml)
        .map(|content| content.contains("name = \"alphacode\""))
        .unwrap_or(false)
}

fn find_alphacode_repo_in_ancestors(start: &Path) -> Option<PathBuf> {
    start
        .ancestors()
        .find(|dir| is_alphacode_repo_dir(dir))
        .map(Path::to_path_buf)
}

fn telemetry_alphacode_repo_dir() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("ALPHACODE_REPO_DIR") {
        let path = PathBuf::from(path);
        if is_alphacode_repo_dir(&path) {
            return Some(path);
        }
    }

    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    if let Some(repo) = find_alphacode_repo_in_ancestors(&manifest_dir) {
        return Some(repo);
    }
    ...
    if let Ok(exe) = std::env::current_exe()
        && let Some(repo) = exe
            ...
            .and_then(Path::parent)
            .filter(|dir| is_alphacode_repo_dir(dir))
    {
    ...
    std::env::current_dir()
        .ok()
        .and_then(|cwd| find_alphacode_repo_in_ancestors(&cwd))
}
...
pub(super) fn build_channel() -> String {
    ...
    if telemetry_alphacode_repo_dir().is_some() {
...
pub(super) fn is_git_checkout() -> bool {
    telemetry_alphacode_repo_dir().is_some()
}

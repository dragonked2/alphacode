...
pub async fn send_email(request: SendEmailRequest<'_>) -> Result<()> {
    ...
    if let Some(cid) = request.cycle_id {
        let msg_id = format!("<ambient-{}@alphacode>", cid);
...
pub fn poll_imap_once(host: &str, port: u16, user: &str, pass: &str) -> Result<Vec<ReplyAction>> {
    ...
    let reply_search = session.search("UNSEEN HEADER In-Reply-To \"@alphacode>\"")?;
    let button_search = session.search("UNSEEN SUBJECT \"[alphacode-perm:\"")?;
    ...
    for message in messages.iter() {
            ...
            && let Some(parsed) = mail_parser::MessageParser::default().parse(body)
        {
            ...
            let cycle_id = if in_reply_to.contains("@alphacode>") {
                in_reply_to
                    .trim_start_matches("<ambient-")
                    .trim_end_matches("@alphacode>")
                    .to_string()
            } else if let Some(start) = subject.find("[alphacode-perm:") {
                let rest = &subject[start + "[alphacode-perm:".len()..];
...
pub fn build_permission_email_html(
    ...
    let approve_subj_raw = format!("[alphacode-perm:{}] Approved", request_id);
    let deny_subj_raw = format!("[alphacode-perm:{}] Denied", request_id);
...
<div class="footer">
  Sent by alphacode ambient mode
</div>
</body>
</html>"#
    )
...
mod tests {
    ...
    fn test_build_permission_email_html() {
        let html = build_permission_email_html(
            ...
            "Touch Cargo.toml",
            "req_123",
            "alphacode@example.com",
        );
        assert!(html.contains("Permission Request"));
        assert!(html.contains("req_123"));
        assert!(html.contains("mailto:alphacode@example.com"));
    }
}

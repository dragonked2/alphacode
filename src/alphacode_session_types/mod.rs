...
use crate::alphacode_message_types::{ContentBlock, Message, Role, ToolCall};
use serde::{Deserialize, Serialize};
use std::collections::HashSet;

/// Identifies a session to resume, across the agent backends alphacode can import
/// from. This is pure data (only ids/paths) with no UI dependency; it lives in
/// `alphacode-session-types` so the foundation/import layer can match on it without
/// depending on any `alphacode-tui-*` crate. The session-picker UI re-exports it.
...
impl ResumeTarget {
    pub fn stable_id(&self) -> &str {
        match self {
            Self::AlphacodeSession { session_id } => session_id,
...
pub struct EnvSnapshot {
    ...
    pub alphacode_git_hash: Option<String>,
    pub alphacode_git_dirty: Option<bool>,
    pub os: String,
    pub arch: String,
    pub pid: u32,
    pub is_selfdev: bool,
...
pub struct SessionSearchReport {
    ...
    pub scanned_alphacode_sessions: usize,
    pub candidate_alphacode_sessions: usize,
...
pub fn format_session_search_results(
    ...
    output.push_str(&format!(
        "_Scanned: {} Alphacode sessions ({} candidates), {} external sessions{}{}._\n\n",
        report.scanned_alphacode_sessions,
        report.candidate_alphacode_sessions,

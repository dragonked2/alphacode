...
fn test_lazy_renderer_also_follows_table_alignment() {
    ...
    let md = "| left | right |\n|:--|--:|\n| a | 1 |\n| bbbb | 1000 |";
    let rendered: Vec<String> = render_markdown_lazy(md, None, 0..100)
        .iter()
        .map(line_to_string)
        .collect();
    let short = rendered
...
fn assert_reasoning_line_fully_dim_italic(line: &str) {
    ...
    let md = reasoning_line_markup(line);
    let lines = render_markdown(&md);
...
fn test_multiline_reasoning_renders_one_visual_line_per_thought() {
    ...
    for thought in thoughts {
        md.push_str(&reasoning_line_markup(thought));
    }
    let lines = render_markdown(&md);
...
fn test_reasoning_emphasis_does_not_leak_into_following_text() {
    ...
    let md = format!(
        "{}\n{}",
        reasoning_line_markup("it's **thinking** with `code`."),
        "Normal answer text."
    );
    let lines = render_markdown(&md);
...
fn test_reasoning_summary_line_markup_folds_to_single_dim_italic_trace() {
    ...
    let many = reasoning_summary_line_markup(3);
    assert!(

//! Adapter: backend-neutral [`crate::alphacode_render_core::Document`] -> ratatui lines.
//!
...
fn push_code_block(
    ...
    block: &crate::alphacode_render_core::Block,
    language: Option<&str>,
) {
...
pub fn render_markdown_via_core(text: &str) -> Vec<Line<'static>> {
    document_to_lines_with_width(&crate::alphacode_render_core::parse_markdown(text), None)
}
...
pub fn render_markdown_via_core_wrapped(text: &str, width: usize) -> Vec<Line<'static>> {
    let doc = crate::alphacode_render_core::parse_markdown(text);

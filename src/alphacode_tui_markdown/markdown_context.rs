use crate::alphacode_tui_markdown::{DiagramDisplayMode, MarkdownSpacingMode};
use std::cell::Cell;
...
pub(super) fn effective_diagram_mode() -> DiagramDisplayMode {
    ...
    crate::alphacode_tui_markdown::config_snapshot().diagram_mode
}
...
pub(super) fn effective_markdown_spacing_mode() -> MarkdownSpacingMode {
    MARKDOWN_SPACING_MODE_OVERRIDE.with(|mode| {
        mode.get()
            .unwrap_or(crate::alphacode_tui_markdown::config_snapshot().markdown_spacing)
    })
}
...
pub fn mermaid_rendering_enabled() -> bool {
    ...
    crate::alphacode_tui_markdown::config_snapshot().mermaid_enabled
}

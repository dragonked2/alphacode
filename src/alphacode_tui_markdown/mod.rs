...
use crate::alphacode_tui_mermaid as mermaid;
...
///
/// These pure-string helpers were moved to `alphacode-render-core` so the
...
///
/// Moved to `alphacode-render-core` (pure/backend-neutral) so the foundation/
/// streaming layer can format it without depending on any `alphacode-tui-*` crate.
/// Re-exported here so the existing
/// `crate::alphacode_tui_markdown::reasoning_summary_line_markup` path keeps working.
pub use crate::alphacode_render_core::reasoning_summary_line_markup;
...
/// Check if a line is visually blank (empty or only empty spans).
/// Optimized with early return on first non-empty span.
fn line_is_blank(line: &Line<'_>) -> bool {
    if line.spans.is_empty() {
        return true;
    }
    // Fast path: check first span before iterating all
    if !line.spans[0].content.is_empty() {
        return false;
    }
...
/// Push a blank separator line if the last line isn't already blank.
/// Uses a lightweight check to avoid unnecessary allocations.
...
/// Push a block separator based on block kind and spacing mode.
...
/// Collapse consecutive blank lines into single blanks and strip
/// trailing blanks. Preserves intentionally blank lines that belong
/// to image/diagram placeholder bodies.
fn normalize_block_separators(lines: &mut Vec<Line<'static>>) {
    let original_len = lines.len();
    let mut normalized: Vec<Line<'static>> = Vec::with_capacity(original_len);
    let mut previous_blank = true;
    let mut preserve_blanks: usize = 0;
    let mut protected_len = 0usize;

    // Drain in-place to avoid cloning
    ...
    // Strip trailing blanks (but not protected placeholder bodies)
...
/// Syntax highlighting cache — keyed by (code_content, language) hash,
/// stores pre-highlighted lines to avoid re-highlighting during streaming.
...
// Modern markdown color palette — high contrast, clear hierarchy.
// Each color is chosen for maximum readability on dark backgrounds
// and consistent visual weight across the rendering pipeline.
...
/// Ensure a blockquote gutter prefix is present at the start of a line's spans.
/// Builds the prefix with a single allocation instead of `repeat()`.
fn ensure_blockquote_prefix(current_spans: &mut Vec<Span<'static>>, blockquote_depth: usize) {
    ...
    let capacity = blockquote_depth * 3;
...
/// Pre-allocates the prefix string to avoid repeated allocations.
fn with_blockquote_prefix(line: Line<'static>, blockquote_depth: usize) -> Line<'static> {
    if blockquote_depth == 0 {
        return line;
    }
    // Pre-allocate with known capacity: "│ " is 3 bytes × depth + capacity for line spans
    let prefix_capacity = blockquote_depth * 3;
    let mut spans = Vec::with_capacity(line.spans.len() + 1);
    // Build the gutter prefix with a single allocation
...
/// Flush accumulated spans into a new line and push to the result vec.
/// Takes ownership of the spans Vec to avoid cloning.
...
/// Center a range of lines within the given width by prepending
/// space padding. Pre-computes the padding string once and reuses it.
fn center_structured_block_ranges(
    ...
    for range in ranges {
        ...
        if pad > 0 {
            // Allocate the padding string once and reuse across all lines
...
/// Recenter structured blocks (code blocks, tables, etc.) within the
/// terminal width. Pre-computes padding strings to avoid repeated
...
/// Render display math as framed lines with pre-allocated capacity.
...
/// Render raw math as framed lines with pre-allocated capacity.

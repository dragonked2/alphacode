...
//! key), moved out of `alphacode-base` so provider edits compile only this crate
...
pub use crate::alphacode_provider_gemini::{
...
impl GeminiProvider {
    fn persisted_catalog_path() -> Result<std::path::PathBuf> {
        Ok(crate::alphacode_base::storage::app_config_dir()?.join("gemini_models_cache.json"))
    }

    fn load_persisted_catalog() -> Option<PersistedCatalog> {
        let path = Self::persisted_catalog_path().ok()?;
        crate::alphacode_base::storage::read_json(&path)
            .ok()
    ...
    fn persist_catalog(models: &[String]) {
        ...
        if let Err(error) = crate::alphacode_base::storage::write_json(&path, &payload) {
    ...
    /// cloudcode-pa tier. Set `ALPHACODE_GEMINI_FORCE_OAUTH=1` to pin OAuth even when
    /// a key is present.
    fn auth_mode() -> GeminiAuthMode {
        let force_oauth = std::env::var("ALPHACODE_GEMINI_FORCE_OAUTH")
            .map(|value| {
        ...
        let response: Value = match self.post_json("loadCodeAssist", &load_req).await {
            ...
            Err(err) if is_gemini_code_assist_unsupported_error(&err) => {
                ...
                return Err(err).context(
                    "Gemini Code Assist is no longer supported for individual Google accounts. \
                     Set GEMINI_API_KEY with a Gemini Developer API key (from https://aistudio.google.com/apikey) \
                     or use the 'gemini-api' provider profile.",
                );
            }
            Err(err) => {
...
impl Provider for GeminiProvider {
    ...
    fn model_routes(&self) -> Vec<crate::alphacode_provider_core::ModelRoute> {
        let api_method = match Self::auth_mode() {
            GeminiAuthMode::ApiKey(_) => "gemini-api-key",
            GeminiAuthMode::Oauth => "code-assist-oauth",
        };
        self.available_models_display()
            .into_iter()
            .map(|model| crate::alphacode_provider_core::ModelRoute {
                model,
...
fn gemini_http_client() -> reqwest::Client {
    reqwest::Client::builder()
        .user_agent("alphacode/1.0 (gemini)")
        .http1_only()
        ...
        .build()
        .unwrap_or_else(|_| crate::alphacode_provider_core::shared_http_client())
}
...
fn is_transient_gemini_transport_error(err: &reqwest::Error) -> bool {
    ...
    err.is_connect()
        || err.is_timeout()
        || crate::alphacode_provider_core::is_transient_transport_error(&err.to_string())
}
...
/// Detect Google's "no longer supported for Gemini Code Assist for
/// individuals" error. When this fires, the Code Assist OAuth path is
/// dead for the account and the user should either configure a Gemini
/// Developer API key (`GEMINI_API_KEY`) or switch to the `gemini-api`
/// provider profile.

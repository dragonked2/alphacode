...
use crate::alphacode_provider_metadata::{is_safe_env_file_name, is_safe_env_key_name};
...
pub fn load_api_key_from_env_or_config(env_key: &str, file_name: &str) -> Option<String> {
    ...
    // Alphax Free (KiloCode gateway): no API key required. The bundled key
    // is empty — the gateway accepts anonymous requests. If the user has set
    // their own key, it was already loaded above.
    if env_key == "ALPHAX_FREE_API_KEY" {
        return None;
    }
...
/// Load a value only from the saved env file under the alphacode config dir,
...
pub fn save_env_value_to_env_file(
    ...
    let config_dir = crate::alphacode_storage::app_config_dir()?;
    let file_path = config_dir.join(file_name);
    crate::alphacode_storage::upsert_env_file_value(&file_path, env_key, value)?;

    if let Some(value) = value {
        crate::alphacode_core::env::set_var(env_key, value);
    } else {
        crate::alphacode_core::env::remove_var(env_key);
    }
...
mod tests {
    ...
    impl EnvGuard {
        fn new(keys: &[&'static str]) -> Self {
            // Take the shared test-env lock so these tests serialize against
            // every other module that mutates ALPHACODE_HOME (auth, storage,
            // build_support, ...) during parallel test runs.
            let lock = crate::alphacode_base::storage::lock_test_env();
            let saved = keys
                .iter()
    ...
    fn loads_api_key_from_env_before_config_file() {
        ...
        let _guard = EnvGuard::new(&["ALPHACODE_HOME", "ALPHACODE_PROVIDER_ENV_TEST_KEY"]);
    ...
    fn loads_and_removes_values_from_sandboxed_config_file() {
        let temp = tempfile::tempdir().expect("tempdir");
        let _guard = EnvGuard::new(&["ALPHACODE_HOME", "ALPHACODE_PROVIDER_ENV_TEST_VALUE"]);
        crate::alphacode_core::env::set_var("ALPHACODE_HOME", temp.path());

        save_env_value_to_env_file(
            "ALPHACODE_PROVIDER_ENV_TEST_VALUE",
            "provider-env-test.env",
            Some("file-value"),
        )
        .expect("save file value");

        crate::alphacode_core::env::remove_var("ALPHACODE_PROVIDER_ENV_TEST_VALUE");
        assert_eq!(
    ...
    fn accepts_legacy_zai_key_for_zhipu() {
        let temp = tempfile::tempdir().expect("tempdir");
        let _guard = EnvGuard::new(&["ALPHACODE_HOME", "ZHIPU_API_KEY", "ZAI_API_KEY"]);
        crate::alphacode_core::env::set_var("ALPHACODE_HOME", temp.path());

        save_env_value_to_env_file("ZAI_API_KEY", "zai.env", Some("legacy-zai-key"))
            .expect("save legacy key");
        crate::alphacode_core::env::remove_var("ZAI_API_KEY");
    ...
    fn loads_api_key_with_zero_width_space_from_config_file() {
        let temp = tempfile::tempdir().expect("tempdir");
        let _guard = EnvGuard::new(&["ALPHACODE_HOME", "ALPHACODE_PROVIDER_FOO_API_KEY"]);
        crate::alphacode_core::env::set_var("ALPHACODE_HOME", temp.path());

        // Write an env file with a U+200B zero-width space prefixed onto the key,
        // mirroring issue #376's reproduction.
        let config_dir = crate::alphacode_storage::app_config_dir().expect("config dir");
        std::fs::create_dir_all(&config_dir).expect("create config dir");
        std::fs::write(
            config_dir.join("provider-foo.env"),
            "ALPHACODE_PROVIDER_FOO_API_KEY=\u{200B}sk-mykey123\n",
        )
        .expect("write env file");

        assert_eq!(
            load_api_key_from_env_or_config("ALPHACODE_PROVIDER_FOO_API_KEY", "provider-foo.env")
                .as_deref(),
            Some("sk-mykey123")
        );
    }

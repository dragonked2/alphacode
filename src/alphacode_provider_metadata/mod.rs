...
mod tests {
    ...
    #[test]
    fn dragonmeta_profile_is_local_openai_compatible_without_required_api_key() {
        assert_eq!(DRAGONMETA_PROFILE.id, "dragonmeta");
        assert_eq!(DRAGONMETA_PROFILE.display_name, "Dragonmeta");
        assert_eq!(DRAGONMETA_PROFILE.api_base, "http://127.0.0.1:8080/v1");
        assert_eq!(DRAGONMETA_PROFILE.api_key_env, "DRAGONMETA_API_KEY");
        assert_eq!(DRAGONMETA_PROFILE.env_file, "dragonmeta.env");
        assert_eq!(DRAGONMETA_PROFILE.default_model, Some("alpha"));
        const {
            assert!(!DRAGONMETA_PROFILE.requires_api_key);
        }

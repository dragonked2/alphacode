...
///
/// Disable entirely with `ALPHACODE_WATCHDOG=0`. Tune with
/// `ALPHACODE_WATCHDOG_STALL_SECS` and `ALPHACODE_WATCHDOG_HEARTBEAT_SECS`.
pub fn start() {
    if std::env::var("ALPHACODE_WATCHDOG").is_ok_and(|value| value == "0" || value == "off") {
        return;
    }
    ...
    std::thread::Builder::new()
        .name("alphacode-watchdog".to_string())

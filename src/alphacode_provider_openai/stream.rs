...
pub fn parse_openai_response_event(
    ...
    if is_websocket_fallback_notice(data) {
        crate::alphacode_logging::warn(&format!("OpenAI stream transport notice: {}", data.trim()));
        return None;
    }
...
fn handle_openai_image_generation_item(
    ...
    let image_bytes = match BASE64_STANDARD.decode(result_b64) {
        ...
        Err(err) => {
            ...
            return Some(StreamEvent::TextDelta(
                "\n[Generated image received, but Alphacode could not decode it.]\n".to_string(),
            ));
        }
    };
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        return Some(StreamEvent::TextDelta(format!(
            "\n[Generated image received ({} bytes), but Alphacode could not save it.]\n",
            image_bytes.len()
        )));
    }
    ...
    if let Err(err) = std::fs::write(&path, image_bytes) {
        crate::alphacode_logging::warn(&format!("Failed to save OpenAI generated image: {}", err));
        return Some(StreamEvent::TextDelta(
            "\n[Generated image received, but Alphacode could not save it.]\n".to_string(),
        ));
    }
    ...
    let metadata_path_string = match serde_json::to_vec_pretty(&metadata).ok().and_then(|bytes| {
        ...
        None => {
            crate::alphacode_logging::warn("Failed to save OpenAI generated image metadata");
            None
        }
    };
...
pub struct OpenAIResponsesStream {
    ...
    utf8: crate::alphacode_core::util::Utf8StreamDecoder,
...
impl OpenAIResponsesStream {
    pub fn new(stream: impl Stream<Item = Result<Bytes, reqwest::Error>> + Send + 'static) -> Self {
        Self {
            ...
            buffer: String::new(),
            utf8: crate::alphacode_core::util::Utf8StreamDecoder::new(),

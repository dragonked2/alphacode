...
/// Endpoints disk cache TTL (2 hours). Matches the runtime's
/// `ENDPOINTS_CACHE_TTL_SECS` — endpoint data rarely changes, and doubling
/// the TTL halves the endpoint-related request volume on free-tier providers.
...
fn configured_cache_namespace() -> String {
    let raw = std::env::var("ALPHACODE_OPENROUTER_CACHE_NAMESPACE")
        .ok()
...
pub fn parse_provider_routing_from_env() -> ProviderRouting {
    ...
    if let Ok(providers) = std::env::var("ALPHACODE_OPENROUTER_PROVIDER") {
    ...
    if std::env::var("ALPHACODE_OPENROUTER_NO_FALLBACK").is_ok() {

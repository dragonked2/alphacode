...
//! recorded across their agent sessions (alphacode + imported Claude/Codex/etc.).
//!
...
//! launch hotkeys (e.g. `Cmd+[`, `Cmd+]`) to "open alphacode here". The ranking is a
...
pub struct RankOptions {
    ...
    /// the user's home directory, which is noise from launching alphacode at `$HOME`.
...
/// alphacode in. Produced by [`build_launch_hotkey_plan`] from a ranking, then
/// persisted to config so the mapping is baked once and does not move around.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PlannedHotkey {
    /// alphacode-style chord string, e.g. `cmd+;` or `cmd+[`.
    pub chord: String,
    /// Absolute directory the hotkey opens alphacode in.
    pub dir: String,
...
/// Scan a alphacode sessions directory and extract one [`SessionLocation`] per
...
pub fn collect_alphacode_session_locations(sessions_dir: &Path) -> Vec<SessionLocation> {
    use std::fs;
...
/// Compute the baked launch-hotkey plan from a alphacode sessions directory.
///
...
pub fn plan_launch_hotkeys_from_sessions(
    ...
    let locations = collect_alphacode_session_locations(sessions_dir);
...
mod tests {
    ...
    fn plan_assigns_top_repo_home_then_next_repos() {
        ...
        assert_eq!(plan[0].dir, "/u/jeremy/alphacode");
    ...
    fn plan_skips_home_if_it_appears_in_ranking() {
        ...
        // Top repo slot is alphacode (home filtered out), then home gets cmd+'.
        assert_eq!(plan[0].dir, "/u/jeremy/alphacode");
        assert_eq!(plan[1].dir, "/u/jeremy");
    ...
    fn collect_reads_working_dirs_and_skips_junk() {
        ...
        write(
            "session_a.json",
            r#"{"working_dir":"/Users/jeremy/alphacode-github","id":"a"}"#,
        );
        write(
            "session_b.json",
        ...
        let mut locs = collect_alphacode_session_locations(dir.path());
        locs.sort_by(|a, b| a.working_dir.cmp(&b.working_dir));
        let dirs: Vec<&str> = locs.iter().map(|l| l.working_dir.as_str()).collect();
        assert_eq!(
            dirs,
            vec!["/Users/jeremy/alphacode-github", "/Users/jeremy/scrollwm"]
        );

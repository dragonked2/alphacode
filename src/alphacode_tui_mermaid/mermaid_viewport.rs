...
fn kitty_viewport_unique_id(hash: u64) -> u32 {
    ...
    loop {
        ...
        if sequence != 0 {
            // Reserve the 0x4Axxxxxx namespace ('J') for alphacode's custom virtual
...
pub(super) fn clear_image_area(area: Rect, buf: &mut Buffer) {
    ...
    crate::alphacode_tui_workspace::color_support::clear_buf(clamped, buf);
}
...
mod kitty_viewport_leak_tests {
    ...
    fn kitty_cache_eviction_queues_terminal_image_deletion() {
        let _guard = crate::alphacode_tui_mermaid::IMAGE_TEST_LOCK
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    fn pending_terminal_cleanup_renders_without_changing_visible_cell_text() {
        ...
        assert!(crate::alphacode_tui_mermaid::render_pending_terminal_image_cleanup(&mut buf));

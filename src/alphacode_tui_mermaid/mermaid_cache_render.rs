...
/// Per-source-hash counts of layout computations performed on the cache-miss
/// path, for tests that assert the layout tier is reused across PNG
/// width-bucket crossings and aspect profiles. Content hashes are unique per
/// test (timestamp-derived diagram bodies), so a global map is safe.
#[cfg(all(test, feature = "renderer"))]
static LAYOUT_COMPUTATIONS_FOR_TEST: LazyLock<Mutex<HashMap<u64, usize>>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));

#[cfg(all(test, feature = "renderer"))]
pub(super) fn record_layout_computation_for_test(hash: u64) {
    if let Ok(mut counts) = LAYOUT_COMPUTATIONS_FOR_TEST.lock() {
        *counts.entry(hash).or_insert(0) += 1;
    }
}

#[cfg(all(test, feature = "renderer"))]
pub(super) fn layout_computations_for_test(hash: u64) -> usize {
    LAYOUT_COMPUTATIONS_FOR_TEST
        .lock()
        .map(|counts| counts.get(&hash).copied().unwrap_or(0))
        .unwrap_or(0)
}

/// Drop both the in-memory entry and any on-disk renditions for `hash`, so the
/// next render must re-rasterize while the layout tier stays warm.
#[cfg(test)]
pub(super) fn evict_render_cache_for_test(hash: u64) {
    evict_render_cache_by_hash(hash);
}

/// Insert a render-cache entry under the ambient (possibly aspect-tagged)
/// profile, exactly as a deferred transcript render does.
#[cfg(test)]
...
/// Profile-agnostic in-memory lookup used by draw/probe paths outside an
/// aspect render scope.
#[cfg(test)]
...
/// Convenience wrapper that hashes the mermaid source for callers that
/// have the raw diagram text rather than a pre-computed hash.
...
fn deferred_render_sender() -> &'static mpsc::Sender<DeferredRenderTask> {
    DEFERRED_RENDER_TX.get_or_init(|| {
        ...
        if let Err(err) = std::thread::Builder::new()
            .name("alphacode-mermaid-deferred".to_string())
            .spawn(move || deferred_render_worker(rx))
        {
            crate::alphacode_tui_mermaid::log_warn(&format!(
...
fn deferred_render_worker(rx: mpsc::Receiver<DeferredRenderTask>) {
    for task in rx {
        ...
        bump_deferred_render_epoch();
        crate::alphacode_tui_mermaid::notify_render_completed();
    }
}

...
fn preferred_aspect_ratio_adjusts_render_height_without_changing_width_bucket() {
    let (default_width, default_height) =
        super::cache_render::calculate_render_size(6, 5, Some(80));
...
fn image_scroll_steady_state_has_no_per_frame_stats_or_rebuilds() {
    ...
    assert!(
        result.retained_image_state_source_bytes <= super::IMAGE_STATE_MAX_SOURCE_BYTES as u64,
...
fn warm_fit_draws_do_not_stat_the_render_cache() {
    ...
    let (hash, _, _) = super::materialize_inline_image("image/png", &b64).expect("materialize");
    ...
    super::render_image_widget_fit(hash, area, &mut buf, false, true);
    let before = super::cache_stat_syscalls();
    super::render_image_widget_fit(hash, area, &mut buf, false, true);
    super::render_image_widget_fit(hash, area, &mut buf, false, true);
    assert_eq!(
        super::cache_stat_syscalls() - before,
        0,
...
fn cache_eviction_recognizes_every_inline_extension() {
    for media_type in [
        ...
        let ext = super::inline_image::mermaid_inline_extension_for_test(media_type);
        assert!(
            super::CACHE_FILE_EXTENSIONS.contains(&ext),
...
fn bounded_bookkeeping_insert_caps_map_growth() {
    ...
    for hash in 0..(super::RENDER_BOOKKEEPING_MAX as u64 * 2) {
        super::bounded_bookkeeping_insert(&mut map, hash, 0);
        assert!(
            map.len() <= super::RENDER_BOOKKEEPING_MAX,
            "bookkeeping map exceeded its cap at {} entries",
            map.len()
        );
    }
    let last = super::RENDER_BOOKKEEPING_MAX as u64 * 2 - 1;
    ...
    let before = map.len();
    super::bounded_bookkeeping_insert(&mut map, last, 1);
    assert_eq!(map.len(), before, "existing-key update must not clear");
}

#[test]
fn source_image_cache_enforces_decoded_byte_budget() {
    let mut cache = super::SourceImageCache::new();
    let accounted = super::SOURCE_CACHE_MAX_BYTES / 2 + 1;
    for hash in 1..=2u64 {
...
fn fitted_source_cache_enforces_budget_and_replaces_resize_variants() {
    let mut cache = super::FittedSourceCache::new();
    let accounted = super::FITTED_SOURCE_CACHE_MAX_BYTES / 2 + 1;
    for hash in 1..=2u64 {
        let key = super::FittedSourceKey {
            hash,
            target_cols: 80,
            target_rows: 16,
            font_size: (8, 16),
...
fn image_state_cache_enforces_source_byte_budget() {
    super::force_test_kitty_picker();
    let picker = super::PICKER
        .get()
        .and_then(|picker| picker.as_ref())
        .expect("test picker");
    let mut cache = super::ImageStateCache::new();
    let accounted = super::IMAGE_STATE_MAX_SOURCE_BYTES / 2 + 1;
    for hash in 1..=2u64 {
        ...
        cache.insert(
            ...
            super::ImageState {
                ...
                last_area: None,
                resize_mode: super::ResizeMode::Fit,
                last_crop_top: false,
                last_viewport: None,
            },
        );
    }
    assert!(cache.total_source_bytes <= super::IMAGE_STATE_MAX_SOURCE_BYTES);
...
fn png_dimension_probe_needs_only_the_header() {
    // Rust test thread names contain `::` module separators, which are not
    // valid in Windows filenames; sanitize to keep the temp file portable.
    let thread = std::thread::current()
        .name()
        .unwrap_or("test")
        .replace(':', "_");
    let path = std::env::temp_dir().join(format!(
        "alphacode-png-header-{}-{}.png",
        std::process::id(),
        thread
    ));
    ...
    assert_eq!(super::get_png_dimensions(&path), Some((123, 456)));
...
fn inline_fit_geometry_and_marker_roundtrip() {
    ...
    let (rows, cols) = super::inline_fit_geometry(1600, 400, 80, super::INLINE_DIAGRAM_MAX_ROWS);
    assert!(rows >= super::INLINE_FIT_MIN_ROWS);
    assert!(rows < super::INLINE_DIAGRAM_MAX_ROWS);
    assert!(cols <= 80);

    // Tall image: height-bound by the cap.
    let (tall_rows, _) = super::inline_fit_geometry(400, 40_000, 80, 20);
    assert_eq!(tall_rows, 20);

    // Placeholder lines round-trip through the parser.
    let lines = super::inline_image_placeholder_lines(0xabcdef, rows, cols);
    assert_eq!(lines.len(), rows as usize);
    let parsed = super::parse_inline_image_placeholder(&lines[0]);
    ...
    assert_eq!(
        super::parse_inline_image_placeholder(&padded),
...
fn inline_transcript_aspect_goal_produces_expected_bucketed_profile() {
    ...
    let goal = super::inline_transcript_aspect_goal_with_font(120, 40, Some((8, 16)));
    assert_eq!(goal, Some(1.75));

    // The goal flows through the standard profile bucketing (per-mille).
    let bucket =
        super::with_preferred_aspect_ratio(goal, super::current_preferred_aspect_ratio_bucket);
    assert_eq!(bucket, Some(1750));

    // Narrow terminals floor at the 4:3 sizing default instead of requesting
    // portrait renders.
    let narrow = super::inline_transcript_aspect_goal_with_font(40, 50, Some((8, 16)));
    assert!(
    ...
    let flat = super::inline_transcript_aspect_goal_with_font(500, 12, Some((8, 16)));
...
fn inline_transcript_aspect_goal_is_stable_under_resize_jitter() {
    let a = super::inline_transcript_aspect_goal_with_font(120, 40, Some((8, 16)));
    let b = super::inline_transcript_aspect_goal_with_font(121, 40, Some((8, 16)));
    let c = super::inline_transcript_aspect_goal_with_font(120, 39, Some((8, 16)));
    ...
    let bucket_a = super::preferred_aspect_ratio_bucket(a);
    let bucket_b = super::preferred_aspect_ratio_bucket(b);
...
fn inline_transcript_aspect_goal_no_geometry_yields_none() {
    assert_eq!(
        super::inline_transcript_aspect_goal_with_font(120, 40, None),
        None,
        "unknown font size must not invent an aspect goal"
    );
    assert_eq!(
        super::inline_transcript_aspect_goal_with_font(0, 40, Some((8, 16))),
        None,
        "zero-width chat area must not produce a goal"
    );
    assert_eq!(
        super::inline_transcript_aspect_goal_with_font(120, 0, Some((8, 16))),
        None,
...
fn transcript_profile_keeps_pinned_pane_aspect_when_present() {
    ...
    let combined =
        super::transcript_preferred_aspect_ratio_with_font(pane_aspect, 120, 40, Some((8, 16)));
    assert_eq!(
        combined, pane_aspect,
        "pinned pane aspect must win over the inline goal"
    );

    let fallback = super::transcript_preferred_aspect_ratio_with_font(None, 120, 40, Some((8, 16)));
    assert_eq!(fallback, Some(1.75), "no pane -> inline goal");

    let no_geometry = super::transcript_preferred_aspect_ratio_with_font(None, 120, 40, None);

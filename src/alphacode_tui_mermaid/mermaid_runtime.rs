...
/// Terminal multiplexers / agent-multiplexers that sit between alphacode and the
...
/// Detect whether alphacode is running inside a known multiplexer, using the same
...
///
/// * `probe_override` is the parsed value of `ALPHACODE_MERMAID_PICKER_PROBE`
...
///   `ALPHACODE_MERMAID_PICKER_PROBE=1`.
...
/// Parse only the explicit `ALPHACODE_MERMAID_PICKER_PROBE` override into a mode,
...
pub(super) fn iterm2_images_opt_in() -> bool {
    std::env::var("ALPHACODE_ITERM2_IMAGES")
        .ok()
        .as_deref()
...
fn query_font_size() -> (u16, u16) {
    match crossterm::terminal::window_size() {
        Ok(ws) if ws.columns > 0 && ws.rows > 0 && ws.width > 0 && ws.height > 0 => {
            ...
            if fw > 0 && fh > 0 {
                crate::alphacode_tui_mermaid::log_info(&format!(
    ...
    match Picker::from_query_stdio() {
        Ok(probed) => {
            ...
            if protocol == ProtocolType::Iterm2 && real_iterm2_without_opt_in() {
                crate::alphacode_tui_mermaid::log_info(
...
pub(crate) fn prewarm_svg_font_db_async() {
    SVG_FONT_DB_PREWARM_STARTED.get_or_init(|| {
        let _ = std::thread::Builder::new()
            .name("alphacode-mermaid-fontdb-prewarm".to_string())
            .spawn(|| {
                let _ = &*SVG_FONT_DB;
            });
    });
...
/// By default alphacode uses environment-based detection and never blocks startup
/// on terminal capability responses. Set ALPHACODE_MERMAID_PICKER_PROBE=1 to run an
...
pub fn init_picker() {
    PICKER.get_or_init(|| {
        ...
        let probe_override = std::env::var("ALPHACODE_MERMAID_PICKER_PROBE")
            .ok()
            .as_deref()
            .and_then(parse_env_bool);
...
mod tests {
    ...
    fn rediscovery_fails_when_the_external_image_file_is_gone() {
        let path = std::env::temp_dir().join(format!(
            "alphacode-missing-external-image-{}-{:?}.png",
            std::process::id(),
            std::thread::current().id()
        ));

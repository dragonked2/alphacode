...
//! `alphacode-base` so provider edits compile only this crate plus a binary
...
//! loading, cache-TTL toggles) stay in `crate::alphacode_base::provider::anthropic`;
...
use crate::alphacode_provider_core::{EventStream, NativeToolResultSender, Provider};
...
use crate::alphacode_message_types::{Message, StreamEvent, ToolDefinition};
use crate::alphacode_provider_anthropic::{
...
fn load_official_claude_client_metadata() -> OAuthClientMetadata {
    let path = match crate::alphacode_base::storage::user_home_path(".claude.json") {
        Ok(path) => path,
...
async fn oauth_preflight_get(
    ...
    if !resp.status().is_success() {
        ...
        let body = crate::alphacode_base::util::http_error_body(resp, "HTTP error").await;
...
async fn ensure_oauth_preflight(
    ...
    let eval = OAuthEvalRequest {
        attributes: OAuthEvalAttributes {
            ...
            subscription_type: crate::alphacode_base::auth::claude::get_subscription_type()
    ...
    if all_ok {
        crate::alphacode_base::logging::info("Claude OAuth preflight completed successfully");
    }
    Ok(())
}

/// Quality-first default shared with model routing and first-run selection.
const DEFAULT_MODEL: &str = crate::alphacode_provider_core::DEFAULT_CLAUDE_MODEL;
...
pub struct AnthropicProvider {
    ...
    /// Explicit `ALPHACODE_ANTHROPIC_MAX_TOKENS` override. When unset, the output
...
impl AnthropicProvider {
    fn is_usage_exhausted() -> bool {
        let usage = crate::alphacode_base::usage::get_sync();
    ...
    /// `ALPHACODE_RUNTIME_PROVIDER` happens to be in the current process (e.g. a
    /// self-dev session may have it set to `claude-api`). This also updates
    /// `ALPHACODE_RUNTIME_PROVIDER` so any provider instances the probes build
    ...
    pub async fn fetch_live_model_ids_for_doctor(&self) -> Result<Vec<String>> {
        ...
        let catalog = if is_oauth {
            crate::alphacode_base::provider::fetch_anthropic_model_catalog_oauth(&token).await?
        } else {
            crate::alphacode_base::provider::fetch_anthropic_model_catalog(&token).await?
        };
    ...
    pub fn new() -> Self {
        let model = std::env::var("ALPHACODE_ANTHROPIC_MODEL").unwrap_or_else(|_| {
        ...
        let _ = tokio::runtime::Handle::try_current().map(|_| {
            tokio::spawn(async {
                let _ = crate::alphacode_base::usage::get().await;
            })
        });

        let max_tokens_override = std::env::var("ALPHACODE_ANTHROPIC_MAX_TOKENS")
            .ok()
            .and_then(|v| v.trim().parse::<u32>().ok());
        let reasoning_effort = crate::alphacode_base::config::config()
            .provider
        ...
        Self {
            ...
            credential_mode: Arc::new(RwLock::new(AnthropicCredentialMode::from_runtime_env(
                crate::alphacode_provider_core::DualAuthProvider::Anthropic,
            ))),
            max_tokens_override,
    ...
    fn model_supports_output_effort(model: &str) -> bool {
        ...
        // generations); see `crate::alphacode_provider_core::anthropic_reasoning_caps`.
        // Fable 5 verified live 2026-07-01; Sonnet 5 verified live 2026-07-07.
        crate::alphacode_provider_core::anthropic_reasoning_caps(model).output_effort
    }

    fn model_supports_adaptive_thinking(model: &str) -> bool {
        crate::alphacode_provider_core::anthropic_reasoning_caps(model).adaptive_thinking
    }

    fn model_supports_manual_thinking(model: &str) -> bool {
        crate::alphacode_provider_core::anthropic_reasoning_caps(model).manual_thinking
    }

    fn model_supports_xhigh_effort(model: &str) -> bool {
        crate::alphacode_provider_core::anthropic_reasoning_caps(model).xhigh_effort
    }
    ...
    fn model_supports_max_effort(model: &str) -> bool {
        crate::alphacode_provider_core::anthropic_reasoning_caps(model).max_effort
    }

    fn model_supports_reasoning_effort(model: &str) -> bool {
        crate::alphacode_provider_core::anthropic_reasoning_caps(model).supports_reasoning_effort()
    }
    ...
    fn actual_effort_for_model(model: &str, effort: &str) -> String {
        if crate::alphacode_base::prompt::is_swarm_effort(effort) {
    ...
    fn store_effort_for_model(model: &str, effort: &str) -> String {
        if crate::alphacode_base::prompt::is_deep_swarm_effort(effort) {
            crate::alphacode_base::prompt::SWARM_DEEP_EFFORT.to_string()
        } else if crate::alphacode_base::prompt::is_swarm_effort(effort) {
            crate::alphacode_base::prompt::SWARM_EFFORT.to_string()
        } else {
    ...
    fn manual_thinking_budget(effort: &str, max_tokens: u32) -> Option<u32> {
        let desired = match effort {
            ...
            "high" => 8_192,
            "xhigh" | "max" => 16_384,
            e if crate::alphacode_base::prompt::is_swarm_effort(e) => 16_384,
            _ => return None,
        };
    ...
    async fn get_access_token(&self) -> Result<(String, bool)> {
        ...
        // alphacode OAuth account may still deserialize successfully even after its
    ...
    async fn get_oauth_access_token(&self) -> Result<(String, bool)> {
        ...
        {
            anyhow::bail!(
                "Claude OAuth credentials are missing the required user:inference scope (scopes: {}). Run `alphacode login --provider claude` to mint a fresh Claude.ai OAuth token, or import/use a fresh Claude Code login.",
        ...
        if fresh_creds.expires_at < now + 300_000 && !fresh_creds.refresh_token.is_empty() {
            ...
            if auth::refresh_state::refresh_token_is_known_rejected(
                ...
                anyhow::bail!(
                    "Claude OAuth refresh token was previously rejected by Anthropic and cannot be refreshed. Run `alphacode login --provider claude` to mint a fresh token."
                );
            }
            ...
            {
                Ok(refreshed) => {
                    crate::alphacode_base::logging::info("OAuth token refreshed successfully");
        ...
        if fresh_creds.expires_at <= now {
            anyhow::bail!(
                "Claude OAuth token is expired and no usable refresh token is available. Run `alphacode login --provider claude` to refresh OAuth credentials"
            );
        }
    ...
    pub(crate) fn set_credential_mode(&self, mode: AnthropicCredentialMode) -> Result<()> {
        ...
        *mode_guard = mode;
        ...
        crate::alphacode_base::auth::AuthStatus::invalidate_cached_status();
        Ok(())
    }
    ...
    fn format_messages(&self, messages: &[Message], is_oauth: bool) -> Vec<ApiMessage> {
        crate::alphacode_provider_anthropic::format_messages(messages, is_oauth)
    }
    ...
    fn format_tools(&self, tools: &[ToolDefinition], is_oauth: bool) -> Vec<ApiTool> {
        crate::alphacode_provider_anthropic::format_tools(tools, is_oauth, is_cache_ttl_1h())
    }
}
...
fn log_anthropic_canonical_input(
    ...
    crate::alphacode_provider_core::fingerprint::log_provider_canonical_input(
        "anthropic",
        model,
        format,
        &payload,
...
impl Provider for AnthropicProvider {
    ...
    fn credential_mode(&self) -> crate::alphacode_provider_core::CredentialMode {
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        let model: &str = if is_1m_model(model)
            && matches!(
                crate::alphacode_provider_core::anthropic_context_mode(model),
                crate::alphacode_provider_core::AnthropicContextMode::Native1M
            ) {
            strip_1m_suffix(model)
        } else {
            model
        };
        // Allow custom model names (e.g. from --model or manual entry).
        // Warn instead of error so users can try models not yet in the known list.
        if !crate::alphacode_base::provider::known_anthropic_model_ids()
            .iter()
            .any(|known| known == model)
        {
            crate::alphacode_logging::warn(&format!(
                "Model '{}' is not in the known Anthropic model list; it may not work. \
                 Use /model to choose from available models if needed.",
                model,
            ));
        }
        *self
    ...
    fn available_models_for_switching(&self) -> Vec<String> {
        crate::alphacode_base::provider::cached_anthropic_model_ids()
            .unwrap_or_else(crate::alphacode_base::provider::known_anthropic_model_ids)
    }
        ...
        .await
        {
            ...
            Err(e) => {
                ...
                if is_oauth && is_oauth_auth_error(&error_str) && !attempted_forced_refresh {
                    ...
                    let _ = tx
                        .send(Ok(StreamEvent::ConnectionPhase {
                            phase: crate::alphacode_message_types::ConnectionPhase::Authenticating,
                        }))
                        .await;
                        ...
                        Err(refresh_err) => {
                            let _ = tx
                                .send(Err(anyhow::anyhow!(
                                    "{}\n\nAutomatic Claude OAuth refresh failed: {}\nRun `alphacode login --provider claude` (preferred) or `claude`, then retry.",
                                    e,
                                    refresh_err
                                )))
                                .await;
                ...
                if is_oauth && is_oauth_auth_error(&error_str) {
                    let _ = tx
                        .send(Err(anyhow::anyhow!(
                            "{}\n\nClaude OAuth authentication failed. Run `alphacode login --provider claude` (preferred) or `claude`, then retry.",
                            e
                        )))
                        .await;
                } else {
...
async fn stream_response(
    ...
    if std::env::var("ALPHACODE_ANTHROPIC_DEBUG")
        .map(|v| v == "1")
        .unwrap_or(false)
        && let Ok(json) = serde_json::to_string_pretty(&request)
    {
        crate::alphacode_base::logging::info(&format!("Anthropic request payload:\n{}", json));
    }
...
fn anthropic_model_quality_rank(model: &str) -> usize {
    ...
    let normalized = crate::alphacode_provider_core::model_id::strip_date_suffix(
        &crate::alphacode_provider_core::model_id::canonical(model),
    )
    .to_string();
    crate::alphacode_provider_core::ALL_CLAUDE_MODELS
        .iter()
        .position(|candidate| {
            crate::alphacode_provider_core::model_id::strip_date_suffix(
                &crate::alphacode_provider_core::model_id::canonical(candidate),
            ) == normalized
        })
        ...
        // available.
        .unwrap_or(crate::alphacode_provider_core::ALL_CLAUDE_MODELS.len())
}
...
fn anthropic_recommended_model_from_error(error_str: &str) -> Option<String> {
    ...
    crate::alphacode_base::provider::known_anthropic_model_ids()
        .into_iter()
...
fn parse_sse_event(buffer: &mut String) -> Option<SseEvent> {
    ...
    for line in event_str.lines() {
        ...
        } else if let Some(rest) = crate::alphacode_base::util::sse_data_line(line) {
            data = rest.to_string();
        }
    }
    ...
    match event.event_type.as_str() {
            ...
            if let Ok(parsed) = serde_json::from_str::<MessageStartEvent>(&event.data) {
                ...
                if let Some(usage) = parsed.message.usage {
                    ...
                    if let Some(tier) = usage.service_tier.as_deref() {
                        ...
                        if std::env::var("ALPHACODE_LOG_SERVICE_TIER").is_ok() {
        ...
        "ping" => {
            ...
            events.push(StreamEvent::ConnectionPhase {
                phase: crate::alphacode_message_types::ConnectionPhase::Streaming,
            });
        }
        "error" => {
            crate::alphacode_base::logging::error(&format!(
...
fn build_system_param(system: &str, is_oauth: bool) -> Option<ApiSystem> {
    crate::alphacode_provider_anthropic::build_system_param(system, is_oauth, is_cache_ttl_1h())
}
...
fn build_system_param_split(
    ...
    crate::alphacode_provider_anthropic::build_system_param_split(
        static_part,
        dynamic_part,
        is_oauth,
        is_cache_ttl_1h(),

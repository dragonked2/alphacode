...
/// `ALPHACODE_CURSOR_CLI_VERSION` if Cursor moves the floor.
const CLI_CLIENT_VERSION_DEFAULT: &str = "cli-2026.07.08-0c04a8a";

fn cli_client_version() -> String {
    std::env::var("ALPHACODE_CURSOR_CLI_VERSION")
        .ok()
...
fn agent_host_from_cursor_cli_config() -> Option<String> {
    let path = match crate::alphacode_base::storage::user_home_path(".cursor/cli-config.json") {
        Ok(path) => path,
        Err(err) => {
...
/// 1. `ALPHACODE_CURSOR_AGENT_HOST` / `CURSOR_AGENT_HOST` (explicit override)
...
fn agent_host() -> String {
    for var in ["ALPHACODE_CURSOR_AGENT_HOST", "CURSOR_AGENT_HOST"] {
...
mod tests {
    ...
    fn agent_host_normalizes_env_overrides() {
        ...
        // shared lock in alphacode-base.
        let _guard = crate::alphacode_base::storage::lock_test_env();
        let prev = std::env::var_os("ALPHACODE_CURSOR_AGENT_HOST");
        ...
        crate::alphacode_base::env::set_var("ALPHACODE_CURSOR_AGENT_HOST", "   ");

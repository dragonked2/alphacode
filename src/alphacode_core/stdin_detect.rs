...
mod windows {
    ...
    /// Windows has no non-invasive equivalent of `/proc/<pid>/syscall` or
    /// macOS `thread_info()`, so this reports `Unknown` by design.
    ///
    /// Determining that another process is blocked in `ReadFile`/`ReadConsole`
    /// on stdin requires `NtQuerySystemInformation` (undocumented, and the
    /// thread-state layout is not a stable contract), `SeDebugPrivilege`, or
    /// code injection. None of those are acceptable costs for a hint, and each
    /// would need elevation the CLI otherwise never asks for.
    ///
    /// `Unknown` is a correct answer rather than a missing one: callers treat
    /// it as "cannot determine" and fall back to their default, whereas a
    /// guessed `Reading`/`NotReading` would make them act on a wrong signal.

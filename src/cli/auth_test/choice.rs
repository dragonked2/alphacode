...
pub(crate) async fn auth_test_choice_plan(
    ...
    Ok(AuthTestChoicePlan::Skip(format!(
        "Skipped: {} local endpoint reported no models. Re-run `alphacode auth-test --provider {} --model <local-model>` or set a default model first.",
...
pub(crate) fn tool_smoke_skip_detail_for_choice(
    ...
    if matches!(choice, super::provider_init::ProviderChoice::Cursor) {
        return Some(
            "Skipped: the Cursor native agent transport is text-only in alphacode (it does not expose \
...
fn effective_openai_compatible_auth_test_model(
    ...
    model
        ...
        .or_else(|| {
            std::env::var("ALPHACODE_OPENROUTER_MODEL")
                .ok()
...
async fn run_provider_tool_smoke_for_choice(
    ...
    run_auth_test_with_retry(async || {
        ...
        validate_auth_test_tool_smoke_transcript(&agent.messages()[transcript_start..], &output)
            .with_context(|| {
                format!(
                    "{} tool-enabled smoke prompt did not complete a valid real Alphacode tool loop",
                    choice.as_arg_value()
                )
            })?;

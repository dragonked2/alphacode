...
impl AuthTestTarget {
    ...
    fn credential_paths(self) -> Result<Vec<String>> {
        match self {
            Self::Claude => Ok(vec![
                crate::alphacode_base::auth::claude::alphacode_path()?.display().to_string(),
            ...
            Self::Gemini => Ok(vec![
                crate::alphacode_base::auth::gemini::tokens_path()?.display().to_string(),
                crate::alphacode_base::auth::gemini::gemini_cli_oauth_path()?
                    .display()
                    .to_string(),
                crate::storage::user_home_path(".local/share/opencode/auth.json")?
                    .display()
            ...
            Self::Antigravity => Ok(vec![
                crate::alphacode_base::auth::antigravity::tokens_path()?
                    .display()
                    .to_string(),
                crate::storage::user_home_path(".local/share/opencode/auth.json")?
                    .display()
            ...
            Self::Google => Ok(vec![
                crate::alphacode_base::auth::google::credentials_path()?
                    .display()
                    .to_string(),
                crate::alphacode_base::auth::google::tokens_path()?.display().to_string(),
            ]),
            Self::Copilot => Ok(vec![
            ...
            Self::Cursor => Ok(vec![
                dirs::config_dir()
                    ...
                    .display()
                    .to_string(),
                crate::alphacode_base::auth::cursor::cursor_auth_file_path()?
                    .display()
                    .to_string(),
...
impl AuthTestSmokeKind {
    ...
    fn success_detail(self) -> &'static str {
        match self {
            ...
            Self::Tool => {
                "Tool-enabled provider request returned AUTH_TEST_OK after one validated real Alphacode bash tool call, successful registry execution, and tool-result followup."
            }
        }
    }

fn generic_credential_paths_for_provider(
    ...
    match provider.target {
        crate::provider_catalog::LoginProviderTarget::Alphacode => {
            vec![config_dir.join(crate::subscription_catalog::ALPHACODE_ENV_FILE)]
        }
        ...
        crate::provider_catalog::LoginProviderTarget::Azure => {
            vec![config_dir.join(crate::alphacode_base::auth::azure::ENV_FILE)]
        }
...
fn probe_generic_provider_auth(
    ...
    report.push_step(
        "refresh_probe",
        true,
        "Skipped: provider does not expose a dedicated refresh probe in alphacode today.".to_string(),
    );
}
...
async fn probe_claude_auth(report: &mut AuthTestProviderReport) {
    if let Some(creds) = push_result_step(
        report,
        "credential_probe",
        crate::alphacode_base::auth::claude::load_credentials(),
        |creds| {
            format!(
        ...
        push_result_step(
            report,
            "refresh_probe",
            crate::alphacode_base::auth::oauth::refresh_claude_tokens(&creds.refresh_token).await,
            |tokens| {
                format!(
...
async fn probe_openai_auth(report: &mut AuthTestProviderReport) {
    if let Some(creds) = push_result_step(
        report,
        "credential_probe",
        crate::alphacode_base::auth::codex::load_credentials(),
        |creds| {
        ...
        if creds.refresh_token.trim().is_empty() {
            ...
            push_result_step(
                report,
                "refresh_probe",
                crate::alphacode_base::auth::oauth::refresh_openai_tokens(&creds.refresh_token).await,
                |tokens| {
                    format!(
...
async fn probe_gemini_auth(report: &mut AuthTestProviderReport) {
    ...
    if crate::alphacode_base::auth::gemini::has_api_key() {
        let detail = match crate::alphacode_base::auth::gemini::api_key() {
    ...
    if push_result_step(
        report,
        "credential_probe",
        crate::alphacode_base::auth::gemini::load_tokens(),
        |tokens| {
            format!(
    ...
    .is_some()
    {
        push_result_step(
            report,
            "refresh_probe",
            crate::alphacode_base::auth::gemini::load_or_refresh_tokens().await,
            |tokens| {
                format!(
...
async fn probe_antigravity_auth(report: &mut AuthTestProviderReport) {
    if push_result_step(
        report,
        "credential_probe",
        crate::alphacode_base::auth::antigravity::load_tokens(),
        |tokens| {
            format!(
    ...
    .is_some()
    {
        push_result_step(
            report,
            "refresh_probe",
            crate::alphacode_base::auth::antigravity::load_or_refresh_tokens().await,
            |tokens| {
                format!(
...
async fn probe_google_auth(report: &mut AuthTestProviderReport) {
    let creds_result = crate::alphacode_base::auth::google::load_credentials();
    let tokens_result = crate::alphacode_base::auth::google::load_tokens();
    match (creds_result, tokens_result) {
        (Ok(creds), Ok(tokens)) => {
            ...
            match crate::alphacode_base::auth::google::get_valid_token().await {
...
async fn probe_copilot_auth(report: &mut AuthTestProviderReport) {
    if let Some(token) = push_result_step(
        ...
        push_result_step(
            report,
            "refresh_probe",
            crate::alphacode_base::auth::copilot::exchange_github_token(&client, &token).await,
            |api_token| {
                format!(
...
async fn probe_cursor_auth(report: &mut AuthTestProviderReport) {
    let has_api_key = crate::alphacode_base::auth::cursor::has_cursor_api_key();
    let has_auth_file = crate::alphacode_base::auth::cursor::has_cursor_auth_file_token();
    let has_vscdb = crate::alphacode_base::auth::cursor::has_cursor_vscdb_token();
    ...
    report.push_step(
        "refresh_probe",
        true,
        "Skipped: Cursor provider does not expose a native refresh-token probe in alphacode today."
            .to_string(),
    );
}

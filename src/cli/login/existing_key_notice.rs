//! Tell the user when `alphacode login` found a credential they already saved.
//!
//! Split out of `login.rs` so the notice can carry its rationale and tests
//! without growing an already-oversized file.

use crate::alphacode_base::provider_catalog::ResolvedOpenAiCompatibleProfile;

/// Build the notice shown when a key is already configured, or `None` when
/// nothing is configured yet.
///
/// Without this the prompt looks identical whether alphacode found an existing

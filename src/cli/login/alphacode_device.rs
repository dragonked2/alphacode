//! CLI orchestration for the Alphacode account device authorization flow.
//!
    ...
    C: Future<Output = std::io::Result<()>>,
{
    ...
    loop {
        ...
        if tokio::time::Instant::now() + delay >= deadline {
            anyhow::bail!(
                "Alphacode account login timed out before browser approval. Run `alphacode account login` to try again."
            );
        }
        tokio::select! {
        ...
        match subscription_api::poll_device_token_once(client, api_base, device_code).await {
            ...
            Ok(TokenPollOutcome::Expired) => anyhow::bail!(
                "The browser approval expired or was already exchanged. Run `alphacode account login` to start a new single-use flow."
            ),
            Ok(TokenPollOutcome::Denied) => {
                anyhow::bail!("Alphacode account login was canceled or denied in the browser.")
            }
...
pub(super) async fn login_alphacode_device_flow(no_browser: bool) -> Result<LoginCompletion> {
    ...
    .context("Failed to start Alphacode account login")?;
    ...
    let completion = match activation {
        Some(ActivationOutcome::Active(me)) => {
            ...
            let tier = me
                ...
                .unwrap_or(me.tier);
            eprintln!(
                "  ✓ {} plan is active. Alphacode account login is complete.",
                tier
            );
        ...
        Some(ActivationOutcome::Revoked) => {
            ...
            anyhow::bail!(
                "The newly issued account key was revoked before plan activation. Local credentials were cleared; run `alphacode account login` again."
            );
        }
        Some(ActivationOutcome::Denied) => {
            crate::subscription_catalog::clear_account_credentials()?;
            anyhow::bail!(
                "The account server denied plan activation checks. Local credentials were cleared; run `alphacode account login` again."
            );
        }
        None => {
    ...
    crate::telemetry::record_auth_success("alphacode-subscription", "device_code_browser");
    Ok(completion)
}

fn print_recovery_actions() {
    eprintln!("  Check:   alphacode account status");

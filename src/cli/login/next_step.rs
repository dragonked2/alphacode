...
mod tests {
    ...
    fn extracted_hints_match_the_strings_login_printed_before_extraction() {
        assert_eq!(
            ...
            "Next step: install a model with `ollama pull llama3.2`, then run `alphacode --provider ollama --model llama3.2 run 'hello'`."
        );
        assert_eq!(
            local_endpoint_hint("lmstudio"),
            "Next step: load a chat model in LM Studio's Local Server, then run alphacode with that exact model id, for example `alphacode --provider lmstudio --model <model-id> run 'hello'`."
        );
        assert_eq!(
            local_endpoint_hint("ollama-turbo"),
            "Next step: run alphacode with a model available on this endpoint, for example `alphacode --provider ollama-turbo --model <model-id> run 'hello'`."
        );
    }

...
pub fn hot_reload(session_id: &str) -> Result<()> {
    ...
    if let Ok(migrate_binary) = std::env::var("ALPHACODE_MIGRATE_BINARY") {
        ...
        if binary_path.exists() {
            ...
            cmd.arg("--resume")
                .arg(session_id)
                .arg("--no-update")
                .env_remove("ALPHACODE_MIGRATE_BINARY")
                .current_dir(cwd);
            let err = crate::platform::replace_process(&mut cmd);
...
pub fn hot_update(session_id: &str) -> Result<()> {
    ...
    match update::check_for_update_blocking() {
        Ok(Some(release)) => {
            let current = crate::alphacode_build_meta::version();
...
/// Minimum interval between `git fetch` update probes across all alphacode
...
fn claim_update_fetch_slot() -> bool {
    let Ok(base) = crate::storage::alphacode_dir() else {
...
pub fn run_auto_update() -> Result<()> {
    ...
    if let Some(session_id) = crate::get_current_session() {
        use crate::alphacode_base::bus::{ClientMaintenanceAction, SessionUpdateStatus};
    ...
    if update::is_release_build() {
        ...
        match update::check_for_update_blocking() {
            Ok(Some(release)) => {
                ...
                reload_server_after_update("installed update");
                update::print_centered("Restart alphacode to use the new version.");
            }
            Ok(None) => {
                if repair_stale_shared_server_after_update_check() {
                    reload_server_after_update("repaired stale server target");
...
fn reload_server_after_update(reason: &str) {
    ...
    let Some(exe) = exe else {
        crate::logging::warn("update: could not find alphacode binary to reload stale server");
        return;
    };

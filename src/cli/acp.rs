...
impl AcpRuntime {
            ...
            _ if method.starts_with('_') => {
                if let Some(id) = message.id {
                    self.write_error_value(
                        ...
                        format!("Unsupported Alphacode ACP extension method: {method}"),
                    )
                    .await?;
                }
            }
        ...
        match self.create_new_session(cwd).await {
            ...
            Err(err) => {
                self.write_error_value(
                    ...
                    format!("Failed to create Alphacode session: {err:#}"),
                )
                .await?;
            }
        }
            ...
            .await
        {
            ...
            Err(err) => {
                self.write_error_value(
                    id,
                    JSONRPC_INTERNAL_ERROR,
                    format!("Failed to attach Alphacode session '{session_id}': {err:#}"),
                )
                .await?;
            }
        }
            ...
            match event {
                ...
                other => {
                    if self.profile.is_extended() {
                        self.write_alphacode_extension_event(&attached_id, &other)
                            .await?;
                    }
                }
            }
    ...
    async fn run_prompt(
        ...
        loop {
            ...
            if self.profile.is_extended() {
                self.write_alphacode_extension_event(&session.session_id, &event)
                    .await?;
            }
            match event {
                ServerEvent::Ack { .. } => {}
    ...
    async fn write_alphacode_extension_event(
        &self,
        session_id: &str,
        event: &ServerEvent,
    ) -> Result<()> {
        self.write_notification(
            "_alphacode/server_event",
            json!({
                "sessionId": session_id,
    ...
    fn map_event(&mut self, event: ServerEvent) -> Vec<Value> {
        match event {
            ...
            ServerEvent::Compaction { trigger, .. } if self.profile.is_extended() => vec![json!({
                ...
                "content": {
                    "type": "text",
                    "text": format!("\n[Alphacode compacted context: {trigger}]\n"),
                }
            })],
...
fn initialize_result(params: &Value, profile: AcpProfile) -> Value {
    ...
    json!({
        ...
        "agentInfo": {
            "name": "alphacode",
            "title": "Alphacode",
            "version": crate::alphacode_build_meta::pkg_version(),
        },
        "authMethods": [],
    })
}
...
fn ensure_no_acp_mcp_servers(params: &Value) -> std::result::Result<(), String> {
    match params.get("mcpServers") {
        ...
        Some(_) => Err(
            "ACP mcpServers are not supported yet; configure MCP servers in Alphacode config.toml"
                .to_string(),
        ),
    }
}
...
mod tests {
    ...
    #[test]
    fn initialize_standard_omits_alphacode_meta() {
    ...
    #[test]
    fn initialize_full_advertises_alphacode_extension_meta() {
        let result = initialize_result(&json!({"protocolVersion": 1}), AcpProfile::Full);
        assert_eq!(
            result["agentCapabilities"]["_meta"]["alphacode"]["profile"],
            "full"
        );
    }

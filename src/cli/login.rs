    ...
    match choice {
        ProviderChoice::Auto => {
            if options.uses_scriptable_flow()? {
                anyhow::bail!(
                    "Scriptable login flags require an explicit provider. Use `alphacode login --provider <provider> ...`."
                );
            }
            ...
            if !io::stdin().is_terminal() {
                anyhow::bail!(
                    "`alphacode login --provider auto` requires an interactive terminal. Use `alphacode login --provider <provider>` in non-interactive mode."
                );
            }
...
pub async fn run_login_provider(
    ...
    let login_result = if explicit_scriptable_flow {
        ...
        match provider.target {
            ...
            LoginProviderTarget::Alphacode => login_alphacode_flow(options.no_browser)
                .await
...
/// Best-effort: tell a running alphacode server that on-disk auth has changed so it
...
async fn notify_running_server_auth_changed_best_effort(provider: Option<&str>) {
    let Ok(mut client) = crate::alphacode_app_core::server::Client::connect().await else {
        crate::logging::auth_event(
...
async fn login_alphacode_flow(no_browser: bool) -> Result<()> {
    eprintln!("Starting alphacode subscription sign-in...");
    let _ = alphacode_device::login_alphacode_device_flow(no_browser).await?;
    Ok(())
}

pub(crate) async fn run_alphacode_account_login(no_browser: bool) -> Result<()> {
    login_alphacode_flow(no_browser).await
}
...
async fn login_claude_flow(requested_label: Option<&str>, no_browser: bool) -> Result<()> {
    ...
    eprintln!(
        "Account '{}' stored at {}",
        label,
        auth::claude::alphacode_path()?.display()
    );
...
fn login_azure_flow() -> Result<()> {
    ...
    eprintln!(
        "Reference: OpenCode supports Azure OpenAI with Entra credentials. alphacode uses Azure OpenAI's newer `/openai/v1` API with either Microsoft Entra ID or an API key.\n"
    );
    ...
    if use_entra {
        ...
        eprintln!(
            "That means alphacode can authenticate via `az login`, managed identity, or Azure environment credentials."
        );
    } else {
...
pub use crate::alphacode_base::secret_input::read_secret_line;
...
fn login_cursor_flow() -> Result<()> {
    ...
    eprintln!("alphacode will use the native Cursor HTTPS transport.");
...
async fn login_copilot_device_flow(no_browser: bool) -> Result<()> {
    ...
    let device_resp = crate::alphacode_base::auth::copilot::initiate_device_flow(&client).await?;
    ...
    let token = crate::alphacode_base::auth::copilot::poll_for_access_token(
        &client,
    ...
    crate::alphacode_base::auth::copilot::save_github_token(&token, &username)?;
...
async fn login_antigravity_flow(no_browser: bool) -> Result<()> {
    ...
    eprintln!(
        "alphacode will authenticate directly with Google Antigravity; the Antigravity desktop app is not required."
    );
    eprintln!(
        "If browser launch fails, or you pass `--no-browser`, alphacode will prompt for the callback URL instead."
    );
    eprintln!(
    ...
    let tokens = crate::alphacode_base::auth::antigravity::login(no_browser).await?;

    eprintln!("Successfully logged in to Antigravity!");
    eprintln!(
        "Tokens saved to {}",
        crate::alphacode_base::auth::antigravity::tokens_path()?.display()
    );
...
async fn login_gemini_flow(no_browser: bool) -> Result<()> {
    ...
    eprintln!(
        "If browser launch fails, or you pass `--no-browser`, alphacode will prompt for the manual authorization code."
    );
    eprintln!(
    ...
    let tokens = crate::alphacode_base::auth::gemini::login(no_browser).await?;

    eprintln!("Successfully logged in to Gemini!");
    eprintln!(
        "Tokens saved to {}",
        crate::alphacode_base::auth::gemini::tokens_path()?.display()
    );
    if let Some(email) = tokens.email.as_deref() {
        eprintln!("Google account: {}", email);
    }
...
fn login_gemini_api_key_flow() -> Result<()> {
    ...
    crate::alphacode_base::auth::gemini::save_api_key(&key)?;
    eprintln!("\nSuccessfully saved Gemini Developer API key!");
    eprintln!(
        "Stored at {}",
        crate::storage::app_config_dir()?
            .join(crate::alphacode_base::auth::gemini::GEMINI_API_KEY_ENV_FILE)
            .display()
    );
    eprintln!(
        ...
        Err(_) => {
            ...
            match input.trim() {
                ...
                "3" => {
                    ...
                    eprintln!("   - Fill in app name (e.g. 'alphacode') and your email");

...
/// `alphacode --resume` opens a searchable picker, so it read as "memorize this ID
/// or lose the session").
fn crash_resume_hint_lines(
    ...
    if crashed.len() == 1 {
        vec![
            ...
            // `alphacode --resume` opens a searchable list (issue #690).
...
mod crash_resume_hint_tests {
    ...
    fn single_session_hint_also_points_at_the_picker() {
        ...
        assert!(
            lines
                .iter()
                .any(|line| line.contains("Or browse all: alphacode --resume")),
...
pub fn init_tui_runtime() -> Result<(ratatui::DefaultTerminal, TuiRuntimeGuard)> {
    ...
    // ALPHACODE_RESUMING describes the session lifecycle, but only a valid modes
    ...
    // leaking them into tools or unrelated child alphacode processes.
...
pub fn print_session_resume_hint(session_id: &str) {
    // Print to BOTH stdout and stderr. The user typically runs `alphacode`
    // from an interactive shell (cmd.exe, PowerShell, bash). If the
    // terminal was attached to stderr (e.g. `2>&1` or TUI) the line will
    // already be visible there, but for the common case where the user
    // just types `alphacode` and Ctrl+C's or quits, the line on stderr
    // gets scrolled past the prompt and disappears. Writing to stdout
    // guarantees the resume command is in their scrollback so they can
    // scroll up, copy it, and paste it back into the same shell to
    // resume the session.

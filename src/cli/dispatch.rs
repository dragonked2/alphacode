...
fn is_file_controlled_debug_client() -> bool {
    std::env::var_os("ALPHACODE_DEBUG_CMD_PATH").is_some()
}
...
pub(crate) async fn run_main(mut args: Args) -> Result<()> {
    ...
    if let Some(tool_profile) = args.tool_profile.as_deref() {
        crate::alphacode_core::env::set_var("ALPHACODE_TOOL_PROFILE", tool_profile);
    }
    if let Some(tools) = args.tools.as_deref() {
        crate::alphacode_core::env::set_var("ALPHACODE_TOOLS", tools);
    }
    if let Some(disabled_tools) = args.disabled_tools.as_deref() {
        crate::alphacode_core::env::set_var("ALPHACODE_DISABLED_TOOLS", disabled_tools);
    }
    if args.disable_base_tools {
        crate::alphacode_core::env::set_var("ALPHACODE_DISABLE_BASE_TOOLS", "1");
    }
    ...
    match args.command {
        ...
        Some(Command::Server { action }) => match action {
            ServerCommand::Start { json } => {
                ...
                if json {
                    ...
                    println!("Alphacode server is running.");
                }
            }
            ...
            ServerCommand::Promote { version, json } => {
                run_server_promote(version.as_deref(), json).await?;
            }
        },
        Some(Command::Run {
            message,
        ...
        Some(Command::Telemetry(subcmd)) => {
            run_telemetry_command(&subcmd)?;
        }
        ...
        Some(Command::ProviderTestCoverage {
            ...
            let colorize = std::io::stdout().is_terminal()
                ...
                && std::env::var_os("ALPHACODE_NO_COLOR").is_none();
        ...
        match resolve_resume_id(&resume_id) {
            ...
            Err(e) => {
                match resume_resolution_failure_action(&resume_id, |key| std::env::var_os(key)) {
                    ...
                    // itself with `--resume <id>` and `ALPHACODE_RESUMING=1`. In the
                    ...
                    // making alphacode unusable after an auto-update (issue #328).
                    ...
                    ResumeResolutionFailureAction::Exit => {
                        ...
                        if !output::quiet_enabled() {
                            eprintln!("\nUse `alphacode --resume` to list available sessions.");
                        }
                        std::process::exit(1);
                    }
                }
...
where
    F: Fn(&str) -> Option<V>,
{
    if var_os("ALPHACODE_RESUMING").is_some() {
        ResumeResolutionFailureAction::DeferToServer
    } else {
        ResumeResolutionFailureAction::Exit
    }
...
async fn run_default_command(args: Args) -> Result<()> {
    ...
    let in_alphacode_repo = build::is_alphacode_repo(&cwd);
    startup_profile::mark("is_alphacode_repo");
    ...
    // hotkeys can reopen alphacode in the last project directory (Cmd+') and the
    // last alphacode repo for self-dev (Cmd+Shift+'). Best-effort; ignored unless a
    ...
    if in_alphacode_repo && !already_in_selfdev && !args.no_selfdev {
        output::stderr_info("📍 Detected alphacode repository - enabling self-dev mode");
        ...
        crate::alphacode_core::env::set_var(selfdev::CLIENT_SELFDEV_ENV, "1");
    ...
    if std::env::var_os("ALPHACODE_RESUMING").is_none() {
        crate::alphacode_tui::tui::theme_detect::prewarm_theme_mode();
    }
    ...
    if !server_running {
        ...
        if server::reap_stale_socket_if_dead(&server::socket_path()).await {
            output::stderr_info("Removed a stale alphacode socket from a previous server.");
        }
    ...
    if std::env::var("ALPHACODE_RESUMING").is_err() && server_running {
...
pub(crate) async fn maybe_prompt_server_bootstrap_login(
    ...
    let cli_bootstrap_requested = std::env::var_os("ALPHACODE_CLI_BOOTSTRAP_LOGIN").is_some();
    ...
    // `ALPHACODE_CLI_BOOTSTRAP_LOGIN` env var.
...
pub(crate) async fn spawn_server(
    ...
    if client_requested_selfdev {
        cmd.env("ALPHACODE_DEBUG_CONTROL", "1");
    }
    ...
    cmd.env("ALPHACODE_DEFERRED_AUTH_BOOTSTRAP", "1");
...
fn run_telemetry_command(subcmd: &super::args::TelemetryCommand) -> Result<()> {
    match subcmd {
        ...
        super::args::TelemetryCommand::Enable => run_telemetry_set(true),
        super::args::TelemetryCommand::Disable => run_telemetry_set(false),
    }
}
...
fn run_telemetry_set(enabled: bool) -> Result<()> {
    let _ = crate::alphacode_telemetry_core::set_usage_telemetry_enabled(enabled);
    if enabled {
        println!("Usage telemetry enabled.");
    } else {
...
async fn run_server_promote(version: Option<&str>, json: bool) -> Result<()> {
    let target = match version {
        Some(v) => v.to_string(),
        None => crate::alphacode_build_support::promote_version_to_shared_server("")
            .ok()
            .flatten()
            .unwrap_or_default(),
    };
    let promoted = crate::alphacode_build_support::promote_version_to_shared_server(&target)?;
    if json {

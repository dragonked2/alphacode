...
pub(crate) use crate::alphacode_app_core::session_launch::resumed_window_title;
    ...
    if let Some(output) = video_output {
        let output_path = if output == "auto" {
            ...
            let safe_name = session_name
                ...
                .collect::<String>();
            std::path::PathBuf::from(format!("alphacode_replay_{}_{}.mp4", safe_name, date))
        } else {
            std::path::PathBuf::from(output)
        };
        eprintln!(
...
pub use crate::alphacode_app_core::session_launch::{
...
pub fn list_sessions() -> Result<()> {
    fn build_resume_target_command(
        exe: &std::path::Path,
        target: &crate::alphacode_tui_session_picker::ResumeTarget,
    ...
    match tui::session_picker::pick_session()? {
        Some(tui::session_picker::PickerResult::TakeOverClaude(target)) => {
            ...
            let crate::alphacode_tui_session_picker::ResumeTarget::AlphacodeSession { session_id } =
                &resolved_target
            else {
                anyhow::bail!("Claude takeover did not produce a Alphacode session");
            };
            ...
            for session_id in recovered {
                ...
                match spawn_resume_in_new_terminal(&exe, &session_id, &session_cwd) {
                    ...
                    Ok(false) => {
                        ...
                        eprintln!("  alphacode --resume {}", session_id);
                    }
                    Err(e) => {

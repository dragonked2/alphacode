...
pub(crate) enum Command {
    ...
    /// Run as an Agent Client Protocol (ACP) adapter backed by the Alphacode daemon
    Acp,

    /// Manage the background server daemon (e.g. `alphacode server stop`).
    Server {
        #[command(subcommand)]
    ...
    Login {
        /// Provider to log in to. Equivalent to --provider for this command, e.g. `alphacode login google`.
        // Distinct clap id: the global `--provider` flag also has id "provider";
        // sharing the id makes clap drop the flag inside `login` (so
        // `alphacode login --provider x` errors) and propagate the global default
        ...
        /// OpenAI-compatible API key. If omitted, alphacode prompts securely when needed.
        #[arg(long)]
    ...
    /// Log in to and manage your Alphacode account
    Account {
        #[command(subcommand)]
    ...
    /// Update alphacode to the latest version
    Update,
    ...
    /// Debug socket CLI - interact with running alphacode server
    Debug {
    ...
    /// Optional Alphacode Cloud/Jade integration commands
    #[command(subcommand)]
    Cloud(CloudCommand),
    ...
    /// Inject externally transcribed text into the active Alphacode TUI
    Transcript {
        /// Transcript text. If omitted, reads from stdin.
        text: Option<String>,

        /// How to apply the transcript inside Alphacode
    ...
    /// Run configured dictation: send to last-focused alphacode client or type raw text
    Dictate {
        /// Type the transcript into the focused app instead of sending to alphacode
        #[arg(long)]
        r#type: bool,
    },

    /// Set up the platform global hotkey to launch alphacode
    SetupHotkey {
    ...
    /// Install a launcher so alphacode appears in your app launcher
    SetupLauncher,
    ...
    /// Save or restore the current set of open alphacode windows across a system reboot
    Restart {
        #[command(subcommand)]
    ...
    ///
    /// This is the endpoint the TypeScript SDK (`@1jehuang/alphacode-sdk`) connects to. It
    ...
    ApiBridge {
        /// Path of the API socket to listen on (default: $XDG_RUNTIME_DIR/alphacode-api.sock)
        ///
...
pub(crate) enum AccountCommand {
    ...
    /// Open the public Alphacode account management page
    Manage,
...
pub(crate) enum ServerCommand {
    ...
    /// binary; run `alphacode server reload` separately to apply it.
    Promote {
...
pub(crate) enum CloudSessionsCommand {
    ...
    Upload {
        /// Path to a local Alphacode session JSON file
        session_file: String,
    ...
    /// Upload the newest local Alphacode session to Jade cloud storage
    UploadLatest {
        /// Directory containing local Alphacode session JSON files
        #[arg(long, default_value = "~/.alphacode/sessions")]
        sessions_dir: String,
    ...
    Sync {
        /// Directory containing local Alphacode session JSON files (default: ~/.alphacode/sessions)
        #[arg(long)]
...
pub(crate) struct JadeCloudOptions {
    ...
    /// Path to the private Jade session helper. Defaults to $ALPHACODE_JADE_SESSIONS_HELPER or ~/jade/scripts/jade_sessions.py.
    #[arg(long)]
...
pub(crate) enum RestartCommand {
    /// Save a reboot snapshot of currently active alphacode windows
    Save {
        /// Restore this reboot snapshot automatically the next time plain `alphacode` starts
        #[arg(long)]
        auto_restore: bool,
    },
...
pub(crate) enum ProviderCommand {
    ...
    Add {
        ...
        /// API key value to store in alphacode's private provider env file. Prefer --api-key-stdin for shell history safety.
        #[arg(long, conflicts_with_all = ["api_key_stdin", "no_api_key"])]
        api_key: Option<String>,

        /// Read the API key from stdin and store it in alphacode's private provider env file
        #[arg(long, conflicts_with = "no_api_key")]
        api_key_stdin: bool,
        ...
        /// Private env file name under alphacode's app config directory for stored API keys
        #[arg(long)]

...
pub(crate) async fn run_login(no_browser: bool) -> Result<()> {
    super::login::run_alphacode_account_login(no_browser).await
}

pub(crate) async fn run_status(json: bool) -> Result<()> {
    let Some(api_key) = crate::subscription_catalog::configured_api_key() else {
        anyhow::bail!(
            "No Alphacode account credential is configured. Run `alphacode account login` to sign in."
        );
    };
    ...
    match subscription_api::fetch_subscription_me_with(&client, &api_base, &api_key).await {
        ...
        Err(AccountApiError::Unauthorized) => {
            ...
            anyhow::bail!(
                "The Alphacode account key was revoked or expired. Local credentials were cleared. Run `alphacode account login` to sign in again."
            )
        }
...
pub(crate) fn run_manage() -> Result<()> {
    ...
    println!("Opening Alphacode account management: {url}");
...
pub(crate) async fn run_logout() -> Result<()> {
    ...
    crate::subscription_catalog::clear_account_credentials()
        .context("Failed to securely clear local Alphacode account credentials")?;
    crate::alphacode_base::auth::AuthStatus::invalidate_cache();

    match (api_key.is_some(), remote) {
        (false, _) => {
            println!("No local Alphacode account credential was present. Local account cache is clear.")
        }
        (true, Ok(())) => println!(
            "Alphacode account key revoked. Local credentials and account cache were securely cleared."
        ),
        (true, Err(AccountApiError::Unauthorized)) => println!(
            "The Alphacode account key was already revoked. Local credentials and account cache were securely cleared."
        ),
            ...
            matches!(
                ...
                Ok(parsed)
                    if parsed.scheme() == "https"
                        && matches!(parsed.host_str(), Some("alphacode.sh" | "www.alphacode.sh" | "solosystems.dev"))
                        && parsed.username().is_empty()
                        && parsed.password().is_none()
            )
        })
        .unwrap_or(crate::subscription_catalog::ALPHACODE_ACCOUNT_URL)
}
...
mod tests {
    ...
    fn manage_url_accepts_only_public_allowlisted_https_origins() {
        assert_eq!(
            public_manage_url(Some("https://alphacode.sh/account")),
            "https://alphacode.sh/account"
        );
        assert_eq!(
            public_manage_url(Some("https://evil.example/?key=jck_live_secret")),
            crate::subscription_catalog::ALPHACODE_ACCOUNT_URL
        );
        assert_eq!(
            public_manage_url(Some("https://user:pass@alphacode.sh/account")),
            crate::subscription_catalog::ALPHACODE_ACCOUNT_URL
        );
    }
}

    ...
    match cmd {
        PluginCommand::List { json } => {
            ...
            if json {
                ...
                print!(
                    "{}",
                    crate::alphacode_app_core::plugin::format_plugin_list(&plugins)
                );
            }
        }
        ...
        PluginCommand::Info { name } => {
            ...
            print!(
                "{}",
                crate::alphacode_app_core::plugin::format_plugin_info(&plugin)
            );
        }
    }
    Ok(())
...
fn cloud_sessions_config_path() -> Result<PathBuf> {
    Ok(crate::storage::alphacode_dir()?.join("cloud_sessions.json"))
}
...
fn cloud_sessions_sync_state_path() -> Result<PathBuf> {
    Ok(crate::storage::alphacode_dir()?.join("cloud_sessions_sync.json"))
}
...
fn resolve_sync_sessions_dir(override_path: Option<&str>) -> Result<PathBuf> {
    ...
    Ok(crate::storage::alphacode_dir()?.join("sessions"))
}
...
fn resolve_jade_sessions_helper(override_path: Option<&str>) -> Result<PathBuf> {
    ...
    if let Some(path) = std::env::var_os("ALPHACODE_JADE_SESSIONS_HELPER")
    ...
    anyhow::bail!(
        "Could not find Jade session helper. Set --helper PATH or ALPHACODE_JADE_SESSIONS_HELPER. Expected a private helper like ~/jade/scripts/jade_sessions.py"
    );
}
...
pub async fn run_transcript_command(
    ...
    let mut client = crate::alphacode_app_core::server::Client::connect_debug().await?;
...
pub fn run_session_rename_command(
    ...
    session.save()?;
    crate::alphacode_tui::tui::session_picker::invalidate_session_list_cache();
...
async fn run_ambient_visible() -> Result<()> {
    use crate::alphacode_app_core::ambient::VisibleCycleContext;
    ...
    let _ = crossterm::execute!(
        std::io::stdout(),
        crossterm::terminal::SetTitle(terminal_title("🤖 alphacode ambient cycle"))
    );
...
pub fn run_memory_command(cmd: MemorySubcommand) -> Result<()> {
    ...
    match cmd {
        ...
        MemorySubcommand::ClearTest => {
            let test_dir = storage::alphacode_dir()?.join("memory").join("test");
            if test_dir.exists() {
...
pub fn run_pair_command(list: bool, revoke: Option<String>) -> Result<()> {
    ...
    if !gw_config.enabled {
        ...
        eprintln!("  Then restart the alphacode server.\n");
    }

    let code = registry.generate_pairing_code();
    let connect_host = resolve_connect_host(&gw_config.bind_addr);
    let pair_uri = format!(
        "alphacode://pair?host={}&port={}&code={}",
        connect_host, gw_config.port, code
    );

    eprintln!();
    eprintln!("  \x1b[1mScan with the alphacode iOS app:\x1b[0m\n");
    ...
    if connect_host == gateway::UNKNOWN_CONNECT_HOST {
        eprintln!(
            "\n  \x1b[33mTip:\x1b[0m set ALPHACODE_GATEWAY_HOST to your reachable Tailscale hostname."
        );
    }
    ...
    match action {
        ...
        "status" => {
            ...
            if status.ready {
                ...
                println!(
                    "\nThe browser bridge is connected, but the installed Firefox extension is out of date for this alphacode build. Run `alphacode browser setup` to repair or update it."
                );
            } else {
                println!("\nRun `alphacode browser setup` to install or repair it.");
            }
        }
        other => {
...
pub async fn run_server_reload_command(force: bool, emit_json: bool) -> Result<()> {
    ...
    // `alphacode server reload` unconditionally after swapping the binary.
    if !crate::alphacode_app_core::server::has_live_listener(&socket).await {
        // Reap a stale socket left by a crashed daemon so the next launch binds
        // cleanly instead of wedging in a connect-retry loop.
        let reaped = crate::alphacode_app_core::server::reap_stale_socket_if_dead(&socket).await;
        let detail = if reaped {
            "No running alphacode server found; cleared a stale socket.".to_string()
        } else {
            "No running alphacode server found; nothing to reload.".to_string()
        };
    ...
    let mut client = crate::alphacode_app_core::server::Client::connect().await?;
    ...
    if skipped && !reloading {
        return emit(ServerReloadReport {
            ...
            handoff_ready: true,
            detail: "alphacode server is already running the newest binary; no reload needed."
                .to_string(),
        });
    }
    ...
    let detail = if handoff_ready {
        "alphacode server reloaded onto the newest binary.".to_string()
    } else {
        "alphacode server reload requested; the new server is still coming up.".to_string()
    };
...
pub async fn run_server_stop_command(force: bool, emit_json: bool) -> Result<()> {
    ...
    if !force {
        let msg = "`alphacode server stop` terminates the daemon and drops any live headless/swarm sessions. \
Prefer `alphacode server reload` to pick up an upgrade gracefully. \
Re-run with `--force` if you really want to stop the server.";
    ...
    let socket = crate::alphacode_app_core::server::socket_path();
    let had_listener = crate::alphacode_app_core::server::has_live_listener(&socket).await;
    ...
    if let Some(info) = server_info.as_ref() {
        ...
        if crate::platform::is_process_running(pid) {
            #[cfg(unix)]
            {
                ...
                match crate::platform::signal_detached_process_group(pid, libc::SIGTERM) {
                    Ok(()) => {
                        ...
                        detail = format!("Sent SIGTERM to alphacode server (pid {pid}).");
                    }
                    Err(e) => {
                        detail = format!("Failed to signal alphacode server (pid {pid}): {e}");
                    }
                }
            }
            #[cfg(not(unix))]
            {
                match crate::platform::signal_detached_process_group(pid, 0) {
                    Ok(()) => {
                        signaled_pid = Some(pid);
                        detail = format!("Terminated alphacode server (pid {pid}).");
                    }
                    Err(e) => {
                        detail = format!("Failed to terminate alphacode server (pid {pid}): {e}");
                    }
                }
            }
        } else {
            detail = format!("Registered alphacode server (pid {pid}) is not running.");
        }
        ...
        detail = "No running alphacode server found.".to_string();
    }
    ...
    if emit_json {
        ...
        if stopped && signaled_pid.is_some() {
            println!("alphacode server stopped.");
        } else if stopped && !had_listener && signaled_pid.is_none() {
            // Nothing was running; this is still a success for an installer.
        } else if !stopped {
            println!(
                "alphacode server did not exit cleanly; it may still be shutting down. Re-run if needed."
            );
        }
        if reaped {
            println!("Cleared a stale alphacode socket.");
        }
    }
...
pub async fn run_single_message_command(
    ...
    // Load MCP servers from ~/.alphacode/mcp.json so headless `alphacode run` has the
    ...
    if run_command_mcp_enabled() {
        ...
        // for it, and a single-turn `alphacode run` locks its tool snapshot before
        // the background connection finishes, so the model would never see those
        // tools. Long-lived sessions recover on a later turn, but `alphacode run`
...
fn run_command_auto_poke_enabled() -> bool {
    std::env::var("ALPHACODE_RUN_AUTO_POKE")
        .ok()
        .map(|value| {
...
/// Whether headless `alphacode run` should load MCP servers from `~/.alphacode/mcp.json`.
/// Enabled by default; set `ALPHACODE_RUN_MCP=0` (or `false`/`off`/`no`) to skip MCP
/// registration for latency-sensitive scripting. (#390)
fn run_command_mcp_enabled() -> bool {
    std::env::var("ALPHACODE_RUN_MCP")
        .ok()
        .map(|value| {
            let value = value.trim().to_ascii_lowercase();
            !matches!(value.as_str(), "0" | "false" | "off" | "no")
...
/// Max time `alphacode run` waits for cold-cache MCP servers to register their
/// tools before running the single turn. Override with `ALPHACODE_RUN_MCP_WAIT_MS`
/// (0 disables the wait).
fn run_command_mcp_cold_wait() -> std::time::Duration {
    let ms = std::env::var("ALPHACODE_RUN_MCP_WAIT_MS")
        .ok()
...
/// Bridge the cold-cache gap for `alphacode run`: if any configured MCP server has
...
async fn wait_for_cold_cache_mcp_tools(registry: &crate::tool::Registry) {
    ...
    crate::logging::info(&format!(
        "alphacode run: waiting up to {}ms for cold-cache MCP server(s) to register tools: {}",
        budget.as_millis(),
        cold_servers.join(", ")
    ));
    ...
    loop {
        ...
        if covered {
            crate::logging::info(
                "alphacode run: cold-cache MCP server(s) registered tools; proceeding",
            );
            return;
        }
        if std::time::Instant::now() >= deadline {
            crate::logging::warn(
                "alphacode run: timed out waiting for cold-cache MCP server(s); \
...
fn run_command_auto_poke_max_turns() -> Option<usize> {
    std::env::var("ALPHACODE_RUN_AUTO_POKE_MAX_TURNS")
        .ok()
    ...
    loop {
        ...
        match build_run_auto_poke_follow_up_from_todos(
            ...
            Some(RunAutoPokeFollowUp::GateDigest { message }) => {
                ...
                eprintln!(
                    "We asked the agent to double-check this turn's weak points. Set ALPHACODE_RUN_AUTO_POKE=0 to disable."
                );
                continue;
            }
            Some(RunAutoPokeFollowUp::ConfidenceSummary {
                ...
                next_message = message;
                eprintln!(
                    "Todos are done. Asking the agent for a final confidence check. Set ALPHACODE_RUN_AUTO_POKE=0 to disable."
                );
                continue;
            }
            Some(RunAutoPokeFollowUp::Incomplete { count, message }) => {
                ...
                next_message = message;
                eprintln!(
                    "{} incomplete todo(s). We poked the agent for you. Set ALPHACODE_RUN_AUTO_POKE=0 to disable.",
                    count
                );
            }
            None => break,

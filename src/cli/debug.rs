...
pub async fn run_debug_command(
    ...
    if !crate::transport::is_socket_path(&debug_socket) {
        ...
        eprintln!("\nMake sure:");
        eprintln!("  1. A alphacode server is running (alphacode or alphacode serve)");
        eprintln!("  2. debug_socket is enabled in ~/.alphacode/config.toml");
        eprintln!("     [display]");
        eprintln!("     debug_socket = true");
        eprintln!("\nOr use 'alphacode debug start' to start a server.");
        eprintln!("Use 'alphacode debug list' to see running servers.");
...
async fn debug_list_servers() -> Result<()> {
    ...
    if servers.is_empty() {
        println!("No running alphacode servers found.");
        println!("\nStart one with: alphacode debug start");
        return Ok(());
    }

    println!("Running alphacode servers:\n");
    ...
    println!("  alphacode debug -s /path/to/socket.sock sessions");
...
async fn debug_start_server(arg: &str, socket_path: Option<String>) -> Result<()> {
            ...
            .is_ok()
    {
        ...
        eprintln!("Use 'alphacode debug list' to see all servers.");
        return Ok(());
    }
    ...
    eprintln!("Starting alphacode server...");
    ...
    if crate::transport::is_socket_path(&debug_socket) {
        ...
        eprintln!("⚠ Debug socket not enabled. Add to ~/.alphacode/config.toml:");
        eprintln!("  [display]");
        eprintln!("  debug_socket = true");
    }

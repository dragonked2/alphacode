...
pub(crate) use crate::alphacode_app_core::external_auth::maybe_run_external_auth_auto_import_flow;
use crate::alphacode_app_core::external_auth::{
...
pub enum ProviderChoice {
    ...
    Baseten,
    Cortecs,
    #[value(alias = "token-harbor", alias = "thk")]
    Tokenharbor,
...
const PROVIDER_CHOICE_LOGIN_PROVIDERS: &[(ProviderChoice, LoginProviderDescriptor)] = &[
    (
        ProviderChoice::Alphacode,
        crate::provider_catalog::ALPHACODE_LOGIN_PROVIDER,
    ),
    (
    ...
    (
        ProviderChoice::Tokenharbor,
        crate::provider_catalog::TOKENHARBOR_LOGIN_PROVIDER,
    ),
    (
    ...
    (
        ...
        crate::provider_catalog::DRAGONMETA_LOGIN_PROVIDER,
    ),
    (
...
pub fn prompt_login_provider_selection(
    ...
    prompt_login_provider_selection_optional(providers, heading)?.ok_or_else(|| {
        anyhow::anyhow!("Login skipped. Run `alphacode login` when you're ready to authenticate.")
    })
}
...
fn provider_login_hint_for_api_key_env(env_key: &str) -> String {
    if env_key == "OPENROUTER_API_KEY" {
        return "alphacode login --provider openrouter".to_string();
    }
    ...
    crate::provider_catalog::openai_compatible_profiles()
        ...
        .find_map(|profile| {
            ...
            (resolved.api_key_env == env_key)
                .then(|| format!("alphacode login --provider {}", resolved.id))
        })
        .unwrap_or_else(|| "alphacode login".to_string())
}
...
fn ensure_external_api_key_auth_allowed_for_explicit_choice(env_key: &str) -> Result<()> {
    ...
    anyhow::bail!(
        "Skipped trusting external {} credentials. Run `{}` to authenticate alphacode directly.",
        provider_name,
        login_hint
    )
}
...
fn ensure_openai_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    anyhow::bail!(
        "Skipped trusting existing ~/.codex/auth.json credentials. Run `alphacode login --provider openai` to authenticate alphacode directly."
    )
}
...
fn ensure_claude_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    if maybe_prompt_for_generic_oauth_source(
        ...
        "alphacode login --provider claude",
        false,
    ...
    anyhow::bail!(
        "Skipped trusting external Claude credentials. Run `alphacode login --provider claude` to authenticate alphacode directly."
    )
}
...
fn ensure_gemini_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    if maybe_prompt_for_generic_oauth_source(
        ...
        "alphacode login --provider gemini",
        false,
    ...
    anyhow::bail!(
        "Skipped trusting Gemini CLI credentials. Run `alphacode login --provider gemini` to authenticate alphacode directly."
    )
}
...
fn ensure_antigravity_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    if maybe_prompt_for_generic_oauth_source(
        ...
        "alphacode login --provider antigravity",
        false,
...
fn ensure_copilot_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    if !can_prompt_for_external_auth() {
        anyhow::bail!(external_auth_blocked_message(
            ...
            source.display_name(),
            &path,
            "alphacode login --provider copilot"
        ));
    }
    ...
    anyhow::bail!(
        "Skipped trusting external Copilot credentials. Run `alphacode login --provider copilot` to authenticate alphacode directly."
    )
}
...
fn ensure_cursor_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    if !can_prompt_for_external_auth() {
        anyhow::bail!(external_auth_blocked_message(
            ...
            source.display_name(),
            &path,
            "alphacode login --provider cursor"
        ));
    }
    ...
    anyhow::bail!(
        "Skipped trusting external Cursor credentials. Run `alphacode login --provider cursor` to authenticate alphacode directly."
    )
}
...
pub async fn login_and_bootstrap_provider(
    ...
    let runtime: Arc<dyn provider::Provider> = match provider.target {
        ...
        LoginProviderTarget::Cursor => {
            ...
            crate::alphacode_core::env::set_var("ALPHACODE_ACTIVE_PROVIDER", "cursor");
            Arc::new(crate::alphacode_provider_cursor_runtime::CursorCliProvider::new())
        }
        ...
        LoginProviderTarget::Gemini => {
            disable_subscription_runtime_mode();
            clear_initial_model_provider();
            crate::alphacode_core::env::set_var("ALPHACODE_ACTIVE_PROVIDER", "gemini");
            Arc::new(crate::alphacode_provider_gemini_runtime::GeminiProvider::new())
        }
        LoginProviderTarget::Antigravity => {
            disable_subscription_runtime_mode();
            clear_initial_model_provider();
            crate::alphacode_core::env::set_var("ALPHACODE_ACTIVE_PROVIDER", "antigravity");
            Arc::new(crate::alphacode_provider_antigravity_runtime::AntigravityProvider::new())
        }
...
async fn init_provider_with_options(
    ...
    if let Ok(profile_name) = std::env::var("ALPHACODE_PROVIDER_PROFILE_NAME")
    ...
    if std::env::var_os("ALPHACODE_PROVIDER_PROFILE_ACTIVE").is_none()
        && std::env::var_os("ALPHACODE_NAMED_PROVIDER_PROFILE").is_none()
    {
    ...
    let provider: Arc<dyn provider::Provider> = match choice {
        ProviderChoice::Alphacode => {
            init_notice("Using Alphacode subscription provider");
            Arc::new(provider::alphacode::AlphacodeProvider::new())
        }
        ProviderChoice::Claude => {
            disable_subscription_runtime_mode();
        ...
        ProviderChoice::ClaudeSubprocess => {
            ...
            crate::alphacode_core::env::set_var("ALPHACODE_USE_CLAUDE_CLI", "1");
            init_notice(
        ...
        | ProviderChoice::OpenaiCompatible => {
            ...
            if std::env::var_os("ALPHACODE_NAMED_PROVIDER_PROFILE").is_none() {
            ...
            if std::env::var_os("ALPHACODE_NAMED_PROVIDER_PROFILE").is_some() {
                let profile_name = std::env::var("ALPHACODE_NAMED_PROVIDER_PROFILE")?;
                let cfg = crate::config::config();
                let profile = cfg.providers.get(&profile_name).ok_or_else(|| {
                    anyhow::anyhow!("Unknown provider profile '{}'", profile_name)
                })?;
                Arc::new(
                    crate::alphacode_provider_openrouter_runtime::OpenRouterProvider::new_named_openai_compatible(
                        &profile_name,
                        profile,
                    )?,
                )
            } else {
                Arc::new(crate::alphacode_provider_openrouter_runtime::OpenRouterProvider::new()?)
            }
        }
        ProviderChoice::Antigravity => {
            disable_subscription_runtime_mode();
        ...
        ProviderChoice::Google => {
            ...
            init_notice(
                "Gmail credentials can be configured with `alphacode login google`; the gmail tool is enabled by default in the full tool profile.",
            );
            clear_initial_model_provider();
            Arc::new(provider::MultiProvider::new_fast())
        }
        ProviderChoice::Auto => {
            ...
            if availability.has_any_provider() {
                ...
                // headless (ALPHACODE_NON_INTERACTIVE) but the user logs in *inside*
                ...
                // via ALPHACODE_DEFERRED_AUTH_BOOTSTRAP, so `alphacode run` and other
                // genuinely headless callers still fail loudly.
                if std::env::var_os("ALPHACODE_DEFERRED_AUTH_BOOTSTRAP").is_some() {
                    crate::logging::info(
                    ...
                    anyhow::bail!(
                        "No credentials configured. Run 'alphacode login' or set ANTHROPIC_API_KEY to authenticate."
                    );
    ...
    if let Some(model_name) = model {
        if let Err(e) = provider.set_model(model_name) {
            // If the model looks like an OpenRouter-style ID (contains '/'),
            // the MultiProvider already tried routing through OpenRouter.
            // Give a more helpful error message.
            let hint = if model_name.contains('/') {
                format!(
                    " (Hint: models like '{}' use the 'provider/model' format. \
                     Try TokenRouter (free tier): `alphacode --provider tokenrouter --model {}` \
                     or OpenRouter: `alphacode --provider openrouter --model {}`)",

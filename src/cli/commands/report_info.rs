        ...
        .map(|provider| {
            ...
            AuthStatusProviderReport {
                ...
                last_refresh: assessment
                    .last_refresh
                    .as_ref()
                    .map(crate::alphacode_base::auth::refresh_state::format_record_label),
...
async fn build_auth_doctor_report(
    ...
    let mut status = crate::alphacode_base::auth::AuthStatus::check();
    ...
    for provider in providers {
        ...
        if validation_result.is_some() {
            ...
            status = crate::alphacode_base::auth::AuthStatus::check();
        }
        let assessment = status.assessment_for_provider(provider);
        ...
        let recommended_actions = crate::alphacode_base::auth::doctor::recommended_actions(
            provider,
            &assessment,
...
async fn run_auth_doctor_validation(
    ...
    .await
    {
        ...
        Err(_) => format!(
            "validation timed out after {}s; run `alphacode auth-test --provider {}` for detailed output",
...
fn auth_doctor_validation_detail(
    record: &crate::alphacode_base::auth::validation::ProviderValidationRecord,
    ...
    AuthDoctorValidationDetail {
        ...
        stale: crate::alphacode_base::auth::doctor::validation_is_stale(record.checked_at_ms),
...
pub(super) fn run_version_command(emit_json: bool) -> Result<()> {
    let report = VersionReport {
        version: crate::alphacode_build_meta::version().to_string(),
        semver: crate::alphacode_build_meta::semver().to_string(),
        base_semver: crate::alphacode_build_meta::base_semver().to_string(),
        update_semver: crate::alphacode_build_meta::update_semver().to_string(),
        git_hash: crate::alphacode_build_meta::git_hash().to_string(),
        git_tag: crate::alphacode_build_meta::git_tag().to_string(),
        build_time: crate::build::current_binary_build_time_string()
            .unwrap_or_else(|| "unknown".to_string()),
        git_date: crate::alphacode_build_meta::git_date().to_string(),
        release_build: crate::alphacode_build_meta::is_release_build(),
    };
...
pub(super) async fn run_usage_command(emit_json: bool) -> Result<()> {
    ...
    if report.providers.is_empty() {
        ...
        println!();
        println!("Next steps:");
        println!("- Use `alphacode login --provider claude` to connect Claude OAuth.");
        println!("- Use `alphacode login --provider openai` to connect ChatGPT / Codex OAuth.");
        return Ok(());
    }
    ...
    if let Some(provider_arg) = provider_arg {
        let provider =
            crate::provider_catalog::resolve_login_provider(provider_arg).ok_or_else(|| {
                anyhow::anyhow!(
                    "Unknown provider '{}'. Use `alphacode provider list` to see valid provider ids.",
                    provider_arg
                )
            })?;
...
fn auth_state_label(state: crate::alphacode_base::auth::AuthState) -> &'static str {
    match state {
        crate::alphacode_base::auth::AuthState::Available => "available",
        crate::alphacode_base::auth::AuthState::Expired => "expired",
        crate::alphacode_base::auth::AuthState::NotConfigured => "not_configured",
    }
}
    ...
    async fn cli_auth_status_doctor_and_login_lifecycle_uses_fresh_sandbox() {
        ...
        assert!(
            before_doctor_provider
                ...
                .iter()
                .any(|line| {
                    line == &format!("Connect it: alphacode login --provider {}", provider.id)
                })
        );
        ...
        crate::alphacode_core::env::remove_var(&resolved.api_key_env);
        crate::alphacode_base::auth::AuthStatus::invalidate_cache();
        ...
        assert!(
            after_doctor_provider
                ...
                .any(|line| {
                    line == &format!(
                        "Run runtime verification: alphacode auth-test --provider {}",
                        provider.id
                    )
                })
        );
        assert!(
            after_doctor_provider
                .recommended_actions
                .iter()
                .any(|line| { line == "Review current state: alphacode auth status --json" })
        );
    }
}

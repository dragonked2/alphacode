...
pub(crate) fn configure_provider_profile(
    ...
    Ok(ProviderSetupReport {
        ...
        run_command: format!(
            "alphacode --provider-profile {} --model {} run 'hello'",
            shell_quote(&name),
            shell_quote(&model)
        ),
        auth_test_command: format!(
            "alphacode --provider-profile {} auth-test --prompt {}",
            shell_quote(&name),
            shell_quote("Reply exactly ALPHACODE_PROVIDER_SETUP_OK")
        ),
    })
}
...
mod tests {
    ...
    fn provider_add_writes_named_profile_env_file_and_default() {
        ...
        let _key = EnvVarGuard::remove("ALPHACODE_PROVIDER_MY_API_API_KEY");
        ...
        assert_eq!(
            ...
            Some("ALPHACODE_PROVIDER_MY_API_API_KEY")
        );
        ...
        assert!(env_content.contains("ALPHACODE_PROVIDER_MY_API_API_KEY=secret-test-key"));
    }

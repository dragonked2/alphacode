//! `alphacode menubar` - a lightweight live indicator of how many alphacode sessions
...
/// Ensure a single background `alphacode menubar` helper is running on macOS so the
/// session-count indicator shows up automatically for every macOS user without
/// them needing to run `alphacode menubar` by hand.
///
/// This is a best-effort, fire-and-forget singleton: it records the helper's
/// PID in the *global* `~/.alphacode/menubar.pid` (see [`global_menubar_dir`]) and
...
/// hard against sandboxed alphacode processes (tests, self-dev, onboarding) ever
/// spawning a helper: each such process runs with a throwaway `$ALPHACODE_HOME`,
...
pub fn ensure_menubar_helper_running() {
    ...
    if std::env::var_os("ALPHACODE_NO_MENUBAR").is_some() {
        return;
    }

    // Sandboxed alphacode (tests / self-dev / onboarding, anything with a throwaway
    // `$ALPHACODE_HOME`) must never manage the real user's global menu bar.
...
/// alphacode process for this user, so this state must live at a fixed location
/// that does **not** depend on `$ALPHACODE_HOME`. Sandboxes (tests, self-dev,
/// onboarding) override `$ALPHACODE_HOME` with throwaway temp dirs; anchoring to
/// the real home (`$HOME/.alphacode`) gives every process the same lock inode so
/// the singleton actually holds across them. For a normal (non-sandboxed)
/// launch this is exactly `crate::storage::alphacode_dir()`, so behavior for the
/// real user is unchanged.
#[cfg(target_os = "macos")]
fn global_menubar_dir() -> Option<std::path::PathBuf> {
    let home = dirs::home_dir()?;
    let dir = home.join(".alphacode");
    let _ = std::fs::create_dir_all(&dir);
    Some(dir)
}

/// True when this process is a sandboxed alphacode that must not own the real
/// user's global menu bar. A throwaway `$ALPHACODE_HOME` (anything other than the
/// real `~/.alphacode`) or an explicit test/temp marker means "sandbox".
#[cfg(target_os = "macos")]
...
/// - A `$ALPHACODE_HOME` that differs from the real `~/.alphacode` is a sandbox home.
...
fn is_menubar_sandbox(
    ...
    real_alphacode_home: Option<&std::path::Path>,
) -> bool {
    if test_session || temp_server {
        return true;
    }

    // No explicit override: the real user's default `~/.alphacode`.
    let Some(custom_home) = custom_home else {
        return false;
    };
    let custom = std::path::Path::new(custom_home);
    let Some(real) = real_alphacode_home else {
...
mod macos {
    ...
    /// `~/.alphacode/menubar.lock` (see [`super::global_menubar_dir`]) so the lock
    /// is shared across every alphacode process for this OS user, including ones
    /// running with a sandboxed `$ALPHACODE_HOME`. The menu bar itself is a single
    ...
    const STATUS_ITEM_AUTOSAVE: &str = "alphacode-menubar";
    ...
    define_class!(
           ...
           #[name = "AlphacodeMenubarHandler"]
           struct MenuHandler;

           impl MenuHandler {
               /// Open the clicked session (stored in the item's representedObject)
               /// in a new terminal window via `alphacode --resume <id>`.
               ...
               fn open_session(&self, sender: &NSMenuItem)
    {
                   ...
                   launch_alphacode_window(vec!["--resume".to_string(), session_id.to_string()]);
               }

               /// Launch a brand-new alphacode session in a new terminal window.
               #[unsafe(method(newWindow:))]
               fn new_window(&self, _sender: &NSMenuItem) {
                   launch_alphacode_window(Vec::new());
               }
           }
       );
    ...
    /// Launch a alphacode window off the main thread so slow terminal startup
    /// (osascript / `open`) never blocks the menu bar UI.
    fn launch_alphacode_window(args: Vec<String>) {
        std::thread::spawn(move || {
            if let Err(err) = crate::setup_hints::launch_alphacode_in_macos_terminal(&args) {
                crate::logging::warn(&format!(
                    "menubar: failed to launch alphacode window ({args:?}): {err}"
                ));
            }
        });
    }
    ...
    pub(super) fn run_status_item_app() {
        let mtm = MainThreadMarker::new()
            .expect("alphacode menubar must run on the main thread (the process entry point)");
        ...
        // bypassed entirely (e.g. a self-dev `target/.../alphacode` and the
        // installed `~/.local/bin/alphacode` both spawn helpers, or a reload
        ...
        // Follow the system's Light/Dark setting explicitly. `alphacode` runs as a
        ...
        if let Some(button) = status_item.button(mtm) {
            let icon = NSImage::imageWithSystemSymbolName_accessibilityDescription(
                ...
                Some(ns_string!("alphacode sessions")),
            );
        ...
        let new_window_item = unsafe {
            NSMenuItem::initWithTitle_action_keyEquivalent(
                ...
                ns_string!("New alphacode Window"),
                Some(sel!(newWindow:)),
                ns_string!("n"),
            )
        };
        ...
        let quit_item = unsafe {
            NSMenuItem::initWithTitle_action_keyEquivalent(
                NSMenuItem::alloc(mtm),
                ns_string!("Quit alphacode menu bar"),
    ...
    ///
    /// `alphacode` runs as a bare executable without an `Info.plist` app bundle, so
...
mod tests {
    ...
    fn session_menu_item_title_loads_persisted_todo_title() {
        ...
        let temp = tempfile::tempdir().expect("create temporary ALPHACODE_HOME");
        ...
        if let Some(previous_home) = previous_home {
            crate::alphacode_core::env::set_var("ALPHACODE_HOME", previous_home);
        } else {
    ...
    fn menubar_sandbox_detection() {
        ...
        let real = Path::new("/Users/me/.alphacode");
        ...
        assert!(!is_menubar_sandbox(
            false,
            false,
            Some(OsStr::new("/Users/me/.alphacode")),
            Some(real),
        ));
        ...
        assert!(is_menubar_sandbox(
            false,
            false,
            Some(OsStr::new("/private/tmp/alphacode-e2e-home-xyz")),
            Some(real),
        ));

...
pub(super) async fn stream_response(
    ...
    if !response.status().is_success() {
        ...
        return Err(OpenAIStreamFailure::Other(
            crate::alphacode_provider_core::retry_after::error_with_retry_after(msg, retry_after),
        ));
    }
...
pub(super) async fn try_persistent_ws_continuation(
    ...
    if state.connected_at.elapsed() >= Duration::from_secs(WEBSOCKET_PERSISTENT_MAX_AGE_SECS) {
        crate::alphacode_base::logging::info("Persistent WS connection too old; forcing reconnect");
        *guard = None;
        log_openai_stream_lifecycle(
            crate::alphacode_base::logging::LogLevel::Info,
    ...
    if incremental_items.is_empty() {
        crate::alphacode_base::logging::info("No incremental items to send; need fresh request");
        *guard = None;
        log_openai_stream_lifecycle(
            crate::alphacode_base::logging::LogLevel::Info,
            "persistent_state_reset",
    ...
    let continuation_payload = serde_json::json!({
        ...
        "previous_response_id_hash": crate::alphacode_provider_core::fingerprint::stable_hash_str(&previous_response_id),
...
fn maybe_record_runtime_model_unavailable_from_stream_error(model: &str, message: &str) {
    ...
    if let Some(reason) = reason {
        crate::alphacode_base::provider::record_model_unavailable_for_account(model, &reason);
        crate::alphacode_base::logging::warn(&format!(
...
pub(super) fn is_retryable_error(error_str: &str) -> bool {
    ...
    crate::alphacode_provider_core::is_transient_transport_error(error_str)
        ...
        // and trying a fallback. Retrying in the transport loop wastes 2-3
        // extra API requests that count against the rate limit. The error
        // message with Retry-After hint is still surfaced to the user.
...
mod stream_runtime_tests {
    ...
    #[test]
    fn rate_limit_is_not_retryable_at_transport_level() {
        // 429 (rate limiting) is intentionally NOT retryable at the transport
        // level. The failover system handles 429 by marking the route
        // unavailable and trying a fallback. Retrying in the transport loop
        // wastes extra API requests that count against the rate limit. The
        // agent-level retry in response_recovery.rs handles 429 with proper
        // backoff.

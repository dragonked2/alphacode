...
fn catalog_error_is_auth_rejection(err: &anyhow::Error) -> bool {
    err.downcast_ref::<crate::alphacode_base::provider::ModelCatalogHttpStatus>()
...
impl Provider for OpenAIProvider {
    ...
    async fn complete(
        ...
        let reasoning_effort = self
            ...
            // No explicit user effort: fall back to the model's alphacode-side
        ...
        let prompt_cache_key_hash = request
            ...
            .map(crate::alphacode_provider_core::fingerprint::stable_hash_json);
        ...
        let usage_snapshot = crate::alphacode_base::usage::get_openai_usage_sync();
                ...
                for attempt in 0..MAX_RETRIES {
                    if attempt > 0 {
                        emit_connection_phase(
                            &tx,
                            crate::alphacode_message_types::ConnectionPhase::Retrying {
                                attempt: attempt + 1,
                                max: MAX_RETRIES,
                            },
                        )
                    ...
                    match result {
                        ...
                        Err(OpenAIStreamFailure::Other(error)) => {
                            ...
                            if is_retryable_error(&error_str) && attempt + 1 < MAX_RETRIES {
                                ...
                                next_retry_delay =
                                    crate::alphacode_provider_core::retry_after::retry_after_from_error(
                                        &error,
                                    );
                                last_error = Some(error);
                                continue;
                            ...
                            log_openai_stream_lifecycle(
                                crate::alphacode_base::logging::LogLevel::Error,
                                "attempt_failed",
                                vec![
                                    ("model", model_for_transport.clone()),
                                    ("attempt", (attempt + 1).to_string()),
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        if self.is_browser_only() && !is_chatgpt_web_model(model) {
            anyhow::bail!(
                "OpenAI API credentials are not available for '{}'. The browser-only runtime can use '{}'; run `alphacode login --provider openai` before selecting API models.",
                model,
                CHATGPT_WEB_MODEL,
            );
        }
                ...
                .any(|known| known == model)
        {
            crate::alphacode_logging::warn(&format!(
                "Model '{}' is not in the known OpenAI model list; it may not work. \
                 Use /model to choose from available models if needed.",
                model,
            ));
        }
        let availability = crate::alphacode_base::provider::model_availability_for_account(model);
        if !is_chatgpt_web_model(model)
            && availability.state
        ...
        if crate::alphacode_provider_core::is_openai_api_only_pro_model(model) {
        ...
        if let Ok(mut current) = self.model.try_write() {
            ...
            *current = model.to_string();
            crate::alphacode_base::provider::clear_model_unavailable_for_account(model);
            drop(current);
            if changed {
    ...
    fn available_models(&self) -> Vec<&'static str> {
        ...
        crate::alphacode_provider_core::ALL_OPENAI_MODELS.to_vec()
    }
    ...
    fn available_models_for_switching(&self) -> Vec<String> {
        ...
        if crate::alphacode_base::provider::openai_platform_api_key_configured() {
            for pro in crate::alphacode_provider_core::OPENAI_API_ONLY_PRO_MODELS {
    ...
    async fn prefetch_models(&self) -> Result<()> {
        ...
        let account_label = crate::alphacode_base::auth::codex::active_account_label();
        ...
        let catalog = if is_chatgpt_mode {
            ...
            match crate::alphacode_base::provider::fetch_openai_model_catalog(&access_token).await {
                Ok(catalog) => catalog,
                ...
                Err(err) if catalog_error_is_auth_rejection(&err) => {
                    ...
                    crate::alphacode_base::provider::fetch_openai_model_catalog(&refreshed).await?
                }
        ...
        if current_credential_identity != credential_identity
            || crate::alphacode_base::auth::codex::active_account_label() != account_label
        {
            crate::alphacode_base::logging::info(
        ...
        self.revalidate_reasoning_effort();
        crate::alphacode_base::provider::persist_openai_model_catalog(&catalog);
        ...
        if !catalog.available_models.is_empty() {
            crate::alphacode_base::provider::populate_account_models(catalog.available_models);
        }
        Ok(())
    }
    ...
    fn set_reasoning_effort(&self, effort: &str) -> Result<()> {
        ...
        if !requested.is_empty()
            && crate::alphacode_provider_core::canonical_reasoning_effort(&requested).is_none()
            && !crate::alphacode_base::prompt::is_swarm_effort(&requested)
        {
            anyhow::bail!(
            ...
            && !crate::alphacode_base::prompt::is_swarm_effort(requested)
        {
            ...
            if !available.contains(&requested) {
                anyhow::bail!(
                    ...
                    available
                        .into_iter()
                        .filter(|effort| !crate::alphacode_base::prompt::is_swarm_effort(effort))
                        .collect::<Vec<_>>()
                        .join(", ")
                );
            }
    ...
    fn available_efforts(&self) -> Vec<&'static str> {
        let model = crate::alphacode_provider_core::model_id::canonical(&self.model());
        ...
        crate::alphacode_provider_core::OPENAI_SELECTABLE_EFFORTS.to_vec()
    }
    ...
    async fn native_compact(
    ...
    ) -> Result<crate::alphacode_provider_core::NativeCompactionResult> {
        ...
        if let Some(encrypted_content) = existing_openai_encrypted_content {
            if !crate::alphacode_base::provider::openai_request::openai_encrypted_content_is_sendable(
                encrypted_content,
            ) {
                anyhow::bail!(
                    "OpenAI native compaction payload is too large to replay ({} chars > safe limit {} chars)",
                    encrypted_content.len(),
                    crate::alphacode_base::provider::openai_request::OPENAI_ENCRYPTED_CONTENT_SAFE_MAX_CHARS,
                );
            }
        ...
        Ok(crate::alphacode_provider_core::NativeCompactionResult {
            summary_text: None,

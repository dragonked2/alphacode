...
//! persistent WebSocket), moved out of `alphacode-base` so provider edits compile
...
//! [`OpenAIProvider`] with `crate::alphacode_base::provider::external` at startup.
//!
//! Model-catalog/account-availability state stays in `crate::alphacode_base::provider`
...
const DEFAULT_MODEL: &str = crate::alphacode_provider_core::DEFAULT_OPENAI_MODEL;
const ORIGINATOR: &str = "codex_cli_rs";

pub(crate) const CHATGPT_WEB_MODEL: &str = crate::alphacode_provider_core::CHATGPT_WEB_MODEL;
...
/// `ALPHACODE_OPENAI_WS_IDLE_RECONNECT_SECS` (0 disables the idle reconnect entirely,
...
static WEBSOCKET_PERSISTENT_IDLE_RECONNECT_SECS: LazyLock<Option<u64>> = LazyLock::new(|| {
    match std::env::var("ALPHACODE_OPENAI_WS_IDLE_RECONNECT_SECS") {
        Ok(raw) => match raw.trim().parse::<u64>() {
            Ok(0) => {
                crate::alphacode_base::logging::info(
                    "OpenAI persistent WS idle reconnect disabled (ALPHACODE_OPENAI_WS_IDLE_RECONNECT_SECS=0); relying on healthcheck + max-age",
                );
                None
            }
            Ok(secs) => {
                crate::alphacode_base::logging::info(&format!(
                    "OpenAI persistent WS idle reconnect threshold set to {}s (ALPHACODE_OPENAI_WS_IDLE_RECONNECT_SECS)",
                    secs
                ));
                Some(secs)
            }
            Err(_) => {
                crate::alphacode_base::logging::info(&format!(
                    "Warning: invalid ALPHACODE_OPENAI_WS_IDLE_RECONNECT_SECS '{}'; using default {}s",
        ...
        match raw.trim().to_ascii_lowercase().as_str() {
            ...
            other => {
                crate::alphacode_base::logging::warn(&format!(
                    "Unknown ALPHACODE_OPENAI_TRANSPORT '{}'; using auto. Use: auto, websocket, or https.",
                    other
                ));
                Self::Auto
            }
...
/// Shared dual-auth credential pin (see `crate::alphacode_provider_core::CredentialMode`).
/// The OpenAI-specific alias is kept so existing call sites read naturally.
pub(crate) use crate::alphacode_provider_core::CredentialMode as OpenAICredentialMode;
...
fn log_openai_stream_lifecycle(
    level: crate::alphacode_base::logging::LogLevel,
    phase: &str,
    ...
    crate::alphacode_base::logging::event(level, "PROVIDER_STREAM_LIFECYCLE", owned);
}
...
async fn emit_connection_phase(
    ...
    phase: crate::alphacode_message_types::ConnectionPhase,
) {
        ...
        loop {
                ...
                >= Duration::from_secs(WEBSOCKET_PERSISTENT_MAX_AGE_SECS)
            {
                log_openai_stream_lifecycle(
                    crate::alphacode_base::logging::LogLevel::Info,
...
impl OpenAIProvider {
    pub(crate) fn supports_extended_prompt_cache_retention(model_id: &str) -> bool {
        crate::alphacode_base::provider::openai::supports_extended_prompt_cache_retention(model_id)
    }
    ...
    fn new_inner(credentials: CodexCredentials, browser_only: bool) -> Self {
        ...
        let mut model = if browser_only {
            ...
            std::env::var("ALPHACODE_OPENAI_MODEL")
                .unwrap_or_else(|_| DEFAULT_MODEL.to_string())
                .trim()
                .to_string()
        };
        if !is_chatgpt_web_model(&model)
            && !crate::alphacode_base::provider::known_openai_model_ids()
                .iter()
        ...
        let prompt_cache_key = std::env::var("ALPHACODE_OPENAI_PROMPT_CACHE_KEY")
            .ok()
            .map(|v| v.trim().to_string())
            .filter(|v| !v.is_empty());
        let prompt_cache_retention = std::env::var("ALPHACODE_OPENAI_PROMPT_CACHE_RETENTION")
            .ok()
            .map(|v| v.trim().to_string())
            .filter(|v| !v.is_empty());
        let prompt_cache_retention = match prompt_cache_retention.as_deref() {
            ...
            Some(other) => {
                crate::alphacode_base::logging::info(&format!(
                    "Warning: Unsupported ALPHACODE_OPENAI_PROMPT_CACHE_RETENTION '{}'; expected 'in_memory' or '24h'",
                    other
                ));
                None
            }
        ...
        let native_compaction_threshold_tokens = crate::alphacode_base::config::config()
            .provider
    ...
    fn default_reasoning_effort_for_model(model: &str) -> Option<String> {
        let key = crate::alphacode_provider_core::model_id::canonical(model);
    ...
    fn revalidate_reasoning_effort(&self) {
        ...
        if crate::alphacode_base::prompt::is_swarm_effort(&current)
    ...
    fn api_reasoning_effort(&self, effort: Option<&str>) -> Option<String> {
        match effort {
            Some(e) if crate::alphacode_base::prompt::is_swarm_effort(e) => {
                let available = self.available_efforts();
                crate::alphacode_provider_core::OPENAI_SELECTABLE_EFFORTS
                    .iter()
                    .rev()
                    .find(|candidate| {
                        !crate::alphacode_base::prompt::is_swarm_effort(candidate)
    ...
    fn parse_max_output_tokens(raw: Option<&str>) -> Option<u32> {
        ...
        match raw.parse::<u32>() {
            ...
            Err(_) => {
                crate::alphacode_base::logging::warn(&format!(
                    "Invalid ALPHACODE_OPENAI_MAX_OUTPUT_TOKENS='{}'; using default {}",
    ...
    fn load_max_output_tokens() -> Option<u32> {
        let raw = std::env::var("ALPHACODE_OPENAI_MAX_OUTPUT_TOKENS").ok();
        let parsed = Self::parse_max_output_tokens(raw.as_deref());
        if raw.is_some() {
            match parsed {
                Some(value) => crate::alphacode_base::logging::info(&format!(
                    "OpenAI max_output_tokens configured to {}",
                    value
                )),
                None => crate::alphacode_base::logging::info(
                    "OpenAI max_output_tokens disabled (ALPHACODE_OPENAI_MAX_OUTPUT_TOKENS=0)",
                ),
            }
        }
        parsed
    ...
    /// `ALPHACODE_OPENAI_API_BASE`, `OPENAI_BASE_URL`, `OPENAI_API_BASE`.
    ///
    ...
    pub(crate) fn resolve_api_base() -> String {
        crate::alphacode_base::provider::openai::resolve_api_base()
    }
    ...
    async fn model_id(&self) -> String {
        ...
        match availability.state {
            ...
            crate::alphacode_base::provider::AccountModelAvailabilityState::Unknown => {
                if crate::alphacode_base::provider::should_refresh_openai_model_catalog()
                    && crate::alphacode_base::provider::begin_openai_model_catalog_refresh()
                {
                    let is_chatgpt_mode = {
                    ...
                    crate::alphacode_base::provider::refresh_openai_model_catalog_in_background(
                        token,
                        is_chatgpt_mode,
            ...
            crate::alphacode_base::provider::AccountModelAvailabilityState::Available => {}
        }

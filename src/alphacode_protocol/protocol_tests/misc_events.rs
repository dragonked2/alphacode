...
fn test_input_shell_result_event_roundtrip() -> Result<()> {
    let event = ServerEvent::InputShellResult {
        result: crate::alphacode_message_types::InputShellResult {
...
fn test_structured_set_route_decodes_as_set_route_not_set_model() -> Result<()> {
    ...
    let request = Request::SetRoute {
        ...
        selection: crate::alphacode_provider_core::RouteSelection {
            ...
            runtime_key: crate::alphacode_provider_core::RuntimeKey::OpenAIApiKey,
...
#[test]
fn test_alphacode_subscription_set_route_is_wire_safe() -> Result<()> {
    let request = Request::SetRoute {
        id: 8,
        selection: crate::alphacode_provider_core::RouteSelection {
            model: "gpt-5.5".to_string(),
            runtime_key: crate::alphacode_provider_core::RuntimeKey::AlphacodeSubscription,
            api_method: "alphacode-subscription".to_string(),
            provider_label: "Alphacode Subscription".to_string(),
    ...
    let line = serde_json::to_string(&request)?;
    assert!(line.contains("\"type\":\"set_route\""));
    assert!(line.contains("\"kind\":\"alphacode-subscription\""));

    let decoded = decode_request(&line)?;
    let Request::SetRoute { id, selection } = decoded else {
        return Err(anyhow!("expected Alphacode SetRoute, got {decoded:?}"));
    };
    assert_eq!(id, 8);
    assert_eq!(selection.model, "gpt-5.5");
    assert_eq!(

...
/// Score a memory entry for ranking in retrieval results.
///
/// Higher scores indicate more relevant, trustworthy, and recently useful
/// memories. The scoring considers:
/// - Recency (exponential decay with 24h half-life)
/// - Access frequency (sqrt-scaled to avoid runaway from repeated hits)
/// - Category importance (corrections > preferences > facts > entities)
/// - Trust level (multiplied as a quality gate)
/// - Reinforcement strength (ln-scaled, diminishing returns)
/// - Confidence decay (exponential decay based on category half-life)
pub fn memory_score(entry: &MemoryEntry) -> f64 {
    ...
    let recency = (-age_hours / 24.0 * 0.693).exp();
    score += recency * 80.0;

    // Access frequency: sqrt-scaled to prevent runaway scores
    score += (entry.access_count as f64).sqrt() * 12.0;
    ...
    // Reinforcement strength: ln-scaled with diminishing returns
    score += (entry.strength.max(1) as f64).ln() * 6.0;

    // Confidence factor: use the time-decayed confidence if available
    let confidence = entry.effective_confidence() as f64;
    score *= confidence.max(0.1); // Floor at 0.1 to avoid zeroing out
...
/// Supports multi-word queries: ALL words must appear (AND logic),
/// enabling more precise retrieval for compound searches like
/// "rust error handling".
pub fn memory_matches_search(memory: &MemoryEntry, normalized_query: &str) -> bool {
    let text = memory.searchable_text();
    // Single query term: simple substring match

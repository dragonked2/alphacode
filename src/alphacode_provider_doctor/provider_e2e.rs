...
//!
//! This powers `alphacode provider-doctor`: it walks the same strict provider/model
...
//! - [`DoctorTier::Offline`]: no API key, no network, no spend. Validates alphacode's
...
pub enum DoctorTier {
    /// No key, no network, no spend. Validates alphacode-side wiring only.
    Offline,
...
///
/// The predicate itself lives in `crate::alphacode_base::auth::doctor` so base-internal
...
pub use crate::alphacode_base::auth::doctor::native_doctor_supports_provider;
...
pub async fn run_claude_native_e2e(
    ...
    // `ALPHACODE_RUNTIME_PROVIDER=claude-api` would silently test the API-key path
        ...
        checks.push(DoctorCheck::failed(
            ...
            format!(
                "could not select the Claude OAuth credential path: {error}. \
                 Run `alphacode login --provider claude` to mint a fresh OAuth token."
            ),
        ));
        return Ok(finish_report(
            provider_id,
    ...
    let catalog_models: Vec<String> = if tier.requires_api_key() {
        match provider_runtime.fetch_live_model_ids_for_doctor().await {
            ...
            Ok(_) => {
                ...
                let fallback = crate::alphacode_base::provider::known_anthropic_model_ids();
                checks.push(DoctorCheck::passed(
                    checkpoints::MODEL_CATALOG_LIVE_ENDPOINT,
                    label_for(checkpoints::MODEL_CATALOG_LIVE_ENDPOINT),
                    format!(
...
/// Google OAuth tokens minted by `alphacode login --provider antigravity`; the
...
/// accepts every schema construct alphacode emits), then any Gemini model, then any
                ...
                checks.push(DoctorCheck::failed(
                    ...
                    format!(
                        "could not resolve an Antigravity credential: {error}. \
                         Run `alphacode login --provider antigravity` to sign in."
                    ),
                ));
                return Ok(finish_report(
                    provider_id,
...
impl NativeProviderKind {
    ...
    fn spec(self) -> NativeProviderSpec {
        match self {
            Self::OpenAi => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "alphacode login --provider openai",
            },
            Self::Gemini => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "alphacode login --provider gemini",
            },
            Self::Cursor => NativeProviderSpec {
                ...
                login_hint: "alphacode login --provider cursor",
            },
            Self::Copilot => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "alphacode login --provider copilot",
            },
            Self::Bedrock => NativeProviderSpec {
                ...
                login_hint: "alphacode login --provider bedrock",
            },
            Self::Alphacode => NativeProviderSpec {
                provider_id: "alphacode",
                label: "Alphacode Subscription",
                ...
                auth_source: "Alphacode subscription API key (ALPHACODE_API_KEY)",
                auth_env_key: Some("ALPHACODE_API_KEY"),
                login_hint: "alphacode login --provider alphacode",
            },
            Self::Azure => NativeProviderSpec {
                ...
                login_hint: "alphacode login --provider azure",
            },
        }
    }
    ...
    fn build_runtime(self) -> anyhow::Result<std::sync::Arc<dyn crate::alphacode_base::provider::Provider>> {
        ...
        let runtime: std::sync::Arc<dyn Provider> = match self {
            ...
            Self::Azure => {
                ...
                if let Some(model) = crate::alphacode_base::auth::azure::load_model() {
        ...
        match self {
            Self::OpenAi => {
                let credentials = crate::alphacode_base::auth::codex::load_credentials()
                    .context("load OpenAI credentials (run `alphacode login --provider openai`)")?;
            ...
            Self::Gemini => {
                let tokens = crate::alphacode_base::auth::gemini::load_or_refresh_tokens()
                    .await
            ...
            Self::Cursor => {
                let key = crate::alphacode_base::auth::cursor::load_api_key()
                    .context("load Cursor credential (run `alphacode login --provider cursor`)")?;
            ...
            Self::Bedrock => {
                if !crate::alphacode_base::provider::bedrock::BedrockProvider::has_credentials() {
                    anyhow::bail!(
            ...
            Self::Alphacode => {
                if !crate::alphacode_base::subscription_catalog::has_credentials() {
                    anyhow::bail!(
                        "no Alphacode subscription credential found (set ALPHACODE_API_KEY or run \
                         `alphacode login --provider alphacode`)"
                    );
                }
                Ok("Alphacode subscription credential resolved".to_string())
            }
            Self::Azure => {
                if !crate::alphacode_base::auth::azure::has_configuration() {
                    anyhow::bail!(
                        "Azure OpenAI is not fully configured (need AZURE_OPENAI_ENDPOINT plus an \
                         API key or Entra ID); run `alphacode login --provider azure`"
                    );
                }
                Ok(format!(
                    "Azure OpenAI configured ({})",
                    crate::alphacode_base::auth::azure::method_detail()
                ))
            }
        }
    }
    ...
    fn cheapest_model(self, catalog: &[String]) -> Option<String> {
        ...
        let cheap_markers: &[&str] = match self {
            ...
            Self::Alphacode => &["mini", "flash", "haiku", "lite", "nano"],
...
struct NativeProviderSpec {
    ...
    label: &'static str,
    /// alphacode-side routing/activation contract for the wiring checkpoints.
    contract: WiringContract,
    ...
    /// `alphacode login` hint surfaced when the credential cannot be resolved.
...
async fn run_generic_native_api_checks(
    provider: &dyn crate::alphacode_base::provider::Provider,
    selected: &str,
    label: &str,
    checks: &mut Vec<DoctorCheck>,
    spend: &mut DoctorSpend,
...
/// The alphacode-side wiring a given compat profile is expected to activate.
///
...
/// and alphacode remaps them to their native runtimes. The doctor must assert the
...
fn wiring_contract(profile: OpenAiCompatibleProfile) -> WiringContract {
    match crate::alphacode_base::auth::lifecycle::normalized_auth_provider_id(Some(profile.id)) {
...
fn run_wiring_checks_for_contract(
    ...
    let auth = AuthChanged {
        provider: crate::alphacode_base::protocol::AuthProviderId::new(auth_provider_id),
...
mod tests {
    ...
    fn native_provider_kind_maps_every_generic_id() {
        for (id, expected) in [
            ...
            ("alphacode", NativeProviderKind::Alphacode),
    ...
    fn native_provider_specs_are_self_consistent() {
        ...
        for kind in [
            ...
            if kind == NativeProviderKind::Alphacode {
                assert!(
                    spec.contract.switch_prefix.is_empty(),
                    "Alphacode switches must use bare managed model ids"
                );
            } else {
                assert!(
    ...
    /// `native_doctor_supports_provider` lives in `crate::alphacode_base::auth::doctor`
    ...
    #[test]
    fn native_alphacode_contract_uses_managed_subscription_identity() {
        let contract = NativeProviderKind::Alphacode.spec().contract;
        assert_eq!(
            contract.api_method,
        ...
        assert_eq!(contract.expected_runtime, "alphacode");
        assert!(contract.expected_namespace.is_none());

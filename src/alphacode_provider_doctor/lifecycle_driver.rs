...
use crate::alphacode_base::auth::test_sandbox::AuthTestSandbox;
use crate::alphacode_base::protocol::{
...
impl AuthLifecycleSpec {
    pub(crate) fn cerebras_fixture(auth_path: AuthLifecycleAuthPath) -> Self {
        let mut spec = Self::openai_compatible_fixture(
            crate::alphacode_base::provider_catalog::CEREBRAS_PROFILE,
            auth_path,
        );
...
impl AuthLifecycleDriver {
    ...
    pub(crate) fn run_openai_compatible_fixture(
        ...
        let auth = AuthChanged {
            provider: crate::alphacode_base::protocol::AuthProviderId::new(spec.provider_id),
        ...
        // (`handle_notify_auth_changed`): once the live catalog lands, alphacode
    ...
    fn apply_credentials(
        &self,
        spec: &AuthLifecycleSpec,
        resolved: &crate::alphacode_base::provider_catalog::ResolvedOpenAiCompatibleProfile,
        ...
        match spec.auth_path {
            ...
            AuthLifecycleAuthPath::ProcessEnvPreseeded => {
                crate::alphacode_base::env::set_var(&resolved.api_key_env, &spec.api_key);
...
#[cfg(test)]
pub(crate) use crate::alphacode_provider_doctor::live_provider_probes::{
...
mod tests {
    ...
    fn is_native_routed_compat_profile(
        ...
        crate::alphacode_base::auth::lifecycle::normalized_auth_provider_id(Some(profile.id))
    ...
    fn live_cerebras_api_key() -> Option<LiveTestApiKey> {
        std::env::var("ALPHACODE_AUTH_LIFECYCLE_CEREBRAS_API_KEY")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
            .map(|secret| LiveTestApiKey {
                auth: crate::live_tests::LiveVerificationAuth::from_secret(
                    "env:ALPHACODE_AUTH_LIFECYCLE_CEREBRAS_API_KEY",
                    Some("ALPHACODE_AUTH_LIFECYCLE_CEREBRAS_API_KEY"),
                    &secret,
                ),
                secret,
            })
    ...
    fn live_opencode_zen_api_key() -> Option<LiveTestApiKey> {
        if let Some(secret) = std::env::var("ALPHACODE_AUTH_LIFECYCLE_OPENCODE_API_KEY")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
        {
            return Some(LiveTestApiKey {
                auth: crate::live_tests::LiveVerificationAuth::from_secret(
                    "env:ALPHACODE_AUTH_LIFECYCLE_OPENCODE_API_KEY",
                    Some("ALPHACODE_AUTH_LIFECYCLE_OPENCODE_API_KEY"),
                    &secret,
                ),
                secret,
            });
        ...
        let resolved = crate::alphacode_base::provider_catalog::resolve_openai_compatible_profile(
            crate::alphacode_base::provider_catalog::OPENCODE_PROFILE,
        );
    ...
    fn openai_compatible_provider_matrix_preserves_identity_catalog_and_picker() {
        ...
        for profile in crate::alphacode_base::provider_catalog::openai_compatible_profiles()
            .iter()
            .copied()
    ...
    async fn cerebras_live_opt_in_catalog_lifecycle_uses_isolated_sandbox() {
        if !env_truthy("ALPHACODE_AUTH_LIFECYCLE_LIVE") {
            eprintln!(
                "skipping live Cerebras auth lifecycle test; set ALPHACODE_AUTH_LIFECYCLE_LIVE=1 and ALPHACODE_AUTH_LIFECYCLE_CEREBRAS_API_KEY"
            );
            return;
        }
        ...
        let default_model = crate::alphacode_base::provider_catalog::CEREBRAS_PROFILE.default_model;
    ...
    async fn opencode_zen_live_opt_in_tool_call_smoke() {
        if !env_truthy("ALPHACODE_OPENCODE_ZEN_LIVE_TOOL_TEST") {
            eprintln!(
                "skipping live OpenCode Zen tool-call smoke; set ALPHACODE_OPENCODE_ZEN_LIVE_TOOL_TEST=1 and provide OPENCODE_API_KEY"
            );
            return;
        }
        let api_key = live_opencode_zen_api_key().expect(
            "ALPHACODE_OPENCODE_ZEN_LIVE_TOOL_TEST=1 requires OPENCODE_API_KEY or ALPHACODE_AUTH_LIFECYCLE_OPENCODE_API_KEY",
        );
        let model = std::env::var("ALPHACODE_OPENCODE_ZEN_LIVE_TOOL_MODEL")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
            .unwrap_or_else(|| "kimi-k2.6".to_string());

        let stream_smoke = env_truthy("ALPHACODE_OPENCODE_ZEN_LIVE_STREAM_TEST")
            || env_truthy("ALPHACODE_AUTH_LIFECYCLE_STREAM_SMOKE");
        let mut stages = vec![
            crate::live_tests::LiveVerificationStage::passed(
                crate::live_tests::checkpoints::AUTH_CREDENTIAL_LOADED,
            )
    ...
    async fn issue_driven_openai_compatible_live_target_smoke() {
        let Some(provider_id) = std::env::var("ALPHACODE_ISSUE_DRIVEN_LIVE_PROVIDER")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
        else {
            eprintln!(
                "skipping issue-driven live provider test; set ALPHACODE_ISSUE_DRIVEN_LIVE_PROVIDER to an OpenAI-compatible profile id"
            );
            return;
        };
        let profile =
        ...
        let requested_model = std::env::var("ALPHACODE_ISSUE_DRIVEN_LIVE_MODEL")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
            .or_else(|| resolved.default_model.clone());
        let spend_smoke = env_truthy("ALPHACODE_ISSUE_DRIVEN_LIVE_COMPLETION_SMOKE");
        let stream_smoke = env_truthy("ALPHACODE_ISSUE_DRIVEN_LIVE_STREAM_SMOKE");
    ...
    fn fresh_start_sandbox_is_unconfigured_then_tui_key_lifecycle_configures_provider() {
        ...
        crate::alphacode_base::auth::AuthStatus::invalidate_cache();
        assert_eq!(
            crate::alphacode_base::auth::AuthStatus::check_fast().state_for_provider(provider),
        ...
        assert_eq!(
            crate::alphacode_base::auth::AuthStatus::check_fast().state_for_provider(provider),
            crate::alphacode_base::auth::AuthState::Available
        );
    }
}

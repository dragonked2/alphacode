...
/// raise it via `ALPHACODE_LIVE_SMOKE_TIMEOUT_SECS` for slow reasoning models (e.g.
...
fn smoke_timeout(default_secs: u64) -> std::time::Duration {
    let secs = std::env::var("ALPHACODE_LIVE_SMOKE_TIMEOUT_SECS")
        .ok()
...
/// bare id alphacode uses for routing and coverage keys.
///
...
pub async fn run_live_openai_compatible_smoke(
    ...
    let request = crate::alphacode_base::provider::shared_http_client()
        .post(&url)
        .json(&body);
    let request = apply_provider_auth(request, &resolved, api_key);
...
mod tests {
    use super::*;
    use crate::alphacode_base::provider_catalog::resolve_openai_compatible_profile;
    use crate::alphacode_provider_metadata::{
...
pub async fn run_live_openai_compatible_tool_smoke(
    ...
    let tool_calls = parsed
        ...
        resolved.display_name,
        crate::alphacode_base::util::truncate_str(text.trim(), 1200)
    );
...
pub async fn run_live_claude_native_smoke(
    ...
    .await?;
        ...
        crate::alphacode_base::util::truncate_str(outcome.text.trim(), 200),
        outcome.diagnostics()
    );
...
/// `tool_execution_loop`, `tool_result_followup`, and `real_alphacode_tool_smoke`
...
pub async fn run_live_claude_native_tool_smoke(
    ...
    .await?;
        ...
        first.stop_reason,
        crate::alphacode_base::util::truncate_str(first.text.trim(), 200)
    );
    ...
    let parsed_arguments = crate::alphacode_base::message::ToolCall::parse_streamed_input_to_object(
    ...
    .await?;
        ...
        crate::alphacode_base::util::truncate_str(second.text.trim(), 200)
    );
...
/// alphacode re-emits as text deltas, so we assert the runtime produced streamed
...
/// `real_alphacode_tool_smoke` checkpoints.
...
///
/// Some native runtimes deliver a single coalesced response that alphacode re-emits
